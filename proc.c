...
static const enum special_token ms16_regs16[] = { T_AX, T_DX, T_BX };
static const enum special_token ms16_regs8[] = { T_AL, T_DL, T_BL };

static const enum special_token ms32_regs32[] = { T_ECX, T_EDX };
static const enum special_token ms32_regs16[] = { T_CX, T_DX };
static const enum special_token ms32_regs8[] = { T_CL, T_DL };

static const enum special_token delphi_regs32[] = { T_EAX, T_EDX, T_ECX };

static const enum special_token thiscall_regs32[] = { T_ECX };
static const enum special_token thiscall_regs16[] = { T_CX };
static const enum special_token thiscall_regs8[] = { T_CL };

/* v2.07: added */
static const int ms32_maxreg[] = {
    sizeof(ms16_regs16) / sizeof(ms16_regs16[0]),
    sizeof(ms32_regs32) / sizeof(ms32_regs32[0]),
};

static const int thiscall_maxreg[] = {
    sizeof(thiscall_regs32) / sizeof(thiscall_regs32[0]),
    sizeof(thiscall_regs32) / sizeof(thiscall_regs32[0]),
};
...
static const enum special_token sysV64_regs64[] = { T_RDI, T_RSI, T_RDX, T_RCX, T_R8, T_R9 };
...
static const enum special_token regcallms64_regs64[] = { T_RAX, T_RCX, T_RDX, T_RDI, T_RSI, T_R8, T_R9, T_R11, T_R12, T_R14, T_R15 };
/*
...
*/
static int watc_pcheck(struct dsym* proc, struct dsym* paranode, int* used, int* vecused)
/***************************************************************************/
{
...
*/
static int ms32_pcheck(struct dsym* proc, struct dsym* paranode, int* used, int* vecused)
/***************************************************************************/
{
    char regname[32];
    int size = SizeFromMemtype(paranode->sym.mem_type, paranode->sym.Ofssize, paranode->sym.type);
    ...
    if (proc->sym.langtype == LANG_REGCALL && proc->sym.output_format == OFORMAT_COFF)
    {
        /*if (!(paranode->sym.mem_type == MT_REAL4 || paranode->sym.mem_type == MT_REAL8
            || (paranode->sym.mem_type == MT_TYPE && _stricmp(paranode->sym.type->name, "__m128") == 0) || paranode->sym.mem_type == MT_OWORD
            || (paranode->sym.mem_type == MT_TYPE && _stricmp(paranode->sym.type->name, "__m256") == 0) || paranode->sym.mem_type == MT_YMMWORD
            || (paranode->sym.mem_type == MT_TYPE && _stricmp(paranode->sym.type->name, "__m512") == 0) || paranode->sym.mem_type == MT_ZMMWORD)
            && (size > CurrWordSize || *used >= regcallms32_maxreg[ModuleInfo.Ofssize]))*/
        if (size > CurrWordSize || *used >= regcallms32_maxreg[ModuleInfo.Ofssize])
        {
    ...
    else  if (proc->sym.langtype == LANG_REGCALL && proc->sym.output_format == OFORMAT_ELF)
    {
        /*if (!(paranode->sym.mem_type == MT_REAL4 || paranode->sym.mem_type == MT_REAL8
            ...
            || (paranode->sym.mem_type == MT_TYPE && _stricmp(paranode->sym.type->name, "__m256") == 0) || paranode->sym.mem_type == MT_YMMWORD
            || (paranode->sym.mem_type == MT_TYPE && _stricmp(paranode->sym.type->name, "__m512") == 0) || paranode->sym.mem_type == MT_ZMMWORD)
            && (size > CurrWordSize || *used >= regcallunix32_maxreg[ModuleInfo.Ofssize]))*/
        if (size > CurrWordSize || *used >= regcallunix32_maxreg[ModuleInfo.Ofssize])
        {
            paranode->sym.string_ptr = NULL;
            return(NOT_ERROR);
        }
    }
    else  if (proc->sym.langtype == LANG_VECTORCALL && proc->sym.output_format == OFORMAT_COFF)
    {
        /*if (!(paranode->sym.mem_type == MT_REAL4 || paranode->sym.mem_type == MT_REAL8
            || (paranode->sym.mem_type == MT_TYPE && _stricmp(paranode->sym.type->name, "__m128") == 0) || paranode->sym.mem_type == MT_OWORD
            || (paranode->sym.mem_type == MT_TYPE && _stricmp(paranode->sym.type->name, "__m256") == 0) || paranode->sym.mem_type == MT_YMMWORD
            || (paranode->sym.mem_type == MT_TYPE && _stricmp(paranode->sym.type->name, "__m512") == 0) || paranode->sym.mem_type == MT_ZMMWORD)
            && (size > CurrWordSize || *used >= ms32_maxreg[ModuleInfo.Ofssize]))*/
    ...
    else if (proc->sym.langtype == LANG_SYSVCALL && proc->sym.output_format == OFORMAT_ELF)
    {
        /*if (!(paranode->sym.mem_type == MT_REAL4 || paranode->sym.mem_type == MT_REAL8
            ...
            || (paranode->sym.mem_type == MT_TYPE && _stricmp(paranode->sym.type->name, "__m256") == 0) || paranode->sym.mem_type == MT_YMMWORD)
            && (((paranode->sym.mem_type == MT_TYPE && _stricmp(paranode->sym.type->name, "__m512") == 0) || paranode->sym.mem_type == MT_ZMMWORD)
                || (size > CurrWordSize || *used >= 1)))*/
        if (size > CurrWordSize || *used >= 1)
        {
            paranode->sym.string_ptr = NULL;
            return(NOT_ERROR);
        }
    ...
    {
        if (proc->sym.langtype == LANG_REGCALL && (proc->sym.output_format == OFORMAT_COFF || proc->sym.output_format == OFORMAT_ELF))
        {
            if (*vecused >= 8)
            {
                paranode->sym.string_ptr = NULL;
        ...
        if (proc->sym.langtype == LANG_REGCALL && (proc->sym.output_format == OFORMAT_COFF || proc->sym.output_format == OFORMAT_ELF))
        {
            GetResWName(regcall32_regsXMM[*vecused], regname);
            paranode->sym.tokval = regcall32_regsXMM[*vecused]; /* Store register tokval so invoke can use it */
        }
        else if (proc->sym.langtype == LANG_VECTORCALL && proc->sym.output_format == OFORMAT_COFF)
        {
            GetResWName(vectorcall32_regsXMM[*vecused], regname);
            paranode->sym.tokval = vectorcall32_regsXMM[*vecused]; /* Store register tokval so invoke can use it */
        }
        else if (proc->sym.langtype == LANG_SYSVCALL && proc->sym.output_format == OFORMAT_ELF)
        {
            GetResWName(sysV32_regsXMM[*vecused], regname);
            paranode->sym.tokval = sysV32_regsXMM[*vecused]; /* Store register tokval so invoke can use it */
        }
    ...
    if ((paranode->sym.mem_type == MT_TYPE && _stricmp(paranode->sym.type->name, "__m256") == 0) || paranode->sym.mem_type == MT_YMMWORD)
    {
        ...
        if (proc->sym.langtype == LANG_REGCALL && (proc->sym.output_format == OFORMAT_COFF || proc->sym.output_format == OFORMAT_ELF))
        {
            GetResWName(regcall32_regsYMM[*vecused], regname);
            paranode->sym.tokval = regcall32_regsYMM[*vecused]; /* Store register tokval so invoke can use it */
        }
        else if (proc->sym.langtype == LANG_VECTORCALL && proc->sym.output_format == OFORMAT_COFF)
        {
            GetResWName(vectorcall32_regsYMM[*vecused], regname);
            paranode->sym.tokval = vectorcall32_regsYMM[*vecused]; /* Store register tokval so invoke can use it */
        }
        /*else if (proc->sym.langtype == LANG_SYSVCALL && proc->sym.output_format == OFORMAT_ELF)
        {
            GetResWName(sysV32_regsYMM[*vecused], regname);
            paranode->sym.tokval = sysV32_regsYMM[*vecused]; / * Store register tokval so invoke can use it * /
        }*/
        paranode->sym.total_size = 0;
        paranode->sym.string_ptr = LclAlloc(strlen(regname) + 1);
        strcpy(paranode->sym.string_ptr, regname);
    ...
    if ((paranode->sym.mem_type == MT_TYPE && _stricmp(paranode->sym.type->name, "__m512") == 0) || paranode->sym.mem_type == MT_ZMMWORD)
    {
        ...
        if (proc->sym.langtype == LANG_REGCALL && (proc->sym.output_format == OFORMAT_COFF || proc->sym.output_format == OFORMAT_ELF))
        {
            GetResWName(regcall32_regsZMM[*vecused], regname);
            paranode->sym.tokval = regcall32_regsZMM[*vecused]; /* Store register tokval so invoke can use it */
        }
        else if (proc->sym.langtype == LANG_VECTORCALL && proc->sym.output_format == OFORMAT_COFF)
        {
            GetResWName(vectorcall32_regsZMM[*vecused], regname);
            paranode->sym.tokval = vectorcall32_regsZMM[*vecused]; /* Store register tokval so invoke can use it */
        }
        /*else if (proc->sym.langtype == LANG_SYSVCALL && proc->sym.output_format == OFORMAT_ELF)
        {
            GetResWName(sysV32_regsZMM[*vecused], regname);
            paranode->sym.tokval = sysV32_regsZMM[*vecused]; / * Store register tokval so invoke can use it * /
        }*/
        paranode->sym.total_size = 0;
        paranode->sym.string_ptr = LclAlloc(strlen(regname) + 1);
        strcpy(paranode->sym.string_ptr, regname);
    ...
    if (proc->sym.langtype == LANG_REGCALL && (proc->sym.output_format == OFORMAT_ELF || proc->sym.output_format == OFORMAT_COFF))
    {
        ...
        if (proc->sym.output_format == OFORMAT_ELF)
        {
            paranode->sym.state = SYM_TMACRO;
            ...
            switch (size)
            {
                case 4:
                    ...
                    paranode->sym.tokval = regcallunix32_regs32[*used]; /* Store register tokval so invoke can use it */
                    break;
                case 2:
                    GetResWName(regcallunix32_regs16[*used], regname);
                    paranode->sym.total_size = 2;
                    paranode->sym.tokval = regcallunix32_regs16[*used]; /* Store register tokval so invoke can use it */
                    break;
                case 1:
                    GetResWName(regcallunix32_regs8[*used], regname);
                    paranode->sym.total_size = 1;
                    paranode->sym.tokval = regcallunix32_regs8[*used];  /* Store register tokval so invoke can use it */
                    break;
            }
        }
        else if (proc->sym.output_format == OFORMAT_COFF)
        {
            paranode->sym.state = SYM_TMACRO;
            ...
            switch (size)
            {
                ...
                case 1:
                    ...
                    paranode->sym.total_size = 1;
                    paranode->sym.tokval = regcallms32_regs8[*used];  /* Store register tokval so invoke can use it */
                    break;
            }
        }
    }
    else if ((proc->sym.langtype == LANG_VECTORCALL || proc->sym.langtype == LANG_FASTCALL) && proc->sym.output_format == OFORMAT_COFF)
    {
        ...
        paranode->sym.state = SYM_TMACRO;
        paranode->sym.regist[0] = ms32_regs32[*used];
        switch (size)
        {
            case 4:
                GetResWName(ms32_regs32[*used], regname);
                paranode->sym.total_size = 4;
                paranode->sym.tokval = ms32_regs32[*used]; /* Store register tokval so invoke can use it */
                break;
            case 2:
                GetResWName(ms32_regs16[*used], regname);
                paranode->sym.total_size = 2;
                paranode->sym.tokval = ms32_regs16[*used]; /* Store register tokval so invoke can use it */
                break;
            case 1:
                GetResWName(ms32_regs8[*used], regname);
                paranode->sym.total_size = 1;
                paranode->sym.tokval = ms32_regs8[*used];  /* Store register tokval so invoke can use it */
                break;
        }
    }
    else if (proc->sym.langtype == LANG_THISCALL && proc->sym.output_format == OFORMAT_COFF)
    {
        ...
        paranode->sym.state = SYM_TMACRO;
        paranode->sym.regist[0] = thiscall_regs32[*used];
        switch (size)
        {
            case 4:
                GetResWName(thiscall_regs32[*used], regname);
                paranode->sym.total_size = 4;
                paranode->sym.tokval = thiscall_regs32[*used]; /* Store register tokval so invoke can use it */
                break;
            case 2:
                GetResWName(thiscall_regs16[*used], regname);
                paranode->sym.total_size = 2;
                paranode->sym.tokval = thiscall_regs16[*used]; /* Store register tokval so invoke can use it */
                break;
            case 1:
                GetResWName(thiscall_regs8[*used], regname);
                paranode->sym.total_size = 1;
                paranode->sym.tokval = thiscall_regs8[*used];  /* Store register tokval so invoke can use it */
                break;
        }
    }
    else
    {
        ...
        if (size > CurrWordSize || (ModuleInfo.Ofssize == USE16 && *used >= 3) || (ModuleInfo.Ofssize == USE32 && *used >= 2) || paranode->sym.is_vararg)
        {
            paranode->sym.string_ptr = NULL;
            return(NOT_ERROR);
        }
        ...
        {
            paranode->sym.state = SYM_TMACRO;
            paranode->sym.regist[0] = ms16_regs16[*used];
            switch (size)
            {
                case 2:
                    GetResWName(ms16_regs16[*used], regname);
                    paranode->sym.total_size = 2;
                    paranode->sym.tokval = ms16_regs16[*used]; /* Store register tokval so invoke can use it */
                    break;
                case 1:
                    GetResWName(ms16_regs8[*used], regname);
                    paranode->sym.total_size = 1;
                    paranode->sym.tokval = ms16_regs8[*used];  /* Store register tokval so invoke can use it */
                    break;
            }
        }
    }
...
/********************************************************/
{
    ...
    {
        if (proc->sym.output_format == OFORMAT_COFF)
        {
        ...
        else if (proc->sym.output_format == OFORMAT_ELF)
        {
...
static int delphi_pcheck(struct dsym* proc, struct dsym* paranode, int* used, int* vecused)
/***************************************************************************/
{
    char regname[32];
    int size = SizeFromMemtype(paranode->sym.mem_type, paranode->sym.Ofssize, paranode->sym.type);
    ...
    */
    ...
    do
    {
        ...
        */
        ...
        if (!local)
        { /* if it failed, an error msg has been written already */
...
/***********************************************************************************************/
{
    ...
    const bool syscallsysv = proc->sym.langtype == LANG_SYSCALL && ((proc->sym.output_format == OFORMAT_ELF || proc->sym.output_format == OFORMAT_MAC) && proc->sym.sub_format == SFORMAT_64BIT);
    const bool syscallsys = proc->sym.langtype == LANG_SYSCALL && !((proc->sym.output_format == OFORMAT_ELF || proc->sym.output_format == OFORMAT_MAC) && proc->sym.sub_format == SFORMAT_64BIT);
    /*
    ...
    */
    if (proc->sym.langtype == LANG_C ||
        ...
        (syscallsysv && ModuleInfo.Ofssize != USE64) ||
        ...
        */
        if (tokenarray[i].token != T_COLON)
        {
            ...
            switch (ti.Ofssize)
            {
                ...
                //case USE64: ti.mem_type = MT_QWORD; break;  // TODO KRAD maybe this is true!!!
#endif
                default:
        ...
        if (paracurr)
        {
            ...
            */
            ...
            {
                struct dsym* lym;
                for (lym = proc->e.procinfo->paralist;
                     lym && (lym->nextparam != paracurr);
                     lym = lym->nextparam);
                paracurr = lym;
            }
            else
        ...
        else
        {
            ...
            {
                paranode = (struct dsym*)SymAlloc(""); /* for PROTO, no param name needed */ // TODO KRAD - Find why proto crash with non default call convention
            }

            if (paranode == NULL)
            { /* error msg has been displayed already */
            ...
            if (ti.mem_type == MT_TYPE)
            {
                ...
                if (proc->sym.langtype == LANG_VECTORCALL && proc->sym.output_format == OFORMAT_COFF)
                {
                ...
                if (proc->sym.langtype == LANG_REGCALL && (proc->sym.output_format == OFORMAT_COFF || proc->sym.output_format == OFORMAT_ELF || proc->sym.output_format == OFORMAT_MAC))
                {
                ...
                if (proc->sym.langtype == LANG_SYSVCALL && (proc->sym.output_format == OFORMAT_ELF || proc->sym.output_format == OFORMAT_MAC))
                {
                    proc->e.procinfo->sysvregsize[cntParam] = ti.symtype->max_mbr_size;
                    proc->e.procinfo->sysvregs[cntParam] = ti.size / ti.symtype->max_mbr_size;
                    proc->e.procinfo->sysvsize += ti.size;
                    ti.size = MT_QWORD;
                }
                if (proc->sym.langtype == LANG_FASTCALL && proc->sym.sub_format == SFORMAT_64BIT && proc->sym.output_format == OFORMAT_COFF)
                {
            ...
            else
            {
                ...
                if (proc->sym.langtype == LANG_SYSVCALL && (proc->sym.output_format == OFORMAT_ELF || proc->sym.output_format == OFORMAT_MAC))
                {
                        ...
                        ti.mem_type == MT_OWORD || ti.mem_type == MT_YMMWORD || ti.mem_type == MT_ZMMWORD)
                    {
                        ...
                        if (ti.size >= 16) proc->e.procinfo->sysvsize += ti.size;
                        ti.size = MT_QWORD;
                    }
                }
                if (proc->sym.langtype == LANG_FASTCALL && proc->sym.sub_format == SFORMAT_64BIT && proc->sym.output_format == OFORMAT_COFF)
                {
                    if (ti.mem_type == MT_REAL4 || ti.mem_type == MT_REAL8)
                    {
                        proc->e.procinfo->ms64vecregsize[cntParam] = ti.size;
                        proc->e.procinfo->ms64vecregs[cntParam] = 1;
                        if (ti.size >= 16) proc->e.procinfo->ms64vsize += ti.size;
                        ti.size = MT_QWORD;
                    }
                }
            }
            ...
            else if (proc->sym.langtype == LANG_VECTORCALL && proc->sym.output_format == OFORMAT_COFF && vectorcall_tab[ModuleInfo.fctype].paramcheck(proc, paranode, &fcint, &vecint))
            {
            }
            else if ((proc->sym.langtype == LANG_SYSVCALL || (proc->sym.langtype == LANG_SYSCALL && proc->sym.sub_format == SFORMAT_64BIT && (proc->sym.output_format == OFORMAT_ELF || proc->sym.output_format == OFORMAT_MAC))) && sysvcall_tab[ModuleInfo.fctype].paramcheck(proc, paranode, &fcint, &vecint))
            {
            }
            else if (proc->sym.langtype == LANG_REGCALL && proc->sym.output_format == OFORMAT_COFF && regcallms_tab[ModuleInfo.fctype].paramcheck(proc, paranode, &fcint, &vecint))
            {
            }
            else if (proc->sym.langtype == LANG_REGCALL && (proc->sym.output_format == OFORMAT_ELF || proc->sym.output_format == OFORMAT_MAC) && regcallunix_tab[ModuleInfo.fctype].paramcheck(proc, paranode, &fcint, &vecint))
            {
            }
            else if (proc->sym.langtype == LANG_THISCALL && thiscall_tab[ModuleInfo.fctype].paramcheck(proc, paranode, &fcint, &vecint))
            {
            }
            ...
            {
                if (proc->sym.langtype == LANG_VECTORCALL || proc->sym.langtype == LANG_SYSVCALL || proc->sym.langtype == LANG_REGCALL || proc->sym.langtype == LANG_FASTCALL)
                {
                    switch (CurrWordSize)
                    {
                        case 8:
            ...
            */
            ...
            switch (proc->sym.langtype)
            {
                ...
                case LANG_SYSVCALL:
                    ...
                    else if (ti.Ofssize == USE32 && ModuleInfo.fctype == FCT_MSC && (proc->sym.langtype == LANG_REGCALL || proc->sym.langtype == LANG_VECTORCALL || proc->sym.langtype == LANG_SYSVCALL || proc->sym.langtype == LANG_FASTCALL || proc->sym.langtype == LANG_THISCALL))
                    {
                        goto left_to_right;
                    }
                default:
...
/***************************************************************************************************************/
{
    ...
    */
    ...
    {
        proc->sym.langtype = langtype;
        proc->sym.output_format = ModuleInfo.output_format;
        proc->sym.sub_format = ModuleInfo.sub_format;
        proc->sym.fctype = ModuleInfo.fctype;
    }
    ...
    */
    ...
    if (tokenarray[i].token == T_ID || tokenarray[i].token == T_DIRECTIVE)
    {
        ...
        {
            if (IsPROC)
            { /* v2.11: ignore PRIVATE for PROTO */
    ...
    */
        ...
        tokenarray[i].tokval == T_FRAME)
    {
        ...
        if (proc->sym.output_format != OFORMAT_COFF && proc->sym.output_format != OFORMAT_ELF && proc->sym.output_format != OFORMAT_BIN && proc->sym.output_format != OFORMAT_MAC
#if PE_SUPPORT
...
/***********************************************************/
{
    ...
    proc->sym.output_format = src->sym.output_format;
    proc->sym.sub_format = src->sym.sub_format;
    proc->sym.fctype = src->sym.fctype;
#if MANGLERSUPP
    ...
    proc->sym.Ofssize = src->sym.Ofssize;
...
/***************************************/
{
    ...
    */
    ...
    if (Parse_Pass == PASS_1 && (ModuleInfo.fctype == FCT_WIN64 || ModuleInfo.fctype == FCT_SYSV64) && (ModuleInfo.win64_flags & W64F_AUTOSTACKSP))
    {
    ...
    if (proc->e.procinfo->isframe)
    {
        ...
        if (proc->sym.output_format != OFORMAT_ELF && proc->sym.output_format != OFORMAT_BIN && proc->sym.output_format != OFORMAT_MAC)/*(proc->sym.langtype != LANG_SYSVCALL)*/ // KRAD ensure no extra windows data header with Unix systems and bin flat mode
            WriteSEHData(proc);
    }
#endif
    if (ModuleInfo.list)
...
/*******************************************/
{
    ...
    if (proc->sym.output_format == OFORMAT_ELF || proc->sym.output_format == OFORMAT_BIN || proc->sym.output_format == OFORMAT_MAC)
        return;
...
/**************************************************************/
{
    ...
    if (CurrProc->sym.output_format != OFORMAT_COFF && CurrProc->sym.output_format != OFORMAT_ELF && CurrProc->sym.output_format != OFORMAT_BIN && CurrProc->sym.output_format != OFORMAT_MAC
#if PE_SUPPORT
        && ModuleInfo.sub_format != SFORMAT_PE
#endif
        )
...
/*******************************************************************/
{
    ...
    if (CurrProc->sym.langtype == LANG_C ||
        CurrProc->sym.langtype == LANG_STDCALL ||
...
/*******************************************************/
{
    ...
    if (info->regslist)
    {
        ...
        if (CurrProc->sym.langtype == LANG_REGCALL)
        {
            ...
            if (regcallfreeshadow)
            {                                                    //skip if all shadow spaces are taken
                if (grcount == 1) memset(info->regcallmshome_used, 1, 11);              //1 register only? don't store it, push it to prevent 'sub rsp,...8'
                else if (grcount == 2 && regcallfreeshadow >= 2)
                {                      //store only one register, another push to prevent 'sub rsp,...8'
                    for (i = 0; i < 11; i++)
                    {
                ...
                {                                              //3 registers?
                    ...
                    if (regcallfreeshadow >= 3)
                    {                                       //even if there is enough space we will store only two
                        for (i = 0; i < 11; i++)
                        {                                      //the third one we will push to keep uneven stack
                ...
                {                                              //5 registers?
                    ...
                    if (regcallfreeshadow >= 4)
                    {                                       //even if there is enough space we will store only four
                        for (i = 0; i < 11; i++)
                        {                                      //the fifth one we will push to keep uneven stack
                            if (info->regcallmshome_used[i] == 0) break;                //found first available
                        }
                        for (++i; i < 11; i++)
                        {
                ...
                {                                              //6 registers?
                    ...
                    if (regcallfreeshadow >= 4 && regcallfreeshadow < 6)
                    {              //even if there is enough space we will store only four
                        for (i = 0; i < 11; i++)
                        {                                      //the fifth one we will push to keep uneven stack
                            if (info->regcallmshome_used[i] == 0) break;                //found first available
                ...
                {                                              //7 registers?
                    ...
                    if (regcallfreeshadow >= 6)
                    {                                       //even if there is enough space we will store only six
                        for (i = 0; i < 11; i++)
                        {                                      //the seventh one we will push to keep uneven stack
                            if (info->regcallmshome_used[i] == 0) break;                //found first available
                        }
                        for (++i; i < 11; i++)
                        {
                ...
                {                                              //8 registers?
                    ...
                    if (regcallfreeshadow >= 6 && regcallfreeshadow < 8)
                    {              //even if there is enough space we will store only six
                        for (i = 0; i < 11; i++)
                        {                                      //the seventh one we will push to keep uneven stack
                            if (info->regcallmshome_used[i] == 0) break;                //found first available
                    ...
                    {                                       //even if there is enough space we will store only eight
                        for (i = 0; i < 11; i++)
                        {                                      //the ninth one we will push to keep uneven stack
                            if (info->regcallmshome_used[i] == 0) break;                //found first available
                        }
                        for (++i; i < 11; i++)
                        {
        ...
        else
        {
            ...
            {                                        //skip if all shadow spaces are taken
                ...
                else if (grcount == 2 && freeshadow >= 2)
                {            //store only one register, another push to prevent 'sub rsp,...8'
                    for (i = 0; i < 4; i++)
                    {
                ...
                else if (grcount == 4 && freeshadow == 4)
                {          //easy case
            ...
            if (param->sym.is_vararg == FALSE)
            {
                ...
                else if (param->sym.used)
                {
                    if (param->sym.mem_type == MT_BYTE || param->sym.mem_type == MT_SBYTE)
                        AddLineQueueX("mov [%r+%u], %r", T_RSP, 8 + i * 8, GetWin64SubReg(regcallms64_regs64[i], 0));
                    else if (param->sym.mem_type == MT_WORD || param->sym.mem_type == MT_SWORD)
                        AddLineQueueX("mov [%r+%u], %r", T_RSP, 8 + i * 8, GetWin64SubReg(regcallms64_regs64[i], 1));
                    else if (param->sym.mem_type == MT_DWORD || param->sym.mem_type == MT_SDWORD)
                        AddLineQueueX("mov [%r+%u], %r", T_RSP, 8 + i * 8, GetWin64SubReg(regcallms64_regs64[i], 2));
                    else
                        AddLineQueueX("mov [%r+%u], %r", T_RSP, 8 + i * 8, regcallms64_regs64[i]);
                }
                param = param->nextparam;
    ...
    else
    {
        for (i = 0, param = info->paralist; param && (i < 4); i++)
        {
            ...
            if (param->sym.is_vararg == FALSE)
            {
                ...
                {
                    if (param->sym.mem_type == MT_BYTE || param->sym.mem_type == MT_SBYTE)
                        AddLineQueueX("mov [%r+%u], %r", T_RSP, 8 + i * 8, GetWin64SubReg(ms64_regs64[i], 0));
                    else if (param->sym.mem_type == MT_WORD || param->sym.mem_type == MT_SWORD)
                        AddLineQueueX("mov [%r+%u], %r", T_RSP, 8 + i * 8, GetWin64SubReg(ms64_regs64[i], 1));
                    else if (param->sym.mem_type == MT_DWORD || param->sym.mem_type == MT_SDWORD)
                        AddLineQueueX("mov [%r+%u], %r", T_RSP, 8 + i * 8, GetWin64SubReg(ms64_regs64[i], 2));
                    else
                        AddLineQueueX("mov [%r+%u], %r", T_RSP, 8 + i * 8, ms64_regs64[i]);
                }
                param = param->nextparam;
        ...
        if (regist)
        {
            for (cnt = *regist++; cnt; cnt--, regist++)
            {
                ...
                {                         //sym_ReservedStack has value
                    ...
                    {
                        AddLineQueueX("%s [%r+%u+%s], %r", "vmovdqu", T_RSP, NUMQUAL i, sym_ReservedStack->name, *regist);
                        if (info->isframe && ModuleInfo.frame_auto)
                    ...
                    {
                        AddLineQueueX("%s [%r+%u+%s], %r", "vmovdqu32", T_RSP, NUMQUAL i, sym_ReservedStack->name, *regist);
                        if (info->isframe && ModuleInfo.frame_auto)
                ...
                {                                   // sym_ReservedStack has no value
                    if (GetValueSp(*regist) & OP_XMM)
                    {
                    ...
                    {
                        AddLineQueueX("%s [%r+%u], %r", "vmovdqu", T_RSP, NUMQUAL i, *regist);
                        if (info->isframe && ModuleInfo.frame_auto)
                    ...
                    {
                        AddLineQueueX("%s [%r+%u+%s], %r", "vmovdqu32", T_RSP, NUMQUAL i, *regist);
                        if (info->isframe && ModuleInfo.frame_auto)
                ...
                for (cnt = *regist++; cnt; cnt--, regist++)
                {
                    if ((GetValueSp(*regist) & OP_XMM) || (GetValueSp(*regist) & OP_YMM) || (GetValueSp(*regist) & OP_ZMM))
                    {
                        if (resstack)
                        {
                            ...
                            {
                                AddLineQueueX("vmovdqu32 [%r+%u+%s], %r", T_RSP, NUMQUAL i, sym_ReservedStack->name, *regist);
                                AddLineQueueX("%r %r, %u+%s", T_DOT_SAVEZMM512, *regist, NUMQUAL i, sym_ReservedStack->name);
                                i += 64;// XYZMMsize;
                            }
                            // }
                        ...
                        else
                        {
                            ...
                            {
                                AddLineQueueX("vmovdqu32 [%r+%u+%s], %r", T_RSP, NUMQUAL i, *regist);
                                AddLineQueueX("%r %r, %u+%s", T_DOT_SAVEZMM512, *regist, NUMQUAL i);
                                i += 64;// XYZMMsize;
                            }
                            // }
                            ...
                            if (pp)
                            {
                                if (pp->sym.ttype)
                                {
                                    if (pp->sym.ttype->e.structinfo->isHFA == 1 || pp->sym.ttype->e.structinfo->isHVA == 1 || pp->sym.ttype->e.structinfo->stype == MM128 || pp->sym.ttype->e.structinfo->stype == MM256 || pp->sym.ttype->e.structinfo->stype == MM512)
                                    {
                                        stackSize = info->localsize + resstack + info->vsize + info->xmmsize + 8 + info->pushed_reg * 8 + n * 8;
                                        if ((stackSize & 7) != 0) stackSize = (stackSize + 7) & (-8);
                                        AddLineQueueX("mov [%r + %d], %r", T_RSP, stackSize, ms64_regs64[n]);
                                        //xsize += 8;
                                        xsize += info->vecregsize[n] * xreg;
                                    }
                                }
                            ...
                            if (pp)
                            {
                                if (pp->sym.ttype)
                                {
                                    if (pp->sym.ttype->e.structinfo->isHFA == 1 || pp->sym.ttype->e.structinfo->isHVA == 1 || pp->sym.ttype->e.structinfo->stype == MM128 || pp->sym.ttype->e.structinfo->stype == MM256 || pp->sym.ttype->e.structinfo->stype == MM512)
                                    {
                                        stackSize = info->localsize + resstack + info->regcsize + info->xmmsize + 8 + info->pushed_reg * 8 + n * 8;
                                        if ((stackSize & 7) != 0) stackSize = (stackSize + 7) & (-8);
                                        AddLineQueueX("mov [%r + %d], %r", T_RSP, stackSize, regcallms64_regs64[n]);
                                        //xsize += 8;
                                        xsize += info->regcregsize[n] * xreg;
                                    }
                                }
            ...
            if (i)
            {
                ...
                for (regs = info->regslist, cnt = *regs++; cnt; cnt--, regs++)
                {
                    ...
                    if (GetValueSp(*regs) & OP_ZMM)
                    {
                        DebugMsg1(("write_win64_default_epilogue(%s): restore %s, offset=%d\n", CurrProc->sym.name, GetResWName(*regs, NULL), i));
                        if (ModuleInfo.win64_flags & W64F_AUTOSTACKSP)
                            AddLineQueueX("%s %r, [%r + %d + %s]", "vmovdqu32", *regs, restoreReg, NUMQUAL i - info->frameofs, sym_ReservedStack->name);
                        else
                            AddLineQueueX("%s %r, [%r + %d]", "vmovdqu32", *regs, restoreReg, NUMQUAL i - info->frameofs);
                        i += 64;
                    }
                }
            }
            ...
            for (regs = info->regslist, cnt = *regs++; cnt; cnt--, regs++)
            {
                if ((GetValueSp(*regs) & OP_XMM) || (GetValueSp(*regs) & OP_YMM) || (GetValueSp(*regs) & OP_ZMM))
                {
                    ...
                    if (resstack)
                    {
                        ...
                        {
                            AddLineQueueX("vmovdqu32 %r, [%r + %u + %s]", *regs, stackreg[ModuleInfo.Ofssize], NUMQUAL i, sym_ReservedStack->name);
                            i += 64;
                        }
                    }
                    else
                    {
                        ...
                        {
                            AddLineQueueX("vmovdqu32 %r, [%r + %u]", *regs, stackreg[ModuleInfo.Ofssize], NUMQUAL i);
                            i += 64;
                        }
                    }
                }
...
/***************************************************************************/
{
    ...
    else
    {
        ...
        {
            case 8:
                GetResWName(sysV64_regs64[*used], regname);
                paranode->sym.total_size = 8;
                paranode->sym.tokval = sysV64_regs64[*used];   /* Store register tokval so invoke can use it */
                break;
            case 4:
...
/****************************************************************/
{
    ...
    //stackadj = 8;
    if (!info->fpo && GetRegNo(info->basereg) != 4 && ((info->parasize != 0 && ModuleInfo.noframe != 1) || info->locallist != NULL))
    {
        AddLineQueueX("push %r", info->basereg);
        AddLineQueueX("mov %r, %r", info->basereg, T_RSP);
        //stackadj = 0;
    ...
    if (info->regslist)
    {
        ...
        if (i)
        {
            ...
            for (regs = info->regslist, cnt = *regs++; cnt; cnt--, regs++)
            {
                ...
                if (GetValueSp(*regs) & OP_YMM)
                {
                    AddLineQueueX("%s %r, [%r + %u]", "vmovdqu", *regs, stackreg[ModuleInfo.Ofssize], NUMQUAL i);
                    i += 32;
                }
                if (GetValueSp(*regs) & OP_ZMM)
                {
                    AddLineQueueX("%s %r, [%r + %u]", "vmovdqu32", *regs, stackreg[ModuleInfo.Ofssize], NUMQUAL i);
                    i += 64;
                }
            }
        }
...
static void SetLocalOffsets_RBP_SYSV(struct proc_info* info)
{
    ...
    */
    ...
    for (curr = info->locallist; curr; curr = curr->nextlocal)
    {
        ...
        {
            if (itemsize == 64)
                info->localsize = ROUND_UP(info->localsize, 64);
            else if (itemsize == 32)
...
/********************************************/
{
    ...
    if (info->fpo && stackadj > 0 && (CurrProc->sym.langtype == LANG_FASTCALL || CurrProc->sym.langtype == LANG_VECTORCALL || (CurrProc->sym.langtype == LANG_REGCALL && CurrProc->sym.output_format == OFORMAT_COFF)) && !info->isleaf)
    {
        if (Options.frameflags)
        {
            if (ModuleInfo.Ofssize == USE64)
                ...
                AddLineQueueX("lea %r, [%r-%d]", T_ESP, T_ESP, stackadj);
        }
        else
        {
            if (ModuleInfo.Ofssize == USE64)
    ...
    {
        if ((CurrProc->sym.langtype == LANG_FASTCALL || CurrProc->sym.langtype == LANG_VECTORCALL || (CurrProc->sym.langtype == LANG_REGCALL && CurrProc->sym.output_format == OFORMAT_COFF)) && ModuleInfo.basereg[ModuleInfo.Ofssize] == T_RSP)
            win64_SaveRegParams_RSP(info);
        else if ((CurrProc->sym.langtype == LANG_FASTCALL || CurrProc->sym.langtype == LANG_VECTORCALL || (CurrProc->sym.langtype == LANG_REGCALL && CurrProc->sym.output_format == OFORMAT_COFF)) && ModuleInfo.basereg[ModuleInfo.Ofssize] == T_RBP)
            win64_SaveRegParams_RBP(info);
    }
...
/********************************************/
{
    ...
    if (ModuleInfo.Ofssize == USE64 && (ModuleInfo.fctype == FCT_WIN64 || ModuleInfo.fctype == FCT_SYSV64) && (ModuleInfo.win64_flags & W64F_AUTOSTACKSP))
        resstack = sym_ReservedStack->value;

    if (ModuleInfo.Ofssize == USE64)
    {
        if (ModuleInfo.basereg[USE64] == T_RSP && (CurrProc->sym.langtype == LANG_FASTCALL || CurrProc->sym.langtype == LANG_VECTORCALL || CurrProc->sym.langtype == LANG_REGCALL) && CurrProc->sym.output_format == OFORMAT_COFF)
        {
            write_win64_default_prologue_RSP(info);
        }
        else if ((ModuleInfo.basereg[USE64] == T_RBP) && (CurrProc->sym.langtype == LANG_FASTCALL || CurrProc->sym.langtype == LANG_VECTORCALL || CurrProc->sym.langtype == LANG_REGCALL) && CurrProc->sym.output_format == OFORMAT_COFF && info->isframe)
        {
            write_win64_default_prologue_RBP(info);
        }
        else if ((ModuleInfo.basereg[USE64] == T_RBP) && (CurrProc->sym.langtype == LANG_FASTCALL || CurrProc->sym.langtype == LANG_VECTORCALL || CurrProc->sym.langtype == LANG_REGCALL) && CurrProc->sym.output_format == OFORMAT_COFF)
        {
            write_generic_prologue(info);
        }
        else if ((ModuleInfo.basereg[USE64] == T_RBP) && ((CurrProc->sym.langtype == LANG_SYSVCALL || CurrProc->sym.langtype == LANG_REGCALL || (CurrProc->sym.langtype == LANG_SYSCALL && CurrProc->sym.sub_format == SFORMAT_64BIT)) && (CurrProc->sym.output_format == OFORMAT_ELF || CurrProc->sym.output_format == OFORMAT_MAC)))
        {
...
/***************************************************/
{
    ...
    if (info->isframe || ((ModuleInfo.fctype == FCT_WIN64 || ModuleInfo.fctype == FCT_SYSV64) && (ModuleInfo.win64_flags & W64F_AUTOSTACKSP)))
    {
        rspalign = TRUE;
        if (ModuleInfo.win64_flags & W64F_STACKALIGN16)
            align = 16;
...
/************************************************/
{
    ...
    if ((ModuleInfo.fctype == FCT_WIN64 || ModuleInfo.fctype == FCT_SYSV64) && (ModuleInfo.win64_flags & W64F_AUTOSTACKSP))
    {
        ...
        else if ((CurrProc->sym.langtype == LANG_VECTORCALL || CurrProc->sym.langtype == LANG_FASTCALL) && CurrProc->sym.output_format == OFORMAT_COFF)
        {
        ...
        else if (CurrProc->sym.langtype == LANG_SYSVCALL || (CurrProc->sym.langtype == LANG_SYSCALL && CurrProc->sym.sub_format == SFORMAT_64BIT) && (CurrProc->sym.output_format == OFORMAT_ELF || CurrProc->sym.output_format == OFORMAT_MAC))
        {
    ...
    if (ModuleInfo.Ofssize == USE64)
    {
        ...
        else
        {
            ...
            if ((CurrProc->sym.output_format == OFORMAT_COFF || CurrProc->sym.output_format == OFORMAT_BIN) && CurrProc->e.procinfo->isframe)
                SetLocalOffsets_RBP(CurrProc->e.procinfo);
            else if ((CurrProc->sym.output_format == OFORMAT_ELF || CurrProc->sym.output_format == OFORMAT_MAC) && CurrProc->sym.sub_format == SFORMAT_64BIT)
        ...
        if (ModuleInfo.epiloguemode != PEM_NONE)
        {
            switch (CurrProc->sym.langtype)
            {
                ...
                case LANG_SYSVCALL:
                    ...
                    break;
                case LANG_REGCALL:
                    if (CurrProc->sym.output_format == OFORMAT_COFF)
                    {
                        regcallms_tab[ModuleInfo.fctype].handlereturn(CurrProc, buffer);
                    }
                    else if (CurrProc->sym.output_format == OFORMAT_ELF || CurrProc->sym.output_format == OFORMAT_MAC)
                    {
                        regcallunix_tab[ModuleInfo.fctype].handlereturn(CurrProc, buffer);
                    }
                    break;
                case LANG_THISCALL:
