...
/***********************************************************************************************/
{
    ...
    for( cntParam = 0 ; tokenarray[i].token != T_FINAL ; cntParam++ ) {
         ...
         * HJWasm always require a colon for PROTO.
         */
        if( tokenarray[i].token != T_COLON ) {
            if ( IsPROC == FALSE ) {
    ...
    if ( IsPROC ) {
	  ...
	  if(proc->e.procinfo->basereg == T_RSP && (ModuleInfo.win64_flags & W64F_AUTOSTACKSP) && (ModuleInfo.win64_flags & W64F_SAVEREGPARAMS))
		offset -= 8;

	  /* now calculate the [E|R]BP offsets */
#if AMD64_SUPPORT
        if (( ModuleInfo.Ofssize == USE64) && (proc->sym.langtype == LANG_FASTCALL || proc->sym.langtype == LANG_VECTORCALL ))
          {
...
/****************************************************/
{
    ...
    if( CurrProc != NULL ) {
        ...
        /* this is not needed for HJWasm, but Masm will reject nested
    ...
    if( Parse_Pass == PASS_1 ) {
        ...
        if( sym == NULL || sym->state == SYM_UNDEFINED ) {
             ...
             * Hence this "feature" isn't active in hjwasm.
             */
				...
				if (param->sym.is_vararg == FALSE) {
					if ((param->sym.mem_type & MT_FLOAT) && param->sym.used) {  // added  && param->sym.used
						if (param->sym.mem_type == MT_REAL8)
							AddLineQueueX("movsd qword ptr[%r+%u], %r", T_RSP, 8 + i * 8, T_XMM0 + i);
						else if (param->sym.mem_type == MT_REAL4)
							AddLineQueueX("movss dword ptr[%r+%u], %r", T_RSP, 8 + i * 8, T_XMM0 + i);
						info->home_used[i] = 1;
						++info->home_taken;
					}
...
/****************************************************************/
{
    ...
    if ( info->fpo || ( info->parasize == 0 && info->locallist == NULL ) ) {
		...
		//if (info->basereg != T_RSP && ModuleInfo.win64_flags != 0)
		//{
//			AddLineQueueX("sub %r, %u", T_RSP, CurrWordSize);
    ...
    if (ModuleInfo.win64_flags & W64F_HABRAN){
      ...
      if ((ModuleInfo.Ofssize == USE64)&&(info->locallist == 0)&&(info->localsize)){
           AddLineQueueX("sub %r, %u", T_RSP, info->localsize);
        }
      }
    if (( info->locallist + resstack) || ((CurrProc->sym.langtype == LANG_VECTORCALL)&&(info->vecused)))  {
...
/***************************************************/
{
    ...
    for( curr = info->locallist; curr; curr = curr->nextlocal ) {
        ...
        if ( itemsize > salign )
            info->localsize = ROUND_UP( info->localsize, salign );
  ...
  int anysize;
    ...
    if (ModuleInfo.fctype == FCT_WIN64 && (ModuleInfo.win64_flags & W64F_AUTOSTACKSP)){
      if (ModuleInfo.win64_flags & W64F_HABRAN){
		...
		if (anysize)
		{
			int stackSize = info->localsize + info->vsize + info->xmmsize;
			if ((stackSize & 7) != 0) stackSize = (stackSize + 7)&(-8);
