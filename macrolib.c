...
void InitAutoMacros64(void)
{
	...
	uint_32 macroLen[] = {17, 11, 29, 3, 3, 8, 37, 33, 37, 33, 7, 6, 10, 6, 7, 7, 7, 8, 8, 10, 3, 7, 11, 19, 10, 2, 10, 2, 18, 9, 13, 6, 39, 39, 39, 39, 12, 5, 2, 20, 21, 2, 2, 11, 38, 38, 11, 45, 91, 6, 10, 10, 37, 1, 1, 1, 1, 256, 10, 6, 6, 106, 134 }; // Count of individual lines of macro-body code.
	char *macCode[] = {
...
/*11 MEMALLOC*/			"IF @Platform EQ 1", "INVOKE HeapAlloc,RV(GetProcessHeap),0,aSize", "ELSE", "INVOKE malloc,aSize", "ENDIF", "MEMALIGN rax, 16", "endm", NULL,
...
/*31 ENDCLASS*/			"curClass ENDS",".data","align 16","% _stat&curClass&vtbl &curClass&vtbl <>","% _stat&curClass& curClass <>","ptrDefS TEXTEQU <psr>","ptrDefS CATSTR ptrDefS, <&curClass&>, < TYPEDEF PTR >, <&curClass&>","% ptrDefS","ptrDefS2 TEXTEQU <psr>","ptrDefS2 CATSTR ptrDefS2, <&curClass&vtbl>, < TYPEDEF PTR >, <&curClass&vtbl>","% ptrDefS2",".code","ENDM", NULL,
...
/*39 _STATICREF*/		"% lea reg, _stat&curClass&", "ENDM", NULL,
...
/*59 COMINTERFACE*/		"curClass TEXTEQU <CName>", "% __&CName&_size = 24", "@CatStr(CName, <vtbl COMSTRUCT >)", "ptrDefS TEXTEQU <psr>", "ptrDefS CATSTR ptrDefS, <&curClass&>, < TYPEDEF PTR >, <&curClass&>","% ptrDefS", "CVIRTUAL QueryInterface, <>, :PTR", "CVIRTUAL AddRef, <>", "CVIRTUAL Release, <>", "ENDM", NULL,
...
/*62:_DEREF*/		    "LOCAL i, ptrstr, typestr, argstr","argstr TEXTEQU < >","i = 0","FOR dref, <argsAndRefs>","IF i LT argCount OR argCount EQ 0","IF argCount GT 0", "argstr CATSTR argstr, <,>, <&dref&>","ENDIF", "ELSE","IF(i - argCount) MOD 2 EQ 0","ptrstr TEXTEQU <&dref&>","ELSE","typestr TEXTEQU <&dref&>","IF(i - argCount) EQ 1","% IF(OPATTR(ptrstr)) EQ 0x30","% mov rcx, &ptrstr&","ELSE","% mov rcx, @CatStr(<[>, <&ptrstr&>, <].>, <&typestr&>)","ENDIF","ELSE","% mov rcx, @CatStr(<[>, <&ptrstr&>, <].>, <&typestr&>)","ENDIF","ENDIF","ENDIF","i = i + 1","ENDM","% IF @SizeStr(<%&argstr&>) GT 3", "argstr SUBSTR argstr, 3", "ENDIF", "IF argCount EQ 0", "argstr TEXTEQU <>", "ENDIF", "% IFNB <&argstr&>", "InterfacePtr TEXTEQU <_>","InterfacePtr CATSTR InterfacePtr, <&itype>, <_>, <&proc>, <Pto>","IF(OPATTR(rcx)) AND 00010000b","% IFNB <&argstr&>", "IF @Platform EQ 1","mov r15,[rcx]","% invoke(InterfacePtr PTR[r15].&itype&vtbl.&proc), rcx, &argstr","ELSE","mov rdi,rcx","mov r15,[rdi]","% invoke(InterfacePtr PTR[r15].&itype&vtbl.&proc), rdi, &argstr","ENDIF","ELSE","IF @Platform EQ 1","mov r15,[rcx]","% invoke(InterfacePtr PTR[r15].&itype&vtbl.&proc), rcx","ELSE","mov rdi,rcx","mov r15,[rdi]","% invoke(InterfacePtr PTR[r15].&itype&vtbl.&proc), rdi","ENDIF","ENDIF","ELSE","mov r15,[rcx]","% IFNB <&argstr&>","FOR arg, <argstr>","IFIDNI <&arg>, <rcx>",".ERR <rcx is not allowed as a Method parameter with indirect object label>","ENDIF","ENDM","% invoke(InterfacePtr PTR[r15].&itype&vtbl.&proc), rcx, &argstr","ELSE","invoke(InterfacePtr PTR[r15].&itype&vtbl.&proc), rcx","ENDIF","ENDIF","ELSE","InterfacePtr TEXTEQU <_>","InterfacePtr CATSTR InterfacePtr, <&itype>, <_>, <&proc>, <Pto>","IF(OPATTR(rcx)) AND 00010000b","% IFNB <&argstr&>","IF @Platform EQ 1","mov r15,[rcx]","% invoke(InterfacePtr PTR[r15].&itype&vtbl.&proc), rcx, &argstr","ELSE","mov rdi,rcx","mov r15,[rdi]","% invoke(InterfacePtr PTR[r15].&itype&vtbl.&proc), rdi, &argstr","ENDIF","ELSE","IF @Platform EQ 1","mov r15,[rcx]","invoke(InterfacePtr PTR[r15].&itype&vtbl.&proc), rcx","ELSE","mov rdi,rcx","mov r15,[rdi]","invoke(InterfacePtr PTR[r15].&itype&vtbl.&proc), rdi","ENDIF","ENDIF","ELSE","mov r15,[rcx]","% IFNB <&argstr&>","FOR arg, <argstr>","IFIDNI <&arg>, <rcx>",".ERR <rcx is not allowed as a Method parameter with indirect object label>","ENDIF","ENDM","% invoke(InterfacePtr PTR[r15].&itype&vtbl.&proc), rcx, &argstr","ELSE","invoke(InterfacePtr PTR[r15].&itype&vtbl.&proc), rcx","ENDIF","ENDIF","ENDIF","ENDM",NULL,
/*63:_DEREFI*/          "LOCAL i, ptrstr, typestr, argstr","argstr TEXTEQU < >","i = 0","FOR dref, <argsAndRefs>","IF i LT argCount OR argCount EQ 0","IF argCount GT 0", "argstr CATSTR argstr, <,>, <&dref&>","ENDIF", "ELSE","IF(i - argCount) MOD 2 EQ 0","ptrstr TEXTEQU <&dref&>","ELSE","typestr TEXTEQU <&dref&>","IF(i - argCount) EQ 1","% IF(OPATTR(ptrstr)) EQ 0x30","% mov rcx, &ptrstr&","ELSE","% mov rcx, @CatStr(<[>, <&ptrstr&>, <].>, <&typestr&>)","ENDIF","ELSE","% mov rcx, @CatStr(<[>, <&ptrstr&>, <].>, <&typestr&>)","ENDIF","ENDIF","ENDIF","i = i + 1","ENDM","% IF @SizeStr(<%&argstr&>) GT 3", "argstr SUBSTR argstr, 3", "ENDIF", "IFNB <argstr>", "InterfacePtr TEXTEQU <_>","InterfacePtr CATSTR InterfacePtr, <&itype>, <_>, <&proc>, <Pto>","IF(OPATTR(rcx)) AND 00010000b","% IFNB <&argstr&>", "IF @Platform EQ 1","mov r15,[rcx]","% invoke(InterfacePtr PTR[r15].&itype&vtbl.&proc), rcx, &argstr","ELSE","mov rdi,rcx","mov r15,[rdi]","% invoke(InterfacePtr PTR[r15].&itype&vtbl.&proc), rdi, &argstr","ENDIF","ELSE","IF @Platform EQ 1","mov r15,[rcx]","% invoke(InterfacePtr PTR[r15].&itype&vtbl.&proc), rcx","ELSE","mov rdi,rcx","mov r15,[rdi]","% invoke(InterfacePtr PTR[r15].&itype&vtbl.&proc), rdi","ENDIF","ENDIF","ELSE","mov r15,[rcx]","% IFNB <&argstr&>","FOR arg, <argstr>","IFIDNI <&arg>, <rcx>",".ERR <rcx is not allowed as a Method parameter with indirect object label>","ENDIF","ENDM","% invoke(InterfacePtr PTR[r15].&itype&vtbl.&proc), rcx, &argstr","ELSE","invoke(InterfacePtr PTR[r15].&itype&vtbl.&proc), rcx","ENDIF","ENDIF","ELSE","InterfacePtr TEXTEQU <_>","InterfacePtr CATSTR InterfacePtr, <&itype>, <_>, <&proc>, <Pto>","IF(OPATTR(rcx)) AND 00010000b","% IFNB <&argstr&>","IF @Platform EQ 1","mov r15,[rcx]","% invoke(InterfacePtr PTR[r15].&itype&vtbl.&proc), rcx, &argstr","ELSE","mov rdi,rcx","mov r15,[rdi]","% invoke(InterfacePtr PTR[r15].&itype&vtbl.&proc), rdi, &argstr","ENDIF","ELSE","IF @Platform EQ 1","mov r15,[rcx]","invoke(InterfacePtr PTR[r15].&itype&vtbl.&proc), rcx","ELSE","mov rdi,rcx","mov r15,[rdi]","invoke(InterfacePtr PTR[r15].&itype&vtbl.&proc), rdi","ENDIF","ENDIF","ELSE","mov r15,[rcx]","% IFNB <&argstr&>","FOR arg, <argstr>","IFIDNI <&arg>, <rcx>",".ERR <rcx is not allowed as a Method parameter with indirect object label>","ENDIF","ENDM","% invoke(InterfacePtr PTR[r15].&itype&vtbl.&proc), rcx, &argstr","ELSE","invoke(InterfacePtr PTR[r15].&itype&vtbl.&proc), rcx","ENDIF","ENDIF","ENDIF","IF @LastReturnType EQ 0","EXITM <al>","ELSEIF @LastReturnType EQ 0x40","EXITM <al>","ELSEIF @LastReturnType EQ 1","EXITM <ax>","ELSEIF @LastReturnType EQ 0x41","EXITM <ax>","ELSEIF @LastReturnType EQ 2","EXITM <eax>","ELSEIF @LastReturnType EQ 0x42","EXITM <eax>","ELSEIF @LastReturnType EQ 3","EXITM <rax>","ELSEIF @LastReturnType EQ 0x43","EXITM <rax>","ELSEIF @LastReturnType EQ 0xc3","EXITM <rax>","ELSEIF @LastReturnType EQ 6","EXITM <xmm0>","ELSEIF @LastReturnType EQ 7","EXITM <ymm0>","ELSEIF @LastReturnType EQ 8","EXITM <zmm0>","ELSEIF @LastReturnType EQ 0x22","EXITM <xmm0>","ELSEIF @LastReturnType EQ 0x23","EXITM <xmm0>","ELSE","EXITM <eax>","ENDIF","ENDM",NULL
    };	
