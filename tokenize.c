    ...
    checkifvar:
        ...
        *p1 = '\0';                  /* make it C string */
        ...
        else
        {
            ...
            c = *p1++ | 0x20;
            if (c != 'k')
            {           /* if it is not the mask register throw an error */
                EmitError(TOO_MANY_DECORATORS);
                return;
            }
            else
    ...
    for (;; )
    {
        ...
        if (extraflags.evex)
        {
            ...
            else if (*p.input == 'v' || *p.input == 'V')
            {
                ...
                if (*p1 != 0 && *p1 != ';')
                {                        /* if 'bcst' present p1 is pointing to "bcst" */
                    ...
                    if (*p1 == 'd' || *p1 == 'D')
                        ...
                        goto nobcst;                       /* let parser throw error */
                    if (*input1 != '\[')
                    {               /* if address is not inside [] */
                        *p1++ = '\[';                      /* force it   */
                        for (; *input1 > ','; p1++, input1++)
                        {  /* input1 points to [address] */
                            if (*input1 == ';')
                                break;
