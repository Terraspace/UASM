    ...
    for (; i < Token_Count; i++)
    {
        ...
        switch (type->init)
        {
            ...
            case INIT_CHAR:
                ...
                if (Options.output_format == OFORMAT_OMF || (ModuleInfo.flat == FALSE && Options.output_format == OFORMAT_BIN && ModuleInfo.sub_format != SFORMAT_PE))
                {
                    EmitErr(NOT_SUPPORTED_WITH_CURR_FORMAT, tokenarray[i].string_ptr);
                }
                else
