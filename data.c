...
extern void OutputBytes(const unsigned char* pbytes, int len, struct fixup* fixup);
...
/****************************************************************************/
{
    ...
    else
    {
        ...
        if (name)
        {
            ...
            if (Parse_Pass == PASS_1)
            {
                if (sym->state == SYM_EXTERNAL && sym->weak == TRUE && sym->isproc == FALSE)
                { /* EXTERNDEF? */
