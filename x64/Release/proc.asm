; Listing generated by Microsoft (R) Optimizing Compiler Version 19.00.23026.0 

include listing.inc

INCLUDELIB LIBCMT
INCLUDELIB OLDNAMES

PUBLIC	stackreg
_DATA	SEGMENT
COMM	ProcStatus:DWORD
COMM	StackAdj:DWORD
COMM	StackAdjHigh:DWORD
_DATA	ENDS
_BSS	SEGMENT
$SG12515 DB	01H DUP (?)
$SG11584 DB	01H DUP (?)
$SG11614 DB	01H DUP (?)
$SG11617 DB	01H DUP (?)
$SG11636 DB	01H DUP (?)
$SG11758 DB	01H DUP (?)
_BSS	ENDS
_DATA	SEGMENT
COMM	decoflags:BYTE
COMM	broadflags:BYTE
COMM	evex:BYTE
COMM	ZEROLOCALS:BYTE
_DATA	ENDS
_BSS	SEGMENT
$SG12092 DB	01H DUP (?)
_BSS	ENDS
_DATA	SEGMENT
COMM	CurrProc:QWORD
COMM	sym_ReservedStack:QWORD
COMM	procidx:DWORD
_DATA	ENDS
CONST	SEGMENT
fastcall_tab DQ	FLAT:ms32_pcheck
	DQ	FLAT:ms32_return
	DQ	FLAT:watc_pcheck
	DQ	FLAT:watc_return
	DQ	FLAT:ms64_pcheck
	DQ	FLAT:ms64_return
stackreg DD	0dH
	DD	015H
	DD	077H
	ORG $+4
fmtstk0	DQ	FLAT:$SG11314
	DQ	FLAT:$SG11315
fmtstk1	DQ	FLAT:$SG11317
	DQ	FLAT:$SG11318
CONST	ENDS
_DATA	SEGMENT
$SG12297 DB	'push %r', 00H
$SG12298 DB	'mov %r, %r', 00H
	ORG $+5
$SG12302 DB	'push %r', 00H
$SG12303 DB	'sub %r, %d + %s', 00H
$SG12308 DB	'add %r, %d', 00H
	ORG $+1
$SG11360 DB	'::', 00H
	ORG $+1
$SG12309 DB	'sub %r, %d', 00H
	ORG $+1
$SG11917 DB	'dw', 00H
	ORG $+1
$SG12311 DB	'push %r', 00H
$SG12312 DB	'mov %r, %s', 00H
	ORG $+1
$SG11919 DB	',', 00H
	ORG $+2
$SG12313 DB	'mov %r, %r', 00H
	ORG $+1
$SG11922 DB	'dw', 00H
	ORG $+1
$SG12315 DB	'push %r', 00H
$SG11314 DB	'sub %r, %d', 00H
	ORG $+1
$SG11315 DB	'%r %d', 00H
	ORG $+2
$SG11363 DB	'%s::%s', 00H
	ORG $+1
$SG12271 DB	'%r', 00H
	ORG $+1
$SG11317 DB	'sub %r, %d + %s', 00H
$SG11318 DB	'%r %d + %s', 00H
	ORG $+1
$SG11372 DB	'%d%c', 00H
	ORG $+3
$SG12408 DB	'pop %r', 00H
	ORG $+1
$SG12410 DB	'pop %r', 00H
	ORG $+1
$SG11390 DB	'%d%c', 00H
	ORG $+3
$SG12459 DB	'pop %r', 00H
	ORG $+5
$SG12438 DB	'vmovdqa %r, [%r + %u + %s]', 00H
	ORG $+5
$SG12439 DB	'vmovdqa %r, [%r + %u]', 00H
	ORG $+2
$SG12445 DB	'add %r, %d + %s', 00H
$SG12446 DB	'add %r, %d + %s', 00H
$SG12447 DB	'add %r, %d', 00H
	ORG $+5
$SG12456 DB	'mov %r, [%r+%u]', 00H
$SG12469 DB	'add %r, %d + %s', 00H
$SG12471 DB	'pop %r', 00H
	ORG $+1
$SG12477 DB	'leave', 00H
	ORG $+2
$SG12482 DB	'add %r, %d', 00H
	ORG $+5
$SG12484 DB	'mov %r, %r', 00H
	ORG $+1
$SG12485 DB	'pop %r', 00H
	ORG $+5
$SG12516 DB	'%s, 0%XH, 0%XH, 0%XH, <<%s>>, <%s>', 00H
	ORG $+5
$SG12518 DB	'option epilogue:none', 0aH, 00H
	ORG $+2
$SG12549 DB	'%d%c', 00H
	ORG $+3
$SG12553 DB	'%d%c', 00H
	ORG $+3
$SG11712 DB	'PRIVATE', 00H
$SG11717 DB	'PUBLIC', 00H
	ORG $+1
$SG11719 DB	'EXPORT', 00H
	ORG $+1
$SG11731 DB	'FORCEFRAME', 00H
	ORG $+1
$SG11734 DB	'LOADDS', 00H
	ORG $+1
$SG11747 DB	'USES', 00H
	ORG $+3
$SG11906 DB	'.xdata', 00H
	ORG $+1
$SG11911 DB	'%s %r', 00H
	ORG $+6
$SG11912 DB	'%s %r align(%u) flat read ''DATA''', 00H
	ORG $+7
$SG11913 DB	'$xdatasym label near', 00H
	ORG $+3
$SG11915 DB	'db %ut + (0%xh shl 3), %ut, %ut, 0%xh + (0%xh shl 4)', 00H
	ORG $+3
$SG11918 DB	'%s 0%xh', 00H
$SG11923 DB	'%r 4', 00H
	ORG $+3
$SG11925 DB	'dd %r %s', 00H
	ORG $+3
$SG11926 DB	'%r 8', 00H
	ORG $+3
$SG11927 DB	'%s %r', 00H
	ORG $+2
$SG11930 DB	'.pdata', 00H
	ORG $+5
$SG11931 DB	'.pdata$%04u', 00H
$SG11934 DB	'%s %r', 00H
	ORG $+6
$SG11935 DB	'%s %r align(%u) flat read ''DATA''', 00H
	ORG $+7
$SG11936 DB	'dd %r %s, %r %s+0%xh, %r $xdatasym+0%xh', 00H
$SG11937 DB	'%s %r', 00H
	ORG $+2
$SG12023 DB	'CODE', 00H
	ORG $+3
$SG12089 DB	'option prologue:none', 0aH, 00H
	ORG $+2
$SG12093 DB	' (%s, 0%XH, 0%XH, 0%XH, <<%s>>, <%s>)', 00H
	ORG $+2
$SG12116 DB	'movq [%r+%u], %r', 00H
	ORG $+7
$SG12118 DB	'mov [%r+%u], %r', 00H
$SG12119 DB	'mov [%r+%u], %r', 00H
$SG12124 DB	'movq [%r+%u], %r', 00H
	ORG $+7
$SG12125 DB	'mov [%r+%u], %r', 00H
$SG12126 DB	'mov [%r+%u], %r', 00H
$SG12186 DB	'mov [%r+%u], %r', 00H
$SG12187 DB	'%r %r, %u', 00H
	ORG $+6
$SG12216 DB	'push %r', 00H
$SG12217 DB	'%r %r', 00H
	ORG $+2
$SG12218 DB	'mov %r, %r', 00H
	ORG $+5
$SG12219 DB	'%r %r, 0', 00H
	ORG $+7
$SG12227 DB	'push %r', 00H
$SG12229 DB	'%r %r', 00H
	ORG $+2
$SG12233 DB	'push %r', 00H
$SG12235 DB	'%r %r', 00H
	ORG $+2
$SG12248 DB	'mov %r, %u', 00H
	ORG $+1
$SG12249 DB	'dec %r', 00H
	ORG $+5
$SG12250 DB	'mov byte ptr [%r + %r], 0', 00H
	ORG $+6
$SG12251 DB	'dw 0F875h', 00H
	ORG $+6
$SG12252 DB	'mov [%r], %r', 00H
	ORG $+3
$SG12253 DB	'mov [%r+8], %r', 00H
	ORG $+1
$SG12254 DB	'xor %r, %r', 00H
	ORG $+5
$SG12255 DB	'mov %r, %u', 00H
	ORG $+5
$SG12256 DB	'mov %r, %r', 00H
	ORG $+5
$SG12257 DB	'add %r, %u', 00H
	ORG $+5
$SG12258 DB	'rep stosb', 00H
	ORG $+6
$SG12259 DB	'mov %r,[%r]', 00H
	ORG $+4
$SG12260 DB	'mov %r,[%r+8]', 00H
	ORG $+2
$SG12265 DB	'vmovdqa [%r+%u+%s], %r', 00H
	ORG $+1
$SG12267 DB	'%r %r, %u+%s', 00H
	ORG $+3
$SG12268 DB	'vmovdqa [%r+%u], %r', 00H
	ORG $+4
$SG12270 DB	'%r %r, %u', 00H
_DATA	ENDS
PUBLIC	__local_stdio_printf_options
PUBLIC	printf
PUBLIC	sprintf
PUBLIC	ParseProc
PUBLIC	CreateProc
PUBLIC	DeleteProc
PUBLIC	CopyPrototype
PUBLIC	RetInstr
PUBLIC	write_prologue
PUBLIC	ProcInit
PUBLIC	ProcCheckOpen
PUBLIC	LocalDir
PUBLIC	UpdateStackBase
PUBLIC	UpdateProcStatus
PUBLIC	ProcDir
PUBLIC	EndpDir
PUBLIC	ExcFrameDirective
EXTRN	__acrt_iob_func:PROC
EXTRN	__stdio_common_vfprintf:PROC
EXTRN	__stdio_common_vsprintf:PROC
EXTRN	atoi:PROC
EXTRN	_stricmp:PROC
EXTRN	EmitError:PROC
EXTRN	EmitErr:PROC
EXTRN	EmitWarn:PROC
EXTRN	AddLinnumDataRef:PROC
EXTRN	LclAlloc:PROC
EXTRN	SymAlloc:PROC
EXTRN	SymFree:PROC
EXTRN	SymCreate:PROC
EXTRN	SymLCreate:PROC
EXTRN	SymAddLocal:PROC
EXTRN	SymFind:PROC
EXTRN	SymClearLocal:PROC
EXTRN	SymSetLocal:PROC
EXTRN	SymGetLocal:PROC
EXTRN	SizeFromMemtype:PROC
EXTRN	SizeFromRegister:PROC
EXTRN	GetLangType:PROC
EXTRN	sym_add_table:PROC
EXTRN	sym_remove_table:PROC
EXTRN	sym_ext2int:PROC
EXTRN	ParseLine:PROC
EXTRN	SetSymSegOfs:PROC
EXTRN	GetSymOfssize:PROC
EXTRN	GetCurrOffset:PROC
EXTRN	GetSegIdx:PROC
EXTRN	SimGetSegName:PROC
EXTRN	AddPublicData:PROC
EXTRN	BackPatch:PROC
EXTRN	GetLineNumber:PROC
EXTRN	get_curr_srcfile:PROC
EXTRN	AddLineQueue:PROC
EXTRN	AddLineQueueX:PROC
EXTRN	RunLineQueue:PROC
EXTRN	Tokenize:PROC
EXTRN	EvalOperand:PROC
EXTRN	EmitConstError:PROC
EXTRN	GetQualifiedType:PROC
EXTRN	RunMacro:PROC
EXTRN	SkipSavedState:PROC
EXTRN	LstWrite:PROC
EXTRN	LstSetPosition:PROC
EXTRN	AlignCurrOffset:PROC
EXTRN	GetResWName:PROC
EXTRN	Options:BYTE
EXTRN	ModuleInfo:BYTE
EXTRN	Parse_Pass:DWORD
EXTRN	SymCmpFunc:QWORD
EXTRN	SymTables:BYTE
EXTRN	SpecialTable:BYTE
EXTRN	LineStoreCurr:QWORD
EXTRN	UseSavedState:BYTE
EXTRN	szDgroup:BYTE
EXTRN	list_pos:DWORD
EXTRN	__ImageBase:BYTE
_DATA	SEGMENT
COMM	?_OptionsStorage@?1??__local_stdio_printf_options@@9@9:QWORD							; `__local_stdio_printf_options'::`2'::_OptionsStorage
_DATA	ENDS
_BSS	SEGMENT
	ALIGN	8

ProcStack DQ	01H DUP (?)
endprolog_found DB 01H DUP (?)
	ALIGN	4

unw_segs_defined DB 01H DUP (?)
	ALIGN	4

unw_info DD	01H DUP (?)
unw_code DW	0102H DUP (?)
?regname@?1??watc_pcheck@@9@9 DB 040H DUP (?)		; `watc_pcheck'::`2'::regname
?regist@?1??watc_pcheck@@9@9 DB 020H DUP (?)		; `watc_pcheck'::`2'::regist
_BSS	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$_vfprintf_l DD imagerel $LN4
	DD	imagerel $LN4+80
	DD	imagerel $unwind$_vfprintf_l
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$printf DD imagerel $LN6
	DD	imagerel $LN6+83
	DD	imagerel $unwind$printf
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$_vsnprintf_l DD imagerel $LN4
	DD	imagerel $LN4+102
	DD	imagerel $unwind$_vsnprintf_l
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$_vsprintf_l DD imagerel $LN6
	DD	imagerel $LN6+98
	DD	imagerel $unwind$_vsprintf_l
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$sprintf DD imagerel $LN8
	DD	imagerel $LN8+90
	DD	imagerel $unwind$sprintf
pdata	ENDS
pdata	SEGMENT
$pdata$ParseProc DD imagerel $LN128
	DD	imagerel $LN128+1925
	DD	imagerel $unwind$ParseProc
$pdata$CreateProc DD imagerel $LN22
	DD	imagerel $LN22+302
	DD	imagerel $unwind$CreateProc
$pdata$DeleteProc DD imagerel $LN15
	DD	imagerel $LN15+23
	DD	imagerel $unwind$DeleteProc
$pdata$0$DeleteProc DD imagerel $LN15+23
	DD	imagerel $LN15+54
	DD	imagerel $chain$0$DeleteProc
$pdata$1$DeleteProc DD imagerel $LN15+54
	DD	imagerel $LN15+59
	DD	imagerel $chain$1$DeleteProc
$pdata$CopyPrototype DD imagerel $LN21
	DD	imagerel $LN21+302
	DD	imagerel $unwind$CopyPrototype
$pdata$RetInstr DD imagerel $LN36
	DD	imagerel $LN36+573
	DD	imagerel $unwind$RetInstr
$pdata$write_prologue DD imagerel $LN14
	DD	imagerel $LN14+198
	DD	imagerel $unwind$write_prologue
$pdata$ProcCheckOpen DD imagerel $LN60
	DD	imagerel $LN60+20
	DD	imagerel $unwind$ProcCheckOpen
$pdata$2$ProcCheckOpen DD imagerel $LN60+20
	DD	imagerel $LN60+544
	DD	imagerel $chain$2$ProcCheckOpen
$pdata$3$ProcCheckOpen DD imagerel $LN60+544
	DD	imagerel $LN60+549
	DD	imagerel $chain$3$ProcCheckOpen
$pdata$ms32_pcheck DD imagerel ms32_pcheck
	DD	imagerel ms32_pcheck+240
	DD	imagerel $unwind$ms32_pcheck
$pdata$watc_pcheck DD imagerel watc_pcheck
	DD	imagerel watc_pcheck+73
	DD	imagerel $unwind$watc_pcheck
$pdata$0$watc_pcheck DD imagerel watc_pcheck+73
	DD	imagerel watc_pcheck+211
	DD	imagerel $chain$0$watc_pcheck
$pdata$2$watc_pcheck DD imagerel watc_pcheck+211
	DD	imagerel watc_pcheck+234
	DD	imagerel $chain$2$watc_pcheck
$pdata$6$watc_pcheck DD imagerel watc_pcheck+234
	DD	imagerel watc_pcheck+255
	DD	imagerel $chain$6$watc_pcheck
$pdata$7$watc_pcheck DD imagerel watc_pcheck+255
	DD	imagerel watc_pcheck+560
	DD	imagerel $chain$7$watc_pcheck
$pdata$8$watc_pcheck DD imagerel watc_pcheck+560
	DD	imagerel watc_pcheck+672
	DD	imagerel $chain$8$watc_pcheck
$pdata$9$watc_pcheck DD imagerel watc_pcheck+672
	DD	imagerel watc_pcheck+715
	DD	imagerel $chain$9$watc_pcheck
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$pushitem DD imagerel pushitem
	DD	imagerel pushitem+50
	DD	imagerel $unwind$pushitem
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$push_proc DD imagerel push_proc
	DD	imagerel push_proc+60
	DD	imagerel $unwind$push_proc
pdata	ENDS
pdata	SEGMENT
$pdata$LocalDir DD imagerel $LN66
	DD	imagerel $LN66+74
	DD	imagerel $unwind$LocalDir
$pdata$2$LocalDir DD imagerel $LN66+74
	DD	imagerel $LN66+759
	DD	imagerel $chain$2$LocalDir
$pdata$4$LocalDir DD imagerel $LN66+759
	DD	imagerel $LN66+778
	DD	imagerel $chain$4$LocalDir
$pdata$5$LocalDir DD imagerel $LN66+778
	DD	imagerel $LN66+798
	DD	imagerel $chain$5$LocalDir
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$ParseParams DD imagerel ParseParams
	DD	imagerel ParseParams+100
	DD	imagerel $unwind$ParseParams
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$1$ParseParams DD imagerel ParseParams+100
	DD	imagerel ParseParams+157
	DD	imagerel $chain$1$ParseParams
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$2$ParseParams DD imagerel ParseParams+157
	DD	imagerel ParseParams+190
	DD	imagerel $chain$2$ParseParams
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$3$ParseParams DD imagerel ParseParams+190
	DD	imagerel ParseParams+1640
	DD	imagerel $chain$3$ParseParams
pdata	ENDS
pdata	SEGMENT
$pdata$ProcDir DD imagerel $LN47
	DD	imagerel $LN47+66
	DD	imagerel $unwind$ProcDir
$pdata$1$ProcDir DD imagerel $LN47+66
	DD	imagerel $LN47+205
	DD	imagerel $chain$1$ProcDir
$pdata$3$ProcDir DD imagerel $LN47+205
	DD	imagerel $LN47+329
	DD	imagerel $chain$3$ProcDir
$pdata$4$ProcDir DD imagerel $LN47+329
	DD	imagerel $LN47+357
	DD	imagerel $chain$4$ProcDir
$pdata$6$ProcDir DD imagerel $LN47+357
	DD	imagerel $LN47+982
	DD	imagerel $chain$6$ProcDir
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$WriteSEHData DD imagerel WriteSEHData
	DD	imagerel WriteSEHData+34
	DD	imagerel $unwind$WriteSEHData
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$1$WriteSEHData DD imagerel WriteSEHData+34
	DD	imagerel WriteSEHData+232
	DD	imagerel $chain$1$WriteSEHData
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$2$WriteSEHData DD imagerel WriteSEHData+232
	DD	imagerel WriteSEHData+394
	DD	imagerel $chain$2$WriteSEHData
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$3$WriteSEHData DD imagerel WriteSEHData+394
	DD	imagerel WriteSEHData+790
	DD	imagerel $chain$3$WriteSEHData
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$4$WriteSEHData DD imagerel WriteSEHData+790
	DD	imagerel WriteSEHData+799
	DD	imagerel $chain$4$WriteSEHData
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$SetLocalOffsets DD imagerel SetLocalOffsets
	DD	imagerel SetLocalOffsets+21
	DD	imagerel $unwind$SetLocalOffsets
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$2$SetLocalOffsets DD imagerel SetLocalOffsets+21
	DD	imagerel SetLocalOffsets+140
	DD	imagerel $chain$2$SetLocalOffsets
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$3$SetLocalOffsets DD imagerel SetLocalOffsets+140
	DD	imagerel SetLocalOffsets+196
	DD	imagerel $chain$3$SetLocalOffsets
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$4$SetLocalOffsets DD imagerel SetLocalOffsets+196
	DD	imagerel SetLocalOffsets+337
	DD	imagerel $chain$4$SetLocalOffsets
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$5$SetLocalOffsets DD imagerel SetLocalOffsets+337
	DD	imagerel SetLocalOffsets+488
	DD	imagerel $chain$5$SetLocalOffsets
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$6$SetLocalOffsets DD imagerel SetLocalOffsets+488
	DD	imagerel SetLocalOffsets+745
	DD	imagerel $chain$6$SetLocalOffsets
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$ProcFini DD imagerel ProcFini
	DD	imagerel ProcFini+11
	DD	imagerel $unwind$ProcFini
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$0$ProcFini DD imagerel ProcFini+11
	DD	imagerel ProcFini+92
	DD	imagerel $chain$0$ProcFini
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$1$ProcFini DD imagerel ProcFini+92
	DD	imagerel ProcFini+199
	DD	imagerel $chain$1$ProcFini
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$2$ProcFini DD imagerel ProcFini+199
	DD	imagerel ProcFini+363
	DD	imagerel $chain$2$ProcFini
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$3$ProcFini DD imagerel ProcFini+363
	DD	imagerel ProcFini+498
	DD	imagerel $chain$3$ProcFini
pdata	ENDS
pdata	SEGMENT
$pdata$EndpDir DD imagerel $LN9
	DD	imagerel $LN9+125
	DD	imagerel $unwind$EndpDir
$pdata$ExcFrameDirective DD imagerel $LN81
	DD	imagerel $LN81+162
	DD	imagerel $unwind$ExcFrameDirective
$pdata$4$ExcFrameDirective DD imagerel $LN81+162
	DD	imagerel $LN81+943
	DD	imagerel $chain$4$ExcFrameDirective
$pdata$6$ExcFrameDirective DD imagerel $LN81+943
	DD	imagerel $LN81+1496
	DD	imagerel $chain$6$ExcFrameDirective
$pdata$7$ExcFrameDirective DD imagerel $LN81+1496
	DD	imagerel $LN81+1552
	DD	imagerel $chain$7$ExcFrameDirective
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$write_userdef_prologue DD imagerel write_userdef_prologue
	DD	imagerel write_userdef_prologue+220
	DD	imagerel $unwind$write_userdef_prologue
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$4$write_userdef_prologue DD imagerel write_userdef_prologue+220
	DD	imagerel write_userdef_prologue+263
	DD	imagerel $chain$4$write_userdef_prologue
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$5$write_userdef_prologue DD imagerel write_userdef_prologue+263
	DD	imagerel write_userdef_prologue+344
	DD	imagerel $chain$5$write_userdef_prologue
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$6$write_userdef_prologue DD imagerel write_userdef_prologue+344
	DD	imagerel write_userdef_prologue+538
	DD	imagerel $chain$6$write_userdef_prologue
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$7$write_userdef_prologue DD imagerel write_userdef_prologue+538
	DD	imagerel write_userdef_prologue+625
	DD	imagerel $chain$7$write_userdef_prologue
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$win64_SaveRegParams DD imagerel win64_SaveRegParams
	DD	imagerel win64_SaveRegParams+362
	DD	imagerel $unwind$win64_SaveRegParams
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$win64_StoreRegHome DD imagerel win64_StoreRegHome
	DD	imagerel win64_StoreRegHome+42
	DD	imagerel $unwind$win64_StoreRegHome
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$1$win64_StoreRegHome DD imagerel win64_StoreRegHome+42
	DD	imagerel win64_StoreRegHome+365
	DD	imagerel $chain$1$win64_StoreRegHome
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$2$win64_StoreRegHome DD imagerel win64_StoreRegHome+365
	DD	imagerel win64_StoreRegHome+490
	DD	imagerel $chain$2$win64_StoreRegHome
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$3$win64_StoreRegHome DD imagerel win64_StoreRegHome+490
	DD	imagerel win64_StoreRegHome+500
	DD	imagerel $chain$3$win64_StoreRegHome
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$4$win64_StoreRegHome DD imagerel win64_StoreRegHome+500
	DD	imagerel win64_StoreRegHome+509
	DD	imagerel $chain$4$win64_StoreRegHome
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$write_win64_default_prologue DD imagerel write_win64_default_prologue
	DD	imagerel write_win64_default_prologue+10
	DD	imagerel $unwind$write_win64_default_prologue
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$3$write_win64_default_prologue DD imagerel write_win64_default_prologue+10
	DD	imagerel write_win64_default_prologue+212
	DD	imagerel $chain$3$write_win64_default_prologue
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$4$write_win64_default_prologue DD imagerel write_win64_default_prologue+212
	DD	imagerel write_win64_default_prologue+486
	DD	imagerel $chain$4$write_win64_default_prologue
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$5$write_win64_default_prologue DD imagerel write_win64_default_prologue+486
	DD	imagerel write_win64_default_prologue+545
	DD	imagerel $chain$5$write_win64_default_prologue
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$6$write_win64_default_prologue DD imagerel write_win64_default_prologue+545
	DD	imagerel write_win64_default_prologue+1336
	DD	imagerel $chain$6$write_win64_default_prologue
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$write_default_prologue DD imagerel write_default_prologue
	DD	imagerel write_default_prologue+139
	DD	imagerel $unwind$write_default_prologue
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$0$write_default_prologue DD imagerel write_default_prologue+139
	DD	imagerel write_default_prologue+147
	DD	imagerel $chain$0$write_default_prologue
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$1$write_default_prologue DD imagerel write_default_prologue+147
	DD	imagerel write_default_prologue+429
	DD	imagerel $chain$1$write_default_prologue
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$2$write_default_prologue DD imagerel write_default_prologue+429
	DD	imagerel write_default_prologue+557
	DD	imagerel $chain$2$write_default_prologue
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$3$write_default_prologue DD imagerel write_default_prologue+557
	DD	imagerel write_default_prologue+691
	DD	imagerel $chain$3$write_default_prologue
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$pop_register DD imagerel pop_register
	DD	imagerel pop_register+20
	DD	imagerel $unwind$pop_register
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$2$pop_register DD imagerel pop_register+20
	DD	imagerel pop_register+140
	DD	imagerel $chain$2$pop_register
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$4$pop_register DD imagerel pop_register+140
	DD	imagerel pop_register+211
	DD	imagerel $chain$4$pop_register
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$5$pop_register DD imagerel pop_register+211
	DD	imagerel pop_register+216
	DD	imagerel $chain$5$pop_register
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$write_win64_default_epilogue DD imagerel write_win64_default_epilogue
	DD	imagerel write_win64_default_epilogue+6
	DD	imagerel $unwind$write_win64_default_epilogue
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$4$write_win64_default_epilogue DD imagerel write_win64_default_epilogue+6
	DD	imagerel write_win64_default_epilogue+611
	DD	imagerel $chain$4$write_win64_default_epilogue
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$5$write_win64_default_epilogue DD imagerel write_win64_default_epilogue+611
	DD	imagerel write_win64_default_epilogue+652
	DD	imagerel $chain$5$write_win64_default_epilogue
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$write_default_epilogue DD imagerel write_default_epilogue
	DD	imagerel write_default_epilogue+393
	DD	imagerel $unwind$write_default_epilogue
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$write_userdef_epilogue DD imagerel write_userdef_epilogue
	DD	imagerel write_userdef_epilogue+81
	DD	imagerel $unwind$write_userdef_epilogue
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$0$write_userdef_epilogue DD imagerel write_userdef_epilogue+81
	DD	imagerel write_userdef_epilogue+92
	DD	imagerel $chain$0$write_userdef_epilogue
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$1$write_userdef_epilogue DD imagerel write_userdef_epilogue+92
	DD	imagerel write_userdef_epilogue+211
	DD	imagerel $chain$1$write_userdef_epilogue
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$3$write_userdef_epilogue DD imagerel write_userdef_epilogue+211
	DD	imagerel write_userdef_epilogue+304
	DD	imagerel $chain$3$write_userdef_epilogue
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$4$write_userdef_epilogue DD imagerel write_userdef_epilogue+304
	DD	imagerel write_userdef_epilogue+438
	DD	imagerel $chain$4$write_userdef_epilogue
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$5$write_userdef_epilogue DD imagerel write_userdef_epilogue+438
	DD	imagerel write_userdef_epilogue+515
	DD	imagerel $chain$5$write_userdef_epilogue
pdata	ENDS
;	COMDAT pdata
pdata	SEGMENT
$pdata$6$write_userdef_epilogue DD imagerel write_userdef_epilogue+515
	DD	imagerel write_userdef_epilogue+546
	DD	imagerel $chain$6$write_userdef_epilogue
pdata	ENDS
CONST	SEGMENT
ms32_regs16 DD	09H
	DD	0bH
	DD	0cH
	ORG $+4
ms32_regs32 DD	012H
	DD	013H
ms32_maxreg DD	03H
	DD	02H
watc_regs8 DD	01H
	DD	03H
	DD	04H
	DD	02H
watc_regs16 DD	09H
	DD	0bH
	DD	0cH
	DD	0aH
watc_regs32 DD	011H
	DD	013H
	DD	014H
	DD	012H
watc_regs_qw DD	09H
	DD	0cH
	DD	0aH
	DD	0bH
ms64_regs DD	074H
	DD	075H
	DD	07bH
	DD	07cH
CONST	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$6$write_userdef_epilogue DD 021H
	DD	imagerel write_userdef_epilogue
	DD	imagerel write_userdef_epilogue+81
	DD	imagerel $unwind$write_userdef_epilogue
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$5$write_userdef_epilogue DD 021H
	DD	imagerel write_userdef_epilogue+81
	DD	imagerel write_userdef_epilogue+92
	DD	imagerel $chain$0$write_userdef_epilogue
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$4$write_userdef_epilogue DD 021H
	DD	imagerel write_userdef_epilogue+92
	DD	imagerel write_userdef_epilogue+211
	DD	imagerel $chain$1$write_userdef_epilogue
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$3$write_userdef_epilogue DD 041021H
	DD	06b7410H
	DD	06a6408H
	DD	imagerel write_userdef_epilogue+92
	DD	imagerel write_userdef_epilogue+211
	DD	imagerel $chain$1$write_userdef_epilogue
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$1$write_userdef_epilogue DD 020821H
	DD	064f408H
	DD	imagerel write_userdef_epilogue+81
	DD	imagerel write_userdef_epilogue+92
	DD	imagerel $chain$0$write_userdef_epilogue
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$0$write_userdef_epilogue DD 020821H
	DD	06dd408H
	DD	imagerel write_userdef_epilogue
	DD	imagerel write_userdef_epilogue+81
	DD	imagerel $unwind$write_userdef_epilogue
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$write_userdef_epilogue DD 060e01H
	DD	065010eH
	DD	0c005e007H
	DD	030025003H
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$write_default_epilogue DD 040a01H
	DD	06340aH
	DD	07006320aH
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$5$write_win64_default_epilogue DD 021H
	DD	imagerel write_win64_default_epilogue
	DD	imagerel write_win64_default_epilogue+6
	DD	imagerel $unwind$write_win64_default_epilogue
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$4$write_win64_default_epilogue DD 0a2121H
	DD	06f421H
	DD	077419H
	DD	0c6414H
	DD	0b540dH
	DD	0a3405H
	DD	imagerel write_win64_default_epilogue
	DD	imagerel write_win64_default_epilogue+6
	DD	imagerel $unwind$write_win64_default_epilogue
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$write_win64_default_epilogue DD 020601H
	DD	0e0027206H
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$5$pop_register DD 021H
	DD	imagerel pop_register
	DD	imagerel pop_register+20
	DD	imagerel $unwind$pop_register
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$4$pop_register DD 060021H
	DD	047400H
	DD	076400H
	DD	063400H
	DD	imagerel pop_register
	DD	imagerel pop_register+20
	DD	imagerel $unwind$pop_register
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$2$pop_register DD 061221H
	DD	047412H
	DD	07640dH
	DD	063405H
	DD	imagerel pop_register
	DD	imagerel pop_register+20
	DD	imagerel $unwind$pop_register
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$pop_register DD 010401H
	DD	04204H
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$3$write_default_prologue DD 021H
	DD	imagerel write_default_prologue
	DD	imagerel write_default_prologue+139
	DD	imagerel $unwind$write_default_prologue
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$2$write_default_prologue DD 021H
	DD	imagerel write_default_prologue+139
	DD	imagerel write_default_prologue+147
	DD	imagerel $chain$0$write_default_prologue
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$1$write_default_prologue DD 020521H
	DD	07e405H
	DD	imagerel write_default_prologue+139
	DD	imagerel write_default_prologue+147
	DD	imagerel $chain$0$write_default_prologue
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$0$write_default_prologue DD 020521H
	DD	067405H
	DD	imagerel write_default_prologue
	DD	imagerel write_default_prologue+139
	DD	imagerel $unwind$write_default_prologue
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$write_default_prologue DD 040a01H
	DD	08340aH
	DD	06006320aH
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$6$write_win64_default_prologue DD 060021H
	DD	0e7400H
	DD	0d6400H
	DD	0c5400H
	DD	imagerel write_win64_default_prologue
	DD	imagerel write_win64_default_prologue+10
	DD	imagerel $unwind$write_win64_default_prologue
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$5$write_win64_default_prologue DD 021H
	DD	imagerel write_win64_default_prologue+10
	DD	imagerel write_win64_default_prologue+212
	DD	imagerel $chain$3$write_win64_default_prologue
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$4$write_win64_default_prologue DD 020521H
	DD	06e405H
	DD	imagerel write_win64_default_prologue+10
	DD	imagerel write_win64_default_prologue+212
	DD	imagerel $chain$3$write_win64_default_prologue
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$3$write_win64_default_prologue DD 08c721H
	DD	0e74c7H
	DD	0d64bfH
	DD	0c54baH
	DD	07d405H
	DD	imagerel write_win64_default_prologue
	DD	imagerel write_win64_default_prologue+10
	DD	imagerel $unwind$write_win64_default_prologue
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$write_win64_default_prologue DD 040a01H
	DD	0f006720aH
	DD	03002c004H
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$4$win64_StoreRegHome DD 021H
	DD	imagerel win64_StoreRegHome
	DD	imagerel win64_StoreRegHome+42
	DD	imagerel $unwind$win64_StoreRegHome
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$3$win64_StoreRegHome DD 021H
	DD	imagerel win64_StoreRegHome+42
	DD	imagerel win64_StoreRegHome+365
	DD	imagerel $chain$1$win64_StoreRegHome
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$2$win64_StoreRegHome DD 020521H
	DD	095405H
	DD	imagerel win64_StoreRegHome+42
	DD	imagerel win64_StoreRegHome+365
	DD	imagerel $chain$1$win64_StoreRegHome
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$1$win64_StoreRegHome DD 040a21H
	DD	0af40aH
	DD	083405H
	DD	imagerel win64_StoreRegHome
	DD	imagerel win64_StoreRegHome+42
	DD	imagerel $unwind$win64_StoreRegHome
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$win64_StoreRegHome DD 040901H
	DD	0e0053209H
	DD	060027003H
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$win64_SaveRegParams DD 0a1801H
	DD	0a6418H
	DD	095418H
	DD	083418H
	DD	0f0143218H
	DD	07010e012H
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$7$write_userdef_prologue DD 021H
	DD	imagerel write_userdef_prologue
	DD	imagerel write_userdef_prologue+220
	DD	imagerel $unwind$write_userdef_prologue
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$6$write_userdef_prologue DD 021H
	DD	imagerel write_userdef_prologue+220
	DD	imagerel write_userdef_prologue+263
	DD	imagerel $chain$4$write_userdef_prologue
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$5$write_userdef_prologue DD 020821H
	DD	06d7408H
	DD	imagerel write_userdef_prologue+220
	DD	imagerel write_userdef_prologue+263
	DD	imagerel $chain$4$write_userdef_prologue
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$4$write_userdef_prologue DD 041021H
	DD	06c6410H
	DD	06a3408H
	DD	imagerel write_userdef_prologue
	DD	imagerel write_userdef_prologue+220
	DD	imagerel $unwind$write_userdef_prologue
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$write_userdef_prologue DD 0a5901H
	DD	064f459H
	DD	065e44dH
	DD	066d442H
	DD	067010bH
	DD	05002c004H
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$3$ProcFini DD 021H
	DD	imagerel ProcFini
	DD	imagerel ProcFini+11
	DD	imagerel $unwind$ProcFini
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$2$ProcFini DD 021H
	DD	imagerel ProcFini+11
	DD	imagerel ProcFini+92
	DD	imagerel $chain$0$ProcFini
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$1$ProcFini DD 020521H
	DD	063405H
	DD	imagerel ProcFini+11
	DD	imagerel ProcFini+92
	DD	imagerel $chain$0$ProcFini
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$0$ProcFini DD 020521H
	DD	047405H
	DD	imagerel ProcFini
	DD	imagerel ProcFini+11
	DD	imagerel $unwind$ProcFini
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$ProcFini DD 010401H
	DD	04204H
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$6$SetLocalOffsets DD 021H
	DD	imagerel SetLocalOffsets
	DD	imagerel SetLocalOffsets+21
	DD	imagerel $unwind$SetLocalOffsets
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$5$SetLocalOffsets DD 020021H
	DD	086400H
	DD	imagerel SetLocalOffsets
	DD	imagerel SetLocalOffsets+21
	DD	imagerel $unwind$SetLocalOffsets
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$4$SetLocalOffsets DD 021H
	DD	imagerel SetLocalOffsets+21
	DD	imagerel SetLocalOffsets+140
	DD	imagerel $chain$2$SetLocalOffsets
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$3$SetLocalOffsets DD 020421H
	DD	0c404H
	DD	imagerel SetLocalOffsets+21
	DD	imagerel SetLocalOffsets+140
	DD	imagerel $chain$2$SetLocalOffsets
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$2$SetLocalOffsets DD 061e21H
	DD	01741eH
	DD	086416H
	DD	073405H
	DD	imagerel SetLocalOffsets
	DD	imagerel SetLocalOffsets+21
	DD	imagerel $unwind$SetLocalOffsets
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$SetLocalOffsets DD 040a01H
	DD	0f006120aH
	DD	05002e004H
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$4$WriteSEHData DD 021H
	DD	imagerel WriteSEHData
	DD	imagerel WriteSEHData+34
	DD	imagerel $unwind$WriteSEHData
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$3$WriteSEHData DD 021H
	DD	imagerel WriteSEHData+34
	DD	imagerel WriteSEHData+232
	DD	imagerel $chain$1$WriteSEHData
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$2$WriteSEHData DD 020821H
	DD	01f7408H
	DD	imagerel WriteSEHData+34
	DD	imagerel WriteSEHData+232
	DD	imagerel $chain$1$WriteSEHData
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$1$WriteSEHData DD 040821H
	DD	01e5408H
	DD	01c3404H
	DD	imagerel WriteSEHData
	DD	imagerel WriteSEHData+34
	DD	imagerel $unwind$WriteSEHData
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$WriteSEHData DD 030b01H
	DD	01a010bH
	DD	06004H
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$3$ParseParams DD 040021H
	DD	0bc400H
	DD	0c6400H
	DD	imagerel ParseParams
	DD	imagerel ParseParams+100
	DD	imagerel $unwind$ParseParams
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$2$ParseParams DD 021H
	DD	imagerel ParseParams
	DD	imagerel ParseParams+100
	DD	imagerel $unwind$ParseParams
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$chain$1$ParseParams DD 041421H
	DD	0bc414H
	DD	0c6405H
	DD	imagerel ParseParams
	DD	imagerel ParseParams+100
	DD	imagerel $unwind$ParseParams
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$ParseParams DD 071e01H
	DD	0f017c21eH
	DD	0d013e015H
	DD	030107011H
	DD	0500fH
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$push_proc DD 020601H
	DD	030023206H
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$pushitem DD 040a01H
	DD	06340aH
	DD	07006320aH
xdata	ENDS
xdata	SEGMENT
$unwind$ParseProc DD 0aea01H
	DD	0ee4eaH
	DD	0f013321aH
	DD	0c00fd011H
	DD	0600c700dH
	DD	0500a300bH
$unwind$CreateProc DD 060f01H
	DD	07640fH
	DD	06340fH
	DD	0700b320fH
$unwind$DeleteProc DD 010401H
	DD	04204H
$chain$0$DeleteProc DD 020521H
	DD	043405H
	DD	imagerel $LN15
	DD	imagerel $LN15+23
	DD	imagerel $unwind$DeleteProc
$chain$1$DeleteProc DD 021H
	DD	imagerel $LN15
	DD	imagerel $LN15+23
	DD	imagerel $unwind$DeleteProc
$unwind$CopyPrototype DD 062a01H
	DD	07642aH
	DD	063425H
	DD	070023206H
$unwind$RetInstr DD 0b1d01H
	DD	055741dH
	DD	054641dH
	DD	053541dH
	DD	052341dH
	DD	050011dH
	DD	0e016H
$unwind$write_prologue DD 020601H
	DD	030023206H
$unwind$ProcCheckOpen DD 010401H
	DD	04204H
$chain$2$ProcCheckOpen DD 061121H
	DD	047411H
	DD	07640aH
	DD	063405H
	DD	imagerel $LN60
	DD	imagerel $LN60+20
	DD	imagerel $unwind$ProcCheckOpen
$chain$3$ProcCheckOpen DD 021H
	DD	imagerel $LN60
	DD	imagerel $LN60+20
	DD	imagerel $unwind$ProcCheckOpen
$unwind$ms32_pcheck DD 040a01H
	DD	0a340aH
	DD	07006720aH
$unwind$watc_pcheck DD 050b01H
	DD	0d007420bH
	DD	05003c005H
	DD	03002H
$chain$0$watc_pcheck DD 020521H
	DD	0ce405H
	DD	imagerel watc_pcheck
	DD	imagerel watc_pcheck+73
	DD	imagerel $unwind$watc_pcheck
$chain$2$watc_pcheck DD 020021H
	DD	0ce400H
	DD	imagerel watc_pcheck
	DD	imagerel watc_pcheck+73
	DD	imagerel $unwind$watc_pcheck
$chain$6$watc_pcheck DD 060e21H
	DD	0b740eH
	DD	0a6405H
	DD	0ce400H
	DD	imagerel watc_pcheck
	DD	imagerel watc_pcheck+73
	DD	imagerel $unwind$watc_pcheck
$chain$7$watc_pcheck DD 020521H
	DD	04f405H
	DD	imagerel watc_pcheck+234
	DD	imagerel watc_pcheck+255
	DD	imagerel $chain$6$watc_pcheck
$chain$8$watc_pcheck DD 021H
	DD	imagerel watc_pcheck+234
	DD	imagerel watc_pcheck+255
	DD	imagerel $chain$6$watc_pcheck
$chain$9$watc_pcheck DD 040021H
	DD	0b7400H
	DD	0ce400H
	DD	imagerel watc_pcheck
	DD	imagerel watc_pcheck+73
	DD	imagerel $unwind$watc_pcheck
$unwind$LocalDir DD 041201H
	DD	0190112H
	DD	050057006H
$chain$2$LocalDir DD 064321H
	DD	01d3443H
	DD	018e414H
	DD	01e6408H
	DD	imagerel $LN66
	DD	imagerel $LN66+74
	DD	imagerel $unwind$LocalDir
$chain$4$LocalDir DD 060021H
	DD	018e400H
	DD	01e6400H
	DD	01d3400H
	DD	imagerel $LN66
	DD	imagerel $LN66+74
	DD	imagerel $unwind$LocalDir
$chain$5$LocalDir DD 021H
	DD	imagerel $LN66
	DD	imagerel $LN66+74
	DD	imagerel $unwind$LocalDir
$unwind$ProcDir DD 020601H
	DD	050025206H
$chain$1$ProcDir DD 041121H
	DD	096411H
	DD	083405H
	DD	imagerel $LN47
	DD	imagerel $LN47+66
	DD	imagerel $unwind$ProcDir
$chain$3$ProcDir DD 040d21H
	DD	0be40dH
	DD	0a7405H
	DD	imagerel $LN47+66
	DD	imagerel $LN47+205
	DD	imagerel $chain$1$ProcDir
$chain$4$ProcDir DD 021H
	DD	imagerel $LN47+66
	DD	imagerel $LN47+205
	DD	imagerel $chain$1$ProcDir
$chain$6$ProcDir DD 080021H
	DD	0be400H
	DD	0a7400H
	DD	096400H
	DD	083400H
	DD	imagerel $LN47
	DD	imagerel $LN47+66
	DD	imagerel $unwind$ProcDir
$unwind$EndpDir DD 020601H
	DD	030023206H
$unwind$ExcFrameDirective DD 051501H
	DD	0160115H
	DD	0d007f009H
	DD	05005H
$chain$4$ExcFrameDirective DD 0a3321H
	DD	014e433H
	DD	015c42bH
	DD	01d7423H
	DD	01c6410H
	DD	01b3408H
	DD	imagerel $LN81
	DD	imagerel $LN81+162
	DD	imagerel $unwind$ExcFrameDirective
$chain$6$ExcFrameDirective DD 0a0021H
	DD	014e400H
	DD	015c400H
	DD	01d7400H
	DD	01c6400H
	DD	01b3400H
	DD	imagerel $LN81
	DD	imagerel $LN81+162
	DD	imagerel $unwind$ExcFrameDirective
$chain$7$ExcFrameDirective DD 021H
	DD	imagerel $LN81
	DD	imagerel $LN81+162
	DD	imagerel $unwind$ExcFrameDirective
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$sprintf DD 041601H
	DD	070125216H
	DD	030106011H
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$_vsprintf_l DD 081401H
	DD	0a6414H
	DD	095414H
	DD	083414H
	DD	070105214H
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$_vsnprintf_l DD 081401H
	DD	0a6414H
	DD	095414H
	DD	083414H
	DD	070105214H
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$printf DD 041b01H
	DD	07017521bH
	DD	030156016H
xdata	ENDS
;	COMDAT xdata
xdata	SEGMENT
$unwind$_vfprintf_l DD 081401H
	DD	0a6414H
	DD	095414H
	DD	083414H
	DD	070105214H
xdata	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
;	COMDAT write_userdef_epilogue
_TEXT	SEGMENT
reglst$ = 64
buffer$ = 192
flag_iret$ = 848
tokenarray$ = 856
is_exitm$ = 864
write_userdef_epilogue PROC				; COMDAT

; 3143 : {

	push	rbx
	push	rbp
	push	r12
	push	r14
	sub	rsp, 808				; 00000328H

; 3144 :     uint_16 *regs;
; 3145 :     int i;
; 3146 :     char *p;
; 3147 :     bool is_exitm;
; 3148 :     struct proc_info   *info;
; 3149 :     int flags = CurrProc->sym.langtype; /* set bits 0-2 */

	mov	rax, QWORD PTR CurrProc
	movzx	ebx, cl

; 3150 :     struct dsym *dir;
; 3151 :     char reglst[128];
; 3152 :     char buffer[MAX_LINE_LEN]; /* stores string for RunMacro() */
; 3153 : 
; 3154 :     dir = (struct dsym *)SymSearch( ModuleInfo.proc_epilogue );

	mov	rcx, QWORD PTR ModuleInfo+328
	mov	r12, rdx
	mov	ebp, DWORD PTR [rax+76]
	call	SymFind
	mov	r14, rax

; 3155 :     if (dir == NULL ||
; 3156 :         dir->sym.state != SYM_MACRO ||

	test	rax, rax
	je	$LN6@write_user
	cmp	DWORD PTR [rax+32], 9
	jne	$LN6@write_user
	test	BYTE PTR [rax+44], 2
	jne	$LN6@write_user

; 3159 :     }
; 3160 : 
; 3161 :     info = CurrProc->e.procinfo;

	mov	rdx, QWORD PTR CurrProc
	mov	QWORD PTR [rsp+872], r13

; 3162 : 
; 3163 : #if AMD64_SUPPORT
; 3164 :     /* to be compatible with ML64, translate FASTCALL to 0 (not 7) */
; 3165 :     if ( CurrProc->sym.langtype == LANG_FASTCALL && ModuleInfo.fctype == FCT_WIN64 )

	xor	r13d, r13d
	mov	QWORD PTR [rsp+800], r15
	mov	ecx, DWORD PTR [rdx+76]
	mov	r15, QWORD PTR [rdx+96]
	cmp	ecx, 7
	jne	SHORT $LN7@write_user
	cmp	DWORD PTR ModuleInfo+376, 2
	cmove	ebp, r13d
$LN7@write_user:
	lea	eax, DWORD PTR [rcx-1]
	cmp	eax, 1
	jbe	SHORT $LN9@write_user
	cmp	ecx, 7
	jne	SHORT $LN8@write_user
$LN9@write_user:

; 3166 :         flags = 0;
; 3167 : #endif
; 3168 :     if ( CurrProc->sym.langtype == LANG_C ||
; 3169 :          CurrProc->sym.langtype == LANG_SYSCALL ||
; 3170 :          CurrProc->sym.langtype == LANG_FASTCALL)
; 3171 :         flags |= 0x10;

	or	ebp, 16
$LN8@write_user:

; 3172 : 
; 3173 :     flags |= ( CurrProc->sym.mem_type == MT_FAR ? 0x20 : 0 );
; 3174 :     flags |= ( CurrProc->sym.ispublic ? 0 : 0x40 );
; 3175 :     /* v2.11: set bit 7, the export flag */
; 3176 :     flags |= ( info->isexport ? 0x80 : 0 );
; 3177 :     flags |= flag_iret ? 0x100 : 0;  /* bit 8: 1 if IRET    */

	movzx	ecx, BYTE PTR [rdx+40]
	mov	r8d, 32					; 00000020H
	movzx	eax, BYTE PTR [r15+84]
	not	cl
	shr	ecx, 1
	and	eax, 4
	shl	eax, 5
	and	ecx, 64					; 00000040H
	or	ecx, eax
	mov	eax, r13d
	cmp	DWORD PTR [rdx+36], 130			; 00000082H
	cmove	eax, r8d
	or	ecx, eax
	neg	bl

; 3178 : 
; 3179 :     p = reglst;

	lea	rbx, QWORD PTR reglst$[rsp]
	sbb	eax, eax
	and	eax, 256				; 00000100H
	or	ecx, eax
	or	ebp, ecx

; 3180 :     if ( info->regslist ) {

	mov	rcx, QWORD PTR [r15]
	test	rcx, rcx
	je	SHORT $LN3@write_user

; 3181 :         int cnt = *info->regslist;

	mov	QWORD PTR [rsp+848], rsi
	mov	QWORD PTR [rsp+856], rdi
	movzx	edi, WORD PTR [rcx]

; 3182 :         regs = info->regslist + cnt;

	lea	rsi, QWORD PTR [rcx+rdi*2]

; 3183 :         for ( ; cnt; regs--, cnt-- ) {

	test	edi, edi
	je	SHORT $LN23@write_user
	npad	2
$LL4@write_user:

; 3184 :             GetResWName( *regs, p );

	movzx	ecx, WORD PTR [rsi]
	mov	rdx, rbx
	call	GetResWName

; 3185 :             p += strlen( p );

	or	rax, -1
	npad	1
$LL21@write_user:
	inc	rax
	cmp	BYTE PTR [rbx+rax], r13b
	jne	SHORT $LL21@write_user
	add	rbx, rax

; 3186 :             if ( cnt != 1 )

	cmp	edi, 1
	je	SHORT $LN2@write_user

; 3187 :                 *p++ = ',';

	mov	BYTE PTR [rbx], 44			; 0000002cH
	inc	rbx
$LN2@write_user:

; 3183 :         for ( ; cnt; regs--, cnt-- ) {

	sub	rsi, 2
	sub	edi, 1
	jne	SHORT $LL4@write_user
$LN23@write_user:
	mov	rsi, QWORD PTR [rsp+848]
	mov	rdi, QWORD PTR [rsp+856]
$LN3@write_user:

; 3188 :         }
; 3189 :     }
; 3190 :     *p = NULLC;

	mov	BYTE PTR [rbx], r13b

; 3191 :     //strcat( reglst, ">" );
; 3192 : 
; 3193 :     /* v2.07: make the numeric arguments more Masm-compatible */
; 3194 :     //sprintf( buffer,"%s %s, %02XH, %02XH, %02XH, <<%s>>, <%s>", ModuleInfo.proc_epilogue,
; 3195 :     sprintf( buffer,"%s, 0%XH, 0%XH, 0%XH, <<%s>>, <%s>",

	lea	rcx, OFFSET FLAT:$SG12515
	mov	rax, QWORD PTR [r15+40]
	lea	rdx, OFFSET FLAT:$SG12516
	mov	r8, QWORD PTR CurrProc
	test	rax, rax
	mov	r9d, ebp
	cmovne	rcx, rax
	lea	rax, QWORD PTR reglst$[rsp]
	mov	QWORD PTR [rsp+56], rcx
	lea	rcx, QWORD PTR buffer$[rsp]
	mov	r8, QWORD PTR [r8+8]
	mov	QWORD PTR [rsp+48], rax
	mov	eax, DWORD PTR [r15+36]
	mov	DWORD PTR [rsp+40], eax
	mov	eax, DWORD PTR [r15+32]
	mov	DWORD PTR [rsp+32], eax
	call	sprintf

; 3196 :             CurrProc->sym.name, flags, info->parasize, info->localsize,
; 3197 :             reglst, info->prologuearg ? info->prologuearg : "" );
; 3198 :     i = Token_Count + 1;

	mov	ebx, DWORD PTR ModuleInfo+496

; 3199 :     Tokenize( buffer, i, tokenarray, TOK_RESCAN );

	lea	rcx, QWORD PTR buffer$[rsp]
	mov	r9d, 1
	mov	r8, r12
	lea	edx, DWORD PTR [rbx+1]
	call	Tokenize

; 3200 : 
; 3201 :     /* if -EP is on, emit "epilogue: none" */
; 3202 :     if ( Options.preprocessor_stdout )

	cmp	BYTE PTR Options+125, r13b
	mov	r15, QWORD PTR [rsp+800]
	je	SHORT $LN12@write_user

; 3203 :         printf( "option epilogue:none\n" );

	lea	rcx, OFFSET FLAT:$SG12518
	call	printf
$LN12@write_user:

; 3204 : 
; 3205 :     RunMacro( dir, i, tokenarray, NULL, 0, &is_exitm );

	lea	rax, QWORD PTR is_exitm$[rsp]
	xor	r9d, r9d
	mov	QWORD PTR [rsp+40], rax
	lea	edx, DWORD PTR [rbx+1]
	mov	r8, r12
	mov	DWORD PTR [rsp+32], r13d
	mov	rcx, r14
	call	RunMacro
	mov	r13, QWORD PTR [rsp+872]

; 3206 :     Token_Count = i - 1;
; 3207 :     return( NOT_ERROR );

	xor	eax, eax
	mov	DWORD PTR ModuleInfo+496, ebx

; 3208 : }

	add	rsp, 808				; 00000328H
	pop	r14
	pop	r12
	pop	rbp
	pop	rbx
	ret	0
$LN6@write_user:

; 3157 :         dir->sym.isfunc == TRUE ) {
; 3158 :         return( EmitErr( EPILOGUE_MUST_BE_MACRO_PROC, ModuleInfo.proc_epilogue ) );

	mov	rdx, QWORD PTR ModuleInfo+328
	mov	ecx, 157				; 0000009dH
	call	EmitErr

; 3208 : }

	add	rsp, 808				; 00000328H
	pop	r14
	pop	r12
	pop	rbp
	pop	rbx
	ret	0
write_userdef_epilogue ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
;	COMDAT write_default_epilogue
_TEXT	SEGMENT
write_default_epilogue PROC				; COMDAT

; 3054 : {

	mov	QWORD PTR [rsp+8], rbx
	push	rdi
	sub	rsp, 32					; 00000020H

; 3055 :     struct proc_info   *info;
; 3056 : #if AMD64_SUPPORT
; 3057 :     int resstack = 0;
; 3058 : #endif
; 3059 : 
; 3060 :     info = CurrProc->e.procinfo;

	mov	rcx, QWORD PTR CurrProc
	xor	edi, edi
	mov	rbx, QWORD PTR [rcx+96]

; 3061 : 
; 3062 : #if AMD64_SUPPORT
; 3063 :     if ( info->isframe ) {

	test	BYTE PTR [rbx+84], 64			; 00000040H
	je	SHORT $LN2@write_defa

; 3064 :         if ( ModuleInfo.frame_auto )

	test	DWORD PTR ModuleInfo+408, 131072	; 00020000H
	je	$LN10@write_defa

; 3065 :             write_win64_default_epilogue( info );

	mov	rcx, rbx

; 3132 : #else
; 3133 :         AddLineQueueX( "pop %r", basereg[ModuleInfo.Ofssize] );
; 3134 : #endif
; 3135 :     }
; 3136 : }

	mov	rbx, QWORD PTR [rsp+48]
	add	rsp, 32					; 00000020H
	pop	rdi

; 3065 :             write_win64_default_epilogue( info );

	jmp	write_win64_default_epilogue
$LN2@write_defa:

; 3066 :         return;
; 3067 :     }
; 3068 :     if ( ModuleInfo.Ofssize == USE64 && ModuleInfo.fctype == FCT_WIN64 && ( ModuleInfo.win64_flags & W64F_AUTOSTACKSP ) ) {

	cmp	BYTE PTR ModuleInfo+404, 2
	jne	SHORT $LN4@write_defa
	cmp	DWORD PTR ModuleInfo+376, 2
	jne	SHORT $LN4@write_defa
	test	BYTE PTR ModuleInfo+413, 2
	je	SHORT $LN4@write_defa

; 3069 :         resstack  = sym_ReservedStack->value;

	mov	r9, QWORD PTR sym_ReservedStack

; 3070 :         /* if no framepointer was pushed, add 8 to align stack on OWORD
; 3071 :          * v2.12: obsolete; localsize contains correct value.
; 3072 :          */
; 3073 :         //if( !(info->localsize || info->stackparam || info->has_vararg || info->forceframe ))
; 3074 :         //    AddLineQueueX( "add %r, 8 + %s", stackreg[ModuleInfo.Ofssize], sym_ReservedStack->name );
; 3075 :         //else
; 3076 :         AddLineQueueX( "add %r, %d + %s", stackreg[ModuleInfo.Ofssize], NUMQUAL info->localsize, sym_ReservedStack->name );

	lea	rcx, OFFSET FLAT:$SG12469
	mov	r8d, DWORD PTR [rbx+36]
	mov	edx, 119				; 00000077H
	mov	edi, DWORD PTR [r9+16]
	mov	r9, QWORD PTR [r9+8]
	call	AddLineQueueX
	mov	rcx, QWORD PTR CurrProc
$LN4@write_defa:

; 3077 :     }
; 3078 : #endif
; 3079 : 
; 3080 :     /* Pop the registers */
; 3081 :     pop_register( CurrProc->e.procinfo->regslist );

	mov	rcx, QWORD PTR [rcx+96]
	mov	rcx, QWORD PTR [rcx]
	call	pop_register

; 3082 : 
; 3083 :     if ( info->loadds ) {

	test	BYTE PTR [rbx+84], 16
	je	SHORT $LN5@write_defa

; 3084 :         AddLineQueueX( "pop %r", T_DS );

	mov	edx, 28
	lea	rcx, OFFSET FLAT:$SG12471
	call	AddLineQueueX
$LN5@write_defa:

; 3085 :     }
; 3086 : 
; 3087 :     if( ( info->locallist == NULL ) &&
; 3088 :        info->stackparam == FALSE &&
; 3089 :        info->has_vararg == FALSE &&
; 3090 : #if AMD64_SUPPORT
; 3091 :        resstack == 0 &&

	mov	rax, QWORD PTR [rbx+16]
	test	rax, rax
	jne	SHORT $LN7@write_defa
	movzx	ecx, BYTE PTR [rbx+84]
	test	cl, 33					; 00000021H
	jne	SHORT $LN6@write_defa
	test	edi, edi
	jne	SHORT $LN6@write_defa
	test	cl, 8
	je	$LN10@write_defa
$LN6@write_defa:

; 3092 : #endif
; 3093 :        info->forceframe == FALSE )
; 3094 :         return;
; 3095 : 
; 3096 :     /* restore registers e/sp and e/bp.
; 3097 :      * emit either "leave" or "mov e/sp,e/bp, pop e/bp".
; 3098 :      */
; 3099 : #if AMD64_SUPPORT
; 3100 :     if( !(info->locallist || info->stackparam || info->has_vararg || info->forceframe ))

	test	rax, rax
	jne	SHORT $LN7@write_defa
	movzx	eax, BYTE PTR [rbx+84]
	test	al, 33					; 00000021H
	jne	SHORT $LN7@write_defa
	test	al, 8
	je	$LN10@write_defa
$LN7@write_defa:

; 3101 :         ;
; 3102 :     else
; 3103 : #endif
; 3104 :     if( info->pe_type ) {

	movzx	eax, BYTE PTR [rbx+84]
	test	al, 2
	je	SHORT $LN9@write_defa

; 3105 :         AddLineQueue( "leave" );

	lea	rcx, OFFSET FLAT:$SG12477

; 3132 : #else
; 3133 :         AddLineQueueX( "pop %r", basereg[ModuleInfo.Ofssize] );
; 3134 : #endif
; 3135 :     }
; 3136 : }

	mov	rbx, QWORD PTR [rsp+48]
	add	rsp, 32					; 00000020H
	pop	rdi

; 3105 :         AddLineQueue( "leave" );

	jmp	AddLineQueue
$LN9@write_defa:

; 3106 :     } else  {
; 3107 : #if STACKBASESUPP
; 3108 :         if ( info->fpo ) {

	test	al, al
	jns	SHORT $LN11@write_defa

; 3109 : #if AMD64_SUPPORT
; 3110 :             if ( ModuleInfo.Ofssize == USE64 && ModuleInfo.fctype == FCT_WIN64 && ( ModuleInfo.win64_flags & W64F_AUTOSTACKSP ) )

	movzx	eax, BYTE PTR ModuleInfo+404
	cmp	al, 2
	jne	SHORT $LN12@write_defa
	cmp	DWORD PTR ModuleInfo+376, 2
	jne	SHORT $LN12@write_defa
	test	BYTE PTR ModuleInfo+413, al
	jne	SHORT $LN10@write_defa
$LN12@write_defa:

; 3111 :                 ;
; 3112 :             else
; 3113 : #endif
; 3114 :             if ( info->localsize )

	mov	r8d, DWORD PTR [rbx+36]
	test	r8d, r8d
	je	SHORT $LN10@write_defa

; 3115 :                 AddLineQueueX( "add %r, %d", stackreg[ModuleInfo.Ofssize], NUMQUAL info->localsize );

	movzx	eax, al
	lea	rcx, OFFSET FLAT:stackreg
	mov	edx, DWORD PTR [rcx+rax*4]
	lea	rcx, OFFSET FLAT:$SG12482

; 3132 : #else
; 3133 :         AddLineQueueX( "pop %r", basereg[ModuleInfo.Ofssize] );
; 3134 : #endif
; 3135 :     }
; 3136 : }

	mov	rbx, QWORD PTR [rsp+48]
	add	rsp, 32					; 00000020H
	pop	rdi

; 3115 :                 AddLineQueueX( "add %r, %d", stackreg[ModuleInfo.Ofssize], NUMQUAL info->localsize );

	jmp	AddLineQueueX
$LN11@write_defa:

; 3116 :             return;
; 3117 :         }
; 3118 : #endif
; 3119 :         /*
; 3120 :          MOV [E|R]SP, [E|R]BP
; 3121 :          POP [E|R]BP
; 3122 :          */
; 3123 :         if( info->localsize != 0 ) {

	cmp	DWORD PTR [rbx+36], 0
	je	SHORT $LN15@write_defa

; 3124 : #if STACKBASESUPP
; 3125 :             AddLineQueueX( "mov %r, %r", stackreg[ModuleInfo.Ofssize], info->basereg );

	movzx	eax, BYTE PTR ModuleInfo+404
	lea	rcx, OFFSET FLAT:stackreg
	movzx	r8d, WORD PTR [rbx+86]
	mov	edx, DWORD PTR [rcx+rax*4]
	lea	rcx, OFFSET FLAT:$SG12484
	call	AddLineQueueX
$LN15@write_defa:

; 3126 : #else
; 3127 :             AddLineQueueX( "mov %r, %r", stackreg[ModuleInfo.Ofssize], basereg[ModuleInfo.Ofssize] );
; 3128 : #endif
; 3129 :         }
; 3130 : #if STACKBASESUPP
; 3131 :         AddLineQueueX( "pop %r", info->basereg );

	movzx	edx, WORD PTR [rbx+86]
	lea	rcx, OFFSET FLAT:$SG12485
	call	AddLineQueueX
$LN10@write_defa:

; 3132 : #else
; 3133 :         AddLineQueueX( "pop %r", basereg[ModuleInfo.Ofssize] );
; 3134 : #endif
; 3135 :     }
; 3136 : }

	mov	rbx, QWORD PTR [rsp+48]
	add	rsp, 32					; 00000020H
	pop	rdi
	ret	0
write_default_epilogue ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
;	COMDAT write_win64_default_epilogue
_TEXT	SEGMENT
info$ = 80
write_win64_default_epilogue PROC			; COMDAT

; 2905 : {

	push	r14
	sub	rsp, 64					; 00000040H
	mov	QWORD PTR [rsp+80], rbx
	mov	r14, rcx
	mov	QWORD PTR [rsp+88], rbp

; 2906 : #if STACKBASESUPP
; 2907 :     /* v2.12: obsolete */
; 2908 :     //if ( GetRegNo( info->basereg ) == 4 || ( info->parasize == 0 && info->locallist == NULL ) )
; 2909 :     //    sizestd = 8;
; 2910 : #endif
; 2911 :     /* restore non-volatile xmm registers */
; 2912 :     if ( info->regslist ) {

	xor	ebp, ebp
	mov	QWORD PTR [rsp+96], rsi
	mov	QWORD PTR [rsp+56], rdi
	mov	rdi, QWORD PTR [rcx]
	mov	QWORD PTR [rsp+48], r15
	lea	r15, OFFSET FLAT:__ImageBase
	test	rdi, rdi
	je	$LN6@write_win6

; 2913 :         uint_16 *regs;
; 2914 :         int cnt;
; 2915 :         int i;
; 2916 : 
; 2917 :         /* v2.12: space for xmm saves is now included in localsize
; 2918 :          * so first thing to do is to count the xmm regs that were saved
; 2919 :          */
; 2920 :         for( regs = info->regslist, cnt = *regs++, i = 0; cnt; cnt--, regs++ )

	movzx	r8d, WORD PTR [rdi]
	lea	rdx, QWORD PTR [rdi+2]
	mov	r9d, ebp
	test	r8d, r8d
	je	$LN6@write_win6
	npad	5
$LL4@write_win6:

; 2921 :             if ( GetValueSp( *regs ) & OP_XYZMM )

	movzx	eax, WORD PTR [rdx]
	lea	rcx, QWORD PTR [rax+rax*2]
	test	BYTE PTR SpecialTable[r15+rcx*4], 128	; 00000080H
	je	SHORT $LN2@write_win6

; 2922 :                 i++;

	inc	r9d
$LN2@write_win6:

; 2913 :         uint_16 *regs;
; 2914 :         int cnt;
; 2915 :         int i;
; 2916 : 
; 2917 :         /* v2.12: space for xmm saves is now included in localsize
; 2918 :          * so first thing to do is to count the xmm regs that were saved
; 2919 :          */
; 2920 :         for( regs = info->regslist, cnt = *regs++, i = 0; cnt; cnt--, regs++ )

	add	rdx, 2
	sub	r8d, 1
	jne	SHORT $LL4@write_win6

; 2923 : 
; 2924 :         DebugMsg1(("write_win64_default_epilogue(%s): %u xmm registers to restore\n", CurrProc->sym.name , i ));
; 2925 : 
; 2926 :         if ( i ) {

	test	r9d, r9d
	je	$LN6@write_win6

; 2927 :             i = ( info->localsize - i * XYZMMsize ) & ~(XYZMMsize-1);
; 2928 :             for( regs = info->regslist, cnt = *regs++; cnt; cnt--, regs++ ) {

	movzx	esi, WORD PTR [rdi]
	add	rdi, 2
	mov	ebx, DWORD PTR [r14+36]
	shl	r9d, 5
	sub	ebx, r9d
	and	ebx, -32				; ffffffffffffffe0H
	test	esi, esi
	je	$LN6@write_win6
	npad	11
$LL7@write_win6:

; 2929 :                 if ( GetValueSp( *regs ) & OP_XYZMM ) {

	movzx	ecx, WORD PTR [rdi]
	lea	rax, QWORD PTR [rcx+rcx*2]
	test	BYTE PTR SpecialTable[r15+rax*4], 128	; 00000080H
	je	SHORT $LN5@write_win6

; 2930 :                     DebugMsg1(("write_win64_default_epilogue(%s): restore %s, offset=%d\n", CurrProc->sym.name , GetResWName( *regs, NULL ), i ));
; 2931 :                     //AddLineQueueX( "movdqa %r, [%r+%u]", *regist, stackreg[ModuleInfo.Ofssize], NUMQUAL info->localsize + sizexmm );
; 2932 :                     /* v2.11: use @ReservedStack only if option win64:2 is set */
; 2933 :                     if ( ModuleInfo.win64_flags & W64F_AUTOSTACKSP )

	test	BYTE PTR ModuleInfo+413, 2

; 2934 :                         AddLineQueueX( "vmovdqa %r, [%r + %u + %s]", *regs, stackreg[ModuleInfo.Ofssize], NUMQUAL i, sym_ReservedStack->name );

	mov	edx, ecx
	mov	r9d, ebx
	je	SHORT $LN15@write_win6
	mov	rax, QWORD PTR sym_ReservedStack
	movzx	r8d, BYTE PTR ModuleInfo+404
	mov	rcx, QWORD PTR [rax+8]
	mov	r8d, DWORD PTR stackreg[r15+r8*4]
	mov	QWORD PTR [rsp+32], rcx
	lea	rcx, OFFSET FLAT:$SG12438
	call	AddLineQueueX
	jmp	SHORT $LN16@write_win6
$LN15@write_win6:

; 2935 :                     else
; 2936 :                         AddLineQueueX( "vmovdqa %r, [%r + %u]", *regs, stackreg[ModuleInfo.Ofssize], NUMQUAL i );

	movzx	eax, BYTE PTR ModuleInfo+404
	lea	rcx, OFFSET FLAT:$SG12439
	mov	r8d, DWORD PTR stackreg[r15+rax*4]
	call	AddLineQueueX
$LN16@write_win6:

; 2937 :                     i += XYZMMsize;

	add	ebx, 32					; 00000020H
$LN5@write_win6:

; 2927 :             i = ( info->localsize - i * XYZMMsize ) & ~(XYZMMsize-1);
; 2928 :             for( regs = info->regslist, cnt = *regs++; cnt; cnt--, regs++ ) {

	add	rdi, 2
	sub	esi, 1
	jne	SHORT $LL7@write_win6
$LN6@write_win6:

; 2938 :                 }
; 2939 :             }
; 2940 :         }
; 2941 :     }
; 2942 : 
; 2943 :     if (ModuleInfo.fctype == FCT_WIN64 && (ModuleInfo.win64_flags & W64F_AUTOSTACKSP)){

	cmp	DWORD PTR ModuleInfo+376, 2
	jne	SHORT $LN17@write_win6
	movzx	eax, BYTE PTR ModuleInfo+413
	test	al, 2
	je	SHORT $LN17@write_win6

; 2944 :       if (ModuleInfo.win64_flags & W64F_HABRAN){
; 2945 :         if (info->localsize + sym_ReservedStack->value)

	mov	r8d, DWORD PTR [r14+36]
	mov	r9, QWORD PTR sym_ReservedStack
	test	al, 11
	je	SHORT $LN19@write_win6
	mov	ecx, DWORD PTR [r9+16]
	add	ecx, r8d
	je	SHORT $LN18@write_win6

; 2946 :           AddLineQueueX("add %r, %d + %s", stackreg[ModuleInfo.Ofssize], NUMQUAL info->localsize, sym_ReservedStack->name);

	movzx	eax, BYTE PTR ModuleInfo+404
	lea	rcx, OFFSET FLAT:$SG12445
	mov	r9, QWORD PTR [r9+8]
	mov	edx, DWORD PTR stackreg[r15+rax*4]
	call	AddLineQueueX

; 2947 :       }

	jmp	SHORT $LN18@write_win6
$LN19@write_win6:

; 2948 :         else
; 2949 :           AddLineQueueX("add %r, %d + %s", stackreg[ModuleInfo.Ofssize], NUMQUAL info->localsize, sym_ReservedStack->name);

	movzx	eax, BYTE PTR ModuleInfo+404
	lea	rcx, OFFSET FLAT:$SG12446
	mov	r9, QWORD PTR [r9+8]
	mov	edx, DWORD PTR stackreg[r15+rax*4]
	call	AddLineQueueX

; 2950 :     }

	jmp	SHORT $LN18@write_win6
$LN17@write_win6:

; 2951 :     else
; 2952 :         AddLineQueueX( "add %r, %d", stackreg[ModuleInfo.Ofssize], NUMQUAL info->localsize );

	movzx	eax, BYTE PTR ModuleInfo+404
	lea	rcx, OFFSET FLAT:$SG12447
	mov	r8d, DWORD PTR [r14+36]
	mov	edx, DWORD PTR stackreg[r15+rax*4]
	call	AddLineQueueX
$LN18@write_win6:

; 2953 :     pop_register( CurrProc->e.procinfo->regslist );

	mov	rax, QWORD PTR CurrProc
	mov	rcx, QWORD PTR [rax+96]
	mov	rcx, QWORD PTR [rcx]
	call	pop_register

; 2954 : #if STACKBASESUPP
; 2955 :     if (ModuleInfo.win64_flags & W64F_HABRAN){

	movzx	eax, BYTE PTR ModuleInfo+413
	test	al, 11
	je	SHORT $LN9@write_win6

; 2956 :       /* restore non-volatile registers from shadow space */
; 2957 :       if (info->regslist) {

	mov	rbx, QWORD PTR [r14]
	test	rbx, rbx
	je	SHORT $LN9@write_win6

; 2958 :         uint_16 *regist = info->regslist;
; 2959 :         int cnt;
; 2960 :         if (ModuleInfo.win64_flags){

	test	al, al
	je	SHORT $LN9@write_win6

; 2962 :           int gprzize = 0;
; 2963 :           for (cnt = *regist++; cnt; cnt--, regist++)

	movzx	edi, WORD PTR [rbx]
	add	rbx, 2
	test	edi, edi
	je	SHORT $LN9@write_win6

; 2961 :           int i = 0;

	mov	rsi, rbp
	npad	6
$LL10@write_win6:

; 2964 :           {
; 2965 :             if (GetValueSp(*regist) & OP_XYZMM) continue;

	movzx	ecx, WORD PTR [rbx]
	lea	rax, QWORD PTR [rcx+rcx*2]
	test	BYTE PTR SpecialTable[r15+rax*4], 128	; 00000080H
	jne	SHORT $LN8@write_win6

; 2966 :             else {
; 2967 :               gprzize += 8;

	add	ebp, 8

; 2968 :               if (gprzize <= 0x20)

	cmp	ebp, 32					; 00000020H
	jg	SHORT $LN8@write_win6

; 2969 :               {
; 2970 :                 if (info->home_used[i] == 0){

	cmp	BYTE PTR [rsi+r14+76], 0
	jne	SHORT $LN28@write_win6

; 2971 :                   AddLineQueueX("mov %r, [%r+%u]", *regist, stackreg[ModuleInfo.Ofssize], NUMQUAL gprzize);

	movzx	eax, BYTE PTR ModuleInfo+404
	mov	edx, ecx
	mov	r9d, ebp
	lea	rcx, OFFSET FLAT:$SG12456
	mov	r8d, DWORD PTR stackreg[r15+rax*4]
	call	AddLineQueueX

; 2972 :                 }

	jmp	SHORT $LN29@write_win6
$LN28@write_win6:

; 2973 :                 else {
; 2974 :                   cnt++; regist--;

	inc	edi
	sub	rbx, 2
$LN29@write_win6:

; 2975 :                 }
; 2976 :                 i++;

	inc	rsi
$LN8@write_win6:

; 2962 :           int gprzize = 0;
; 2963 :           for (cnt = *regist++; cnt; cnt--, regist++)

	add	rbx, 2
	sub	edi, 1
	jne	SHORT $LL10@write_win6
$LN9@write_win6:

; 2977 :               }
; 2978 :             }
; 2979 :           }
; 2980 :         }
; 2981 :       }
; 2982 :     }
; 2983 :     //if ( !info->fpo )
; 2984 :     if ( GetRegNo( info->basereg ) != 4 && ( info->parasize != 0 || info->locallist != NULL ) )

	movzx	ecx, WORD PTR [r14+86]
	mov	rdi, QWORD PTR [rsp+56]
	mov	rsi, QWORD PTR [rsp+96]
	mov	rbp, QWORD PTR [rsp+88]
	mov	rbx, QWORD PTR [rsp+80]
	lea	rax, QWORD PTR [rcx+rcx*2]
	cmp	BYTE PTR SpecialTable[r15+rax*4+10], 4
	mov	r15, QWORD PTR [rsp+48]
	je	SHORT $LN30@write_win6
	cmp	DWORD PTR [r14+32], 0
	jne	SHORT $LN31@write_win6
	cmp	QWORD PTR [r14+16], 0
	je	SHORT $LN30@write_win6
$LN31@write_win6:

; 2985 :         AddLineQueueX( "pop %r", info->basereg );

	mov	edx, ecx
	lea	rcx, OFFSET FLAT:$SG12459

; 2986 : #else
; 2987 :     AddLineQueueX( "pop %r", basereg[ModuleInfo.Ofssize] );
; 2988 : #endif
; 2989 :     return;
; 2990 : }

	add	rsp, 64					; 00000040H
	pop	r14

; 2985 :         AddLineQueueX( "pop %r", info->basereg );

	jmp	AddLineQueueX
$LN30@write_win6:

; 2986 : #else
; 2987 :     AddLineQueueX( "pop %r", basereg[ModuleInfo.Ofssize] );
; 2988 : #endif
; 2989 :     return;
; 2990 : }

	add	rsp, 64					; 00000040H
	pop	r14
	ret	0
write_win64_default_epilogue ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
;	COMDAT pop_register
_TEXT	SEGMENT
regist$ = 48
pop_register PROC					; COMDAT

; 2871 : {

	sub	rsp, 40					; 00000028H

; 2872 :     int cnt;
; 2873 :     if( regist == NULL )

	test	rcx, rcx
	je	$LN6@pop_regist

; 2874 :         return;
; 2875 :     cnt = *regist;
; 2876 :     regist += cnt;
; 2877 :     if (ModuleInfo.win64_flags & W64F_HABRAN)

	test	BYTE PTR ModuleInfo+413, 11
	mov	QWORD PTR [rsp+48], rbx
	movzx	ebx, WORD PTR [rcx]
	mov	QWORD PTR [rsp+56], rsi
	mov	QWORD PTR [rsp+32], rdi
	lea	rdi, QWORD PTR [rcx+rbx*2]
	je	SHORT $LN9@pop_regist

; 2878 :     {
; 2879 :       for (cnt = CurrProc->e.procinfo->pushed_reg; cnt; cnt--, regist--) {

	mov	rax, QWORD PTR CurrProc
	mov	rcx, QWORD PTR [rax+96]
	mov	ebx, DWORD PTR [rcx+64]
	test	ebx, ebx
	je	$LN24@pop_regist
	lea	rsi, OFFSET FLAT:SpecialTable
	npad	7
$LL4@pop_regist:

; 2880 :         /* don't "pop" xmm registers */
; 2881 :         if (GetValueSp(*regist) & OP_XYZMM){

	movzx	ecx, WORD PTR [rdi]
	lea	rax, QWORD PTR [rcx+rcx*2]
	test	BYTE PTR [rsi+rax*4], 128		; 00000080H
	je	SHORT $LN11@pop_regist

; 2882 :           cnt++;

	inc	ebx

; 2883 :           continue;

	jmp	SHORT $LN2@pop_regist
$LN11@pop_regist:

; 2884 :         }
; 2885 :         AddLineQueueX("pop %r", *regist);

	mov	edx, ecx
	lea	rcx, OFFSET FLAT:$SG12408
	call	AddLineQueueX
$LN2@pop_regist:

; 2878 :     {
; 2879 :       for (cnt = CurrProc->e.procinfo->pushed_reg; cnt; cnt--, regist--) {

	sub	rdi, 2
	sub	ebx, 1
	jne	SHORT $LL4@pop_regist
	mov	rsi, QWORD PTR [rsp+56]
	mov	rbx, QWORD PTR [rsp+48]
	mov	rdi, QWORD PTR [rsp+32]

; 2894 :       }
; 2895 :     }
; 2896 : }

	add	rsp, 40					; 00000028H
	ret	0
$LN9@pop_regist:

; 2886 :       }
; 2887 :     }
; 2888 :     else {
; 2889 :       for (; cnt; cnt--, regist--) {

	test	ebx, ebx
	je	SHORT $LN24@pop_regist
	lea	rsi, OFFSET FLAT:SpecialTable
	npad	9
$LL7@pop_regist:

; 2890 :         /* don't "pop" xmm registers */
; 2891 :         if (GetValueSp(*regist) & OP_XYZMM)

	movzx	ecx, WORD PTR [rdi]
	lea	rax, QWORD PTR [rcx+rcx*2]
	test	BYTE PTR [rsi+rax*4], 128		; 00000080H
	jne	SHORT $LN5@pop_regist

; 2892 :           continue;
; 2893 :         AddLineQueueX("pop %r", *regist);

	mov	edx, ecx
	lea	rcx, OFFSET FLAT:$SG12410
	call	AddLineQueueX
$LN5@pop_regist:

; 2886 :       }
; 2887 :     }
; 2888 :     else {
; 2889 :       for (; cnt; cnt--, regist--) {

	sub	rdi, 2
	sub	ebx, 1
	jne	SHORT $LL7@pop_regist
$LN24@pop_regist:
	mov	rsi, QWORD PTR [rsp+56]
	mov	rbx, QWORD PTR [rsp+48]
	mov	rdi, QWORD PTR [rsp+32]
$LN6@pop_regist:

; 2894 :       }
; 2895 :     }
; 2896 : }

	add	rsp, 40					; 00000028H
	ret	0
pop_register ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
;	COMDAT write_default_prologue
_TEXT	SEGMENT
write_default_prologue PROC				; COMDAT

; 2530 : {

	mov	QWORD PTR [rsp+24], rbx
	push	rsi
	sub	rsp, 32					; 00000020H

; 2531 :     struct proc_info    *info;
; 2532 :     uint_16             *regist;
; 2533 :     uint_8              oldlinenumbers;
; 2534 :     int                 cnt;
; 2535 : #if AMD64_SUPPORT
; 2536 :     int                 resstack = 0;
; 2537 : #endif
; 2538 : 
; 2539 :     info = CurrProc->e.procinfo;

	mov	r8, QWORD PTR CurrProc
	xor	ebx, ebx
	mov	rsi, QWORD PTR [r8+96]

; 2540 : 
; 2541 : #if AMD64_SUPPORT
; 2542 :     if ( info->isframe ) {

	movzx	edx, BYTE PTR [rsi+84]
	test	dl, 64					; 00000040H
	je	SHORT $LN8@write_defa

; 2543 :         //DebugMsg(("write_default_prologue: isframe\n"));
; 2544 :         if ( ModuleInfo.frame_auto ) {

	test	DWORD PTR ModuleInfo+408, 131072	; 00020000H
	je	$LN28@write_defa

; 2545 :             write_win64_default_prologue( info );

	mov	rcx, rsi
	call	write_win64_default_prologue

; 2546 :             /* v2.11: line queue is now run here */
; 2547 :             goto runqueue;

	jmp	$runqueue$41
$LN8@write_defa:

; 2548 :         }
; 2549 :         return( NOT_ERROR );
; 2550 :     }
; 2551 :     if ( ModuleInfo.Ofssize == USE64 && ModuleInfo.fctype == FCT_WIN64 && ( ModuleInfo.win64_flags & W64F_AUTOSTACKSP ) )

	movzx	ecx, BYTE PTR ModuleInfo+404
	movzx	r10d, BYTE PTR ModuleInfo+413
	mov	r9d, DWORD PTR ModuleInfo+376
	cmp	cl, 2
	jne	SHORT $LN10@write_defa
	cmp	r9d, 2
	jne	SHORT $LN10@write_defa
	test	r10b, cl
	je	SHORT $LN10@write_defa

; 2552 :         resstack = sym_ReservedStack->value;

	mov	rax, QWORD PTR sym_ReservedStack
	mov	ebx, DWORD PTR [rax+16]
$LN10@write_defa:

; 2553 : #endif
; 2554 :     /* default processing. if no params/locals are defined, continue */
; 2555 :     if( info->forceframe == FALSE &&
; 2556 :        info->localsize == 0 &&
; 2557 :        info->stackparam == FALSE &&
; 2558 :        info->has_vararg == FALSE &&
; 2559 : #if AMD64_SUPPORT
; 2560 :        resstack == 0 &&

	test	dl, 8
	jne	SHORT $LN11@write_defa
	cmp	DWORD PTR [rsi+36], 0
	jne	SHORT $LN11@write_defa
	test	dl, 33					; 00000021H
	jne	SHORT $LN11@write_defa
	test	ebx, ebx
	jne	SHORT $LN11@write_defa
	cmp	QWORD PTR [rsi], 0
	je	$LN28@write_defa
$LN11@write_defa:
	mov	QWORD PTR [rsp+48], rdi

; 2561 : #endif
; 2562 :        info->regslist == NULL )
; 2563 :         return( NOT_ERROR );
; 2564 : 
; 2565 :     /* v2.11: now done in write_prologue() */
; 2566 :     //info->localsize = ROUND_UP( info->localsize, CurrWordSize );
; 2567 :     regist = info->regslist;

	mov	rdi, QWORD PTR [rsi]
	mov	QWORD PTR [rsp+56], r14

; 2568 : 
; 2569 : #if AMD64_SUPPORT
; 2570 :     /* initialize shadow space for register params */
; 2571 :     if ( ModuleInfo.Ofssize == USE64 &&
; 2572 :         CurrProc->sym.langtype == LANG_FASTCALL &&
; 2573 :         ModuleInfo.fctype == FCT_WIN64 &&

	cmp	cl, 2
	jne	SHORT $LN12@write_defa
	cmp	DWORD PTR [r8+76], 7
	jne	SHORT $LN12@write_defa
	cmp	r9d, 2
	jne	SHORT $LN12@write_defa
	test	r10b, 1
	je	SHORT $LN12@write_defa

; 2574 :         ( ModuleInfo.win64_flags & W64F_SAVEREGPARAMS ) )
; 2575 :         win64_SaveRegParams( info );

	mov	rcx, rsi
	call	win64_SaveRegParams
	movzx	ecx, BYTE PTR ModuleInfo+404
$LN12@write_defa:

; 2576 : #endif
; 2577 :     if( info->locallist || info->stackparam || info->has_vararg || info->forceframe ) {

	cmp	QWORD PTR [rsi+16], 0
	lea	r14, OFFSET FLAT:stackreg
	jne	SHORT $LN14@write_defa
	test	BYTE PTR [rsi+84], 41			; 00000029H
	je	SHORT $LN15@write_defa
$LN14@write_defa:

; 2578 : 
; 2579 :         /* write 80386 prolog code
; 2580 :          * PUSH [E|R]BP
; 2581 :          * MOV  [E|R]BP, [E|R]SP
; 2582 :          * SUB  [E|R]SP, localsize
; 2583 :          */
; 2584 : #if STACKBASESUPP
; 2585 :         if ( !info->fpo ) {

	test	BYTE PTR [rsi+84], 128			; 00000080H
	jne	SHORT $LN15@write_defa

; 2586 :             AddLineQueueX( "push %r", info->basereg );

	movzx	edx, WORD PTR [rsi+86]
	lea	rcx, OFFSET FLAT:$SG12297
	call	AddLineQueueX

; 2587 :             AddLineQueueX( "mov %r, %r", info->basereg, stackreg[ModuleInfo.Ofssize] );

	movzx	eax, BYTE PTR ModuleInfo+404
	lea	rcx, OFFSET FLAT:$SG12298
	movzx	edx, WORD PTR [rsi+86]
	mov	r8d, DWORD PTR [r14+rax*4]
	call	AddLineQueueX
	movzx	ecx, BYTE PTR ModuleInfo+404
$LN15@write_defa:

; 2588 :         }
; 2589 : #else
; 2590 :         AddLineQueueX( "push %r", basereg[ModuleInfo.Ofssize] );
; 2591 :         AddLineQueueX( "mov %r, %r", basereg[ModuleInfo.Ofssize], stackreg[ModuleInfo.Ofssize] );
; 2592 : #endif
; 2593 :     }
; 2594 : #if AMD64_SUPPORT
; 2595 :     if( resstack ) {

	test	ebx, ebx
	je	SHORT $LN16@write_defa

; 2596 :         /* in this case, push the USES registers BEFORE the stack space is reserved */
; 2597 :         if ( regist ) {

	test	rdi, rdi
	je	SHORT $LN18@write_defa

; 2598 :             for( cnt = *regist++; cnt; cnt--, regist++ )

	movzx	ebx, WORD PTR [rdi]
	add	rdi, 2
	test	ebx, ebx
	je	SHORT $LN3@write_defa
	npad	1
$LL4@write_defa:

; 2599 :                 AddLineQueueX( "push %r", *regist );

	movzx	edx, WORD PTR [rdi]
	lea	rcx, OFFSET FLAT:$SG12302
	call	AddLineQueueX
	lea	rdi, QWORD PTR [rdi+2]
	sub	ebx, 1
	jne	SHORT $LL4@write_defa

; 2598 :             for( cnt = *regist++; cnt; cnt--, regist++ )

	movzx	ecx, BYTE PTR ModuleInfo+404
$LN3@write_defa:

; 2600 :             regist = NULL;

	xor	edi, edi
$LN18@write_defa:

; 2601 :         }
; 2602 :         /* if no framepointer was pushed, add 8 to align stack on OWORD.
; 2603 :          * v2.12: obsolete, localsize contains correct value in this case.
; 2604 :          */
; 2605 :         //if( !(info->localsize || info->stackparam || info->has_vararg || info->forceframe ))
; 2606 :         //    AddLineQueueX( "sub %r, 8 + %s", stackreg[ModuleInfo.Ofssize], sym_ReservedStack->name );
; 2607 :         //else
; 2608 :         AddLineQueueX( "sub %r, %d + %s", stackreg[ModuleInfo.Ofssize], NUMQUAL info->localsize, sym_ReservedStack->name );

	mov	r9, QWORD PTR sym_ReservedStack
	mov	r8d, DWORD PTR [rsi+36]
	movzx	eax, cl
	lea	rcx, OFFSET FLAT:$SG12303
	mov	r9, QWORD PTR [r9+8]
	mov	edx, DWORD PTR [r14+rax*4]
	call	AddLineQueueX

; 2609 :     } else

	jmp	SHORT $LN21@write_defa
$LN16@write_defa:

; 2610 : #endif
; 2611 :     if( info->localsize  ) {

	mov	r8d, DWORD PTR [rsi+36]
	test	r8d, r8d
	je	SHORT $LN21@write_defa

; 2612 :         /* using ADD and the 2-complement has one advantage:
; 2613 :          * it will generate short instructions up to a size of 128.
; 2614 :          * with SUB, short instructions work up to 127 only.
; 2615 :          */
; 2616 :         if ( Options.masm_compat_gencode || info->localsize == 128 )

	cmp	BYTE PTR Options+128, 0
	jne	SHORT $LN22@write_defa
	cmp	r8d, 128				; 00000080H
	je	SHORT $LN22@write_defa

; 2618 :         else
; 2619 :             AddLineQueueX( "sub %r, %d", stackreg[ModuleInfo.Ofssize], NUMQUAL info->localsize );

	movzx	eax, cl
	lea	rcx, OFFSET FLAT:$SG12309
	jmp	SHORT $LN39@write_defa
$LN22@write_defa:

; 2617 :             AddLineQueueX( "add %r, %d", stackreg[ModuleInfo.Ofssize], NUMQUAL - info->localsize );

	movzx	eax, cl
	neg	r8d
	lea	rcx, OFFSET FLAT:$SG12308
$LN39@write_defa:
	mov	edx, DWORD PTR [r14+rax*4]
	call	AddLineQueueX
$LN21@write_defa:

; 2620 :     }
; 2621 : 
; 2622 :     if ( info->loadds ) {

	test	BYTE PTR [rsi+84], 16
	mov	r14, QWORD PTR [rsp+56]
	je	SHORT $LN23@write_defa

; 2623 :         AddLineQueueX( "push %r", T_DS );

	mov	edx, 28
	lea	rcx, OFFSET FLAT:$SG12311
	call	AddLineQueueX

; 2624 :         AddLineQueueX( "mov %r, %s", T_AX, szDgroup );

	mov	ebx, 9
	lea	r8, OFFSET FLAT:szDgroup
	mov	edx, ebx
	lea	rcx, OFFSET FLAT:$SG12312
	call	AddLineQueueX

; 2625 :         AddLineQueueX( "mov %r, %r", T_DS, ModuleInfo.Ofssize ? T_EAX : T_AX );

	cmp	BYTE PTR ModuleInfo+404, 0
	lea	rcx, OFFSET FLAT:$SG12313
	mov	eax, 17
	cmovne	ebx, eax
	mov	r8d, ebx
	lea	edx, QWORD PTR [rax+11]
	call	AddLineQueueX
$LN23@write_defa:

; 2626 :     }
; 2627 : 
; 2628 :     /* Push the GPR registers of the USES clause */
; 2629 :     if ( regist ) {

	test	rdi, rdi
	je	SHORT $LN38@write_defa

; 2630 :         for( cnt = *regist++; cnt; cnt--, regist++ ) {

	movzx	ebx, WORD PTR [rdi]
	add	rdi, 2
	test	ebx, ebx
	je	SHORT $LN38@write_defa
	npad	7
$LL7@write_defa:

; 2631 :             AddLineQueueX( "push %r", *regist );

	movzx	edx, WORD PTR [rdi]
	lea	rcx, OFFSET FLAT:$SG12315
	call	AddLineQueueX
	lea	rdi, QWORD PTR [rdi+2]
	sub	ebx, 1
	jne	SHORT $LL7@write_defa
$LN38@write_defa:
	mov	rdi, QWORD PTR [rsp+48]
$runqueue$41:

; 2632 :         }
; 2633 :     }
; 2634 : 
; 2635 : #if AMD64_SUPPORT
; 2636 : runqueue:
; 2637 : #endif
; 2638 : 
; 2639 : #if FASTPASS
; 2640 :     /* special case: generated code runs BEFORE the line.*/
; 2641 :     if ( ModuleInfo.list && UseSavedState )

	test	DWORD PTR ModuleInfo+408, 2048		; 00000800H
	je	SHORT $LN27@write_defa
	cmp	BYTE PTR UseSavedState, 0
	je	SHORT $LN27@write_defa

; 2642 :         if ( Parse_Pass == PASS_1 )

	cmp	DWORD PTR Parse_Pass, 0
	jne	SHORT $LN26@write_defa

; 2643 :             info->prolog_list_pos = list_pos;

	mov	eax, DWORD PTR list_pos
	mov	DWORD PTR [rsi+80], eax
	jmp	SHORT $LN27@write_defa
$LN26@write_defa:

; 2644 :         else
; 2645 :             list_pos = info->prolog_list_pos;

	mov	eax, DWORD PTR [rsi+80]
	mov	DWORD PTR list_pos, eax
$LN27@write_defa:

; 2646 : #endif
; 2647 :     /* line number debug info also needs special treatment
; 2648 :      * because current line number is the first true src line
; 2649 :      * IN the proc.
; 2650 :      */
; 2651 :     oldlinenumbers = Options.line_numbers;

	movzx	ebx, BYTE PTR Options+1

; 2652 :     Options.line_numbers = FALSE; /* temporarily disable line numbers */

	mov	BYTE PTR Options+1, 0

; 2653 :     RunLineQueue();

	call	RunLineQueue

; 2654 :     Options.line_numbers = oldlinenumbers;
; 2655 : 
; 2656 : #if FASTPASS
; 2657 :     if ( ModuleInfo.list && UseSavedState && (Parse_Pass > PASS_1))

	test	DWORD PTR ModuleInfo+408, 2048		; 00000800H
	mov	BYTE PTR Options+1, bl
	je	SHORT $LN28@write_defa
	cmp	BYTE PTR UseSavedState, 0
	je	SHORT $LN28@write_defa
	cmp	DWORD PTR Parse_Pass, 0
	jbe	SHORT $LN28@write_defa

; 2658 :          LineStoreCurr->list_pos = list_pos;

	mov	rax, QWORD PTR LineStoreCurr
	mov	ecx, DWORD PTR list_pos
	mov	DWORD PTR [rax+12], ecx
$LN28@write_defa:

; 2659 : #endif
; 2660 : 
; 2661 :     return( NOT_ERROR );

	xor	eax, eax

; 2662 : }

	mov	rbx, QWORD PTR [rsp+64]
	add	rsp, 32					; 00000020H
	pop	rsi
	ret	0
write_default_prologue ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
;	COMDAT write_win64_default_prologue
_TEXT	SEGMENT
info$ = 96
write_win64_default_prologue PROC			; COMDAT

; 2332 : {

	push	rbx
	push	r12
	push	r15
	sub	rsp, 64					; 00000040H
	mov	QWORD PTR [rsp+56], r13
	mov	rbx, rcx

; 2333 :     uint_16             *regist;
; 2334 :     const char * const  *ppfmt;
; 2335 :     int                 cntxmm;
; 2336 :     int                 resstack = ( ( ModuleInfo.win64_flags & W64F_AUTOSTACKSP ) ? sym_ReservedStack->value : 0 );

	xor	r13d, r13d
	test	BYTE PTR ModuleInfo+413, 2
	je	SHORT $LN47@write_win6
	mov	rax, QWORD PTR sym_ReservedStack
	mov	r15d, DWORD PTR [rax+16]
	jmp	SHORT $LN48@write_win6
$LN47@write_win6:
	mov	r15d, r13d
$LN48@write_win6:

; 2337 : 
; 2338 :     DebugMsg1(("write_win64_default_prologue enter\n"));
; 2339 : 
; 2340 :     if ( ModuleInfo.win64_flags & W64F_SAVEREGPARAMS )

	test	BYTE PTR ModuleInfo+413, 1
	je	SHORT $LN11@write_win6

; 2341 :         win64_SaveRegParams( info );

	call	win64_SaveRegParams
$LN11@write_win6:

; 2342 :     if (ModuleInfo.win64_flags & W64F_HABRAN) 

	test	BYTE PTR ModuleInfo+413, 11
	je	SHORT $LN12@write_win6

; 2343 :       win64_StoreRegHome(info);

	mov	rcx, rbx
	call	win64_StoreRegHome
$LN12@write_win6:

; 2344 :     /*
; 2345 :      * PUSH RBP
; 2346 :      * .PUSHREG RBP
; 2347 :      * MOV RBP, RSP
; 2348 :      * .SETFRAME RBP, 0
; 2349 :      */
; 2350 : #if STACKBASESUPP
; 2351 :     /* info->locallist tells whether there are local variables ( info->localsize doesn't! ) */
; 2352 :     if ( info->fpo || ( info->parasize == 0 && info->locallist == NULL ) ) {

	test	BYTE PTR [rbx+84], 128			; 00000080H
	jne	SHORT $LN15@write_win6
	cmp	DWORD PTR [rbx+32], r13d
	jne	SHORT $LN13@write_win6
	cmp	QWORD PTR [rbx+16], r13
	je	SHORT $LN15@write_win6
$LN13@write_win6:

; 2353 :         DebugMsg1(("write_win64_default_prologue: no frame register needed\n"));
; 2354 :         //sizestd += 8; /* v2.12: obsolete */
; 2355 :     } else {
; 2356 :         AddLineQueueX( "push %r", info->basereg );

	movzx	edx, WORD PTR [rbx+86]
	lea	rcx, OFFSET FLAT:$SG12216
	call	AddLineQueueX

; 2357 :         AddLineQueueX( "%r %r", T_DOT_PUSHREG, info->basereg );

	movzx	r8d, WORD PTR [rbx+86]
	lea	rcx, OFFSET FLAT:$SG12217
	mov	edx, 412				; 0000019cH
	call	AddLineQueueX

; 2358 :         AddLineQueueX( "mov %r, %r", info->basereg, T_RSP );

	movzx	edx, WORD PTR [rbx+86]
	lea	rcx, OFFSET FLAT:$SG12218
	mov	r8d, 119				; 00000077H
	call	AddLineQueueX

; 2359 :         AddLineQueueX( "%r %r, 0", T_DOT_SETFRAME, info->basereg );

	movzx	r8d, WORD PTR [rbx+86]
	lea	rcx, OFFSET FLAT:$SG12219
	mov	edx, 416				; 000001a0H
	call	AddLineQueueX
$LN15@write_win6:

; 2360 :     }
; 2361 : #else
; 2362 :     AddLineQueueX( "push %r", basereg[USE64] );
; 2363 :     AddLineQueueX( "%r %r", T_DOT_PUSHREG, basereg[USE64] );
; 2364 :     AddLineQueueX( "mov %r, %r", basereg[USE64], T_RSP );
; 2365 :     AddLineQueueX( "%r %r, 0", T_DOT_SETFRAME, basereg[USE64] );
; 2366 : #endif
; 2367 :     if (ModuleInfo.win64_flags & W64F_HABRAN){

	test	BYTE PTR ModuleInfo+413, 11
	lea	r12, OFFSET FLAT:SpecialTable
	mov	QWORD PTR [rsp+96], rbp
	mov	QWORD PTR [rsp+104], rsi

; 2368 :       cntxmm = 0;

	mov	esi, r13d
	mov	QWORD PTR [rsp+112], rdi

; 2369 :       if (info->regslist) {

	mov	rdi, QWORD PTR [rbx]
	mov	QWORD PTR [rsp+48], r14
	je	$LN16@write_win6
	test	rdi, rdi
	je	$LN6@write_win6

; 2370 :         int cnt;
; 2371 :         int n = 0;
; 2372 :         info->pushed_reg = 0; /*count of pushed registers */

	mov	DWORD PTR [rbx+64], r13d
	mov	r14d, r13d

; 2373 :         regist = info->regslist;
; 2374 :         for (cnt = *regist++; cnt; cnt--, regist++) {

	movzx	ebp, WORD PTR [rdi]
	add	rdi, 2
	test	ebp, ebp
	je	$LN6@write_win6
	npad	2
$LL4@write_win6:

; 2375 :           if (GetValueSp(*regist) & OP_XYZMM) {

	movzx	eax, WORD PTR [rdi]
	lea	rcx, QWORD PTR [rax+rax*2]
	test	BYTE PTR [r12+rcx*4], 128		; 00000080H
	je	SHORT $LN19@write_win6

; 2376 :             cntxmm += 1;

	inc	esi

; 2377 :           }

	jmp	SHORT $LN2@write_win6
$LN19@write_win6:

; 2378 :           else {
; 2379 :             if (n < info->stored_reg) n++;

	cmp	r14d, DWORD PTR [rbx+60]
	jge	SHORT $LN21@write_win6
	inc	r14d
	jmp	SHORT $LN2@write_win6
$LN21@write_win6:

; 2380 :             else{
; 2381 :               info->pushed_reg += 1;

	inc	DWORD PTR [rbx+64]

; 2382 :               AddLineQueueX("push %r", *regist);

	lea	rcx, OFFSET FLAT:$SG12227
	movzx	edx, WORD PTR [rdi]
	call	AddLineQueueX

; 2383 :               if ((1 << GetRegNo(*regist)) & win64_nvgpr) {

	movzx	edx, WORD PTR [rdi]
	lea	rax, QWORD PTR [rdx+rdx*2]
	movzx	ecx, BYTE PTR [r12+rax*4+10]
	mov	eax, 1
	shl	eax, cl
	test	eax, 61672				; 0000f0e8H
	je	SHORT $LN2@write_win6

; 2384 :                 AddLineQueueX("%r %r", T_DOT_PUSHREG, *regist);

	mov	r8d, edx
	lea	rcx, OFFSET FLAT:$SG12229
	mov	edx, 412				; 0000019cH
	call	AddLineQueueX
$LN2@write_win6:

; 2373 :         regist = info->regslist;
; 2374 :         for (cnt = *regist++; cnt; cnt--, regist++) {

	add	rdi, 2
	sub	ebp, 1
	jne	SHORT $LL4@write_win6

; 2385 :               }
; 2386 :             }
; 2387 :           }
; 2388 :         } /* end for */
; 2389 :       }
; 2390 :     }

	jmp	SHORT $LN6@write_win6
$LN16@write_win6:

; 2391 :     else{
; 2392 :       /* after the "push rbp", the stack is xmmword aligned */
; 2393 : 
; 2394 :       /* Push the registers */
; 2395 :       cntxmm = 0;
; 2396 :       if (info->regslist) {

	test	rdi, rdi
	je	SHORT $LN6@write_win6

; 2397 :         int cnt;
; 2398 :         regist = info->regslist;
; 2399 :         for (cnt = *regist++; cnt; cnt--, regist++) {

	movzx	ebp, WORD PTR [rdi]
	add	rdi, 2
	test	ebp, ebp
	je	SHORT $LN6@write_win6
	npad	7
$LL7@write_win6:

; 2400 :           if (GetValueSp(*regist) & OP_XYZMM) {

	movzx	ecx, WORD PTR [rdi]
	lea	rax, QWORD PTR [rcx+rcx*2]
	test	BYTE PTR [r12+rax*4], 128		; 00000080H
	je	SHORT $LN25@write_win6

; 2401 :             cntxmm += 1;

	inc	esi

; 2402 :           }

	jmp	SHORT $LN5@write_win6
$LN25@write_win6:

; 2403 :           else {
; 2404 :             AddLineQueueX("push %r", *regist);

	mov	edx, ecx
	lea	rcx, OFFSET FLAT:$SG12233
	call	AddLineQueueX

; 2405 :             if ((1 << GetRegNo(*regist)) & win64_nvgpr) {

	movzx	edx, WORD PTR [rdi]
	lea	rax, QWORD PTR [rdx+rdx*2]
	movzx	ecx, BYTE PTR [r12+rax*4+10]
	mov	eax, 1
	shl	eax, cl
	test	eax, 61672				; 0000f0e8H
	je	SHORT $LN5@write_win6

; 2406 :               AddLineQueueX("%r %r", T_DOT_PUSHREG, *regist);

	mov	r8d, edx
	lea	rcx, OFFSET FLAT:$SG12235
	mov	edx, 412				; 0000019cH
	call	AddLineQueueX
$LN5@write_win6:

; 2397 :         int cnt;
; 2398 :         regist = info->regslist;
; 2399 :         for (cnt = *regist++; cnt; cnt--, regist++) {

	add	rdi, 2
	sub	ebp, 1
	jne	SHORT $LL7@write_win6
$LN6@write_win6:

; 2407 :             }
; 2408 :           }
; 2409 :         } /* end for */
; 2410 :       }
; 2411 :     }
; 2412 :     /* v2.11: now done in write_prologue() */
; 2413 :     //info->localsize = ROUND_UP( info->localsize, CurrWordSize );
; 2414 :     if (ModuleInfo.win64_flags & W64F_HABRAN){

	test	BYTE PTR ModuleInfo+413, 11
	mov	r14, QWORD PTR [rsp+48]
	je	SHORT $LN30@write_win6

; 2415 :       if (Parse_Pass && sym_ReservedStack->hasinvoke == 0) resstack = 0;

	cmp	DWORD PTR Parse_Pass, r13d
	je	SHORT $LN29@write_win6
	mov	rax, QWORD PTR sym_ReservedStack
	cmp	DWORD PTR [rax+16], r13d
	cmove	r15d, r13d
$LN29@write_win6:

; 2416 :       if (!(info->locallist) && !(resstack)) info->localsize = 0;

	cmp	QWORD PTR [rbx+16], r13
	jne	SHORT $LN30@write_win6
	test	r15d, r15d
	jne	SHORT $LN30@write_win6
	mov	DWORD PTR [rbx+36], r13d
$LN30@write_win6:

; 2417 :     }
; 2418 :     /* alloc space for local variables */
; 2419 :     //if( info->localsize + sizestd ) {
; 2420 :     if( info->localsize + resstack ) {

	mov	ecx, DWORD PTR [rbx+36]
	mov	r13, QWORD PTR [rsp+56]
	lea	eax, DWORD PTR [rcx+r15]
	test	eax, eax
	je	$LN9@write_win6

; 2421 :         DebugMsg1(("write_win64_default_prologue: localsize=%u resstack=%u\n", info->localsize, resstack ));
; 2422 :         if (ModuleInfo.win64_flags & W64F_HABRAN){

	test	BYTE PTR ModuleInfo+413, 11
	je	SHORT $LN33@write_win6

; 2423 :           if (((info->pushed_reg & 1) && (info->localsize & 0xF)) ||

	mov	eax, DWORD PTR [rbx+64]
	and	eax, 1
	je	SHORT $LN35@write_win6
	test	cl, 15
	jne	SHORT $LN34@write_win6
$LN35@write_win6:
	test	eax, eax
	jne	SHORT $LN33@write_win6
	test	cl, 15
	jne	SHORT $LN33@write_win6
	test	esi, esi
	jne	SHORT $LN33@write_win6
$LN34@write_win6:

; 2424 :             ((!(info->pushed_reg & 1)) && (!(info->localsize & 0xF))) && (!(info->pushed_reg & 1)) && (!(cntxmm)))
; 2425 :             info->localsize += 8;

	lea	eax, DWORD PTR [rcx+8]
	mov	DWORD PTR [rbx+36], eax
$LN33@write_win6:

; 2426 :         }
; 2427 :           /*
; 2428 :          * SUB  RSP, localsize
; 2429 :          * .ALLOCSTACK localsize
; 2430 :          */
; 2431 :         ppfmt = ( resstack ? fmtstk1 : fmtstk0 );

	test	r15d, r15d
	lea	rax, OFFSET FLAT:fmtstk1
	lea	rdi, OFFSET FLAT:fmtstk0
	cmovne	rdi, rax

; 2432 : #if STACKPROBE
; 2433 :         if ( info->localsize + resstack > 0x1000 ) {
; 2434 :             AddLineQueueX( *(ppfmt+2), T_RAX, NUMQUAL info->localsize, sym_ReservedStack->name );
; 2435 :             AddLineQueue(  "externdef __chkstk:PROC" );
; 2436 :             AddLineQueue(  "call __chkstk" );
; 2437 :             AddLineQueueX( "mov %r, %r", T_RSP, T_RAX );
; 2438 :         } else
; 2439 : #endif
; 2440 : 			if (ZEROLOCALS && info->localsize) {

	cmp	BYTE PTR ZEROLOCALS, 0
	je	$LN36@write_win6
	mov	r8d, DWORD PTR [rbx+36]
	test	r8d, r8d
	je	$LN36@write_win6

; 2441 : 				if (info->localsize <= 128) {
; 2442 : 					AddLineQueueX(*(ppfmt + 0), T_RSP, NUMQUAL info->localsize, sym_ReservedStack->name);

	mov	r9, QWORD PTR sym_ReservedStack
	mov	edx, 119				; 00000077H
	mov	rcx, QWORD PTR [rdi]
	mov	r9, QWORD PTR [r9+8]
	cmp	r8d, 128				; 00000080H
	ja	SHORT $LN38@write_win6
	call	AddLineQueueX

; 2443 : 					AddLineQueueX(*(ppfmt + 1), T_DOT_ALLOCSTACK, NUMQUAL info->localsize, sym_ReservedStack->name);

	mov	r9, QWORD PTR sym_ReservedStack
	mov	edx, 409				; 00000199H
	mov	r8d, DWORD PTR [rbx+36]
	mov	rcx, QWORD PTR [rdi+8]
	mov	r9, QWORD PTR [r9+8]
	call	AddLineQueueX

; 2444 : 
; 2445 : 					AddLineQueueX("mov %r, %u", T_EAX, info->localsize);

	mov	r8d, DWORD PTR [rbx+36]
	lea	rcx, OFFSET FLAT:$SG12248
	mov	edx, 17
	call	AddLineQueueX

; 2446 : 					AddLineQueueX("dec %r", T_EAX);

	mov	edx, 17
	lea	rcx, OFFSET FLAT:$SG12249
	call	AddLineQueueX

; 2447 : 					AddLineQueueX("mov byte ptr [%r + %r], 0", T_RSP, T_RAX);

	mov	edx, 119				; 00000077H
	lea	rcx, OFFSET FLAT:$SG12250
	lea	r8d, QWORD PTR [rdx-4]
	call	AddLineQueueX

; 2448 : 					AddLineQueueX("dw 0F875h");

	lea	rcx, OFFSET FLAT:$SG12251
	call	AddLineQueueX

; 2449 : 				}

	jmp	$LN37@write_win6
$LN38@write_win6:

; 2450 : 				else
; 2451 : 				{
; 2452 : 					AddLineQueueX(*(ppfmt + 0), T_RSP, NUMQUAL info->localsize + 16, sym_ReservedStack->name);

	add	r8d, 16
	call	AddLineQueueX

; 2453 : 					AddLineQueueX(*(ppfmt + 1), T_DOT_ALLOCSTACK, NUMQUAL info->localsize + 16, sym_ReservedStack->name);

	mov	r9, QWORD PTR sym_ReservedStack
	mov	edx, 409				; 00000199H
	mov	r8d, DWORD PTR [rbx+36]
	mov	rcx, QWORD PTR [rdi+8]
	add	r8d, 16
	mov	r9, QWORD PTR [r9+8]
	call	AddLineQueueX

; 2454 : 					AddLineQueueX("mov [%r], %r", T_RSP, T_RDI);

	mov	edx, 119				; 00000077H
	lea	rcx, OFFSET FLAT:$SG12252
	lea	r8d, QWORD PTR [rdx+3]
	call	AddLineQueueX

; 2455 : 					AddLineQueueX("mov [%r+8], %r", T_RSP, T_RCX);

	mov	edx, 119				; 00000077H
	lea	rcx, OFFSET FLAT:$SG12253
	lea	r8d, QWORD PTR [rdx-3]
	call	AddLineQueueX

; 2456 : 					AddLineQueueX("xor %r, %r", T_EAX, T_EAX);

	mov	edx, 17
	lea	rcx, OFFSET FLAT:$SG12254
	mov	r8d, edx
	call	AddLineQueueX

; 2457 : 					AddLineQueueX("mov %r, %u", T_ECX, info->localsize);

	mov	r8d, DWORD PTR [rbx+36]
	lea	rcx, OFFSET FLAT:$SG12255
	mov	edx, 18
	call	AddLineQueueX

; 2458 : 					AddLineQueueX("mov %r, %r", T_RDI, T_RSP);

	mov	edx, 122				; 0000007aH
	lea	rcx, OFFSET FLAT:$SG12256
	lea	r8d, QWORD PTR [rdx-3]
	call	AddLineQueueX

; 2459 : 					AddLineQueueX("add %r, %u", T_RDI, 16);

	mov	edx, 122				; 0000007aH
	lea	rcx, OFFSET FLAT:$SG12257
	lea	r8d, QWORD PTR [rdx-106]
	call	AddLineQueueX

; 2460 : 					AddLineQueueX("rep stosb");

	lea	rcx, OFFSET FLAT:$SG12258
	call	AddLineQueueX

; 2461 : 					AddLineQueueX("mov %r,[%r]", T_RDI, T_RSP);

	mov	edx, 122				; 0000007aH
	lea	rcx, OFFSET FLAT:$SG12259
	lea	r8d, QWORD PTR [rdx-3]
	call	AddLineQueueX

; 2462 : 					AddLineQueueX("mov %r,[%r+8]", T_RCX, T_RSP);

	mov	edx, 116				; 00000074H
	lea	rcx, OFFSET FLAT:$SG12260
	lea	r8d, QWORD PTR [rdx+3]
	call	AddLineQueueX

; 2463 : 				}
; 2464 : 			}

	jmp	SHORT $LN37@write_win6
$LN36@write_win6:

; 2465 : 			else {
; 2466 : 				AddLineQueueX(*(ppfmt + 0), T_RSP, NUMQUAL info->localsize, sym_ReservedStack->name);

	mov	r9, QWORD PTR sym_ReservedStack
	mov	edx, 119				; 00000077H
	mov	r8d, DWORD PTR [rbx+36]
	mov	rcx, QWORD PTR [rdi]
	mov	r9, QWORD PTR [r9+8]
	call	AddLineQueueX

; 2467 : 				AddLineQueueX(*(ppfmt + 1), T_DOT_ALLOCSTACK, NUMQUAL info->localsize, sym_ReservedStack->name);

	mov	r9, QWORD PTR sym_ReservedStack
	mov	edx, 409				; 00000199H
	mov	r8d, DWORD PTR [rbx+36]
	mov	rcx, QWORD PTR [rdi+8]
	mov	r9, QWORD PTR [r9+8]
	call	AddLineQueueX
$LN37@write_win6:

; 2468 : 			}
; 2469 : 
; 2470 : 
; 2471 :         /* save xmm registers */
; 2472 :         if ( cntxmm ) {

	test	esi, esi
	je	$LN9@write_win6

; 2473 :             int i;
; 2474 :             int cnt;
; 2475 :             regist = info->regslist;

	mov	rdi, QWORD PTR [rbx]

; 2476 :             i = ( info->localsize - cntxmm * 32 ) & ~(32-1);

	mov	ebx, DWORD PTR [rbx+36]
	shl	esi, 5
	sub	ebx, esi

; 2477 :             for( cnt = *regist++; cnt; cnt--, regist++ ) {

	movzx	esi, WORD PTR [rdi]
	and	ebx, -32				; ffffffffffffffe0H
	add	rdi, 2
	test	esi, esi
	je	$LN9@write_win6
$LL10@write_win6:

; 2478 :                 if ( GetValueSp( *regist ) & OP_XYZMM ) {

	movzx	ecx, WORD PTR [rdi]
	lea	rax, QWORD PTR [rcx+rcx*2]
	test	BYTE PTR [r12+rax*4], 128		; 00000080H
	je	$LN8@write_win6

; 2479 :                     if ( resstack ) {
; 2480 :                         AddLineQueueX( "vmovdqa [%r+%u+%s], %r", T_RSP, NUMQUAL i, sym_ReservedStack->name, *regist );

	mov	r8d, ebx
	mov	edx, 119				; 00000077H
	test	r15d, r15d
	je	SHORT $LN42@write_win6
	mov	r9, QWORD PTR sym_ReservedStack
	mov	DWORD PTR [rsp+32], ecx
	lea	rcx, OFFSET FLAT:$SG12265
	mov	r9, QWORD PTR [r9+8]
	call	AddLineQueueX

; 2481 :                         if ( ( 1 << GetRegNo( *regist ) ) & win64_nvxmm )  {

	movzx	edx, WORD PTR [rdi]
	lea	rax, QWORD PTR [rdx+rdx*2]
	movzx	ecx, BYTE PTR [r12+rax*4+10]
	mov	eax, 1
	shl	eax, cl
	test	eax, 65472				; 0000ffc0H
	je	SHORT $LN45@write_win6

; 2482 :                             AddLineQueueX( "%r %r, %u+%s", T_DOT_SAVEYMM256, *regist, NUMQUAL i, sym_ReservedStack->name );

	mov	rax, QWORD PTR sym_ReservedStack
	mov	r8d, edx
	mov	r9d, ebx
	mov	edx, 415				; 0000019fH
	mov	rcx, QWORD PTR [rax+8]
	mov	QWORD PTR [rsp+32], rcx
	lea	rcx, OFFSET FLAT:$SG12267
	call	AddLineQueueX

; 2483 :                         }
; 2484 :                     } else {

	jmp	SHORT $LN45@write_win6
$LN42@write_win6:

; 2485 :                         AddLineQueueX( "vmovdqa [%r+%u], %r", T_RSP, NUMQUAL i, *regist );

	mov	r9d, ecx
	lea	rcx, OFFSET FLAT:$SG12268
	call	AddLineQueueX

; 2486 :                         if ( ( 1 << GetRegNo( *regist ) ) & win64_nvxmm )  {

	movzx	edx, WORD PTR [rdi]
	lea	rax, QWORD PTR [rdx+rdx*2]
	movzx	ecx, BYTE PTR [r12+rax*4+10]
	mov	eax, 1
	shl	eax, cl
	test	eax, 65472				; 0000ffc0H
	je	SHORT $LN45@write_win6

; 2487 :                             AddLineQueueX( "%r %r, %u", T_DOT_SAVEYMM256, *regist, NUMQUAL i );

	mov	r8d, edx
	lea	rcx, OFFSET FLAT:$SG12270
	mov	edx, 415				; 0000019fH
	mov	r9d, ebx
	call	AddLineQueueX
$LN45@write_win6:

; 2488 :                         }
; 2489 :                     }
; 2490 :                     i += 32;

	add	ebx, 32					; 00000020H
$LN8@write_win6:

; 2477 :             for( cnt = *regist++; cnt; cnt--, regist++ ) {

	add	rdi, 2
	sub	esi, 1
	jne	$LL10@write_win6
$LN9@write_win6:

; 2491 :                 }
; 2492 :             }
; 2493 :         }
; 2494 : 
; 2495 :     }
; 2496 :     AddLineQueueX( "%r", T_DOT_ENDPROLOG );

	mov	edx, 410				; 0000019aH
	lea	rcx, OFFSET FLAT:$SG12271
	mov	rdi, QWORD PTR [rsp+112]
	mov	rsi, QWORD PTR [rsp+104]
	mov	rbp, QWORD PTR [rsp+96]

; 2497 : 
; 2498 :     /* v2.11: linequeue is now run in write_default_prologue() */
; 2499 :     return;
; 2500 : }

	add	rsp, 64					; 00000040H
	pop	r15
	pop	r12
	pop	rbx

; 2491 :                 }
; 2492 :             }
; 2493 :         }
; 2494 : 
; 2495 :     }
; 2496 :     AddLineQueueX( "%r", T_DOT_ENDPROLOG );

	jmp	AddLineQueueX
write_win64_default_prologue ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
;	COMDAT win64_StoreRegHome
_TEXT	SEGMENT
info$ = 64
win64_StoreRegHome PROC					; COMDAT

; 2254 : {

	push	rsi
	push	rdi
	push	r14
	sub	rsp, 32					; 00000020H

; 2255 :   int			       i = 0;
; 2256 :   int			       cnt;
; 2257 :   int            grcount = 0;
; 2258 :   int			       sizestd = 0;
; 2259 :   int            freeshadow = 4;
; 2260 :   uint_16        *regist;
; 2261 :   info->stored_reg = 0;
; 2262 :   if (info->regslist) {

	mov	rdi, QWORD PTR [rcx]
	xor	r14d, r14d
	mov	DWORD PTR [rcx+60], r14d
	mov	rsi, rcx
	mov	edx, r14d
	test	rdi, rdi
	je	$LN24@win64_Stor

; 2263 :     for (regist = info->regslist, cnt = *regist++; cnt; cnt--, regist++, i++) {

	movzx	r8d, WORD PTR [rdi]
	add	rdi, 2
	mov	QWORD PTR [rsp+64], rbx
	mov	QWORD PTR [rsp+80], r15
	lea	r15, OFFSET FLAT:SpecialTable
	test	r8d, r8d
	je	SHORT $LN3@win64_Stor
$LL4@win64_Stor:

; 2264 :       if (GetValueSp(*regist) & OP_XYZMM) continue;

	movzx	eax, WORD PTR [rdi]
	lea	rcx, QWORD PTR [rax+rax*2]
	test	BYTE PTR [r15+rcx*4], 128		; 00000080H
	jne	SHORT $LN2@win64_Stor

; 2265 :       else ++grcount;                                       //find how many general registers to save

	inc	edx
$LN2@win64_Stor:

; 2263 :     for (regist = info->regslist, cnt = *regist++; cnt; cnt--, regist++, i++) {

	add	rdi, 2
	sub	r8d, 1
	jne	SHORT $LL4@win64_Stor
$LN3@win64_Stor:

; 2266 :     }
; 2267 :     //for (i = 0, freeshadow = 0; i<4; i++){                  
; 2268 :     //  if (info->home_used[i] == 0) ++freeshadow;
; 2269 :     //}
; 2270 :     freeshadow -= info->home_taken;                         //find out how many free shadows

	mov	r8d, 4
	mov	eax, r8d
	sub	eax, DWORD PTR [rsi+68]

; 2271 :     if (freeshadow){                                        //skip if all shadow spaces are taken  

	je	$LN44@win64_Stor

; 2272 :       if (grcount == 1) memset(info->home_used, 1, 4);      //1 register only? don't store it, push it to prevent 'sub rsp,...8'

	cmp	edx, 1
	jne	SHORT $LN30@win64_Stor
	mov	rax, 72340172838076673			; 0101010101010101H
	mov	DWORD PTR [rsi+76], eax
	jmp	$LN44@win64_Stor
$LN30@win64_Stor:

; 2273 :       else if (grcount == 2 && freeshadow >= 2){            //store only one egister, another push to prevent 'sub rsp,...8'

	cmp	edx, 2
	jne	SHORT $LN32@win64_Stor
	cmp	eax, edx
	jl	$LN44@win64_Stor

; 2274 :         for (i = 0; i<4; i++){

	mov	ecx, r14d
	mov	rax, r14
$LL7@win64_Stor:

; 2275 :           if (info->home_used[i] == 0) break;               //we need only one space   

	cmp	BYTE PTR [rsi+rax+76], r14b
	je	SHORT $LN60@win64_Stor

; 2274 :         for (i = 0; i<4; i++){

	inc	ecx
	inc	rax
	cmp	rax, r8
	jl	SHORT $LL7@win64_Stor
$LN60@win64_Stor:

; 2276 :         }
; 2277 :         for (++i; i<4; i++)                                 //the rest of free spaces render as taken

	lea	eax, DWORD PTR [rcx+1]

; 2278 :           info->home_used[i] = 1;
; 2279 :       }

	jmp	SHORT $LN92@win64_Stor
$LN32@win64_Stor:

; 2280 :       else if (grcount == 3){                               //3 registers?

	cmp	edx, 3
	jne	SHORT $LN35@win64_Stor

; 2281 :         if (freeshadow == 1) memset(info->home_used, 1, 4); //if only 1 free shadow, don't store it, push them

	cmp	eax, 1
	jne	SHORT $LN37@win64_Stor
	mov	rax, 72340172838076673			; 0101010101010101H
	mov	DWORD PTR [rsi+76], eax

; 2282 :         if (freeshadow >= 3){                               //enen if there is enough space we will store only two

	jmp	$LN44@win64_Stor
$LN37@win64_Stor:
	cmp	eax, 3
	jl	$LN44@win64_Stor

; 2283 :           for (i = 0; i<4; i++){                            //the third one we will push to keep uneven stack 

	mov	eax, r14d
	mov	rcx, r14
$LL13@win64_Stor:

; 2284 :             if (info->home_used[i] == 0) break;             //found first availible

	cmp	BYTE PTR [rsi+rcx+76], r14b
	je	SHORT $LN61@win64_Stor

; 2283 :           for (i = 0; i<4; i++){                            //the third one we will push to keep uneven stack 

	inc	eax
	inc	rcx
	cmp	rcx, r8
	jl	SHORT $LL13@win64_Stor
$LN61@win64_Stor:

; 2285 :           }
; 2286 :           for (++i; i<4; i++){

	inc	eax
	movsxd	rcx, eax
	cmp	rcx, r8
	jge	SHORT $LN88@win64_Stor
$LL16@win64_Stor:

; 2287 :             if (info->home_used[i] == 0) break;            //found second availible

	cmp	BYTE PTR [rsi+rcx+76], r14b
	je	SHORT $LN88@win64_Stor

; 2285 :           }
; 2286 :           for (++i; i<4; i++){

	inc	eax
	inc	rcx
	cmp	rcx, r8
	jl	SHORT $LL16@win64_Stor
$LN88@win64_Stor:

; 2288 :           }
; 2289 :           for (++i; i<4; i++)

	inc	eax
$LN92@win64_Stor:
	movsxd	rcx, eax
	cmp	rcx, r8
	jge	SHORT $LN44@win64_Stor
	lea	rdi, QWORD PTR [rsi+76]
	mov	eax, 1
	add	rdi, rcx
	sub	r8, rcx
	mov	rcx, r8
	rep stosb

; 2290 :             info->home_used[i] = 1;                        //render the rest of the shadow spaces as taken
; 2291 :         }
; 2292 :       }

	jmp	SHORT $LN44@win64_Stor
$LN35@win64_Stor:

; 2293 :       else if (grcount == 4 && freeshadow == 4){          //easy case

	cmp	edx, r8d
	jne	SHORT $LN91@win64_Stor
	cmp	eax, r8d
	jne	SHORT $LN44@win64_Stor

; 2294 :         info->home_used[3] = 1;                           //render the last shadow spaces as taken

	mov	BYTE PTR [rsi+79], 1

; 2295 :       }                                                   //remember the first space is 0 than 1, 2 and 3 follow 

	jmp	SHORT $LN44@win64_Stor
$LN91@win64_Stor:

; 2296 :       else if (grcount > 4){                              //if more registars than spaces

	jle	SHORT $LN44@win64_Stor

; 2297 :         freeshadow = grcount - freeshadow;                //find out how many we can store

	sub	edx, eax

; 2298 :         if (!(freeshadow & 1)){                           //if it is even number

	test	dl, 1
	jne	SHORT $LN44@win64_Stor

; 2299 :           for (i = 0; i<4; i++){                          //than we have to disable one space

	mov	ecx, r14d
	mov	rax, r14
$LL22@win64_Stor:

; 2300 :             if (info->home_used[i] == 0) break;           //find the first free space 

	cmp	BYTE PTR [rsi+rax+76], r14b
	je	SHORT $LN63@win64_Stor

; 2299 :           for (i = 0; i<4; i++){                          //than we have to disable one space

	inc	ecx
	inc	rax
	cmp	rax, r8
	jl	SHORT $LL22@win64_Stor
$LN63@win64_Stor:

; 2301 :           }
; 2302 :           info->home_used[i] = 1;                         //and render it as taken

	movsxd	rax, ecx
	mov	BYTE PTR [rax+rsi+76], 1
$LN44@win64_Stor:

; 2303 :         }
; 2304 :       }
; 2305 :     }
; 2306 :     for (i = 0, regist = info->regslist, cnt = *regist++; cnt; cnt--, regist++, i++) {

	mov	rdi, QWORD PTR [rsi]
	movzx	ebx, WORD PTR [rdi]
	add	rdi, 2
	test	ebx, ebx
	je	$LN90@win64_Stor
	mov	QWORD PTR [rsp+72], rbp
	mov	rbp, r14
	npad	11
$LL25@win64_Stor:

; 2307 :       if (GetValueSp(*regist) & (OP_XYZMM )) continue;

	movzx	ecx, WORD PTR [rdi]
	lea	rax, QWORD PTR [rcx+rcx*2]
	test	BYTE PTR [r15+rax*4], 128		; 00000080H
	jne	SHORT $LN23@win64_Stor

; 2308 :       else {
; 2309 :         sizestd += 8;

	add	r14d, 8

; 2310 :         if (i < 4)

	cmp	rbp, 4
	jge	SHORT $LN23@win64_Stor

; 2311 :         {
; 2312 :           if (info->home_used[i] == 0){

	cmp	BYTE PTR [rsi+rbp+76], 0
	jne	SHORT $LN49@win64_Stor

; 2313 :             AddLineQueueX("mov [%r+%u], %r", T_RSP, NUMQUAL sizestd, *regist);

	mov	r9d, ecx
	mov	r8d, r14d
	lea	rcx, OFFSET FLAT:$SG12186
	mov	edx, 119				; 00000077H
	call	AddLineQueueX

; 2314 :             AddLineQueueX("%r %r, %u", T_DOT_SAVEREG, *regist, NUMQUAL sizestd);

	movzx	r8d, WORD PTR [rdi]
	lea	rcx, OFFSET FLAT:$SG12187
	mov	r9d, r14d
	mov	edx, 413				; 0000019dH
	call	AddLineQueueX

; 2315 :             info->stored_reg++;

	inc	DWORD PTR [rsi+60]

; 2316 :           }

	jmp	SHORT $LN23@win64_Stor
$LN49@win64_Stor:

; 2317 :           else {
; 2318 :             cnt++; regist--;

	inc	ebx
	sub	rdi, 2
$LN23@win64_Stor:

; 2303 :         }
; 2304 :       }
; 2305 :     }
; 2306 :     for (i = 0, regist = info->regslist, cnt = *regist++; cnt; cnt--, regist++, i++) {

	add	rdi, 2
	inc	rbp
	sub	ebx, 1
	jne	SHORT $LL25@win64_Stor
	mov	rbp, QWORD PTR [rsp+72]
$LN90@win64_Stor:
	mov	rbx, QWORD PTR [rsp+64]
	mov	r15, QWORD PTR [rsp+80]
$LN24@win64_Stor:

; 2319 :           }
; 2320 :         }
; 2321 :       }
; 2322 :     }/* end for */
; 2323 :   }
; 2324 :   return;
; 2325 : }

	add	rsp, 32					; 00000020H
	pop	r14
	pop	rdi
	pop	rsi
	ret	0
win64_StoreRegHome ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
;	COMDAT win64_SaveRegParams
_TEXT	SEGMENT
info$ = 64
win64_SaveRegParams PROC				; COMDAT

; 2199 : {

	mov	QWORD PTR [rsp+8], rbx
	mov	QWORD PTR [rsp+16], rbp
	mov	QWORD PTR [rsp+24], rsi
	push	rdi
	push	r14
	push	r15
	sub	rsp, 32					; 00000020H

; 2200 :     int i;
; 2201 :     struct dsym *param;
; 2202 :     if (ModuleInfo.win64_flags & W64F_HABRAN){
; 2203 :       int			   cnt;
; 2204 :       uint_16        *regist;
; 2205 :       info->home_taken = 0;

	xor	ebp, ebp
	mov	r14, rcx
	test	BYTE PTR ModuleInfo+413, 11
	je	$LN8@win64_Save

; 2206 :       memset(info->home_used, 0, 4);

	xor	eax, eax
	mov	DWORD PTR [rcx+68], ebp
	lea	r15, QWORD PTR [rcx+76]
	mov	DWORD PTR [r15], eax

; 2207 :       if (info->regslist)
; 2208 :         regist = info->regslist;
; 2209 : 
; 2210 :       for (i = 0, param = info->paralist; param && (i < 4); i++) {

	mov	rdi, QWORD PTR [rcx+8]
	test	rdi, rdi
	je	$LN29@win64_Save
	mov	ebx, 8
	lea	rsi, OFFSET FLAT:ms64_regs
	npad	1
$LL4@win64_Save:
	cmp	ebx, 40					; 00000028H
	jge	$LN29@win64_Save

; 2211 :         /* v2.05: save XMMx if type is float/double */
; 2212 :         if (param->sym.is_vararg == FALSE) {

	test	BYTE PTR [rdi+47], 32			; 00000020H
	jne	SHORT $LN11@win64_Save

; 2213 :           if ((param->sym.mem_type & MT_FLOAT) && param->sym.used){  // added  && param->sym.used

	test	BYTE PTR [rdi+36], 32			; 00000020H
	je	SHORT $LN13@win64_Save
	test	BYTE PTR [rdi+40], 1
	je	SHORT $LN15@win64_Save

; 2214 :             AddLineQueueX("movq [%r+%u], %r", T_RSP, 8 + i * 8, T_XMM0 + i);

	lea	r9d, DWORD PTR [rbp+40]
	lea	rcx, OFFSET FLAT:$SG12116

; 2215 :             info->home_used[i] = 1;
; 2216 :             ++info->home_taken;
; 2217 :           }

	jmp	SHORT $LN31@win64_Save
$LN13@win64_Save:

; 2218 :           else{
; 2219 :             if (param->sym.used){                                    //here as well 

	test	BYTE PTR [rdi+40], 1
	je	SHORT $LN15@win64_Save

; 2220 :               AddLineQueueX("mov [%r+%u], %r", T_RSP, 8 + i * 8, ms64_regs[i]);

	mov	r9d, DWORD PTR [rsi]
	lea	rcx, OFFSET FLAT:$SG12118
$LN31@win64_Save:
	mov	r8d, ebx
	mov	edx, 119				; 00000077H
	call	AddLineQueueX

; 2221 :               info->home_used[i] = 1;

	mov	BYTE PTR [r15], 1

; 2222 :               ++info->home_taken;

	inc	DWORD PTR [r14+68]
$LN15@win64_Save:

; 2223 :             }
; 2224 :           }
; 2225 :           param = param->nextparam;

	mov	rdi, QWORD PTR [rdi+112]

; 2226 :         }

	jmp	SHORT $LN2@win64_Save
$LN11@win64_Save:

; 2227 :         else { /* v2.09: else branch added */
; 2228 :           AddLineQueueX("mov [%r+%u], %r", T_RSP, 8 + i * 8, ms64_regs[i]);

	mov	r9d, DWORD PTR [rsi]
	lea	rcx, OFFSET FLAT:$SG12119
	mov	r8d, ebx
	mov	edx, 119				; 00000077H
	call	AddLineQueueX

; 2229 :           info->home_used[i] = 1;

	mov	BYTE PTR [r15], 1

; 2230 :           ++info->home_taken;

	inc	DWORD PTR [r14+68]
$LN2@win64_Save:

; 2207 :       if (info->regslist)
; 2208 :         regist = info->regslist;
; 2209 : 
; 2210 :       for (i = 0, param = info->paralist; param && (i < 4); i++) {

	inc	ebp
	add	ebx, 8
	add	rsi, 4
	inc	r15
	test	rdi, rdi
	jne	$LL4@win64_Save

; 2231 :         }
; 2232 :       }
; 2233 :     }

	jmp	SHORT $LN29@win64_Save
$LN8@win64_Save:

; 2234 :     else {
; 2235 :       for (i = 0, param = info->paralist; param && (i < 4); i++) {

	mov	rdi, QWORD PTR [rcx+8]
	test	rdi, rdi
	je	SHORT $LN29@win64_Save
	mov	ebx, 8
	lea	rsi, OFFSET FLAT:ms64_regs
	npad	2
$LL7@win64_Save:
	cmp	ebx, 40					; 00000028H
	jge	SHORT $LN29@win64_Save

; 2236 :         /* v2.05: save XMMx if type is float/double */
; 2237 :         if (param->sym.is_vararg == FALSE) {

	test	BYTE PTR [rdi+47], 32			; 00000020H

; 2239 :             AddLineQueueX("movq [%r+%u], %r", T_RSP, 8 + i * 8, T_XMM0 + i);

	mov	r8d, ebx
	mov	edx, 119				; 00000077H
	jne	SHORT $LN16@win64_Save

; 2238 :           if (param->sym.mem_type & MT_FLOAT)

	test	BYTE PTR [rdi+36], 32			; 00000020H
	je	SHORT $LN18@win64_Save

; 2239 :             AddLineQueueX("movq [%r+%u], %r", T_RSP, 8 + i * 8, T_XMM0 + i);

	lea	r9d, DWORD PTR [rbp+40]
	lea	rcx, OFFSET FLAT:$SG12124

; 2240 :           else
; 2241 :             AddLineQueueX("mov [%r+%u], %r", T_RSP, 8 + i * 8, ms64_regs[i]);

	call	AddLineQueueX

; 2242 :           param = param->nextparam;

	mov	rdi, QWORD PTR [rdi+112]

; 2243 :         }

	jmp	SHORT $LN5@win64_Save
$LN18@win64_Save:

; 2240 :           else
; 2241 :             AddLineQueueX("mov [%r+%u], %r", T_RSP, 8 + i * 8, ms64_regs[i]);

	mov	r9d, DWORD PTR [rsi]
	lea	rcx, OFFSET FLAT:$SG12125
	call	AddLineQueueX

; 2242 :           param = param->nextparam;

	mov	rdi, QWORD PTR [rdi+112]

; 2243 :         }

	jmp	SHORT $LN5@win64_Save
$LN16@win64_Save:

; 2244 :         else { /* v2.09: else branch added */
; 2245 :           AddLineQueueX("mov [%r+%u], %r", T_RSP, 8 + i * 8, ms64_regs[i]);

	mov	r9d, DWORD PTR [rsi]
	lea	rcx, OFFSET FLAT:$SG12126
	call	AddLineQueueX
$LN5@win64_Save:

; 2234 :     else {
; 2235 :       for (i = 0, param = info->paralist; param && (i < 4); i++) {

	inc	ebp
	add	ebx, 8
	add	rsi, 4
	test	rdi, rdi
	jne	SHORT $LL7@win64_Save
$LN29@win64_Save:

; 2246 :         }
; 2247 :       }
; 2248 :     }
; 2249 :     return;
; 2250 : }

	mov	rbx, QWORD PTR [rsp+64]
	mov	rbp, QWORD PTR [rsp+72]
	mov	rsi, QWORD PTR [rsp+80]
	add	rsp, 32					; 00000020H
	pop	r15
	pop	r14
	pop	rdi
	ret	0
win64_SaveRegParams ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
;	COMDAT write_userdef_prologue
_TEXT	SEGMENT
reglst$ = 64
buffer$ = 192
tokenarray$ = 848
is_exitm$ = 856
write_userdef_prologue PROC				; COMDAT

; 2108 : {

	push	rbp
	push	r12
	sub	rsp, 824				; 00000338H

; 2109 :     int                 len;
; 2110 :     int                 i;
; 2111 :     struct proc_info    *info;
; 2112 :     char                *p;
; 2113 :     bool                is_exitm;
; 2114 :     struct dsym         *dir;
; 2115 :     //int                 align = CurrWordSize;
; 2116 :     int                 flags = CurrProc->sym.langtype; /* set bits 0-2 */
; 2117 :     uint_16             *regs;
; 2118 :     char                reglst[128];
; 2119 :     char                buffer[MAX_LINE_LEN];
; 2120 : 
; 2121 : #if FASTPASS
; 2122 :     if ( Parse_Pass > PASS_1 && UseSavedState )

	cmp	DWORD PTR Parse_Pass, 0
	mov	r12, rcx
	mov	r8, QWORD PTR CurrProc
	mov	edx, DWORD PTR [r8+76]
	mov	ebp, edx
	jbe	SHORT $LN8@write_user
	cmp	BYTE PTR UseSavedState, 0
	je	SHORT $LN8@write_user

; 2123 :         return( NOT_ERROR );

	xor	eax, eax

; 2191 : }

	add	rsp, 824				; 00000338H
	pop	r12
	pop	rbp
	ret	0
$LN8@write_user:
	mov	QWORD PTR [rsp+816], r13

; 2124 : #endif
; 2125 : 
; 2126 :     info = CurrProc->e.procinfo;
; 2127 :     /* v2.11: now done in write_prologue() */
; 2128 :     //info->localsize = ROUND_UP( info->localsize, CurrWordSize );
; 2129 : #if AMD64_SUPPORT
; 2130 :     /* to be compatible with ML64, translate FASTCALL to 0 (not 7) */
; 2131 :     if ( CurrProc->sym.langtype == LANG_FASTCALL && ModuleInfo.fctype == FCT_WIN64 )

	xor	r13d, r13d
	mov	QWORD PTR [rsp+808], r14
	mov	r14, QWORD PTR [r8+96]
	mov	QWORD PTR [rsp+800], r15
	cmp	edx, 7
	jne	SHORT $LN9@write_user
	cmp	DWORD PTR ModuleInfo+376, 2
	cmove	ebp, r13d
$LN9@write_user:
	lea	eax, DWORD PTR [rdx-1]
	cmp	eax, 1
	jbe	SHORT $LN11@write_user
	cmp	edx, 7
	jne	SHORT $LN10@write_user
$LN11@write_user:

; 2132 :         flags = 0;
; 2133 : #endif
; 2134 :     /* set bit 4 if the caller restores (E)SP */
; 2135 :     if ( CurrProc->sym.langtype == LANG_C ||
; 2136 :         CurrProc->sym.langtype == LANG_SYSCALL ||
; 2137 :         CurrProc->sym.langtype == LANG_FASTCALL )
; 2138 :         flags |= 0x10;

	or	ebp, 16
$LN10@write_user:

; 2139 : 
; 2140 :     /* set bit 5 if proc is far */
; 2141 :     /* set bit 6 if proc is private */
; 2142 :     /* v2.11: set bit 7 if proc is export */
; 2143 :     flags |= ( CurrProc->sym.mem_type == MT_FAR ? 0x20 : 0 );
; 2144 :     flags |= ( CurrProc->sym.ispublic ? 0 : 0x40 );
; 2145 :     flags |= ( info->isexport ? 0x80 : 0 );

	movzx	ecx, BYTE PTR [r8+40]
	mov	edx, 32					; 00000020H
	movzx	eax, BYTE PTR [r14+84]
	not	cl
	shr	ecx, 1
	and	eax, 4
	and	ecx, 64					; 00000040H
	shl	eax, 5
	or	ecx, eax
	mov	eax, r13d
	cmp	DWORD PTR [r8+36], 130			; 00000082H
	cmove	eax, edx
	or	ecx, eax
	or	ebp, ecx

; 2146 : 
; 2147 :     dir = (struct dsym *)SymSearch( ModuleInfo.proc_prologue );

	mov	rcx, QWORD PTR ModuleInfo+320
	call	SymFind
	mov	r15, rax

; 2148 :     if ( dir == NULL || dir->sym.state != SYM_MACRO || dir->sym.isfunc != TRUE ) {

	test	rax, rax
	je	$LN13@write_user
	cmp	DWORD PTR [rax+32], 9
	jne	$LN13@write_user
	test	BYTE PTR [rax+44], 2
	je	$LN13@write_user

; 2150 :     }
; 2151 : 
; 2152 :     /* if -EP is on, emit "prologue: none" */
; 2153 :     if ( Options.preprocessor_stdout )

	cmp	BYTE PTR Options+125, r13b
	mov	QWORD PTR [rsp+848], rbx
	mov	QWORD PTR [rsp+864], rsi
	je	SHORT $LN14@write_user

; 2154 :         printf( "option prologue:none\n" );

	lea	rcx, OFFSET FLAT:$SG12089
	call	printf
$LN14@write_user:

; 2155 : 
; 2156 :     p = reglst;
; 2157 :     if ( info->regslist ) {

	mov	rsi, QWORD PTR [r14]
	lea	rbx, QWORD PTR reglst$[rsp]
	test	rsi, rsi
	je	SHORT $LN3@write_user

; 2158 :         regs = info->regslist;
; 2159 :         for ( len = *regs++; len; len--, regs++ ) {

	mov	QWORD PTR [rsp+872], rdi
	movzx	edi, WORD PTR [rsi]
	add	rsi, 2
	test	edi, edi
	je	SHORT $LN34@write_user
	npad	6
$LL4@write_user:

; 2160 :             GetResWName( *regs, p );

	movzx	ecx, WORD PTR [rsi]
	mov	rdx, rbx
	call	GetResWName

; 2161 :             p += strlen( p );

	or	rax, -1
	npad	1
$LL30@write_user:
	inc	rax
	cmp	BYTE PTR [rbx+rax], r13b
	jne	SHORT $LL30@write_user
	add	rbx, rax

; 2162 :             if ( len > 1 )

	cmp	edi, 1
	jle	SHORT $LN2@write_user

; 2163 :                 *p++ = ',';

	mov	BYTE PTR [rbx], 44			; 0000002cH
	inc	rbx
$LN2@write_user:

; 2158 :         regs = info->regslist;
; 2159 :         for ( len = *regs++; len; len--, regs++ ) {

	add	rsi, 2
	sub	edi, 1
	jne	SHORT $LL4@write_user
$LN34@write_user:
	mov	rdi, QWORD PTR [rsp+872]
$LN3@write_user:

; 2164 :         }
; 2165 :     }
; 2166 :     *p = NULLC;

	mov	BYTE PTR [rbx], r13b

; 2167 : 
; 2168 :     /* v2.07: make this work with radix != 10 */
; 2169 :     /* leave a space at pos 0 of buffer, because the buffer is used for
; 2170 :      * both macro arguments and EXITM return value.
; 2171 :      */
; 2172 :     sprintf( buffer," (%s, 0%XH, 0%XH, 0%XH, <<%s>>, <%s>)",

	lea	rcx, OFFSET FLAT:$SG12092
	mov	rax, QWORD PTR [r14+40]
	lea	rdx, OFFSET FLAT:$SG12093
	mov	r8, QWORD PTR CurrProc
	test	rax, rax
	mov	r9d, ebp
	cmovne	rcx, rax
	lea	rax, QWORD PTR reglst$[rsp]
	mov	QWORD PTR [rsp+56], rcx
	lea	rcx, QWORD PTR buffer$[rsp]
	mov	r8, QWORD PTR [r8+8]
	mov	QWORD PTR [rsp+48], rax
	mov	eax, DWORD PTR [r14+36]
	mov	DWORD PTR [rsp+40], eax
	mov	eax, DWORD PTR [r14+32]
	mov	DWORD PTR [rsp+32], eax
	call	sprintf

; 2173 :              CurrProc->sym.name, flags, info->parasize, info->localsize,
; 2174 :             reglst, info->prologuearg ? info->prologuearg : "" );
; 2175 :     i = Token_Count + 1;

	mov	ebx, DWORD PTR ModuleInfo+496

; 2176 :     Token_Count = Tokenize( buffer, i, tokenarray, TOK_RESCAN );

	lea	rcx, QWORD PTR buffer$[rsp]
	mov	r9d, 1
	mov	r8, r12
	lea	edx, DWORD PTR [rbx+1]
	call	Tokenize
	mov	DWORD PTR ModuleInfo+496, eax

; 2177 : 
; 2178 :     RunMacro( dir, i, tokenarray, buffer, 0, &is_exitm );

	lea	r9, QWORD PTR buffer$[rsp]
	lea	rax, QWORD PTR is_exitm$[rsp]
	mov	r8, r12
	mov	QWORD PTR [rsp+40], rax
	lea	edx, DWORD PTR [rbx+1]
	mov	rcx, r15
	mov	DWORD PTR [rsp+32], r13d
	call	RunMacro

; 2179 :     Token_Count = i - 1;
; 2180 :     DebugMsg(("write_userdef_prologue: macro %s returned >%s<\n", ModuleInfo.proc_prologue, buffer ));
; 2181 : 
; 2182 :     if ( Parse_Pass == PASS_1 ) {

	cmp	DWORD PTR Parse_Pass, r13d
	mov	rsi, QWORD PTR [rsp+864]
	mov	DWORD PTR ModuleInfo+496, ebx
	mov	rbx, QWORD PTR [rsp+848]
	jne	SHORT $LN6@write_user

; 2183 :         struct dsym *curr;
; 2184 :         len = atoi( buffer ) - info->localsize;

	lea	rcx, QWORD PTR buffer$[rsp]
	call	atoi
	sub	eax, DWORD PTR [r14+36]

; 2185 :         for ( curr = info->locallist; curr; curr = curr->nextlocal ) {

	mov	rcx, QWORD PTR [r14+16]
	test	rcx, rcx
	je	SHORT $LN6@write_user
$LL7@write_user:

; 2186 :             curr->sym.offset -= len;

	sub	DWORD PTR [rcx+16], eax
	mov	rcx, QWORD PTR [rcx+112]
	test	rcx, rcx
	jne	SHORT $LL7@write_user
$LN6@write_user:

; 2187 :         }
; 2188 :     }
; 2189 : 
; 2190 :     return ( NOT_ERROR );

	xor	eax, eax
	jmp	SHORT $LN33@write_user
$LN13@write_user:

; 2149 :         return( EmitError( PROLOGUE_MUST_BE_MACRO_FUNC ) );

	mov	ecx, 156				; 0000009cH
	call	EmitError
$LN33@write_user:
	mov	r13, QWORD PTR [rsp+816]
	mov	r14, QWORD PTR [rsp+808]
	mov	r15, QWORD PTR [rsp+800]

; 2191 : }

	add	rsp, 824				; 00000338H
	pop	r12
	pop	rbp
	ret	0
write_userdef_prologue ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
_TEXT	SEGMENT
opndx$ = 48
i$ = 208
tokenarray$ = 216
ExcFrameDirective PROC

; 1862 : {

$LN81:
	mov	DWORD PTR [rsp+8], ecx
	push	rbp
	push	r13
	push	r15
	lea	rbp, QWORD PTR [rsp-71]
	sub	rsp, 176				; 000000b0H
	mov	eax, DWORD PTR Options+144
	mov	r15, rdx

; 1863 :     struct expr opndx;
; 1864 :     int token;
; 1865 :     unsigned int size;
; 1866 :     uint_8 oldcodes = unw_info.CountOfCodes;

	movzx	r13d, BYTE PTR unw_info+2
	add	eax, -2					; fffffffeH
	cmp	eax, 1
	jbe	SHORT $LN6@ExcFrameDi

; 1867 :     uint_8 reg;
; 1868 :     uint_8 ofs;
; 1869 :     UNWIND_CODE *puc;
; 1870 : 
; 1871 :     DebugMsg1(("ExcFrameDirective(%s) enter\n", tokenarray[i].string_ptr ));
; 1872 :     /* v2.05: accept directives for windows only */
; 1873 : 	if (Options.output_format != OFORMAT_COFF && Options.output_format != OFORMAT_ELF /* John Hankinson 2016-02-10 Added elf win64 hack */
; 1874 : #if PE_SUPPORT
; 1875 :         && ModuleInfo.sub_format != SFORMAT_PE

	cmp	DWORD PTR ModuleInfo+372, 2
	je	SHORT $LN6@ExcFrameDi

; 1876 : #endif
; 1877 :        ) {
; 1878 :         return( EmitErr( NOT_SUPPORTED_WITH_CURR_FORMAT, GetResWName( tokenarray[i].tokval, NULL ) ) );

	movsxd	rax, ecx
	xor	edx, edx
	shl	rax, 5
	mov	ecx, DWORD PTR [rax+r15+16]
	call	GetResWName
	mov	rdx, rax
	mov	ecx, 224				; 000000e0H
	call	EmitErr

; 2090 : }

	add	rsp, 176				; 000000b0H
	pop	r15
	pop	r13
	pop	rbp
	ret	0
$LN6@ExcFrameDi:

; 1879 :     }
; 1880 :     if ( CurrProc == NULL || endprolog_found == TRUE ) {

	mov	rax, QWORD PTR CurrProc
	test	rax, rax
	je	$LN8@ExcFrameDi
	cmp	BYTE PTR endprolog_found, 1
	je	$LN8@ExcFrameDi

; 1882 :     }
; 1883 :     if ( CurrProc->e.procinfo->isframe == FALSE ) {

	mov	rax, QWORD PTR [rax+96]
	test	BYTE PTR [rax+84], 64			; 00000040H
	jne	SHORT $LN9@ExcFrameDi

; 1884 :         return( EmitError( MISSING_FRAME_IN_PROC ) );

	mov	ecx, 255				; 000000ffH
	call	EmitError

; 2090 : }

	add	rsp, 176				; 000000b0H
	pop	r15
	pop	r13
	pop	rbp
	ret	0
$LN9@ExcFrameDi:
	mov	QWORD PTR [rsp+216], rbx
	mov	QWORD PTR [rsp+224], rsi

; 1885 :     }
; 1886 : 
; 1887 :     puc = &unw_code[unw_info.CountOfCodes];

	lea	rsi, OFFSET FLAT:__ImageBase
	movzx	eax, r13b
	mov	QWORD PTR [rsp+232], rdi
	mov	QWORD PTR [rsp+168], r12
	mov	QWORD PTR [rsp+160], r14
	lea	rbx, QWORD PTR unw_code[rax*2]
	add	rbx, rsi

; 1888 : 
; 1889 :     ofs = GetCurrOffset() - CurrProc->sym.offset;

	call	GetCurrOffset

; 1890 :     token = tokenarray[i].tokval;

	movsxd	r8, DWORD PTR i$[rbp-105]
	mov	r12d, eax
	mov	rcx, QWORD PTR CurrProc
	sub	r12b, BYTE PTR [rcx+16]
	mov	rcx, r8
	shl	rcx, 5

; 1891 :     i++;

	inc	r8d
	mov	edi, DWORD PTR [rcx+r15+16]
	mov	DWORD PTR i$[rbp-105], r8d

; 1892 : 
; 1893 :     /* note: since the codes will be written from "right to left",
; 1894 :      * the opcode item has to be written last!
; 1895 :      */
; 1896 : 
; 1897 :     switch ( token ) {

	lea	eax, DWORD PTR [rdi-409]
	cmp	eax, 7
	ja	$LN62@ExcFrameDi
	movsxd	rcx, eax
	mov	eax, DWORD PTR $LN60@ExcFrameDi[rsi+rcx*4]
	add	rax, rsi
	jmp	rax
$LN10@ExcFrameDi:

; 1898 :     case T_DOT_ALLOCSTACK: /* syntax: .ALLOCSTACK size */
; 1899 :         if ( ERROR == EvalOperand( &i, tokenarray, Token_Count, &opndx, 0 ) )

	mov	r8d, DWORD PTR ModuleInfo+496
	lea	r9, QWORD PTR opndx$[rbp-105]
	mov	rdx, r15
	mov	BYTE PTR [rsp+32], 0
	lea	rcx, QWORD PTR i$[rbp-105]
	call	EvalOperand
	cmp	eax, -1
	je	$LN76@ExcFrameDi

; 1900 :             return( ERROR );
; 1901 :         if ( opndx.kind == EXPR_ADDR && opndx.sym->state == SYM_UNDEFINED ) /* v2.11: allow forward references */

	mov	eax, DWORD PTR opndx$[rbp-45]
	cmp	eax, 1
	jne	SHORT $LN12@ExcFrameDi
	mov	rax, QWORD PTR opndx$[rbp-25]
	cmp	DWORD PTR [rax+32], 0

; 1902 :              ;
; 1903 :         else if ( opndx.kind != EXPR_CONST ) {

	jmp	SHORT $LN69@ExcFrameDi
$LN12@ExcFrameDi:
	test	eax, eax
$LN69@ExcFrameDi:
	jne	$LN75@ExcFrameDi

; 1904 :             return( EmitError( CONSTANT_EXPECTED ) );
; 1905 :         }
; 1906 :         /* v2.11: check added */
; 1907 :         if ( opndx.hvalue ) {

	cmp	DWORD PTR opndx$[rbp-101], 0
	jne	$LN73@ExcFrameDi

; 1908 :             return( EmitConstError( &opndx ) );
; 1909 :         }
; 1910 :         if ( opndx.uvalue == 0 ) {

	mov	eax, DWORD PTR opndx$[rbp-105]
	test	eax, eax
	jne	SHORT $LN16@ExcFrameDi

; 1911 :             return( EmitError( NONZERO_VALUE_EXPECTED ) );

	mov	ecx, 257				; 00000101H
	call	EmitError
	jmp	$LN68@ExcFrameDi
$LN16@ExcFrameDi:

; 1912 :         }
; 1913 :         if ( opndx.value & 7 ) {

	test	al, 7
	jne	$LN74@ExcFrameDi

; 1919 :                 /* allocation size 512k - 4G-8 */
; 1920 :                 /* v2.11: value is stored UNSCALED in 2 WORDs! */
; 1921 :                 puc->FrameOffset = ( opndx.uvalue >> 16 );
; 1922 :                 puc++;
; 1923 :                 puc->FrameOffset = opndx.uvalue & 0xFFFF;
; 1924 :                 puc++;
; 1925 :                 unw_info.CountOfCodes += 2;

	movzx	ecx, BYTE PTR unw_info+2
	cmp	eax, 128				; 00000080H
	jbe	SHORT $LN18@ExcFrameDi

; 1914 :             return( EmitError( BAD_ALIGNMENT_FOR_OFFSET_IN_UNWIND_CODE ) );
; 1915 :         }
; 1916 :         //opndx.value -= 8; /* v2.11: subtract 8 only for UWOP_ALLOC_SMALL! */
; 1917 :         if ( opndx.uvalue > 16*8 ) {
; 1918 :             if ( opndx.uvalue >= 65536*8 ) {

	cmp	eax, 524288				; 00080000H
	jb	SHORT $LN20@ExcFrameDi

; 1926 :                 puc->OpInfo = 1;
; 1927 :                 DebugMsg1(("ExcFrameDirective: UWOP_ALLOC_LARGE, operation info 1, size=%Xh\n", opndx.value ));
; 1928 :             } else {
; 1929 :                 /* allocation size 128+8 - 512k-8 */
; 1930 :                 puc->FrameOffset = ( opndx.uvalue >> 3 );
; 1931 :                 puc++;
; 1932 :                 unw_info.CountOfCodes++;
; 1933 :                 puc->OpInfo = 0;
; 1934 :                 DebugMsg1(("ExcFrameDirective: UWOP_ALLOC_LARGE, operation info 0, size=%Xh\n", opndx.value ));
; 1935 :             }
; 1936 :             puc->UnwindOp = UWOP_ALLOC_LARGE;
; 1937 :         } else {
; 1938 :             /* allocation size 8-128 bytes */
; 1939 :             puc->UnwindOp = UWOP_ALLOC_SMALL;
; 1940 :             /* v2.11: subtract 8 only for UWOP_ALLOC_SMALL! */
; 1941 :             //puc->OpInfo = ( opndx.value >> 3 );
; 1942 :             puc->OpInfo = ( (opndx.uvalue - 8 ) >> 3 );
; 1943 :             DebugMsg1(("ExcFrameDirective: UWOP_ALLOC_SMALL, size=%Xh\n", opndx.value ));
; 1944 :         }
; 1945 :         puc->CodeOffset = ofs;
; 1946 :         unw_info.CountOfCodes++;
; 1947 :         break;

	mov	r8d, DWORD PTR i$[rbp-105]
	add	cl, 2
	shr	eax, 16
	mov	WORD PTR [rbx], ax
	movzx	eax, WORD PTR opndx$[rbp-105]
	mov	WORD PTR [rbx+2], ax
	add	rbx, 4
	and	BYTE PTR [rbx+1], 15
	or	BYTE PTR [rbx+1], 16
	and	BYTE PTR [rbx+1], 241			; 000000f1H
	or	BYTE PTR [rbx+1], 1
	inc	cl
	mov	BYTE PTR [rbx], r12b
	mov	BYTE PTR unw_info+2, cl
	jmp	$LN4@ExcFrameDi
$LN20@ExcFrameDi:
	mov	r8d, DWORD PTR i$[rbp-105]
	inc	cl
	shr	eax, 3
	mov	WORD PTR [rbx], ax
	add	rbx, 2
	and	BYTE PTR [rbx+1], 15
	and	BYTE PTR [rbx+1], 241			; 000000f1H
	or	BYTE PTR [rbx+1], 1
	inc	cl
	mov	BYTE PTR [rbx], r12b
	mov	BYTE PTR unw_info+2, cl
	jmp	SHORT $LN4@ExcFrameDi
$LN18@ExcFrameDi:
	mov	r8d, DWORD PTR i$[rbp-105]
	add	eax, -8					; fffffff8H
	shr	eax, 3
	shl	al, 4
	or	al, 2
	inc	cl
	mov	BYTE PTR [rbx+1], al
	mov	BYTE PTR unw_info+2, cl
	mov	BYTE PTR [rbx], r12b
	jmp	SHORT $LN4@ExcFrameDi
$LN22@ExcFrameDi:

; 1948 :     case T_DOT_ENDPROLOG: /* syntax: .ENDPROLOG */
; 1949 :         opndx.value = GetCurrOffset() - CurrProc->sym.offset;

	call	GetCurrOffset
	mov	rcx, QWORD PTR CurrProc
	sub	eax, DWORD PTR [rcx+16]
	mov	DWORD PTR opndx$[rbp-105], eax

; 1950 :         if ( opndx.uvalue > 255 ) {

	cmp	eax, 255				; 000000ffH
	jbe	SHORT $LN23@ExcFrameDi

; 1951 :             return( EmitError( SIZE_OF_PROLOG_TOO_BIG ) );

	mov	ecx, 258				; 00000102H
	call	EmitError
	jmp	$LN68@ExcFrameDi
$LN23@ExcFrameDi:

; 1952 :         }
; 1953 :         unw_info.SizeOfProlog = (uint_8)opndx.uvalue;

	movzx	eax, BYTE PTR opndx$[rbp-105]
	mov	BYTE PTR unw_info+1, al

; 1954 :         endprolog_found = TRUE;

	mov	BYTE PTR endprolog_found, 1
$LN63@ExcFrameDi:

; 1955 :         break;

	mov	r8d, DWORD PTR i$[rbp-105]
$LN62@ExcFrameDi:

; 2045 :             }
; 2046 :             puc->CodeOffset = ofs;
; 2047 :             puc->OpInfo = reg;

	movzx	ecx, BYTE PTR unw_info+2
$LN4@ExcFrameDi:

; 2077 :             break;
; 2078 :         }
; 2079 :         break;
; 2080 :     }
; 2081 :     if ( tokenarray[i].token != T_FINAL ) {

	movsxd	rax, r8d
	shl	rax, 5
	cmp	BYTE PTR [rax+r15], 0
	je	$LN54@ExcFrameDi

; 2082 :         return( EmitErr( SYNTAX_ERROR_EX, tokenarray[i].string_ptr ) );

	mov	rdx, QWORD PTR [rax+r15+8]
	jmp	$LN78@ExcFrameDi
$LN24@ExcFrameDi:

; 1956 :     case T_DOT_PUSHFRAME: /* syntax: .PUSHFRAME [code] */
; 1957 :         puc->CodeOffset = ofs;
; 1958 :         puc->UnwindOp = UWOP_PUSH_MACHFRAME;
; 1959 :         puc->OpInfo = 0;
; 1960 :         if ( tokenarray[i].token == T_ID && (_stricmp( tokenarray[i].string_ptr, "CODE") == 0 ) ) {

	movsxd	rax, r8d
	shl	rax, 5
	mov	BYTE PTR [rbx], r12b
	mov	BYTE PTR [rbx+1], 10
	cmp	BYTE PTR [rax+r15], 8
	jne	SHORT $LN25@ExcFrameDi
	mov	rcx, QWORD PTR [rax+r15+8]
	lea	rdx, OFFSET FLAT:$SG12023
	call	_stricmp

; 1962 :             i++;

	mov	r8d, DWORD PTR i$[rbp-105]
	test	eax, eax
	jne	SHORT $LN25@ExcFrameDi

; 1961 :             puc->OpInfo = 1;

	and	BYTE PTR [rbx+1], 15
	or	BYTE PTR [rbx+1], 16

; 1962 :             i++;

	inc	r8d
	mov	DWORD PTR i$[rbp-105], r8d
$LN25@ExcFrameDi:

; 1963 :         }
; 1964 :         unw_info.CountOfCodes++;

	movzx	ecx, BYTE PTR unw_info+2
	inc	cl
	mov	BYTE PTR unw_info+2, cl

; 1965 :         break;

	jmp	SHORT $LN4@ExcFrameDi
$LN26@ExcFrameDi:

; 1966 :     case T_DOT_PUSHREG: /* syntax: .PUSHREG r64 */
; 1967 :         if ( tokenarray[i].token != T_REG || !( GetValueSp( tokenarray[i].tokval ) & OP_R64 ) ) {

	movsxd	r9, r8d
	shl	r9, 5
	add	r9, r15
	cmp	BYTE PTR [r9], 2
	jne	SHORT $LN28@ExcFrameDi
	mov	eax, DWORD PTR [r9+16]
	lea	rcx, QWORD PTR [rax+rax*2]
	test	BYTE PTR SpecialTable[rsi+rcx*4], 8
	je	SHORT $LN28@ExcFrameDi

; 1969 :         }
; 1970 :         puc->CodeOffset = ofs;
; 1971 :         puc->UnwindOp = UWOP_PUSH_NONVOL;

	and	BYTE PTR [rbx+1], 240			; 000000f0H
	movzx	edx, BYTE PTR [rbx+1]

; 1972 :         puc->OpInfo = GetRegNo( tokenarray[i].tokval );

	and	dl, 15
	mov	BYTE PTR [rbx], r12b
	mov	eax, DWORD PTR [r9+16]
	lea	rcx, QWORD PTR [rax+rax*2]
	movzx	eax, BYTE PTR SpecialTable[rsi+rcx*4+10]

; 1973 :         unw_info.CountOfCodes++;

	movzx	ecx, BYTE PTR unw_info+2
	shl	al, 4
	inc	cl
	or	al, dl
	mov	BYTE PTR unw_info+2, cl

; 1974 :         i++;

	inc	r8d
	mov	BYTE PTR [rbx+1], al
	mov	DWORD PTR i$[rbp-105], r8d

; 1975 :         break;

	jmp	$LN4@ExcFrameDi
$LN28@ExcFrameDi:

; 1968 :             return( EmitErr( SYNTAX_ERROR_EX, tokenarray[i].string_ptr ) );

	mov	rdx, QWORD PTR [r9+8]
	jmp	SHORT $LN78@ExcFrameDi
$LN29@ExcFrameDi:

; 1976 :     case T_DOT_SAVEREG:    /* syntax: .SAVEREG r64, offset       */
; 1977 :     case T_DOT_SAVEXMM128: /* syntax: .SAVEXMM128 xmmreg, offset */
; 1978 :     case T_DOT_SAVEYMM256: /* syntax: .SAVEYMM256 ymmreg, offset */
; 1979 : #if EVEXSUPP
; 1980 :     case T_DOT_SAVEZMM512: /* syntax: .SAVEXMM512 zmmreg, offset */
; 1981 : #endif
; 1982 :     case T_DOT_SETFRAME:   /* syntax: .SETFRAME r64, offset      */
; 1983 :         if ( tokenarray[i].token != T_REG ) {

	movsxd	rdx, r8d
	shl	rdx, 5
	add	rdx, r15
	cmp	BYTE PTR [rdx], 2
	jne	SHORT $LN77@ExcFrameDi

; 1984 :             return( EmitErr( SYNTAX_ERROR_EX, tokenarray[i].string_ptr ) );
; 1985 :         }
; 1986 :         if ( token == T_DOT_SAVEXMM128 ) {
; 1987 :             if ( !( GetValueSp( tokenarray[i].tokval ) & OP_XMM ) ) {

	mov	eax, DWORD PTR [rdx+16]
	lea	rcx, QWORD PTR [rax+rax*2]
	lea	rcx, QWORD PTR [rcx*4]
	cmp	edi, 414				; 0000019eH
	jne	SHORT $LN31@ExcFrameDi
	test	BYTE PTR SpecialTable[rcx+rsi], 32	; 00000020H
$LN79@ExcFrameDi:

; 1992 :           if (!(GetValueSp(tokenarray[i].tokval) & OP_YMM)) {

	jne	SHORT $LN37@ExcFrameDi
$LN77@ExcFrameDi:

; 1993 :             return(EmitErr(SYNTAX_ERROR_EX, tokenarray[i].string_ptr));

	mov	rdx, QWORD PTR [rdx+8]
$LN78@ExcFrameDi:

; 2017 :             return( EmitErr( SYNTAX_ERROR_EX, tokenarray[i].string_ptr ) );

	mov	ecx, 209				; 000000d1H
	call	EmitErr
$LN68@ExcFrameDi:
	mov	r12, QWORD PTR [rsp+168]
	mov	rdi, QWORD PTR [rsp+232]
	mov	rsi, QWORD PTR [rsp+224]
	mov	rbx, QWORD PTR [rsp+216]
	mov	r14, QWORD PTR [rsp+160]

; 2090 : }

	add	rsp, 176				; 000000b0H
	pop	r15
	pop	r13
	pop	rbp
	ret	0
$LN31@ExcFrameDi:

; 1988 :                 return( EmitErr( SYNTAX_ERROR_EX, tokenarray[i].string_ptr ) );
; 1989 :             }
; 1990 :         } 
; 1991 :         else if (token == T_DOT_SAVEYMM256) {

	cmp	edi, 415				; 0000019fH
	jne	SHORT $LN34@ExcFrameDi

; 1992 :           if (!(GetValueSp(tokenarray[i].tokval) & OP_YMM)) {

	test	BYTE PTR SpecialTable[rcx+rsi], 128	; 00000080H

; 1993 :             return(EmitErr(SYNTAX_ERROR_EX, tokenarray[i].string_ptr));

	jmp	SHORT $LN79@ExcFrameDi
$LN34@ExcFrameDi:

; 1994 :           }
; 1995 :         }
; 1996 : #if EVEXSUPP
; 1997 :         else if (token == T_DOT_SAVEZMM512) {
; 1998 :           if (!(GetValueSp(tokenarray[i].tokval) & OP_ZMM)) {
; 1999 :             return(EmitErr(SYNTAX_ERROR_EX, tokenarray[i].string_ptr));
; 2000 :           }
; 2001 :         }
; 2002 : #endif
; 2003 :         else {
; 2004 :             if ( !( GetValueSp( tokenarray[i].tokval ) & OP_R64 ) ) {

	test	BYTE PTR SpecialTable[rcx+rsi], 8

; 2005 :                 return( EmitErr( SYNTAX_ERROR_EX, tokenarray[i].string_ptr ) );

	jmp	SHORT $LN79@ExcFrameDi
$LN37@ExcFrameDi:

; 2006 :             }
; 2007 :         }
; 2008 :         reg = GetRegNo( tokenarray[i].tokval );

	movzx	esi, BYTE PTR SpecialTable[rcx+rsi+10]

; 2009 : 
; 2010 :         if ( token == T_DOT_SAVEREG )

	cmp	edi, 413				; 0000019dH
	mov	eax, 8
	mov	r14d, 16
	cmove	r14d, eax

; 2011 :             size = 8;
; 2012 :         else
; 2013 :             size = 16;
; 2014 : 
; 2015 :         i++;

	inc	r8d

; 2016 :         if ( tokenarray[i].token != T_COMMA ) {

	movsxd	rdx, r8d
	shl	rdx, 5
	mov	DWORD PTR i$[rbp-105], r8d
	cmp	BYTE PTR [rdx+r15], 44			; 0000002cH
	je	SHORT $LN40@ExcFrameDi

; 2017 :             return( EmitErr( SYNTAX_ERROR_EX, tokenarray[i].string_ptr ) );

	mov	rdx, QWORD PTR [rdx+r15+8]
	jmp	$LN78@ExcFrameDi
$LN40@ExcFrameDi:

; 2018 :         }
; 2019 :         i++;

	inc	r8d

; 2020 :         if ( ERROR == EvalOperand( &i, tokenarray, Token_Count, &opndx, 0 ) )

	mov	BYTE PTR [rsp+32], 0
	mov	DWORD PTR i$[rbp-105], r8d
	lea	r9, QWORD PTR opndx$[rbp-105]
	mov	r8d, DWORD PTR ModuleInfo+496
	lea	rcx, QWORD PTR i$[rbp-105]
	mov	rdx, r15
	call	EvalOperand
	cmp	eax, -1
	jne	SHORT $LN41@ExcFrameDi
$LN76@ExcFrameDi:

; 2021 :             return( ERROR );

	or	eax, -1
	jmp	$LN68@ExcFrameDi
$LN41@ExcFrameDi:

; 2022 :         if ( opndx.kind == EXPR_ADDR && opndx.sym->state == SYM_UNDEFINED ) /* v2.11: allow forward references */

	mov	eax, DWORD PTR opndx$[rbp-45]
	cmp	eax, 1
	jne	SHORT $LN42@ExcFrameDi
	mov	rax, QWORD PTR opndx$[rbp-25]
	cmp	DWORD PTR [rax+32], 0

; 2023 :              ;
; 2024 :         else if ( opndx.kind != EXPR_CONST ) {

	jmp	SHORT $LN70@ExcFrameDi
$LN42@ExcFrameDi:
	test	eax, eax
$LN70@ExcFrameDi:
	je	SHORT $LN44@ExcFrameDi
$LN75@ExcFrameDi:

; 2025 :             return( EmitError( CONSTANT_EXPECTED ) );

	mov	ecx, 65					; 00000041H
	call	EmitError
	jmp	$LN68@ExcFrameDi
$LN44@ExcFrameDi:

; 2026 :         }
; 2027 :         if ( opndx.value & (size - 1) ) {

	mov	ecx, DWORD PTR opndx$[rbp-105]
	lea	eax, DWORD PTR [r14-1]
	test	eax, ecx
	je	SHORT $LN45@ExcFrameDi
$LN74@ExcFrameDi:

; 2028 :             return( EmitError( BAD_ALIGNMENT_FOR_OFFSET_IN_UNWIND_CODE ) );

	mov	ecx, 256				; 00000100H
	call	EmitError
	jmp	$LN68@ExcFrameDi
$LN45@ExcFrameDi:

; 2029 :         }
; 2030 :         switch ( token ) {

	sub	edi, 413				; 0000019dH
	je	$LN46@ExcFrameDi
	sub	edi, 1
	je	SHORT $LN49@ExcFrameDi
	cmp	edi, 2
	jne	$LN63@ExcFrameDi

; 2066 :         case T_DOT_SETFRAME:
; 2067 :             if ( opndx.uvalue > 240 ) {

	cmp	ecx, 240				; 000000f0H
	jbe	SHORT $LN53@ExcFrameDi
$LN73@ExcFrameDi:

; 2068 :                 return( EmitConstError( &opndx ) );

	lea	rcx, QWORD PTR opndx$[rbp-105]
	call	EmitConstError
	jmp	$LN68@ExcFrameDi
$LN53@ExcFrameDi:

; 2069 :             }
; 2070 :             unw_info.FrameRegister = reg;
; 2071 :             unw_info.FrameOffset = ( opndx.uvalue >> 4 );
; 2072 :             puc->CodeOffset = ofs;
; 2073 :             puc->UnwindOp = UWOP_SET_FPREG;
; 2074 :             //puc->OpInfo = ( opndx.uvalue >> 4 );
; 2075 :             puc->OpInfo = reg;
; 2076 :             unw_info.CountOfCodes++;

	mov	r8d, DWORD PTR i$[rbp-105]
	movzx	eax, sil
	shr	ecx, 4
	and	al, 15
	shl	cl, 4
	or	cl, al
	shl	sil, 4
	mov	BYTE PTR unw_info+3, cl
	or	sil, 3
	movzx	ecx, BYTE PTR unw_info+2
	inc	cl
	mov	BYTE PTR [rbx], r12b
	mov	BYTE PTR unw_info+2, cl
	mov	BYTE PTR [rbx+1], sil
	jmp	$LN4@ExcFrameDi
$LN49@ExcFrameDi:

; 2048 :             break;
; 2049 :         case T_DOT_SAVEXMM128:
; 2050 :             if ( opndx.value > 65536 * size ) {

	shl	r14d, 16
	cmp	ecx, r14d
	jbe	SHORT $LN50@ExcFrameDi

; 2051 :                 puc->FrameOffset = ( opndx.value >> 20 );

	mov	eax, ecx

; 2052 :                 puc++;
; 2053 :                 puc->FrameOffset = ( opndx.value >> 4 );

	sar	ecx, 4
	sar	eax, 20
	mov	WORD PTR [rbx], ax
	mov	WORD PTR [rbx+2], cx

; 2054 :                 puc++;

	add	rbx, 4

; 2055 :                 puc->UnwindOp = UWOP_SAVE_XMM128_FAR;
; 2056 :                 unw_info.CountOfCodes += 3;

	movzx	ecx, BYTE PTR unw_info+2
	and	BYTE PTR [rbx+1], 249			; 000000f9H
	or	BYTE PTR [rbx+1], 9
	add	cl, 3

; 2057 :             } else {

	jmp	SHORT $LN71@ExcFrameDi
$LN50@ExcFrameDi:

; 2058 :                 puc->FrameOffset = ( opndx.value >> 4 );

	sar	ecx, 4
	mov	WORD PTR [rbx], cx

; 2059 :                 puc++;

	add	rbx, 2

; 2060 :                 puc->UnwindOp = UWOP_SAVE_XMM128;
; 2061 :                 unw_info.CountOfCodes += 2;

	movzx	ecx, BYTE PTR unw_info+2
	and	BYTE PTR [rbx+1], 248			; 000000f8H
	or	BYTE PTR [rbx+1], 8
	add	cl, 2
$LN71@ExcFrameDi:

; 2062 :             }
; 2063 :             puc->CodeOffset = ofs;
; 2064 :             puc->OpInfo = reg;

	and	BYTE PTR [rbx+1], 15

; 2065 :             break;

	mov	r8d, DWORD PTR i$[rbp-105]
	shl	sil, 4
	or	BYTE PTR [rbx+1], sil
	mov	BYTE PTR unw_info+2, cl
	mov	BYTE PTR [rbx], r12b
	jmp	$LN4@ExcFrameDi
$LN46@ExcFrameDi:

; 2031 :         case T_DOT_SAVEREG:
; 2032 :             puc->OpInfo = reg;

	and	BYTE PTR [rbx+1], 15
	shl	sil, 4
	or	BYTE PTR [rbx+1], sil

; 2033 :             if ( opndx.value > 65536 * size ) {

	shl	r14d, 16
	cmp	ecx, r14d
	jbe	SHORT $LN47@ExcFrameDi

; 2034 :                 puc->FrameOffset = ( opndx.value >> 19 );

	mov	eax, ecx

; 2035 :                 puc++;
; 2036 :                 puc->FrameOffset = ( opndx.value >> 3 );

	sar	ecx, 3
	sar	eax, 19
	mov	WORD PTR [rbx], ax
	mov	WORD PTR [rbx+2], cx

; 2037 :                 puc++;

	add	rbx, 4

; 2038 :                 puc->UnwindOp = UWOP_SAVE_NONVOL_FAR;
; 2039 :                 unw_info.CountOfCodes += 3;

	movzx	ecx, BYTE PTR unw_info+2
	and	BYTE PTR [rbx+1], 245			; 000000f5H
	or	BYTE PTR [rbx+1], 5
	add	cl, 3

; 2040 :             } else {

	jmp	SHORT $LN72@ExcFrameDi
$LN47@ExcFrameDi:

; 2041 :                 puc->FrameOffset = ( opndx.value >> 3 );

	sar	ecx, 3
	mov	WORD PTR [rbx], cx

; 2042 :                 puc++;

	add	rbx, 2

; 2043 :                 puc->UnwindOp = UWOP_SAVE_NONVOL;
; 2044 :                 unw_info.CountOfCodes += 2;

	movzx	ecx, BYTE PTR unw_info+2
	and	BYTE PTR [rbx+1], 244			; 000000f4H
	or	BYTE PTR [rbx+1], 4
	add	cl, 2
$LN72@ExcFrameDi:

; 2045 :             }
; 2046 :             puc->CodeOffset = ofs;
; 2047 :             puc->OpInfo = reg;

	and	BYTE PTR [rbx+1], 15
	or	BYTE PTR [rbx+1], sil
	mov	r8d, DWORD PTR i$[rbp-105]
	mov	BYTE PTR unw_info+2, cl
	mov	BYTE PTR [rbx], r12b
	jmp	$LN4@ExcFrameDi
$LN54@ExcFrameDi:

; 2083 :     }
; 2084 :     /* v2.11: check if the table of codes has been exceeded */
; 2085 :     if ( oldcodes > unw_info.CountOfCodes ) {

	cmp	r13b, cl
	jbe	SHORT $LN55@ExcFrameDi

; 2086 :         return( EmitErr( TOO_MANY_UNWIND_CODES_IN_FRAME_PROC ) );

	mov	ecx, 210				; 000000d2H
	call	EmitErr
	jmp	$LN68@ExcFrameDi
$LN55@ExcFrameDi:

; 2087 :     }
; 2088 :     DebugMsg1(("ExcFrameDirective() exit, ok\n" ));
; 2089 :     return( NOT_ERROR );

	xor	eax, eax
	jmp	$LN68@ExcFrameDi
$LN8@ExcFrameDi:

; 1881 :         return( EmitError( ENDPROLOG_FOUND_BEFORE_EH_DIRECTIVES ) );

	mov	ecx, 254				; 000000feH
	call	EmitError

; 2090 : }

	add	rsp, 176				; 000000b0H
	pop	r15
	pop	r13
	pop	rbp
	ret	0
	npad	1
$LN60@ExcFrameDi:
	DD	$LN10@ExcFrameDi
	DD	$LN22@ExcFrameDi
	DD	$LN24@ExcFrameDi
	DD	$LN26@ExcFrameDi
	DD	$LN29@ExcFrameDi
	DD	$LN29@ExcFrameDi
	DD	$LN29@ExcFrameDi
	DD	$LN29@ExcFrameDi
ExcFrameDirective ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
_TEXT	SEGMENT
i$ = 48
tokenarray$ = 56
EndpDir	PROC

; 1833 : {

$LN9:
	push	rbx
	sub	rsp, 32					; 00000020H
	mov	rbx, rdx

; 1834 :     DebugMsg1(("EndpDir(%s) enter, curr ofs=% " I32_SPEC "X, CurrProc=%s\n", tokenarray[0].string_ptr, GetCurrOffset(), CurrProc ? CurrProc->sym.name : "NULL" ));
; 1835 :     if( i != 1 || tokenarray[2].token != T_FINAL ) {

	cmp	ecx, 1
	jne	SHORT $LN3@EndpDir
	cmp	BYTE PTR [rdx+64], 0
	jne	SHORT $LN3@EndpDir

; 1837 :     }
; 1838 :     /* v2.10: "+ 1" added to CurrProc->sym.name_size */
; 1839 :     if( CurrProc &&

	mov	rcx, QWORD PTR CurrProc
	test	rcx, rcx
	je	SHORT $LN4@EndpDir
	movzx	eax, BYTE PTR [rcx+72]
	mov	rdx, QWORD PTR [rdx+8]
	inc	eax
	mov	rcx, QWORD PTR [rcx+8]
	movsxd	r8, eax
	call	QWORD PTR SymCmpFunc
	test	eax, eax
	jne	SHORT $LN4@EndpDir

; 1840 :        ( SymCmpFunc(CurrProc->sym.name, tokenarray[0].string_ptr, CurrProc->sym.name_size + 1 ) == 0 ) ) {
; 1841 :         ProcFini( CurrProc );

	mov	rcx, QWORD PTR CurrProc
	call	ProcFini

; 1844 :     }
; 1845 :     return( NOT_ERROR );

	xor	eax, eax

; 1846 : }

	add	rsp, 32					; 00000020H
	pop	rbx
	ret	0
$LN4@EndpDir:

; 1842 :     } else {
; 1843 :         return( EmitErr( UNMATCHED_BLOCK_NESTING, tokenarray[0].string_ptr ) );

	mov	rdx, QWORD PTR [rbx+8]
	mov	ecx, 142				; 0000008eH

; 1846 : }

	add	rsp, 32					; 00000020H
	pop	rbx

; 1842 :     } else {
; 1843 :         return( EmitErr( UNMATCHED_BLOCK_NESTING, tokenarray[0].string_ptr ) );

	jmp	EmitErr
$LN3@EndpDir:

; 1836 :         return( EmitErr( SYNTAX_ERROR_EX, tokenarray[i].tokpos ) );

	movsxd	rdx, ecx
	mov	ecx, 209				; 000000d1H
	shl	rdx, 5
	mov	rdx, QWORD PTR [rdx+rbx+24]

; 1846 : }

	add	rsp, 32					; 00000020H
	pop	rbx

; 1836 :         return( EmitErr( SYNTAX_ERROR_EX, tokenarray[i].tokpos ) );

	jmp	EmitErr
EndpDir	ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
;	COMDAT ProcFini
_TEXT	SEGMENT
proc$ = 48
ProcFini PROC						; COMDAT

; 1757 : {

	sub	rsp, 40					; 00000028H

; 1758 :     struct dsym *curr;
; 1759 :     /* v2.06: emit an error if current segment isn't equal to
; 1760 :      * the one of the matching PROC directive. Close the proc anyway!
; 1761 :      */
; 1762 :     if ( proc->sym.segment == &CurrSeg->sym ) {

	mov	rax, QWORD PTR ModuleInfo+432
	mov	QWORD PTR [rsp+32], rdi
	mov	rdi, rcx
	cmp	QWORD PTR [rcx+24], rax
	jne	SHORT $LN14@ProcFini

; 1763 :         proc->sym.total_size = GetCurrOffset() - proc->sym.offset;

	call	GetCurrOffset

; 1764 :     } else {

	jmp	SHORT $LN52@ProcFini
$LN14@ProcFini:

; 1765 :         DebugMsg1(("ProcFini(%s): unmatched block nesting error, proc->seg=%s, CurrSeg=%s\n",
; 1766 :                    proc->sym.name, proc->sym.segment->name, CurrSeg ? CurrSeg->sym.name : "NULL" ));
; 1767 :         EmitErr( UNMATCHED_BLOCK_NESTING, proc->sym.name );

	mov	rdx, QWORD PTR [rcx+8]
	mov	ecx, 142				; 0000008eH
	call	EmitErr

; 1768 :         proc->sym.total_size = CurrProc->sym.segment->offset - proc->sym.offset;

	mov	rax, QWORD PTR CurrProc
	mov	rcx, QWORD PTR [rax+24]
	mov	eax, DWORD PTR [rcx+16]
$LN52@ProcFini:
	sub	eax, DWORD PTR [rdi+16]
	mov	DWORD PTR [rdi+56], eax

; 1769 :     }
; 1770 : 
; 1771 :     /* v2.03: for W3+, check for unused params and locals */
; 1772 :     if ( Options.warning_level > 2 && Parse_Pass == PASS_1 ) {

	cmp	BYTE PTR Options+13, 2
	jbe	SHORT $LN49@ProcFini
	cmp	DWORD PTR Parse_Pass, 0
	jne	$LN50@ProcFini

; 1773 :         for ( curr = proc->e.procinfo->paralist; curr; curr = curr->nextparam ) {

	mov	rax, QWORD PTR [rdi+96]
	mov	QWORD PTR [rsp+48], rbx
	mov	rbx, QWORD PTR [rax+8]
	test	rbx, rbx
	je	SHORT $LN3@ProcFini
	npad	6
$LL4@ProcFini:

; 1774 :             if ( curr->sym.used == FALSE )

	test	BYTE PTR [rbx+40], 1
	jne	SHORT $LN2@ProcFini

; 1775 :                 EmitWarn( 3, PROCEDURE_ARGUMENT_OR_LOCAL_NOT_REFERENCED, curr->sym.name );

	mov	r8, QWORD PTR [rbx+8]
	mov	edx, 265				; 00000109H
	mov	ecx, 3
	call	EmitWarn
$LN2@ProcFini:

; 1773 :         for ( curr = proc->e.procinfo->paralist; curr; curr = curr->nextparam ) {

	mov	rbx, QWORD PTR [rbx+112]
	test	rbx, rbx
	jne	SHORT $LL4@ProcFini
$LN3@ProcFini:

; 1776 :         }
; 1777 :         for ( curr = proc->e.procinfo->locallist; curr; curr = curr->nextlocal ) {

	mov	rax, QWORD PTR [rdi+96]
	mov	rbx, QWORD PTR [rax+16]
	test	rbx, rbx
	je	SHORT $LN51@ProcFini
	npad	1
$LL7@ProcFini:

; 1778 :             if ( curr->sym.used == FALSE )

	test	BYTE PTR [rbx+40], 1
	jne	SHORT $LN5@ProcFini

; 1779 :                 EmitWarn( 3, PROCEDURE_ARGUMENT_OR_LOCAL_NOT_REFERENCED, curr->sym.name );

	mov	r8, QWORD PTR [rbx+8]
	mov	edx, 265				; 00000109H
	mov	ecx, 3
	call	EmitWarn
$LN5@ProcFini:

; 1776 :         }
; 1777 :         for ( curr = proc->e.procinfo->locallist; curr; curr = curr->nextlocal ) {

	mov	rbx, QWORD PTR [rbx+112]
	test	rbx, rbx
	jne	SHORT $LL7@ProcFini
$LN51@ProcFini:
	mov	rbx, QWORD PTR [rsp+48]
$LN49@ProcFini:

; 1780 :         }
; 1781 :     }
; 1782 : #if AMD64_SUPPORT
; 1783 :     /* save stack space reserved for INVOKE if OPTION WIN64:2 is set */
; 1784 :     if ( Parse_Pass == PASS_1 &&
; 1785 :         ModuleInfo.fctype == FCT_WIN64 &&

	cmp	DWORD PTR Parse_Pass, 0
	jne	SHORT $LN50@ProcFini
	cmp	DWORD PTR ModuleInfo+376, 2
	jne	SHORT $LN50@ProcFini
	test	BYTE PTR ModuleInfo+413, 2
	je	SHORT $LN50@ProcFini

; 1786 :         ( ModuleInfo.win64_flags & W64F_AUTOSTACKSP ) ) {
; 1787 :         proc->e.procinfo->ReservedStack = sym_ReservedStack->value;

	mov	rax, QWORD PTR sym_ReservedStack
	mov	rdx, QWORD PTR [rdi+96]
	mov	ecx, DWORD PTR [rax+16]
	mov	DWORD PTR [rdx+56], ecx

; 1788 :         DebugMsg1(("ProcFini(%s): localsize=%u ReservedStack=%u\n", proc->sym.name, proc->e.procinfo->localsize, proc->e.procinfo->ReservedStack ));
; 1789 : #if STACKBASESUPP
; 1790 :         if ( proc->e.procinfo->fpo ) {

	mov	rdx, QWORD PTR [rdi+96]
	test	BYTE PTR [rdx+84], 128			; 00000080H
	je	SHORT $LN50@ProcFini

; 1791 :             for ( curr = proc->e.procinfo->locallist; curr; curr = curr->nextlocal ) {

	mov	rdx, QWORD PTR [rdx+16]
	test	rdx, rdx
	je	SHORT $LN9@ProcFini
	npad	10
$LL10@ProcFini:

; 1792 :                 DebugMsg1(("ProcFini(%s): FPO, offset for %s %8d -> %8d\n", proc->sym.name, curr->sym.name, curr->sym.offset, curr->sym.offset + proc->e.procinfo->ReservedStack ));
; 1793 :                 curr->sym.offset += proc->e.procinfo->ReservedStack;

	mov	rax, QWORD PTR [rdi+96]
	mov	ecx, DWORD PTR [rax+56]
	add	DWORD PTR [rdx+16], ecx
	mov	rdx, QWORD PTR [rdx+112]
	test	rdx, rdx
	jne	SHORT $LL10@ProcFini
$LN9@ProcFini:

; 1794 :             }
; 1795 :             for ( curr = proc->e.procinfo->paralist; curr; curr = curr->nextparam ) {

	mov	rax, QWORD PTR [rdi+96]
	mov	rdx, QWORD PTR [rax+8]
	test	rdx, rdx
	je	SHORT $LN50@ProcFini
$LL13@ProcFini:

; 1796 :                 DebugMsg1(("ProcFini(%s): FPO, offset for %s %8d -> %8d\n", proc->sym.name, curr->sym.name, curr->sym.offset, curr->sym.offset + proc->e.procinfo->ReservedStack ));
; 1797 :                 curr->sym.offset += proc->e.procinfo->ReservedStack;

	mov	rax, QWORD PTR [rdi+96]
	mov	ecx, DWORD PTR [rax+56]
	add	DWORD PTR [rdx+16], ecx
	mov	rdx, QWORD PTR [rdx+112]
	test	rdx, rdx
	jne	SHORT $LL13@ProcFini
$LN50@ProcFini:

; 1798 :             }
; 1799 :         }
; 1800 : #endif
; 1801 :     }
; 1802 : 
; 1803 :     /* create the .pdata and .xdata stuff */
; 1804 :     if ( proc->e.procinfo->isframe ) {

	mov	rax, QWORD PTR [rdi+96]
	test	BYTE PTR [rax+84], 64			; 00000040H
	je	SHORT $LN21@ProcFini

; 1805 : #if FASTPASS
; 1806 :         LstSetPosition(); /* needed if generated code is done BEFORE the line is listed */

	call	LstSetPosition

; 1807 : #endif
; 1808 :         WriteSEHData( proc );

	mov	rcx, rdi
	call	WriteSEHData
$LN21@ProcFini:

; 1809 :     }
; 1810 : #endif
; 1811 :     if ( ModuleInfo.list )

	test	DWORD PTR ModuleInfo+408, 2048		; 00000800H
	mov	rdi, QWORD PTR [rsp+32]
	je	SHORT $LN22@ProcFini

; 1812 :         LstWrite( LSTTYPE_LABEL, 0, NULL );

	xor	edx, edx
	xor	r8d, r8d
	lea	ecx, QWORD PTR [rdx+7]
	call	LstWrite
$LN22@ProcFini:

; 1813 : 
; 1814 :     /* create the list of locals */
; 1815 :     if ( Parse_Pass == PASS_1 ) {

	cmp	DWORD PTR Parse_Pass, 0
	jne	SHORT $LN23@ProcFini

; 1816 :         /* in case the procedure is empty, init addresses of local variables ( for proper listing ) */
; 1817 :         if( ProcStatus & PRST_PROLOGUE_NOT_DONE )

	test	BYTE PTR ProcStatus, -128		; ffffffffffffff80H
	je	SHORT $LN24@ProcFini

; 1818 :             SetLocalOffsets( CurrProc->e.procinfo );

	mov	rcx, QWORD PTR CurrProc
	mov	rcx, QWORD PTR [rcx+96]
	call	SetLocalOffsets
$LN24@ProcFini:

; 1819 :         SymGetLocal( (struct asym *)CurrProc );

	mov	rcx, QWORD PTR CurrProc
	call	SymGetLocal
$LN23@ProcFini:

; 1820 :     }
; 1821 : 
; 1822 :     CurrProc = pop_proc();

	mov	rcx, QWORD PTR ProcStack
	test	rcx, rcx
	jne	SHORT $LN28@ProcFini
	xor	eax, eax
	mov	QWORD PTR CurrProc, rax

; 1825 : 
; 1826 :     ProcStatus = 0; /* in case there was an empty PROC/ENDP pair */

	mov	DWORD PTR ProcStatus, eax

; 1827 : }

	add	rsp, 40					; 00000028H
	ret	0

; 1820 :     }
; 1821 : 
; 1822 :     CurrProc = pop_proc();

$LN28@ProcFini:
	mov	rax, QWORD PTR [rcx]
	mov	QWORD PTR ProcStack, rax
	mov	rcx, QWORD PTR [rcx+8]
	mov	QWORD PTR CurrProc, rcx

; 1823 :     if ( CurrProc )

	test	rcx, rcx
	je	SHORT $LN48@ProcFini

; 1824 :         SymSetLocal( (struct asym *)CurrProc );  /* restore local symbol table */

	call	SymSetLocal
$LN48@ProcFini:

; 1825 : 
; 1826 :     ProcStatus = 0; /* in case there was an empty PROC/ENDP pair */

	xor	eax, eax
	mov	DWORD PTR ProcStatus, eax

; 1827 : }

	add	rsp, 40					; 00000028H
	ret	0
ProcFini ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
;	COMDAT SetLocalOffsets
_TEXT	SEGMENT
paramadj$1 = 48
info$ = 48
SetLocalOffsets PROC					; COMDAT

; 2677 : {

	push	rbp
	push	r14
	push	r15
	sub	rsp, 16

; 2678 :     struct dsym *curr;
; 2679 : #if AMD64_SUPPORT || STACKBASESUPP
; 2680 :     int         cntxmm = 0;
; 2681 :     int         cntstd = 0;
; 2682 :     int         start = 0;
; 2683 : #endif
; 2684 : #if AMD64_SUPPORT
; 2685 :     int         rspalign = FALSE;
; 2686 : #endif
; 2687 :     int         align = CurrWordSize;
; 2688 : 
; 2689 : #if AMD64_SUPPORT
; 2690 :     if ( info->isframe || ( ModuleInfo.fctype == FCT_WIN64 && ( ModuleInfo.win64_flags & W64F_AUTOSTACKSP ) ) ) {

	movzx	r9d, BYTE PTR ModuleInfo+413
	xor	r10d, r10d
	mov	QWORD PTR [rsp+56], rbx
	xor	r14d, r14d
	movzx	ebx, BYTE PTR ModuleInfo+406
	xor	ebp, ebp
	mov	QWORD PTR [rsp+64], rsi
	xor	r15d, r15d
	mov	QWORD PTR [rsp+8], rdi
	mov	r8, rcx
	movzx	edi, BYTE PTR [rcx+84]
	mov	esi, ebx
	test	dil, 64					; 00000040H
	jne	SHORT $LN15@SetLocalOf
	cmp	DWORD PTR ModuleInfo+376, 2
	jne	SHORT $LN16@SetLocalOf
	test	r9b, 2
	je	SHORT $LN16@SetLocalOf
$LN15@SetLocalOf:

; 2691 :         rspalign = TRUE;
; 2692 :         if ( ModuleInfo.win64_flags & W64F_STACKALIGN16 )

	movzx	eax, r9b
	mov	ecx, 16
	and	al, 4
	mov	r15d, 1
	cmovne	esi, ecx
$LN16@SetLocalOf:

; 2693 :             align = 16;
; 2694 :     }
; 2695 : #endif
; 2696 : #if AMD64_SUPPORT || STACKBASESUPP
; 2697 :     /* in 64-bit, if the FRAME attribute is set, the space for the registers
; 2698 :      * saved by the USES clause is located ABOVE the local variables!
; 2699 :      * v2.09: if stack space is to be reserved for INVOKE ( option WIN64:2 ),
; 2700 :      * the registers are also saved ABOVE the local variables.
; 2701 :      */
; 2702 :     if (
; 2703 : #if STACKBASESUPP
; 2704 :         info->fpo
; 2705 : #endif
; 2706 : #if AMD64_SUPPORT
; 2707 :         || rspalign

	shr	dil, 7
	test	dil, dil
	jne	SHORT $LN18@SetLocalOf
	test	r15d, r15d
	je	$LN29@SetLocalOf
$LN18@SetLocalOf:

; 2708 : #endif
; 2709 :        ) {
; 2710 :         /* count registers to be saved ABOVE local variables.
; 2711 :          * v2.06: the list may contain xmm registers, which have size 16!
; 2712 :          */
; 2713 :         if ( info->regslist ) {

	mov	rdx, QWORD PTR [r8]
	test	rdx, rdx
	je	SHORT $LN3@SetLocalOf

; 2714 :             int         cnt;
; 2715 :             uint_16     *regs;
; 2716 :             for( regs = info->regslist, cnt = *regs++; cnt; cnt--, regs++ )

	movzx	r11d, WORD PTR [rdx]
	add	rdx, 2
	test	r11d, r11d
	je	SHORT $LN3@SetLocalOf
	mov	QWORD PTR [rsp], r12
	lea	r12, OFFSET FLAT:SpecialTable
	npad	9
$LL4@SetLocalOf:

; 2717 :                 if ( GetValueSp( *regs ) & OP_XYZMM )

	movzx	eax, WORD PTR [rdx]
	lea	rcx, QWORD PTR [rax+rax*2]
	test	BYTE PTR [r12+rcx*4], 128		; 00000080H
	je	SHORT $LN20@SetLocalOf

; 2718 :                     cntxmm++;

	inc	r10d
	jmp	SHORT $LN2@SetLocalOf
$LN20@SetLocalOf:

; 2719 :                 else
; 2720 :                     cntstd++;

	inc	r14d
$LN2@SetLocalOf:

; 2714 :             int         cnt;
; 2715 :             uint_16     *regs;
; 2716 :             for( regs = info->regslist, cnt = *regs++; cnt; cnt--, regs++ )

	add	rdx, 2
	sub	r11d, 1
	jne	SHORT $LL4@SetLocalOf
	mov	r12, QWORD PTR [rsp]
$LN3@SetLocalOf:

; 2721 :         }        
; 2722 :         /* in case there's no frame register, adjust start offset. */
; 2723 :         if ( info->parasize == 0 && info->locallist == NULL  )

	cmp	DWORD PTR [r8+32], ebp
	jne	SHORT $LN22@SetLocalOf
	cmp	QWORD PTR [r8+16], rbp
	cmove	ebp, ebx
$LN22@SetLocalOf:

; 2724 :             start = CurrWordSize;
; 2725 :         if (info->fpo && !(ModuleInfo.win64_flags & W64F_HABRAN)) start = CurrWordSize;

	test	dil, dil
	je	SHORT $LN23@SetLocalOf
	movzx	eax, r9b
	and	al, 11
	cmove	ebp, ebx
$LN23@SetLocalOf:

; 2726 : #if AMD64_SUPPORT
; 2727 :         if (ModuleInfo.win64_flags & W64F_HABRAN){

	test	r9b, 11
	je	SHORT $LN24@SetLocalOf

; 2728 :           info->localsize += start;

	add	DWORD PTR [r8+36], ebp

; 2729 :           cntstd = info->pushed_reg;

	mov	r14d, DWORD PTR [r8+64]

; 2730 :           if (rspalign && cntxmm) {

	test	r15d, r15d
	je	SHORT $LN66@SetLocalOf
	test	r10d, r10d
	je	SHORT $LN66@SetLocalOf

; 2731 :               if (!(cntstd & 1)) info->localsize += 8;

	test	r14b, 1
	jne	SHORT $LN27@SetLocalOf
	add	DWORD PTR [r8+36], 8
$LN27@SetLocalOf:

; 2732 :               info->localsize += 32 * cntxmm;

	shl	r10d, 5
	add	DWORD PTR [r8+36], r10d

; 2733 :               }
; 2734 :            }

	jmp	SHORT $LN66@SetLocalOf
$LN24@SetLocalOf:

; 2735 :         else {
; 2736 :           if (rspalign) {

	test	r15d, r15d
	je	SHORT $LN29@SetLocalOf

; 2737 :             info->localsize = start + cntstd * CurrWordSize;

	imul	ebx, r14d
	lea	eax, DWORD PTR [rbx+rbp]
	mov	DWORD PTR [r8+36], eax

; 2738 :             if (cntxmm) {

	test	r10d, r10d
	je	SHORT $LN66@SetLocalOf

; 2739 :               info->localsize += 16 * cntxmm;

	shl	r10d, 4

; 2740 :               info->localsize = ROUND_UP(info->localsize, 16);

	add	eax, 15
	add	eax, r10d
	and	eax, -16				; fffffff0H
	mov	DWORD PTR [r8+36], eax
$LN66@SetLocalOf:
	movzx	r9d, BYTE PTR ModuleInfo+413
$LN29@SetLocalOf:

; 2741 :             }
; 2742 :           }
; 2743 :         }
; 2744 : #endif
; 2745 :         DebugMsg1(("SetLocalOffsets(%s): cntxmm=%u cntstd=%u start=%u align=%u localsize=%u\n", CurrProc->sym.name, cntxmm, cntstd, start, align, info->localsize ));
; 2746 :     }
; 2747 : #endif
; 2748 : 
; 2749 :     /* scan the locals list and set member sym.offset */
; 2750 :     for( curr = info->locallist; curr; curr = curr->nextlocal ) {

	mov	r10, QWORD PTR [r8+16]
	mov	rdi, QWORD PTR [rsp+8]
	mov	rbx, QWORD PTR [rsp+56]
	test	r10, r10
	je	$LN6@SetLocalOf
$LL7@SetLocalOf:

; 2751 :         uint_32 itemsize = ( curr->sym.total_size == 0 ? 0 : curr->sym.total_size / curr->sym.total_length );

	mov	r11d, DWORD PTR [r10+56]
	test	r11d, r11d
	jne	SHORT $LN48@SetLocalOf
	xor	edx, edx
	jmp	SHORT $LN49@SetLocalOf
$LN48@SetLocalOf:
	xor	edx, edx
	mov	eax, r11d
	div	DWORD PTR [r10+64]
	mov	edx, eax
$LN49@SetLocalOf:

; 2752 :         if (ModuleInfo.win64_flags & W64F_HABRAN){

	test	r9b, 11
	je	SHORT $LN30@SetLocalOf

; 2753 :           int n = 0;

	xor	ecx, ecx

; 2754 :           if (curr->sym.isarray) n = curr->sym.total_size & 0x7;

	test	BYTE PTR [r10+41], 2
	je	SHORT $LN31@SetLocalOf
	mov	ecx, r11d
	and	ecx, 7
$LN31@SetLocalOf:

; 2755 :           curr->sym.offset = info->localsize + n; //that works

	add	ecx, DWORD PTR [r8+36]
	mov	DWORD PTR [r10+16], ecx
$LN30@SetLocalOf:

; 2756 :         }
; 2757 :         info->localsize += curr->sym.total_size;

	add	DWORD PTR [r8+36], r11d
	mov	eax, DWORD PTR [r8+36]

; 2758 :         if ( itemsize > align )

	cmp	edx, esi
	jbe	SHORT $LN32@SetLocalOf

; 2759 :             info->localsize = ROUND_UP( info->localsize, align );

	lea	ecx, DWORD PTR [rax-1]
	add	ecx, esi
	lea	eax, DWORD PTR [rsi-1]
	jmp	SHORT $LN68@SetLocalOf
$LN32@SetLocalOf:

; 2760 :         else if ( itemsize ) /* v2.04: skip if size == 0 */

	test	edx, edx
	je	SHORT $LN34@SetLocalOf

; 2761 :             info->localsize = ROUND_UP( info->localsize, itemsize );

	lea	ecx, DWORD PTR [rax-1]
	add	ecx, edx
	lea	eax, DWORD PTR [rdx-1]
$LN68@SetLocalOf:
	not	eax
	and	ecx, eax
	mov	DWORD PTR [r8+36], ecx
$LN34@SetLocalOf:

; 2762 :         if (!(ModuleInfo.win64_flags & W64F_HABRAN))

	movzx	r9d, BYTE PTR ModuleInfo+413
	test	r9b, 11
	jne	SHORT $LN5@SetLocalOf

; 2763 :         curr->sym.offset = - info->localsize;

	mov	eax, DWORD PTR [r8+36]
	neg	eax
	mov	DWORD PTR [r10+16], eax
	movzx	r9d, BYTE PTR ModuleInfo+413
$LN5@SetLocalOf:

; 2741 :             }
; 2742 :           }
; 2743 :         }
; 2744 : #endif
; 2745 :         DebugMsg1(("SetLocalOffsets(%s): cntxmm=%u cntstd=%u start=%u align=%u localsize=%u\n", CurrProc->sym.name, cntxmm, cntstd, start, align, info->localsize ));
; 2746 :     }
; 2747 : #endif
; 2748 : 
; 2749 :     /* scan the locals list and set member sym.offset */
; 2750 :     for( curr = info->locallist; curr; curr = curr->nextlocal ) {

	mov	r10, QWORD PTR [r10+112]
	test	r10, r10
	jne	$LL7@SetLocalOf
$LN6@SetLocalOf:
	mov	rsi, QWORD PTR [rsp+64]

; 2764 :         DebugMsg1(("SetLocalOffsets(%s): offset of %s (size=%u) set to %d\n", CurrProc->sym.name, curr->sym.name, curr->sym.total_size, curr->sym.offset));
; 2765 :     }
; 2766 : 
; 2767 :     /* v2.11: localsize must be rounded before offset adjustment if fpo */
; 2768 :     if (!(ModuleInfo.win64_flags & W64F_HABRAN))

	test	r9b, 11
	jne	SHORT $LN36@SetLocalOf

; 2769 :     info->localsize = ROUND_UP( info->localsize, CurrWordSize );

	movzx	ecx, BYTE PTR ModuleInfo+406
	lea	edx, DWORD PTR [rcx-1]
	add	edx, DWORD PTR [r8+36]
	lea	eax, DWORD PTR [rcx-1]
	not	eax
	and	edx, eax
	mov	DWORD PTR [r8+36], edx
	movzx	r9d, BYTE PTR ModuleInfo+413
$LN36@SetLocalOf:

; 2770 : #if AMD64_SUPPORT
; 2771 :     /* RSP 16-byte alignment? */
; 2772 :     if ( rspalign ) {

	test	r15d, r15d
	je	SHORT $LN39@SetLocalOf

; 2773 :       if (ModuleInfo.win64_flags & W64F_HABRAN)info->localsize = ROUND_UP(info->localsize, 8);

	mov	eax, DWORD PTR [r8+36]
	test	r9b, 11
	je	SHORT $LN38@SetLocalOf
	add	eax, 7
	and	eax, -8					; fffffff8H
	jmp	SHORT $LN69@SetLocalOf
$LN38@SetLocalOf:

; 2774 :       else  info->localsize = ROUND_UP( info->localsize, 16 );

	add	eax, 15
	and	eax, -16				; fffffff0H
$LN69@SetLocalOf:
	mov	DWORD PTR [r8+36], eax
	movzx	r9d, BYTE PTR ModuleInfo+413
$LN39@SetLocalOf:

; 2775 :     }
; 2776 : #endif
; 2777 : 
; 2778 :     DebugMsg1(("SetLocalOffsets(%s): localsize=%u after processing locals\n", CurrProc->sym.name, info->localsize ));
; 2779 : 
; 2780 : #if STACKBASESUPP
; 2781 :     /* v2.11: recalculate offsets for params and locals if there's no frame pointer.
; 2782 :      * Problem in 64-bit: option win64:2 triggers the "stack space reservation" feature -
; 2783 :      * but the final value of this space is known at the procedure's END only.
; 2784 :      * Hence in this case the values calculated below are "preliminary".
; 2785 :      */
; 2786 :     if (info->fpo) {

	test	BYTE PTR [r8+84], 128			; 00000080H
	je	$LN12@SetLocalOf

; 2787 :       unsigned localadj;
; 2788 :       unsigned paramadj;
; 2789 : #if AMD64_SUPPORT
; 2790 :       if (rspalign) {

	test	r15d, r15d
	je	SHORT $LN41@SetLocalOf

; 2791 :         localadj = info->localsize;

	mov	r10d, DWORD PTR [r8+36]

; 2792 :         if (!(ModuleInfo.win64_flags & W64F_HABRAN))

	test	r9b, 11
	jne	SHORT $LN67@SetLocalOf

; 2793 :           paramadj = info->localsize - CurrWordSize - start;

	movzx	eax, BYTE PTR ModuleInfo+406
	mov	edx, r10d
	sub	edx, eax
	sub	edx, ebp

; 2794 :       }

	jmp	SHORT $LN42@SetLocalOf
$LN67@SetLocalOf:

; 2798 :         paramadj = info->localsize + cntstd * CurrWordSize - CurrWordSize;

	mov	edx, DWORD PTR paramadj$1[rsp]
	jmp	SHORT $LN42@SetLocalOf
$LN41@SetLocalOf:

; 2795 :       else {
; 2796 : #endif
; 2797 :         localadj = info->localsize + cntstd * CurrWordSize;

	movzx	eax, BYTE PTR ModuleInfo+406

; 2798 :         paramadj = info->localsize + cntstd * CurrWordSize - CurrWordSize;

	lea	edx, DWORD PTR [r14-1]
	mov	r10d, eax
	imul	edx, eax
	imul	r10d, r14d
	add	r10d, DWORD PTR [r8+36]
	add	edx, DWORD PTR [r8+36]
$LN42@SetLocalOf:

; 2799 : #if AMD64_SUPPORT
; 2800 :       }
; 2801 : #endif
; 2802 :       if (!(ModuleInfo.win64_flags & W64F_HABRAN)){

	test	r9b, 11
	jne	SHORT $LN46@SetLocalOf

; 2803 :         DebugMsg1(("SetLocalOffsets(%s): FPO, adjusting offsets\n", CurrProc->sym.name));
; 2804 :         /* subtract CurrWordSize value for params, since no space is required to save the frame pointer value */
; 2805 :         for (curr = info->locallist; curr; curr = curr->nextlocal) {

	mov	rax, QWORD PTR [r8+16]
	test	rax, rax
	je	SHORT $LN9@SetLocalOf
	npad	2
$LL10@SetLocalOf:

; 2806 :           DebugMsg1(("SetLocalOffsets(%s): FPO, offset for %s %4d -> %4d\n", CurrProc->sym.name, curr->sym.name, curr->sym.offset, curr->sym.offset + localadj));
; 2807 :           curr->sym.offset += localadj;

	add	DWORD PTR [rax+16], r10d
	mov	rax, QWORD PTR [rax+112]
	test	rax, rax
	jne	SHORT $LL10@SetLocalOf

; 2803 :         DebugMsg1(("SetLocalOffsets(%s): FPO, adjusting offsets\n", CurrProc->sym.name));
; 2804 :         /* subtract CurrWordSize value for params, since no space is required to save the frame pointer value */
; 2805 :         for (curr = info->locallist; curr; curr = curr->nextlocal) {

	movzx	r9d, BYTE PTR ModuleInfo+413
$LN9@SetLocalOf:

; 2808 :         }
; 2809 :         for (curr = info->paralist; curr; curr = curr->nextparam) {

	mov	rax, QWORD PTR [r8+8]
	test	rax, rax
	je	SHORT $LN12@SetLocalOf
	npad	2
$LL13@SetLocalOf:

; 2810 :           DebugMsg1(("SetLocalOffsets(%s): FPO, offset for %s %4d -> %4d\n", CurrProc->sym.name, curr->sym.name, curr->sym.offset, curr->sym.offset + paramadj));
; 2811 :           curr->sym.offset += paramadj;

	add	DWORD PTR [rax+16], edx
	mov	rax, QWORD PTR [rax+112]
	test	rax, rax
	jne	SHORT $LL13@SetLocalOf

; 2808 :         }
; 2809 :         for (curr = info->paralist; curr; curr = curr->nextparam) {

	movzx	r9d, BYTE PTR ModuleInfo+413
$LN12@SetLocalOf:

; 2812 :         }
; 2813 :       }
; 2814 :     }
; 2815 : #endif
; 2816 : 
; 2817 : #if AMD64_SUPPORT
; 2818 :     /* v2.12: if the space used for register saves has been added to localsize,
; 2819 :      * the part that covers "pushed" GPRs has to be subtracted now, before prologue code is generated.
; 2820 :      */
; 2821 :     if (!(ModuleInfo.win64_flags & W64F_HABRAN)){

	test	r9b, 11
	jne	SHORT $LN46@SetLocalOf

; 2822 :       if (rspalign) {

	test	r15d, r15d
	je	SHORT $LN46@SetLocalOf

; 2823 :         info->localsize -= cntstd * 8 + start;

	neg	ebp
	lea	eax, DWORD PTR [r14*8]
	sub	ebp, eax
	add	DWORD PTR [r8+36], ebp
$LN46@SetLocalOf:

; 2824 :         DebugMsg1(("SetLocalOffsets(%s): final localsize=%u\n", CurrProc->sym.name, info->localsize));
; 2825 :       }
; 2826 :     }
; 2827 : #endif
; 2828 : }

	add	rsp, 16
	pop	r15
	pop	r14
	pop	rbp
	ret	0
SetLocalOffsets ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
;	COMDAT WriteSEHData
_TEXT	SEGMENT
segnamebuff$ = 64
buffer$ = 80
proc$ = 224
WriteSEHData PROC					; COMDAT

; 1657 : {

	mov	rax, rsp
	push	rsi
	sub	rsp, 208				; 000000d0H

; 1658 :     struct dsym *xdata;
; 1659 :     char *segname = ".xdata";
; 1660 :     int i;
; 1661 :     int simplespec;
; 1662 :     uint_8 olddotname;
; 1663 :     uint_32 xdataofs = 0;
; 1664 :     char segnamebuff[12];
; 1665 :     char buffer[128];
; 1666 : 
; 1667 : 	/* 2016-02-10 John Hankinson - Don't bother writing SEH data for ELF64 Win64 ABI hack */
; 1668 : 	if (Options.output_format == OFORMAT_ELF)

	cmp	DWORD PTR Options+144, 3
	mov	rsi, rcx
	je	$LN1@WriteSEHDa

; 1669 : 	return;
; 1670 : 
; 1671 :     if ( endprolog_found == FALSE ) {

	cmp	BYTE PTR endprolog_found, 0
	mov	QWORD PTR [rax+8], rbx
	mov	QWORD PTR [rax+24], rbp
	jne	SHORT $LN6@WriteSEHDa

; 1672 :         EmitErr( MISSING_ENDPROLOG, proc->sym.name );

	mov	rdx, QWORD PTR [rcx+8]
	mov	ecx, 259				; 00000103H
	call	EmitErr
$LN6@WriteSEHDa:

; 1673 :     }
; 1674 :     if ( unw_segs_defined )

	cmp	BYTE PTR unw_segs_defined, 0

; 1675 :         AddLineQueueX("%s %r", segname, T_SEGMENT );

	lea	rdx, OFFSET FLAT:$SG11906
	mov	r8d, 436				; 000001b4H
	je	SHORT $LN7@WriteSEHDa
	lea	rcx, OFFSET FLAT:$SG11911
	call	AddLineQueueX
	jmp	SHORT $LN8@WriteSEHDa
$LN7@WriteSEHDa:

; 1676 :     else {
; 1677 :         AddLineQueueX("%s %r align(%u) flat read 'DATA'", segname, T_SEGMENT, 8 );

	mov	r9d, 8
	lea	rcx, OFFSET FLAT:$SG11912
	call	AddLineQueueX

; 1678 :         AddLineQueue("$xdatasym label near");

	lea	rcx, OFFSET FLAT:$SG11913
	call	AddLineQueue
$LN8@WriteSEHDa:

; 1679 :     }
; 1680 :     xdataofs = 0;
; 1681 :     xdata = (struct dsym *)SymSearch( segname );

	lea	rcx, OFFSET FLAT:$SG11906
	xor	ebp, ebp
	call	SymFind

; 1682 :     if ( xdata ) {

	test	rax, rax
	je	SHORT $LN9@WriteSEHDa

; 1683 :         /* v2.11: changed offset to max_offset.
; 1684 :          * However, value structinfo.current_loc might even be better.
; 1685 :          */
; 1686 :         xdataofs = xdata->sym.max_offset;

	mov	ebp, DWORD PTR [rax+56]
$LN9@WriteSEHDa:

; 1687 :     }
; 1688 : 
; 1689 :     /* write the .xdata stuff (a UNWIND_INFO entry )
; 1690 :      * v2.11: 't'-suffix added to ensure the values are correct if radix is != 10.
; 1691 :      */
; 1692 :     AddLineQueueX( "db %ut + (0%xh shl 3), %ut, %ut, 0%xh + (0%xh shl 4)",

	movzx	edx, BYTE PTR unw_info+3
	movzx	r8d, BYTE PTR unw_info
	mov	ecx, edx
	movzx	eax, BYTE PTR unw_info+2
	and	edx, 15
	movzx	r9d, BYTE PTR unw_info+1
	shr	ecx, 4
	mov	DWORD PTR [rsp+48], ecx
	lea	rcx, OFFSET FLAT:$SG11915
	mov	DWORD PTR [rsp+40], edx
	mov	edx, 1
	shr	r8d, 3
	mov	DWORD PTR [rsp+32], eax
	call	AddLineQueueX

; 1693 :             UNW_VERSION, unw_info.Flags, unw_info.SizeOfProlog,
; 1694 :             unw_info.CountOfCodes, unw_info.FrameRegister, unw_info.FrameOffset );
; 1695 :     if ( unw_info.CountOfCodes ) {

	movzx	eax, BYTE PTR unw_info+2
	test	al, al
	je	$LN3@WriteSEHDa

; 1696 :         char *pfx = "dw";

	mov	QWORD PTR [rsp+248], rdi
	lea	r8, OFFSET FLAT:$SG11917

; 1697 :         buffer[0] = NULLC;
; 1698 :         /* write the codes from right to left */
; 1699 :         for ( i = unw_info.CountOfCodes; i ; i-- ) {

	movzx	ebx, al
	lea	rdi, OFFSET FLAT:unw_code
	lea	rdi, QWORD PTR [rdi-2]
	mov	BYTE PTR buffer$[rsp], 0
	lea	rdi, QWORD PTR [rdi+rbx*2]
	npad	2
$LL4@WriteSEHDa:

; 1700 :             /* v2.11: use field FrameOffset */
; 1701 :             //sprintf( buffer + strlen( buffer ), "%s 0%xh", pfx, unw_code[i-1] );
; 1702 :             sprintf( buffer + strlen( buffer ), "%s 0%xh", pfx, unw_code[i-1].FrameOffset );

	movzx	r9d, WORD PTR [rdi]
	lea	rcx, QWORD PTR buffer$[rsp]
	or	rax, -1
	npad	3
$LL26@WriteSEHDa:
	inc	rax
	cmp	BYTE PTR [rcx+rax], 0
	jne	SHORT $LL26@WriteSEHDa
	lea	rcx, QWORD PTR buffer$[rsp]
	add	rcx, rax
	lea	rdx, OFFSET FLAT:$SG11918
	call	sprintf

; 1703 :             pfx = ",";

	lea	r8, OFFSET FLAT:$SG11919

; 1704 :             if ( i == 1 || strlen( buffer ) > 72 ) {

	cmp	rbx, 1
	je	SHORT $LN12@WriteSEHDa
	lea	rcx, QWORD PTR buffer$[rsp]
	or	rax, -1
$LL25@WriteSEHDa:
	inc	rax
	cmp	BYTE PTR [rcx+rax], 0
	jne	SHORT $LL25@WriteSEHDa
	cmp	rax, 72					; 00000048H
	jbe	SHORT $LN2@WriteSEHDa
$LN12@WriteSEHDa:

; 1705 :                 AddLineQueue( buffer );

	lea	rcx, QWORD PTR buffer$[rsp]
	call	AddLineQueue

; 1706 :                 buffer[0] = NULLC;
; 1707 :                 pfx = "dw";

	lea	r8, OFFSET FLAT:$SG11922
	mov	BYTE PTR buffer$[rsp], 0
$LN2@WriteSEHDa:

; 1697 :         buffer[0] = NULLC;
; 1698 :         /* write the codes from right to left */
; 1699 :         for ( i = unw_info.CountOfCodes; i ; i-- ) {

	sub	rdi, 2
	sub	rbx, 1
	jne	SHORT $LL4@WriteSEHDa
	mov	rdi, QWORD PTR [rsp+248]
$LN3@WriteSEHDa:

; 1708 :             }
; 1709 :         }
; 1710 :     }
; 1711 :     /* make sure the unwind codes array has an even number of entries */
; 1712 :     AddLineQueueX( "%r 4", T_ALIGN );

	mov	edx, 434				; 000001b2H
	lea	rcx, OFFSET FLAT:$SG11923
	call	AddLineQueueX

; 1713 : 
; 1714 :     if ( proc->e.procinfo->exc_handler ) {

	mov	rax, QWORD PTR [rsi+96]
	mov	r8, QWORD PTR [rax+48]
	test	r8, r8
	je	SHORT $LN13@WriteSEHDa

; 1715 :         AddLineQueueX( "dd %r %s", T_IMAGEREL, proc->e.procinfo->exc_handler->name );

	mov	r8, QWORD PTR [r8+8]
	lea	rcx, OFFSET FLAT:$SG11925
	mov	edx, 233				; 000000e9H
	call	AddLineQueueX

; 1716 :         AddLineQueueX( "%r 8", T_ALIGN );

	mov	edx, 434				; 000001b2H
	lea	rcx, OFFSET FLAT:$SG11926
	call	AddLineQueueX
$LN13@WriteSEHDa:

; 1717 :     }
; 1718 :     AddLineQueueX( "%s %r", segname, T_ENDS );

	mov	r8d, 437				; 000001b5H
	lea	rdx, OFFSET FLAT:$SG11906
	lea	rcx, OFFSET FLAT:$SG11927
	call	AddLineQueueX

; 1719 : 
; 1720 :     /* v2.07: ensure that .pdata items are sorted */
; 1721 :     if ( 0 == strcmp( SimGetSegName( SIM_CODE ), proc->sym.segment->name ) ) {

	mov	rbx, QWORD PTR [rsi+24]
	xor	ecx, ecx
	call	SimGetSegName
	mov	r9, QWORD PTR [rbx+8]
	sub	r9, rax
	npad	7
$LL23@WriteSEHDa:
	movzx	edx, BYTE PTR [rax]
	movzx	ecx, BYTE PTR [rax+r9]
	sub	edx, ecx
	jne	SHORT $LN24@WriteSEHDa
	inc	rax
	test	ecx, ecx
	jne	SHORT $LL23@WriteSEHDa
$LN24@WriteSEHDa:
	test	edx, edx
	jne	SHORT $LN14@WriteSEHDa

; 1722 :         segname = ".pdata";
; 1723 :         simplespec = ( unw_segs_defined & 1 );

	movzx	eax, BYTE PTR unw_segs_defined
	lea	rbx, OFFSET FLAT:$SG11930

; 1724 :         unw_segs_defined = 3;

	mov	BYTE PTR unw_segs_defined, 3
	and	eax, 1

; 1730 :     }
; 1731 : 
; 1732 :     if ( simplespec )

	je	SHORT $LN16@WriteSEHDa

; 1733 :         AddLineQueueX( "%s %r", segname, T_SEGMENT );

	mov	r8d, 436				; 000001b4H
	lea	rcx, OFFSET FLAT:$SG11934
	mov	rdx, rbx
	call	AddLineQueueX
	jmp	SHORT $LN17@WriteSEHDa
$LN14@WriteSEHDa:

; 1725 :     } else {
; 1726 :         segname = segnamebuff;
; 1727 :         sprintf( segname, ".pdata$%04u", GetSegIdx( proc->sym.segment ) );

	mov	rcx, QWORD PTR [rsi+24]
	lea	rbx, QWORD PTR segnamebuff$[rsp]
	call	GetSegIdx
	mov	r8d, eax
	lea	rdx, OFFSET FLAT:$SG11931
	lea	rcx, QWORD PTR segnamebuff$[rsp]
	call	sprintf

; 1728 :         simplespec = 0;
; 1729 :         unw_segs_defined |= 2;

	or	BYTE PTR unw_segs_defined, 2
$LN16@WriteSEHDa:

; 1734 :     else
; 1735 :         AddLineQueueX( "%s %r align(%u) flat read 'DATA'", segname, T_SEGMENT, 4 );

	mov	r9d, 4
	lea	rcx, OFFSET FLAT:$SG11935
	mov	r8d, 436				; 000001b4H
	mov	rdx, rbx
	call	AddLineQueueX
$LN17@WriteSEHDa:

; 1736 :     /* write the .pdata stuff ( type IMAGE_RUNTIME_FUNCTION_ENTRY )*/
; 1737 :     AddLineQueueX( "dd %r %s, %r %s+0%xh, %r $xdatasym+0%xh",

	mov	r8, QWORD PTR [rsi+8]
	lea	rcx, OFFSET FLAT:$SG11936
	mov	eax, DWORD PTR [rsi+56]
	mov	edx, 233				; 000000e9H
	mov	DWORD PTR [rsp+56], ebp
	mov	r9d, edx
	mov	DWORD PTR [rsp+48], 233			; 000000e9H
	mov	DWORD PTR [rsp+40], eax
	mov	QWORD PTR [rsp+32], r8
	call	AddLineQueueX

; 1738 :                   T_IMAGEREL, proc->sym.name,
; 1739 :                   T_IMAGEREL, proc->sym.name, proc->sym.total_size,
; 1740 :                   T_IMAGEREL, xdataofs );
; 1741 :     AddLineQueueX("%s %r", segname, T_ENDS );

	mov	r8d, 437				; 000001b5H
	lea	rcx, OFFSET FLAT:$SG11937
	mov	rdx, rbx
	call	AddLineQueueX

; 1742 :     olddotname = ModuleInfo.dotname;

	mov	eax, DWORD PTR ModuleInfo+408
	mov	ebx, eax

; 1743 :     ModuleInfo.dotname = TRUE; /* set OPTION DOTNAME because .pdata and .xdata */

	or	eax, 16
	shr	ebx, 4
	and	bl, 1
	mov	DWORD PTR ModuleInfo+408, eax

; 1744 :     RunLineQueue();

	call	RunLineQueue

; 1745 :     ModuleInfo.dotname = olddotname;

	mov	eax, DWORD PTR ModuleInfo+408
	mov	rbp, QWORD PTR [rsp+240]
	movzx	ecx, bl
	mov	rbx, QWORD PTR [rsp+224]
	shl	ecx, 4
	xor	ecx, eax
	and	ecx, 16
	xor	eax, ecx
	mov	DWORD PTR ModuleInfo+408, eax
$LN1@WriteSEHDa:

; 1746 :     return;
; 1747 : }

	add	rsp, 208				; 000000d0H
	pop	rsi
	ret	0
WriteSEHData ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
_TEXT	SEGMENT
i$ = 64
tokenarray$ = 72
ProcDir	PROC

; 1423 : {

$LN47:
	push	rbp
	sub	rsp, 48					; 00000030H
	mov	rbp, rdx

; 1424 :     struct asym         *sym;
; 1425 :     unsigned int        ofs;
; 1426 :     char                *name;
; 1427 :     bool                oldpubstate;
; 1428 :     bool                is_global;
; 1429 : 
; 1430 :     DebugMsg1(("ProcDir enter, curr ofs=%X\n", GetCurrOffset() ));
; 1431 :     if( i != 1 ) {

	cmp	ecx, 1
	je	SHORT $LN2@ProcDir

; 1432 :         return( EmitErr( SYNTAX_ERROR_EX, tokenarray[i].string_ptr ) );

	movsxd	rdx, ecx
	mov	ecx, 209				; 000000d1H
	shl	rdx, 5
	mov	rdx, QWORD PTR [rdx+rbp+8]

; 1612 : }

	add	rsp, 48					; 00000030H
	pop	rbp

; 1432 :         return( EmitErr( SYNTAX_ERROR_EX, tokenarray[i].string_ptr ) );

	jmp	EmitErr
$LN2@ProcDir:

; 1433 :     }
; 1434 :     /* v2.04b: check was missing */
; 1435 :     if( CurrSeg == NULL ) {

	cmp	QWORD PTR ModuleInfo+432, 0
	jne	SHORT $LN3@ProcDir

; 1436 :         return( EmitError( MUST_BE_IN_SEGMENT_BLOCK ) );

	mov	ecx, 82					; 00000052H

; 1612 : }

	add	rsp, 48					; 00000030H
	pop	rbp

; 1436 :         return( EmitError( MUST_BE_IN_SEGMENT_BLOCK ) );

	jmp	EmitError
$LN3@ProcDir:
	mov	QWORD PTR [rsp+64], rbx

; 1437 :     }
; 1438 : 
; 1439 :     name = tokenarray[0].string_ptr;
; 1440 : 
; 1441 :     if( CurrProc != NULL ) {

	mov	rbx, QWORD PTR CurrProc
	mov	QWORD PTR [rsp+72], rsi
	mov	rsi, QWORD PTR [rdx+8]
	test	rbx, rbx
	je	SHORT $LN39@ProcDir

; 1442 : 
; 1443 :         /* this is not needed for HJWasm, but Masm will reject nested
; 1444 :          * procs if there are params, locals or used registers.
; 1445 :          */
; 1446 :         if ( CurrProc->e.procinfo->paralist ||
; 1447 : #if AMD64_SUPPORT
; 1448 :             CurrProc->e.procinfo->isframe ||
; 1449 : #endif
; 1450 :             CurrProc->e.procinfo->locallist ||

	mov	rax, QWORD PTR [rbx+96]
	cmp	QWORD PTR [rax+8], 0
	jne	$LN6@ProcDir
	test	BYTE PTR [rax+84], 64			; 00000040H
	jne	$LN6@ProcDir
	cmp	QWORD PTR [rax+16], 0
	jne	$LN6@ProcDir
	cmp	QWORD PTR [rax], 0
	jne	$LN6@ProcDir

; 1453 :         }
; 1454 :         /* nested procs ... push currproc on a stack */
; 1455 :         push_proc( CurrProc );

	cmp	DWORD PTR Parse_Pass, 0
	jne	SHORT $LN37@ProcDir
	mov	rcx, rbx
	call	SymGetLocal
$LN37@ProcDir:
	mov	ecx, 16
	call	LclAlloc
	mov	rcx, QWORD PTR ProcStack
	mov	QWORD PTR [rax], rcx
	mov	QWORD PTR [rax+8], rbx
	mov	QWORD PTR ProcStack, rax
$LN39@ProcDir:

; 1456 :     }
; 1457 : 
; 1458 : 
; 1459 :     if ( ModuleInfo.procalign ) {

	movzx	eax, BYTE PTR ModuleInfo+399
	test	al, al
	je	SHORT $LN7@ProcDir

; 1460 :         AlignCurrOffset( ModuleInfo.procalign );

	movzx	ecx, al
	call	AlignCurrOffset
$LN7@ProcDir:

; 1461 :     }
; 1462 : 
; 1463 :     i++; /* go past PROC */
; 1464 : 
; 1465 :     sym = SymSearch( name );

	mov	QWORD PTR [rsp+80], rdi
	mov	rcx, rsi
	mov	QWORD PTR [rsp+88], r14
	call	SymFind

; 1466 : 
; 1467 :     if( Parse_Pass == PASS_1 ) {

	cmp	DWORD PTR Parse_Pass, 0
	mov	rbx, rax
	jne	$LN8@ProcDir

; 1468 : 
; 1469 :         oldpubstate = sym ? sym->ispublic : FALSE;

	test	rax, rax
	je	$LN32@ProcDir
	movzx	edi, BYTE PTR [rax+40]

; 1470 :         if( sym == NULL || sym->state == SYM_UNDEFINED ) {

	mov	eax, DWORD PTR [rax+32]
	shr	dil, 7
	test	eax, eax
	je	$LN12@ProcDir

; 1473 :         } else if ( sym->state == SYM_EXTERNAL && sym->weak == TRUE ) {

	cmp	eax, 2
	jne	SHORT $LN13@ProcDir
	test	BYTE PTR [rbx+47], 8
	je	SHORT $LN13@ProcDir

; 1474 :             /* PROTO or EXTERNDEF item */
; 1475 :             is_global = TRUE;
; 1476 :             if ( sym->isproc == TRUE  ) {

	test	BYTE PTR [rbx+41], 8
	mov	r14b, 1
	je	SHORT $LN15@ProcDir

; 1477 :                 /* don't create the procinfo extension; it exists already */
; 1478 :                 procidx++; /* v2.04: added */

	inc	DWORD PTR procidx

; 1479 :                 if ( Options.line_numbers ) {

	cmp	BYTE PTR Options+1, 0
	je	SHORT $LN14@ProcDir

; 1480 :                     sym->debuginfo = LclAlloc( sizeof( struct debug_info ) );

	lea	ecx, QWORD PTR [rax+22]
	call	LclAlloc
	mov	QWORD PTR [rbx+64], rax

; 1481 :                     sym->debuginfo->file = get_curr_srcfile();

	call	get_curr_srcfile
	mov	rcx, QWORD PTR [rbx+64]
	mov	WORD PTR [rcx+14], ax

; 1482 :                 }
; 1483 :             } else {

	jmp	SHORT $LN14@ProcDir
$LN6@ProcDir:

; 1451 :             CurrProc->e.procinfo->regslist ) {
; 1452 :             return( EmitErr( CANNOT_NEST_PROCEDURES, name ) );

	mov	rdx, rsi
	mov	ecx, 93					; 0000005dH
	mov	rbx, QWORD PTR [rsp+64]
	mov	rsi, QWORD PTR [rsp+72]

; 1612 : }

	add	rsp, 48					; 00000030H
	pop	rbp

; 1451 :             CurrProc->e.procinfo->regslist ) {
; 1452 :             return( EmitErr( CANNOT_NEST_PROCEDURES, name ) );

	jmp	EmitErr
$LN15@ProcDir:

; 1484 :                 /* it's a simple EXTERNDEF. Create a PROC item!
; 1485 :                  * this will be SYM_INTERNAL */
; 1486 :                 /* v2.03: don't call dir_free(), it'll clear field Ofssize */
; 1487 :                 //dir_free( (struct dsym *)sym );
; 1488 :                 sym = CreateProc( sym, name, SYM_INTERNAL );

	mov	r8d, 1
	mov	rdx, rsi
	mov	rcx, rbx
	call	CreateProc

; 1489 :             }
; 1490 :         } else {

	jmp	SHORT $LN45@ProcDir
$LN13@ProcDir:

; 1491 :             /* Masm won't reject a redefinition if "certain" parameters
; 1492 :              * won't change. However, in a lot of cases one gets "internal assembler error".
; 1493 :              * Hence this "feature" isn't active in hjwasm.
; 1494 :              */
; 1495 :             //} else if ( sym->state != SYM_INTERNAL || sym->isproc != TRUE ||
; 1496 :             //           sym->offset != GetCurrOffset() || sym->segment != &CurrSeg->sym ) {
; 1497 :             return( EmitErr( SYMBOL_REDEFINITION, sym->name ) );

	mov	rdx, QWORD PTR [rbx+8]
	mov	ecx, 143				; 0000008fH
	call	EmitErr
	jmp	$LN44@ProcDir
$LN32@ProcDir:

; 1468 : 
; 1469 :         oldpubstate = sym ? sym->ispublic : FALSE;

	xor	dil, dil
$LN12@ProcDir:

; 1471 :             sym = CreateProc( sym, name, SYM_INTERNAL );

	mov	r8d, 1
	mov	rdx, rsi
	mov	rcx, rbx
	call	CreateProc

; 1472 :             is_global = FALSE;

	xor	r14b, r14b
$LN45@ProcDir:

; 1471 :             sym = CreateProc( sym, name, SYM_INTERNAL );

	mov	rbx, rax
$LN14@ProcDir:

; 1498 :         }
; 1499 :         SetSymSegOfs( sym );

	mov	rcx, rbx
	call	SetSymSegOfs

; 1500 : 
; 1501 :         SymClearLocal();

	call	SymClearLocal

; 1502 : 
; 1503 : #if STACKBASESUPP
; 1504 :         /* v2.11: added. Note that fpo flag is only set if there ARE params! */
; 1505 :         ((struct dsym *)sym)->e.procinfo->basereg = ModuleInfo.basereg[ModuleInfo.Ofssize];

	movzx	eax, BYTE PTR ModuleInfo+404
	lea	rsi, OFFSET FLAT:__ImageBase
	mov	rcx, QWORD PTR [rbx+96]

; 1506 : #endif
; 1507 :         /* CurrProc must be set, it's used inside SymFind() and SymLCreate()! */
; 1508 :         CurrProc = (struct dsym *)sym;
; 1509 :         if( ParseProc( (struct dsym *)sym, i, tokenarray, TRUE, ModuleInfo.langtype ) == ERROR ) {

	mov	r9b, 1
	mov	r8, rbp
	mov	edx, 2
	movzx	eax, WORD PTR ModuleInfo[rsi+rax*4+500]
	mov	WORD PTR [rcx+86], ax
	mov	rcx, rbx
	mov	eax, DWORD PTR ModuleInfo+364
	mov	DWORD PTR [rsp+32], eax
	mov	QWORD PTR CurrProc, rbx
	call	ParseProc
	cmp	eax, -1
	jne	SHORT $LN18@ProcDir

; 1510 :             CurrProc = NULL;

	xor	edi, edi
	mov	QWORD PTR CurrProc, rdi

; 1511 :             return( ERROR );

	or	eax, eax
	jmp	$LN44@ProcDir
$LN18@ProcDir:

; 1512 :         }
; 1513 :         /* v2.04: added */
; 1514 :         if ( is_global && Options.masm8_proc_visibility )

	test	r14b, r14b
	je	SHORT $LN19@ProcDir
	cmp	BYTE PTR Options+129, 0
	je	SHORT $LN19@ProcDir

; 1515 :             sym->ispublic = TRUE;

	or	BYTE PTR [rbx+40], 128			; 00000080H
$LN19@ProcDir:

; 1516 : 
; 1517 :         /* if there was a PROTO (or EXTERNDEF name:PROTO ...),
; 1518 :          * change symbol to SYM_INTERNAL!
; 1519 :          */
; 1520 :         if ( sym->state == SYM_EXTERNAL && sym->isproc == TRUE ) {

	cmp	DWORD PTR [rbx+32], 2
	jne	SHORT $LN20@ProcDir
	test	BYTE PTR [rbx+41], 8
	je	SHORT $LN20@ProcDir

; 1521 :             sym_ext2int( sym );

	mov	rcx, rbx
	call	sym_ext2int

; 1522 :             /* v2.11: added ( may be better to call CreateProc() - currently not possible ) */
; 1523 :             if ( SymTables[TAB_PROC].head == NULL )

	cmp	QWORD PTR SymTables+64, 0
	jne	SHORT $LN21@ProcDir

; 1524 :                 SymTables[TAB_PROC].head = (struct dsym *)sym;

	mov	QWORD PTR SymTables+64, rbx
	jmp	SHORT $LN22@ProcDir
$LN21@ProcDir:

; 1525 :             else {
; 1526 :                 SymTables[TAB_PROC].tail->nextproc = (struct dsym *)sym;

	mov	rax, QWORD PTR SymTables+72
	mov	QWORD PTR [rax+112], rbx
$LN22@ProcDir:

; 1527 :             }
; 1528 :             SymTables[TAB_PROC].tail = (struct dsym *)sym;

	mov	QWORD PTR SymTables+72, rbx
$LN20@ProcDir:

; 1529 :         }
; 1530 : 
; 1531 :         /* v2.11: sym->isproc is set inside ParseProc() */
; 1532 :         //sym->isproc = TRUE;
; 1533 : #if STACKBASESUPP
; 1534 :         /* v2.11: Note that fpo flag is only set if there ARE params ( or locals )! */
; 1535 :         if ( CurrProc->e.procinfo->paralist && GetRegNo( CurrProc->e.procinfo->basereg ) == 4 )

	mov	rax, QWORD PTR CurrProc
	mov	rdx, QWORD PTR [rax+96]
	cmp	QWORD PTR [rdx+8], 0
	je	SHORT $LN23@ProcDir
	movzx	eax, WORD PTR [rdx+86]
	lea	rcx, QWORD PTR [rax+rax*2]
	cmp	BYTE PTR SpecialTable[rsi+rcx*4+10], 4
	jne	SHORT $LN23@ProcDir

; 1536 :             CurrProc->e.procinfo->fpo = TRUE;

	or	BYTE PTR [rdx+84], 128			; 00000080H
$LN23@ProcDir:

; 1537 : #endif
; 1538 :         if( sym->ispublic == TRUE && oldpubstate == FALSE )

	test	BYTE PTR [rbx+40], 128			; 00000080H
	je	SHORT $LN24@ProcDir
	test	dil, dil
	jne	SHORT $LN24@ProcDir

; 1539 :             AddPublicData( sym );

	mov	rcx, rbx
	call	AddPublicData
$LN24@ProcDir:

; 1540 : 
; 1541 :         /* v2.04: add the proc to the list of labels attached to curr segment.
; 1542 :          * this allows to reduce the number of passes (see fixup.c)
; 1543 :          */
; 1544 :         ((struct dsym *)sym)->next = (struct dsym *)CurrSeg->e.seginfo->label_list;

	mov	rax, QWORD PTR ModuleInfo+432
	mov	rcx, QWORD PTR [rax+96]
	mov	rax, QWORD PTR [rcx+32]
	mov	QWORD PTR [rbx+104], rax

; 1545 :         CurrSeg->e.seginfo->label_list = sym;

	mov	rax, QWORD PTR ModuleInfo+432
	mov	rcx, QWORD PTR [rax+96]
	mov	QWORD PTR [rcx+32], rbx

; 1546 : 
; 1547 :     } else {

	jmp	SHORT $LN26@ProcDir
$LN8@ProcDir:

; 1548 :         /**/myassert( sym != NULL );
; 1549 : 
; 1550 :         procidx++;

	inc	DWORD PTR procidx

; 1551 :         sym->isdefined = TRUE;
; 1552 : 
; 1553 :         SymSetLocal( sym );

	mov	rcx, rax
	or	BYTE PTR [rax+40], 2
	call	SymSetLocal

; 1554 : 
; 1555 :         /* it's necessary to check for a phase error here
; 1556 :          as it is done in LabelCreate() and data_dir()!
; 1557 :          */
; 1558 :         ofs = GetCurrOffset();

	call	GetCurrOffset

; 1559 : 
; 1560 :         if ( ofs != sym->offset) {

	cmp	eax, DWORD PTR [rbx+16]
	je	SHORT $LN25@ProcDir

; 1561 :             DebugMsg(("ProcDir(%s): %spass %u, old ofs=%" I32_SPEC "X, new ofs=%" I32_SPEC "X\n",
; 1562 :                     sym->name,
; 1563 :                     ModuleInfo.PhaseError ? "" : "phase error ",
; 1564 :                     Parse_Pass+1, sym->offset, ofs ));
; 1565 :             sym->offset = ofs;

	mov	DWORD PTR [rbx+16], eax

; 1566 :             ModuleInfo.PhaseError = TRUE;

	mov	BYTE PTR ModuleInfo+422, 1
$LN25@ProcDir:

; 1567 :         }
; 1568 :         CurrProc = (struct dsym *)sym;

	mov	QWORD PTR CurrProc, rbx

; 1569 : #if AMD64_SUPPORT
; 1570 :         /* check if the exception handler set by FRAME is defined */
; 1571 :         if ( CurrProc->e.procinfo->isframe &&
; 1572 :             CurrProc->e.procinfo->exc_handler &&

	mov	rdx, QWORD PTR [rbx+96]
	test	BYTE PTR [rdx+84], 64			; 00000040H
	je	SHORT $LN26@ProcDir
	mov	rdx, QWORD PTR [rdx+48]
	test	rdx, rdx
	je	SHORT $LN26@ProcDir
	cmp	DWORD PTR [rdx+32], 0
	jne	SHORT $LN26@ProcDir

; 1573 :             CurrProc->e.procinfo->exc_handler->state == SYM_UNDEFINED ) {
; 1574 :             EmitErr( SYMBOL_NOT_DEFINED, CurrProc->e.procinfo->exc_handler->name );

	mov	rdx, QWORD PTR [rdx+8]
	mov	ecx, 102				; 00000066H
	call	EmitErr
$LN26@ProcDir:

; 1575 :         }
; 1576 : #endif
; 1577 :     }
; 1578 : 
; 1579 :     /* v2.11: init @ProcStatus - prologue not written yet, optionally set FPO flag */
; 1580 : #if STACKBASESUPP
; 1581 :     ProcStatus = PRST_PROLOGUE_NOT_DONE | ( CurrProc->e.procinfo->fpo ? PRST_FPO : 0 );

	mov	rdx, QWORD PTR CurrProc

; 1582 :     StackAdj = 0;  /* init @StackBase to 0 */

	xor	edi, edi
	mov	rax, QWORD PTR [rdx+96]
	movzx	ecx, BYTE PTR [rax+84]
	shr	ecx, 5
	and	ecx, 4
	mov	DWORD PTR StackAdj, edi
	bts	ecx, 7

; 1583 :     StackAdjHigh = 0;

	mov	DWORD PTR StackAdjHigh, edi
	mov	DWORD PTR ProcStatus, ecx

; 1584 : #else
; 1585 :     ProcStatus = PRST_PROLOGUE_NOT_DONE;
; 1586 : #endif
; 1587 : 
; 1588 : #if AMD64_SUPPORT
; 1589 :     if ( CurrProc->e.procinfo->isframe ) {

	mov	rax, QWORD PTR [rdx+96]
	test	BYTE PTR [rax+84], 64			; 00000040H
	je	SHORT $LN28@ProcDir

; 1590 :         endprolog_found = FALSE;
; 1591 :         /* v2.11: clear all fields */
; 1592 :         memset( &unw_info, 0, sizeof( unw_info ) );

	xor	eax, eax
	mov	BYTE PTR endprolog_found, dil
	mov	DWORD PTR unw_info, eax

; 1593 :         if ( CurrProc->e.procinfo->exc_handler )

	mov	rax, QWORD PTR [rdx+96]
	cmp	QWORD PTR [rax+48], rdi
	je	SHORT $LN28@ProcDir

; 1594 :             unw_info.Flags = UNW_FLAG_FHANDLER;

	movzx	eax, BYTE PTR unw_info
	and	al, 7
	or	al, 24
	mov	BYTE PTR unw_info, al
$LN28@ProcDir:

; 1595 :     }
; 1596 : #endif
; 1597 : 
; 1598 :     sym->asmpass = Parse_Pass;

	movzx	eax, BYTE PTR Parse_Pass
	mov	BYTE PTR [rbx+46], al

; 1599 :     if ( ModuleInfo.list )

	test	DWORD PTR ModuleInfo+408, 2048		; 00000800H
	je	SHORT $LN29@ProcDir

; 1600 :         LstWrite( LSTTYPE_LABEL, 0, NULL );

	xor	edx, edx
	xor	r8d, r8d
	lea	ecx, QWORD PTR [rdx+7]
	call	LstWrite
$LN29@ProcDir:

; 1601 : 
; 1602 :     if( Options.line_numbers ) {

	cmp	BYTE PTR Options+1, dil
	je	SHORT $LN30@ProcDir

; 1603 : #if COFF_SUPPORT
; 1604 :         AddLinnumDataRef( get_curr_srcfile(), Options.output_format == OFORMAT_COFF ? 0 : GetLineNumber() );

	cmp	DWORD PTR Options+144, 2
	je	SHORT $LN35@ProcDir
	call	GetLineNumber
	mov	edi, eax
$LN35@ProcDir:
	call	get_curr_srcfile
	mov	ecx, eax
	mov	edx, edi
	call	AddLinnumDataRef
$LN30@ProcDir:

; 1605 : #else
; 1606 :         AddLinnumDataRef( get_curr_srcfile(), GetLineNumber() );
; 1607 : #endif
; 1608 :     }
; 1609 : 
; 1610 :     BackPatch( sym );

	mov	rcx, rbx
	call	BackPatch

; 1611 :     return( NOT_ERROR );

	xor	eax, eax
$LN44@ProcDir:
	mov	rdi, QWORD PTR [rsp+80]
	mov	r14, QWORD PTR [rsp+88]
	mov	rbx, QWORD PTR [rsp+64]
	mov	rsi, QWORD PTR [rsp+72]

; 1612 : }

	add	rsp, 48					; 00000030H
	pop	rbp
	ret	0
ProcDir	ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
;	COMDAT ParseParams
_TEXT	SEGMENT
fcint$ = 32
ti$ = 40
init_done$1$ = 160
proc$ = 160
i$ = 168
tokenarray$ = 176
IsPROC$ = 184
ParseParams PROC					; COMDAT

; 632  : {

	mov	BYTE PTR [rsp+32], r9b
	mov	QWORD PTR [rsp+24], r8
	mov	DWORD PTR [rsp+16], edx
	push	rbp
	push	rbx
	push	rdi
	push	r13
	push	r14
	push	r15
	mov	rbp, rsp
	sub	rsp, 104				; 00000068H

; 633  :     char            *name;
; 634  :     struct asym     *sym;
; 635  :     int             cntParam;
; 636  :     int             offset;
; 637  :     //int             size;
; 638  :     int             fcint = 0;

	xor	r15d, r15d
	movsxd	r10, edx

; 639  :     struct qualified_type ti;
; 640  :     bool            is_vararg;
; 641  :     bool            init_done;
; 642  :     struct dsym     *paranode;
; 643  :     struct dsym     *paracurr;
; 644  :     int             curr;
; 645  : 
; 646  :     /*
; 647  :      * find "first" parameter ( that is, the first to be pushed in INVOKE ).
; 648  :      */
; 649  :     if (proc->sym.langtype == LANG_C ||
; 650  :         proc->sym.langtype == LANG_SYSCALL ||
; 651  : #if AMD64_SUPPORT
; 652  :         ( proc->sym.langtype == LANG_FASTCALL && ModuleInfo.Ofssize != USE64 ) ||

	mov	edx, DWORD PTR [rcx+76]
	movzx	ebx, r9b
	mov	r9, r8
	mov	DWORD PTR fcint$[rbp-104], r15d
	mov	r14, rcx
	lea	eax, DWORD PTR [rdx-1]
	cmp	eax, 1
	jbe	SHORT $LN37@ParseParam
	cmp	edx, 7
	jne	SHORT $LN38@ParseParam
	cmp	BYTE PTR ModuleInfo+404, 2
	jne	SHORT $LN37@ParseParam
$LN38@ParseParam:
	cmp	edx, 3
	je	SHORT $LN37@ParseParam

; 658  :     else
; 659  :         paracurr = proc->e.procinfo->paralist;

	mov	rax, QWORD PTR [rcx+96]
	mov	rdi, QWORD PTR [rax+8]
$LN162@ParseParam:

; 660  : 
; 661  :     /* v2.11: proc_info.init_done has been removed, sym.isproc flag is used instead */
; 662  :     init_done = proc->sym.isproc;

	movzx	eax, BYTE PTR [rcx+41]

; 663  : 
; 664  :     for( cntParam = 0 ; tokenarray[i].token != T_FINAL ; cntParam++ ) {

	mov	r13d, r15d
	shr	al, 3
	and	al, 1
	mov	QWORD PTR [rsp+96], rsi
	mov	BYTE PTR init_done$1$[rbp-104], al
	mov	rax, r10
	shl	rax, 5
	mov	QWORD PTR [rsp+88], r12
	lea	rcx, QWORD PTR [rax+r8]
$LN170@ParseParam:
	movzx	eax, BYTE PTR [rcx]
	test	al, al
	je	$LN161@ParseParam
	mov	edx, 3

; 665  : 
; 666  :         if ( tokenarray[i].token == T_ID ) {

	cmp	al, 8
	jne	SHORT $LN39@ParseParam

; 667  :             name = tokenarray[i++].string_ptr;

	mov	r12, QWORD PTR [rcx+8]
	inc	r10d
	mov	DWORD PTR i$[rbp-104], r10d
	jmp	SHORT $LN44@ParseParam
$LN37@ParseParam:

; 653  : #else
; 654  :         proc->sym.langtype == LANG_FASTCALL ||
; 655  : #endif
; 656  :         proc->sym.langtype == LANG_STDCALL)
; 657  :         for ( paracurr = proc->e.procinfo->paralist; paracurr && paracurr->nextparam; paracurr = paracurr->nextparam );

	mov	rax, QWORD PTR [rcx+96]
	mov	rdi, QWORD PTR [rax+8]
	test	rdi, rdi
	je	SHORT $LN162@ParseParam
	npad	6
$LL4@ParseParam:
	mov	rax, QWORD PTR [rdi+112]
	test	rax, rax
	je	SHORT $LN162@ParseParam
	mov	rdi, rax
	jmp	SHORT $LL4@ParseParam
$LN39@ParseParam:

; 668  :         } else if ( IsPROC == FALSE && tokenarray[i].token == T_COLON ) {

	test	bl, bl
	jne	$LN41@ParseParam
	cmp	al, 58					; 0000003aH
	jne	$LN41@ParseParam

; 669  :             if ( paracurr )

	test	rdi, rdi
	je	SHORT $LN43@ParseParam

; 670  :                 name = paracurr->sym.name;

	mov	r12, QWORD PTR [rdi+8]
	jmp	SHORT $LN44@ParseParam
$LN43@ParseParam:

; 671  :             else
; 672  :                 name = "";

	lea	r12, OFFSET FLAT:$SG11584
$LN44@ParseParam:

; 677  :         }
; 678  : 
; 679  :         ti.symtype = NULL;
; 680  :         ti.is_ptr = 0;
; 681  :         ti.ptr_memtype = MT_EMPTY;
; 682  :         /* v2.02: init is_far depending on memory model */
; 683  :         //ti.is_far = FALSE;
; 684  :         if ( SIZE_DATAPTR & ( 1 << ModuleInfo.model ) )

	mov	ecx, DWORD PTR ModuleInfo+360
	mov	eax, 1
	shl	al, cl

; 685  :             ti.is_far = TRUE;
; 686  :         else
; 687  :             ti.is_far = FALSE;
; 688  :         ti.Ofssize = ModuleInfo.Ofssize;

	movzx	ecx, BYTE PTR ModuleInfo+404
	test	al, 104					; 00000068H

; 689  :         ti.size = CurrWordSize;

	movzx	eax, BYTE PTR ModuleInfo+406
	mov	DWORD PTR ti$[rbp-104], eax
	setne	BYTE PTR ti$[rbp-83]

; 690  : 
; 691  :         is_vararg = FALSE;
; 692  : 
; 693  :         /* read colon. It's optional for PROC.
; 694  :          * Masm also allows a missing colon for PROTO - if there's
; 695  :          * just one parameter. Probably a Masm bug.
; 696  :          * HJWasm always require a colon for PROTO.
; 697  :          */
; 698  :         if( tokenarray[i].token != T_COLON ) {

	movsxd	rax, r10d
	xor	sil, sil
	shl	rax, 5
	mov	QWORD PTR ti$[rbp-96], r15
	mov	BYTE PTR ti$[rbp-84], r15b
	mov	DWORD PTR ti$[rbp-80], 192		; 000000c0H
	mov	BYTE PTR ti$[rbp-82], cl
	cmp	BYTE PTR [rax+r9], 58			; 0000003aH
	je	SHORT $LN47@ParseParam

; 699  :             if ( IsPROC == FALSE ) {

	test	bl, bl
	je	$LN123@ParseParam

; 701  :             }
; 702  :             switch ( ti.Ofssize ) {

	test	cl, cl
	mov	r9d, 1
	cmovne	r9d, edx
	mov	DWORD PTR ti$[rbp-88], r9d

; 703  :             case USE16:
; 704  :                 ti.mem_type = MT_WORD; break;
; 705  : #if AMD64_SUPPORT
; 706  :                 /* v2.08: default size for arguments is DWORD in 64-bit ( Win64 ) */
; 707  :                 //case USE64: ti.mem_type = MT_QWORD; break;
; 708  : #endif
; 709  :             default:
; 710  :                 ti.mem_type = MT_DWORD; break;
; 711  :             }
; 712  :         } else {

	jmp	$LN57@ParseParam
$LN47@ParseParam:

; 713  :             i++;

	inc	r10d

; 714  :             if (( tokenarray[i].token == T_RES_ID ) && ( tokenarray[i].tokval == T_VARARG )) {

	movsxd	rcx, r10d
	shl	rcx, 5
	mov	DWORD PTR i$[rbp-104], r10d
	cmp	BYTE PTR [rcx+r9], 7
	jne	SHORT $LN52@ParseParam
	cmp	DWORD PTR [rcx+r9+16], 262		; 00000106H
	jne	SHORT $LN52@ParseParam

; 715  :                 switch( proc->sym.langtype ) {

	mov	eax, DWORD PTR [r14+76]
	test	eax, eax
	je	$LN54@ParseParam
	add	eax, -3					; fffffffdH
	cmp	eax, 3
	jbe	$LN54@ParseParam

; 722  :                 }
; 723  :                 /* v2.05: added check */
; 724  :                 if ( tokenarray[i+1].token != T_FINAL )

	cmp	BYTE PTR [rcx+r9+32], r15b
	je	SHORT $LN55@ParseParam

; 725  :                     EmitError( VARARG_PARAMETER_MUST_BE_LAST );

	mov	ecx, 147				; 00000093H
	call	EmitError
	mov	r10d, DWORD PTR i$[rbp-104]
	jmp	SHORT $LN56@ParseParam
$LN55@ParseParam:

; 726  :                 else
; 727  :                     is_vararg = TRUE;

	mov	sil, 1
$LN56@ParseParam:

; 728  :                 ti.mem_type = MT_EMPTY;

	mov	r9d, 192				; 000000c0H

; 729  :                 ti.size = 0;

	mov	DWORD PTR ti$[rbp-104], r15d

; 730  :                 i++;

	inc	r10d
	mov	DWORD PTR ti$[rbp-88], r9d
	mov	DWORD PTR i$[rbp-104], r10d

; 731  :             } else {

	jmp	SHORT $LN57@ParseParam
$LN52@ParseParam:

; 732  :                 if ( GetQualifiedType( &i, tokenarray, &ti ) == ERROR )

	lea	r8, QWORD PTR ti$[rbp-104]
	mov	rdx, r9
	lea	rcx, QWORD PTR i$[rbp-104]
	call	GetQualifiedType
	cmp	eax, -1
	je	$LN124@ParseParam
	mov	r10d, DWORD PTR i$[rbp-104]
	mov	r9d, DWORD PTR ti$[rbp-88]
$LN57@ParseParam:

; 734  :             }
; 735  :         }
; 736  : 
; 737  :         /* check if parameter name is defined already */
; 738  :         if (( IsPROC ) && ( sym = SymSearch( name ) ) && sym->state != SYM_UNDEFINED ) {

	test	bl, bl
	je	SHORT $LN58@ParseParam
	mov	rcx, r12
	call	SymFind
	test	rax, rax
	je	SHORT $LN160@ParseParam
	cmp	DWORD PTR [rax+32], r15d
	jne	$LN125@ParseParam
$LN160@ParseParam:
	mov	r10d, DWORD PTR i$[rbp-104]
	mov	r9d, DWORD PTR ti$[rbp-88]
$LN58@ParseParam:

; 741  :         }
; 742  : 
; 743  :         /* redefinition? */
; 744  :         if ( paracurr ) {

	test	rdi, rdi
	je	$LN59@ParseParam

; 745  : #if 0 /* was active till v2.04 */
; 746  :             int newsize = ti.size;
; 747  :             int oldsize;
; 748  : 
; 749  :             /* check size only (so UINT <-> DWORD wont cause an error) */
; 750  :             if ( paracurr->sym.type )
; 751  :                 oldsize = paracurr->sym.total_size;
; 752  :             else if ( paracurr->sym.mem_type == MT_EMPTY )
; 753  :                 oldsize = 0;
; 754  :             else if ( paracurr->sym.mem_type == MT_PTR )
; 755  :                 oldsize = SizeFromMemtype( paracurr->sym.isfar ? MT_FAR : MT_NEAR, paracurr->sym.Ofssize, NULL );
; 756  :             else
; 757  :                 oldsize = SizeFromMemtype( paracurr->sym.mem_type, paracurr->sym.Ofssize, paracurr->sym.type );
; 758  :             if ( oldsize != newsize ) {
; 759  :                 DebugMsg(("ParseParams: old memtype=%u, new memtype=%u\n", paracurr->sym.mem_type, ti.mem_type));
; 760  :                 EmitErr( CONFLICTING_PARAMETER_DEFINITION, name );
; 761  :                 //return( ERROR );
; 762  :             }
; 763  :             /* the parameter type used in PROC has highest priority! */
; 764  :             if ( IsPROC ) {
; 765  :                 if ( ti.symtype ) {
; 766  :                     paracurr->sym.type = ti.symtype;
; 767  :                     paracurr->sym.mem_type = MT_TYPE;
; 768  :                 } else
; 769  :                     paracurr->sym.mem_type = ti.mem_type;
; 770  :             }
; 771  : #else
; 772  :             struct asym *to;
; 773  :             struct asym *tn;
; 774  :             char oo;
; 775  :             char on;
; 776  :             for( tn = ti.symtype; tn && tn->type; tn = tn->type );

	mov	r8, QWORD PTR ti$[rbp-96]
	test	r8, r8
	je	SHORT $LN163@ParseParam
$LL14@ParseParam:
	mov	rax, QWORD PTR [r8+80]
	test	rax, rax
	je	SHORT $LN163@ParseParam
	mov	r8, rax
	jmp	SHORT $LL14@ParseParam
$LN163@ParseParam:

; 777  :             /* v2.12: don't assume pointer type if mem_type is != MT_TYPE!
; 778  :              * regression test proc9.asm.
; 779  :              */
; 780  :             //to = ( paracurr->sym.mem_type == MT_TYPE ) ? paracurr->sym.type : paracurr->sym.target_type;
; 781  :             if ( paracurr->sym.mem_type == MT_TYPE )

	mov	esi, DWORD PTR [rdi+36]
	cmp	esi, 196				; 000000c4H
	jne	SHORT $LN61@ParseParam

; 782  :                 to = paracurr->sym.type;

	mov	rdx, QWORD PTR [rdi+80]
	jmp	SHORT $LN104@ParseParam
$LN61@ParseParam:

; 783  :             else
; 784  :                 to = ( paracurr->sym.mem_type == MT_PTR ? paracurr->sym.target_type : NULL );

	cmp	esi, 195				; 000000c3H
	jne	SHORT $LN103@ParseParam
	mov	rdx, QWORD PTR [rdi+48]
$LN104@ParseParam:

; 785  :             for( ; to && to->type; to = to->type );

	test	rdx, rdx
	je	SHORT $LN16@ParseParam
$LL17@ParseParam:
	mov	rax, QWORD PTR [rdx+80]
	test	rax, rax
	je	SHORT $LN16@ParseParam
	mov	rdx, rax
	jmp	SHORT $LL17@ParseParam
$LN103@ParseParam:

; 783  :             else
; 784  :                 to = ( paracurr->sym.mem_type == MT_PTR ? paracurr->sym.target_type : NULL );

	mov	rdx, r15
$LN16@ParseParam:

; 786  :             oo = ( paracurr->sym.Ofssize != USE_EMPTY ) ? paracurr->sym.Ofssize : ModuleInfo.Ofssize;

	movzx	ecx, BYTE PTR ModuleInfo+404
	movzx	eax, BYTE PTR [rdi+44]
	mov	r11d, ecx
	cmp	al, 254					; 000000feH

; 787  :             on = ( ti.Ofssize != USE_EMPTY ) ? ti.Ofssize : ModuleInfo.Ofssize;

	mov	ebx, ecx
	movzx	ecx, BYTE PTR ti$[rbp-82]
	cmovne	r11d, eax
	cmp	cl, 254					; 000000feH
	cmovne	ebx, ecx

; 788  :             if ( ti.mem_type != paracurr->sym.mem_type ||
; 789  :                 ( ti.mem_type == MT_TYPE && tn != to ) ||

	cmp	r9d, esi
	jne	SHORT $LN66@ParseParam
	cmp	r9d, 196				; 000000c4H
	jne	SHORT $LN65@ParseParam
	cmp	r8, rdx
	jne	SHORT $LN66@ParseParam
	jmp	SHORT $LN63@ParseParam
$LN65@ParseParam:
	cmp	r9d, 195				; 000000c3H
	jne	SHORT $LN63@ParseParam
	movzx	eax, BYTE PTR [rdi+47]
	shr	al, 4
	and	al, 1
	cmp	BYTE PTR ti$[rbp-83], al
	jne	SHORT $LN66@ParseParam
	cmp	bl, r11b
	jne	SHORT $LN66@ParseParam
	movzx	eax, BYTE PTR [rdi+46]
	cmp	DWORD PTR ti$[rbp-80], eax
	jne	SHORT $LN66@ParseParam
	cmp	r8, rdx
	je	SHORT $LN63@ParseParam
$LN66@ParseParam:

; 790  :                 ( ti.mem_type == MT_PTR &&
; 791  :                  ( ti.is_far != paracurr->sym.isfar ||
; 792  :                   on != oo ||
; 793  :                   ti.ptr_memtype != paracurr->sym.ptr_memtype ||
; 794  :                   tn != to ))) {
; 795  :                 DebugMsg(("ParseParams: old-new memtype=%X-%X type=%X(%s)-%X(%s) far=%u-%u ind=%u-%u ofss=%d-%d pmt=%X-%X\n",
; 796  :                           paracurr->sym.mem_type, ti.mem_type, 
; 797  :                           (paracurr->sym.mem_type == MT_TYPE) ? paracurr->sym.type : paracurr->sym.target_type,
; 798  :                           (paracurr->sym.mem_type == MT_TYPE) ? paracurr->sym.type->name : paracurr->sym.target_type ? paracurr->sym.target_type->name : "",
; 799  :                           ti.symtype, ti.symtype ? ti.symtype->name : "",
; 800  :                           paracurr->sym.isfar, ti.is_far,
; 801  :                           paracurr->sym.is_ptr, ti.is_ptr,
; 802  :                           paracurr->sym.Ofssize, ti.Ofssize,
; 803  :                           paracurr->sym.ptr_memtype, ti.ptr_memtype ));
; 804  :                 EmitErr( CONFLICTING_PARAMETER_DEFINITION, name );

	mov	rdx, r12
	mov	ecx, 137				; 00000089H
	call	EmitErr
	mov	r10d, DWORD PTR i$[rbp-104]
$LN63@ParseParam:

; 805  :                 //return( ERROR );
; 806  :             }
; 807  : #endif
; 808  :             if ( IsPROC ) {

	movzx	ebx, BYTE PTR IsPROC$[rbp-104]
	test	bl, bl
	je	SHORT $LN67@ParseParam

; 809  :                 DebugMsg1(("ParseParams: calling SymAddLocal(%s, %s)\n", paracurr->sym.name, name ));
; 810  :                 /* it has been checked already that the name isn't found - SymAddLocal() shouldn't fail */
; 811  :                 SymAddLocal( &paracurr->sym, name );

	mov	rdx, r12
	mov	rcx, rdi
	call	SymAddLocal
	mov	r10d, DWORD PTR i$[rbp-104]
$LN67@ParseParam:

; 812  :             }
; 813  :             /* set paracurr to next parameter */
; 814  :             if ( proc->sym.langtype == LANG_C ||
; 815  :                 proc->sym.langtype == LANG_SYSCALL ||
; 816  : #if AMD64_SUPPORT
; 817  :                 ( proc->sym.langtype == LANG_FASTCALL && ti.Ofssize != USE64 ) ||

	mov	ecx, DWORD PTR [r14+76]
	lea	eax, DWORD PTR [rcx-1]
	cmp	eax, 1
	jbe	SHORT $LN70@ParseParam
	cmp	ecx, 7
	jne	SHORT $LN71@ParseParam
	cmp	BYTE PTR ti$[rbp-82], 2
	jne	SHORT $LN70@ParseParam
$LN71@ParseParam:
	cmp	ecx, 3
	je	SHORT $LN70@ParseParam

; 827  :             } else
; 828  :                 paracurr = paracurr->nextparam;

	mov	rdi, QWORD PTR [rdi+112]
	jmp	$LN21@ParseParam
$LN70@ParseParam:

; 818  : #else
; 819  :                 proc->sym.langtype == LANG_FASTCALL ||
; 820  : #endif
; 821  :                 proc->sym.langtype == LANG_STDCALL) {
; 822  :                 struct dsym *l;
; 823  :                 for (l = proc->e.procinfo->paralist;

	mov	rax, QWORD PTR [r14+96]
	mov	rcx, QWORD PTR [rax+8]

; 824  :                      l && ( l->nextparam != paracurr );

	test	rcx, rcx
	je	SHORT $LN164@ParseParam
	npad	3
$LL20@ParseParam:
	mov	rax, QWORD PTR [rcx+112]
	cmp	rax, rdi
	je	SHORT $LN164@ParseParam

; 825  :                      l = l->nextparam );

	mov	rcx, rax
	test	rax, rax
	jne	SHORT $LL20@ParseParam
$LN164@ParseParam:

; 826  :                 paracurr = l;

	mov	rdi, rcx

; 829  : 
; 830  :         //} else if ( proc->e.procinfo->init_done == TRUE ) {
; 831  :         } else if ( init_done == TRUE ) {

	jmp	$LN21@ParseParam
$LN59@ParseParam:
	cmp	BYTE PTR init_done$1$[rbp-104], 1
	je	$LN126@ParseParam

; 835  :         } else {
; 836  :             if ( IsPROC ) {

	test	bl, bl
	je	SHORT $LN74@ParseParam

; 837  :                 paranode = (struct dsym *)SymLCreate( name );

	mov	rcx, r12
	call	SymLCreate

; 838  :             } else

	jmp	SHORT $LN165@ParseParam
$LN74@ParseParam:

; 839  :                 paranode = (struct dsym *)SymAlloc( "" );/* for PROTO, no param name needed */

	lea	rcx, OFFSET FLAT:$SG11617
	call	SymAlloc
$LN165@ParseParam:
	mov	rbx, rax

; 840  : 
; 841  :             if( paranode == NULL ) { /* error msg has been displayed already */

	test	rax, rax
	je	$LN124@ParseParam

; 842  :                 DebugMsg(("ParseParams: SymLCreate(%s) failed\n", name ));
; 843  :                 return( ERROR );
; 844  :             }
; 845  :             paranode->sym.isdefined = TRUE;

	or	BYTE PTR [rax+40], 2

; 846  :             paranode->sym.mem_type = ti.mem_type;

	mov	eax, DWORD PTR ti$[rbp-88]
	mov	DWORD PTR [rbx+36], eax

; 847  :             if ( ti.mem_type == MT_TYPE ) {

	cmp	DWORD PTR ti$[rbp-88], 196		; 000000c4H

; 848  :                 paranode->sym.type = ti.symtype;

	mov	rax, QWORD PTR ti$[rbp-96]
	jne	SHORT $LN77@ParseParam
	mov	QWORD PTR [rbx+80], rax

; 849  :             } else {

	jmp	SHORT $LN78@ParseParam
$LN77@ParseParam:

; 850  :                 paranode->sym.target_type = ti.symtype;

	mov	QWORD PTR [rbx+48], rax
$LN78@ParseParam:

; 851  :             }
; 852  : 
; 853  :             /* v2.05: moved BEFORE fastcall_tab() */
; 854  :             paranode->sym.isfar   = ti.is_far;

	movzx	eax, BYTE PTR ti$[rbp-83]
	shl	al, 4
	xor	al, BYTE PTR [rbx+47]
	and	al, 16

; 855  :             paranode->sym.Ofssize = ti.Ofssize;
; 856  :             paranode->sym.is_ptr  = ti.is_ptr;
; 857  :             paranode->sym.ptr_memtype = ti.ptr_memtype;
; 858  :             paranode->sym.is_vararg = is_vararg;

	shl	sil, 5
	xor	BYTE PTR [rbx+47], al
	movzx	eax, BYTE PTR ti$[rbp-82]
	xor	sil, BYTE PTR [rbx+47]
	mov	BYTE PTR [rbx+44], al
	and	sil, 32					; 00000020H
	movzx	eax, BYTE PTR ti$[rbp-84]
	mov	BYTE PTR [rbx+45], al
	movzx	eax, BYTE PTR ti$[rbp-80]
	xor	BYTE PTR [rbx+47], sil
	mov	BYTE PTR [rbx+46], al

; 859  :             if ( proc->sym.langtype == LANG_FASTCALL &&

	cmp	DWORD PTR [r14+76], 7
	jne	SHORT $LN79@ParseParam
	movsxd	rax, DWORD PTR ModuleInfo+376
	lea	r8, QWORD PTR fcint$[rbp-104]
	add	rax, rax
	lea	r9, OFFSET FLAT:fastcall_tab
	mov	rdx, rbx
	mov	rcx, r14
	call	QWORD PTR [r9+rax*8]
	test	eax, eax
	jne	SHORT $LN80@ParseParam
$LN79@ParseParam:

; 860  :                 fastcall_tab[ModuleInfo.fctype].paramcheck( proc, paranode, &fcint ) ) {
; 861  :             } else {
; 862  :                 paranode->sym.state = SYM_STACK;

	mov	DWORD PTR [rbx+32], 5
$LN80@ParseParam:

; 863  :             }
; 864  : 
; 865  :             paranode->sym.total_length = 1; /* v2.04: added */
; 866  :             paranode->sym.total_size = ti.size;
; 867  : 
; 868  :             if( paranode->sym.is_vararg == FALSE )

	test	BYTE PTR [rbx+47], 32			; 00000020H
	mov	DWORD PTR [rbx+64], 1
	mov	eax, DWORD PTR ti$[rbp-104]
	mov	DWORD PTR [rbx+56], eax
	jne	SHORT $LN81@ParseParam

; 869  :                 /* v2.11: CurrWordSize does reflect the default parameter size only for PROCs.
; 870  :                  * For PROTOs and TYPEs use member seg_ofssize.
; 871  :                  */
; 872  :                 //proc->e.procinfo->parasize += ROUND_UP( ti.size, CurrWordSize );
; 873  :                 proc->e.procinfo->parasize += ROUND_UP( ti.size, IsPROC ? CurrWordSize : ( 2 << proc->sym.seg_ofssize ) );

	cmp	BYTE PTR IsPROC$[rbp-104], r15b
	je	SHORT $LN109@ParseParam
	movzx	r8d, BYTE PTR ModuleInfo+406
	jmp	SHORT $LN166@ParseParam
$LN109@ParseParam:
	movzx	ecx, BYTE PTR [r14+47]
	mov	r8d, 2
	and	ecx, 3
	shl	r8d, cl
$LN166@ParseParam:
	mov	ecx, DWORD PTR ti$[rbp-104]
	lea	eax, DWORD PTR [r8-1]
	mov	rdx, QWORD PTR [r14+96]
	dec	ecx
	add	ecx, r8d
	not	eax
	and	ecx, eax
	mov	r9d, r8d
	add	DWORD PTR [rdx+32], ecx
$LN81@ParseParam:

; 874  : 
; 875  :             /* v2.05: the PROC's vararg flag has been set already */
; 876  :             //proc->e.procinfo->is_vararg |= paranode->sym.is_vararg;
; 877  : 
; 878  :             /* Parameters usually are stored in "push" order.
; 879  :              * However, for Win64, it's better to store them
; 880  :              * the "natural" way from left to right, since the
; 881  :              * arguments aren't "pushed".
; 882  :              */
; 883  : 
; 884  :             switch( proc->sym.langtype ) {

	mov	eax, DWORD PTR [r14+76]
	cmp	eax, 4
	jl	SHORT $LN88@ParseParam
	cmp	eax, 6
	jle	SHORT $left_to_right$172
	cmp	eax, 7
	jne	SHORT $LN88@ParseParam

; 902  :             case LANG_FASTCALL:
; 903  : #if AMD64_SUPPORT
; 904  :                 if ( ti.Ofssize == USE64 )

	movzx	eax, BYTE PTR ti$[rbp-82]
	cmp	al, 2
	je	SHORT $left_to_right$172

; 905  :                     goto left_to_right;
; 906  : #endif
; 907  :                 /* v2.07: MS fastcall 16-bit is PASCAL! */
; 908  :                 if ( ti.Ofssize == USE16 && ModuleInfo.fctype == FCT_MSC )

	test	al, al
	jne	SHORT $LN88@ParseParam
	cmp	DWORD PTR ModuleInfo+376, r15d
	jne	SHORT $LN88@ParseParam
$left_to_right$172:

; 885  :             case LANG_BASIC:
; 886  :             case LANG_FORTRAN:
; 887  :             case LANG_PASCAL:
; 888  :             left_to_right:
; 889  :                 paranode->nextparam = NULL;

	mov	QWORD PTR [rbx+112], r15

; 890  :                 if( proc->e.procinfo->paralist == NULL ) {

	mov	rax, QWORD PTR [r14+96]
	mov	rcx, QWORD PTR [rax+8]
	test	rcx, rcx
	je	SHORT $LN167@ParseParam

; 894  :                         if( paracurr->nextparam == NULL ) {

	mov	rax, QWORD PTR [rcx+112]
	test	rax, rax
	je	SHORT $LN122@ParseParam
	npad	1
$LL25@ParseParam:

; 891  :                     proc->e.procinfo->paralist = paranode;
; 892  :                 } else {
; 893  :                     for( paracurr = proc->e.procinfo->paralist;; paracurr = paracurr->nextparam ) {

	mov	rcx, rax

; 894  :                         if( paracurr->nextparam == NULL ) {

	mov	rax, QWORD PTR [rax+112]
	test	rax, rax
	jne	SHORT $LL25@ParseParam
$LN122@ParseParam:

; 895  :                             break;
; 896  :                         }
; 897  :                     }
; 898  :                     paracurr->nextparam = paranode;

	mov	QWORD PTR [rcx+112], rbx

; 899  :                     paracurr = NULL;

	mov	rdi, r15

; 900  :                 }
; 901  :                 break;

	jmp	SHORT $LN168@ParseParam
$LN88@ParseParam:

; 909  :                     goto left_to_right;
; 910  :             default:
; 911  :                 paranode->nextparam = proc->e.procinfo->paralist;

	mov	rax, QWORD PTR [r14+96]
	mov	rcx, QWORD PTR [rax+8]
	mov	QWORD PTR [rbx+112], rcx

; 912  :                 proc->e.procinfo->paralist = paranode;

	mov	rax, QWORD PTR [r14+96]
$LN167@ParseParam:
	mov	QWORD PTR [rax+8], rbx
$LN168@ParseParam:
	mov	r10d, DWORD PTR i$[rbp-104]
	movzx	ebx, BYTE PTR IsPROC$[rbp-104]
$LN21@ParseParam:

; 913  :                 break;
; 914  :             }
; 915  :         }
; 916  :         if ( tokenarray[i].token != T_FINAL ) {

	mov	r9, QWORD PTR tokenarray$[rbp-104]
	movsxd	rax, r10d
	shl	rax, 5
	movzx	ecx, BYTE PTR [rax+r9]
	test	cl, cl
	je	SHORT $LN5@ParseParam

; 917  :             if( tokenarray[i].token != T_COMMA ) {

	cmp	cl, 44					; 0000002cH
	jne	SHORT $LN128@ParseParam

; 920  :             }
; 921  :             i++;    /* go past comma */

	inc	r10d
	mov	DWORD PTR i$[rbp-104], r10d
$LN5@ParseParam:

; 663  : 
; 664  :     for( cntParam = 0 ; tokenarray[i].token != T_FINAL ; cntParam++ ) {

	inc	r13d
	movsxd	rax, r10d
	shl	rax, 5
	lea	rcx, QWORD PTR [rax+r9]
	jmp	$LN170@ParseParam
$LN123@ParseParam:

; 700  :                 return( EmitError( COLON_EXPECTED ) );

	mov	ecx, 84					; 00000054H
	call	EmitError
	jmp	$LN1@ParseParam
$LN54@ParseParam:

; 716  :                 case LANG_NONE:
; 717  :                 case LANG_BASIC:
; 718  :                 case LANG_FORTRAN:
; 719  :                 case LANG_PASCAL:
; 720  :                 case LANG_STDCALL:
; 721  :                     return( EmitError( VARARG_REQUIRES_C_CALLING_CONVENTION ) );

	mov	ecx, 94					; 0000005eH
	call	EmitError
	jmp	$LN1@ParseParam
$LN125@ParseParam:

; 739  :             DebugMsg(("ParseParams: %s defined already, state=%u, local=%u\n", sym->name, sym->state, sym->scoped ));
; 740  :             return( EmitErr( SYMBOL_REDEFINITION, name ) );

	mov	rdx, r12
	mov	ecx, 143				; 0000008fH
	call	EmitErr
	jmp	$LN1@ParseParam
$LN128@ParseParam:

; 918  :                 DebugMsg(("ParseParams: error, cntParam=%u, found %s\n", cntParam, tokenarray[i].tokpos ));
; 919  :                 return( EmitErr( EXPECTING_COMMA, tokenarray[i].tokpos ) );

	movsxd	rdx, r10d
	mov	ecx, 39					; 00000027H
	shl	rdx, 5
	mov	rdx, QWORD PTR [rdx+r9+24]
	call	EmitErr
	jmp	$LN1@ParseParam
$LN126@ParseParam:

; 832  :             /* second definition has more parameters than first */
; 833  :             DebugMsg(("ParseParams: different param count\n"));
; 834  :             return( EmitErr( CONFLICTING_PARAMETER_DEFINITION, "" ) );

	lea	rdx, OFFSET FLAT:$SG11614
	mov	ecx, 137				; 00000089H
	call	EmitErr
	jmp	$LN1@ParseParam
$LN124@ParseParam:

; 733  :                     return( ERROR );

	or	eax, -1
	jmp	$LN1@ParseParam
$LN41@ParseParam:

; 673  :         } else {
; 674  :             /* PROC needs a parameter name, PROTO accepts <void> also */
; 675  :             DebugMsg(("ParseParams: name missing/invalid for parameter %u, i=%u\n", cntParam+1, i));
; 676  :             return( EmitErr( SYNTAX_ERROR_EX, tokenarray[i].string_ptr ) );

	movsxd	rdx, r10d
	mov	ecx, 209				; 000000d1H
	shl	rdx, 5
	mov	rdx, QWORD PTR [rdx+r9+8]
	call	EmitErr
	jmp	$LN1@ParseParam
$LN161@ParseParam:

; 922  :         }
; 923  :     } /* end for */
; 924  : 
; 925  :     //if ( proc->e.procinfo->init_done == TRUE ) {
; 926  :     if ( init_done == TRUE ) {

	cmp	BYTE PTR init_done$1$[rbp-104], 1
	jne	SHORT $LN93@ParseParam

; 927  :         if ( paracurr ) {

	test	rdi, rdi
	je	SHORT $LN93@ParseParam

; 928  :             /* first definition has more parameters than second */
; 929  :             DebugMsg(("ParseParams: a param is left over, cntParam=%u\n", cntParam));
; 930  :             return( EmitErr( CONFLICTING_PARAMETER_DEFINITION, "" ) );

	lea	rdx, OFFSET FLAT:$SG11636
	mov	ecx, 137				; 00000089H
	call	EmitErr
	jmp	$LN1@ParseParam
$LN93@ParseParam:

; 931  :         }
; 932  :     }
; 933  : 
; 934  :     if ( IsPROC ) {

	test	bl, bl
	je	$LN30@ParseParam

; 935  :         /* calc starting offset for parameters,
; 936  :          * offset from [E|R]BP : return addr + old [E|R]BP
; 937  :          * NEAR: 2 * wordsize, FAR: 3 * wordsize
; 938  :          *         NEAR  FAR
; 939  :          *-------------------------
; 940  :          * USE16   +4    +6
; 941  :          * USE32   +8    +12
; 942  :          * USE64   +16   +24
; 943  :          * without frame pointer:
; 944  :          * USE16   +2    +4
; 945  :          * USE32   +4    +8
; 946  :          * USE64   +8    +16
; 947  :          */
; 948  :         offset = ( ( 2 + ( proc->sym.mem_type == MT_FAR ? 1 : 0 ) ) * CurrWordSize );

	cmp	DWORD PTR [r14+36], 130			; 00000082H
	movzx	eax, BYTE PTR ModuleInfo+406
	sete	r15b
	add	r15d, 2
	imul	r15d, eax

; 949  : 
; 950  :         /* now calculate the [E|R]BP offsets */
; 951  : 
; 952  : #if AMD64_SUPPORT
; 953  :         if ( ModuleInfo.Ofssize == USE64 && proc->sym.langtype == LANG_FASTCALL ) {

	cmp	BYTE PTR ModuleInfo+404, 2
	jne	SHORT $LN95@ParseParam
	cmp	DWORD PTR [r14+76], 7
	jne	SHORT $LN95@ParseParam

; 954  :             for ( paranode = proc->e.procinfo->paralist; paranode ;paranode = paranode->nextparam )

	mov	rax, QWORD PTR [r14+96]
	mov	r8, QWORD PTR [rax+8]
	test	r8, r8
	je	$LN30@ParseParam
	npad	3
$LL28@ParseParam:

; 955  :                 if ( paranode->sym.state == SYM_TMACRO ) /* register param */

	cmp	DWORD PTR [r8+32], 10
	je	SHORT $LN26@ParseParam

; 956  :                     ;
; 957  :                 else {
; 958  :                     paranode->sym.offset = offset;

	mov	DWORD PTR [r8+16], r15d

; 959  :                     proc->e.procinfo->stackparam = TRUE;

	mov	rax, QWORD PTR [r14+96]
	or	BYTE PTR [rax+84], 32			; 00000020H

; 960  :                     offset += ROUND_UP( paranode->sym.total_size, CurrWordSize );

	movzx	ecx, BYTE PTR ModuleInfo+406
	lea	edx, DWORD PTR [rcx-1]
	add	edx, DWORD PTR [r8+56]
	lea	eax, DWORD PTR [rcx-1]
	not	eax
	and	edx, eax
	add	r15d, edx

; 961  :                     //set isparam var for W64F_HABRAN
; 962  :                     if (ModuleInfo.win64_flags & W64F_HABRAN) paranode->sym.isparam = TRUE;

	test	BYTE PTR ModuleInfo+413, 11
	je	SHORT $LN26@ParseParam
	or	BYTE PTR [r8+41], 128			; 00000080H
$LN26@ParseParam:

; 954  :             for ( paranode = proc->e.procinfo->paralist; paranode ;paranode = paranode->nextparam )

	mov	r8, QWORD PTR [r8+112]
	test	r8, r8
	jne	SHORT $LL28@ParseParam

; 963  :                 }
; 964  :         } else

	jmp	SHORT $LN30@ParseParam
$LN95@ParseParam:

; 965  : #endif
; 966  :         for ( ; cntParam; cntParam-- ) {

	test	r13d, r13d
	je	SHORT $LN30@ParseParam
	npad	7
$LL31@ParseParam:

; 967  :             for ( curr = 1, paranode = proc->e.procinfo->paralist; curr < cntParam;paranode = paranode->nextparam, curr++ );

	mov	rax, QWORD PTR [r14+96]
	mov	rax, QWORD PTR [rax+8]
	cmp	r13d, 1
	jle	SHORT $LN33@ParseParam
	lea	edx, DWORD PTR [r13-1]
$LL34@ParseParam:
	mov	rax, QWORD PTR [rax+112]
	sub	rdx, 1
	jne	SHORT $LL34@ParseParam
$LN33@ParseParam:

; 968  :             DebugMsg1(("ParseParams: parm=%s, ofs=%u, size=%d\n", paranode->sym.name, offset, paranode->sym.total_size));
; 969  :             if ( paranode->sym.state == SYM_TMACRO ) /* register param? */

	cmp	DWORD PTR [rax+32], 10
	je	SHORT $LN29@ParseParam

; 970  :                 ;
; 971  :             else {
; 972  :                 paranode->sym.offset = offset;

	mov	DWORD PTR [rax+16], r15d

; 973  :                 proc->e.procinfo->stackparam = TRUE;

	mov	rcx, QWORD PTR [r14+96]
	or	BYTE PTR [rcx+84], 32			; 00000020H

; 974  :                 offset += ROUND_UP( paranode->sym.total_size, CurrWordSize );

	movzx	edx, BYTE PTR ModuleInfo+406
	lea	r8d, DWORD PTR [rdx-1]
	add	r8d, DWORD PTR [rax+56]
	lea	eax, DWORD PTR [rdx-1]
	not	eax
	and	r8d, eax
	add	r15d, r8d
$LN29@ParseParam:

; 965  : #endif
; 966  :         for ( ; cntParam; cntParam-- ) {

	sub	r13d, 1
	jne	SHORT $LL31@ParseParam
$LN30@ParseParam:

; 975  :             }
; 976  :         }
; 977  :     }
; 978  :     return ( NOT_ERROR );

	xor	eax, eax
$LN1@ParseParam:
	mov	r12, QWORD PTR [rsp+88]
	mov	rsi, QWORD PTR [rsp+96]

; 979  : }

	add	rsp, 104				; 00000068H
	pop	r15
	pop	r14
	pop	r13
	pop	rdi
	pop	rbx
	pop	rbp
	ret	0
ParseParams ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
_TEXT	SEGMENT
sym$ = 8
opnd$ = 16
UpdateProcStatus PROC

; 621  :     sym->value = ( CurrProc ? ProcStatus : 0 );

	xor	eax, eax
	cmp	QWORD PTR CurrProc, rax
	cmovne	eax, DWORD PTR ProcStatus
	mov	DWORD PTR [rcx+16], eax

; 622  : }

	ret	0
UpdateProcStatus ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
_TEXT	SEGMENT
sym$ = 8
opnd$ = 16
UpdateStackBase PROC

; 608  :     if ( opnd ) {

	test	rdx, rdx
	je	SHORT $LN4@UpdateStac

; 609  :         StackAdj = opnd->uvalue;

	mov	r8d, DWORD PTR [rdx]
	mov	DWORD PTR StackAdj, r8d

; 610  :         StackAdjHigh = opnd->hvalue;

	mov	eax, DWORD PTR [rdx+4]
	mov	DWORD PTR StackAdjHigh, eax

; 611  :     }
; 612  :     sym->value = StackAdj;

	mov	DWORD PTR [rcx+16], r8d

; 613  :     sym->value3264 = StackAdjHigh;

	mov	eax, DWORD PTR StackAdjHigh
	mov	DWORD PTR [rcx+56], eax

; 614  : }

	ret	0
$LN4@UpdateStac:

; 611  :     }
; 612  :     sym->value = StackAdj;

	mov	eax, DWORD PTR StackAdj
	mov	DWORD PTR [rcx+16], eax

; 613  :     sym->value3264 = StackAdjHigh;

	mov	eax, DWORD PTR StackAdjHigh
	mov	DWORD PTR [rcx+56], eax

; 614  : }

	ret	0
UpdateStackBase ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
_TEXT	SEGMENT
ti$ = 48
opndx$1 = 80
i$ = 224
tokenarray$ = 232
LocalDir PROC

; 441  : {

$LN66:
	mov	DWORD PTR [rsp+8], ecx
	push	rbp
	push	rdi
	lea	rbp, QWORD PTR [rsp-79]
	sub	rsp, 200				; 000000c8H

; 442  :     char        *name;
; 443  :     struct dsym *local;
; 444  :     struct dsym *curr;
; 445  :     struct proc_info *info;
; 446  :     //int         size;
; 447  :     //int         idx;
; 448  :     struct qualified_type ti;
; 449  : 
; 450  :     if ( Parse_Pass != PASS_1 ) /* everything is done in pass 1 */

	cmp	DWORD PTR Parse_Pass, 0
	mov	rdi, rdx
	mov	r8d, ecx
	je	SHORT $LN13@LocalDir

; 451  :         return( NOT_ERROR );

	xor	eax, eax

; 599  : }

	add	rsp, 200				; 000000c8H
	pop	rdi
	pop	rbp
	ret	0
$LN13@LocalDir:

; 452  : 
; 453  :     DebugMsg1(("LocalDir(%u) entry\n", i));
; 454  : 
; 455  :     if( !( ProcStatus & PRST_PROLOGUE_NOT_DONE ) || CurrProc == NULL ) {

	test	BYTE PTR ProcStatus, -128		; ffffffffffffff80H
	je	$LN15@LocalDir
	mov	rax, QWORD PTR CurrProc
	test	rax, rax
	je	$LN15@LocalDir

; 457  :     }
; 458  : 
; 459  :     info = CurrProc->e.procinfo;

	mov	QWORD PTR [rsp+240], rsi
	mov	rsi, QWORD PTR [rax+96]
	mov	QWORD PTR [rsp+192], r14

; 460  : #if STACKBASESUPP
; 461  :     /* ensure the fpo bit is set - it's too late to set it in write_prologue().
; 462  :      * Note that the fpo bit is set only IF there are locals or arguments.
; 463  :      * fixme: what if pass > 1?
; 464  :      */
; 465  :     if ( GetRegNo( info->basereg ) == 4 ) {

	movzx	eax, WORD PTR [rsi+86]
	lea	rcx, QWORD PTR [rax+rax*2]
	lea	rax, OFFSET FLAT:SpecialTable+10
	cmp	BYTE PTR [rax+rcx*4], 4
	jne	SHORT $LN16@LocalDir

; 466  :         info->fpo = TRUE;

	or	BYTE PTR [rsi+84], 128			; 00000080H

; 467  :         ProcStatus |= PRST_FPO;

	or	DWORD PTR ProcStatus, 4
	mov	r8d, DWORD PTR i$[rbp-121]
$LN16@LocalDir:

; 468  :     }
; 469  : #endif
; 470  : 
; 471  :     i++; /* go past LOCAL */

	inc	r8d
	mov	QWORD PTR [rsp+232], rbx
	mov	DWORD PTR i$[rbp-121], r8d
	xor	r14d, r14d
$LL4@LocalDir:

; 472  : 
; 473  :     do  {
; 474  :         if( tokenarray[i].token != T_ID ) {

	movsxd	rax, r8d
	shl	rax, 5
	cmp	BYTE PTR [rax+rdi], 8
	jne	$LN46@LocalDir

; 476  :         }
; 477  :         name = tokenarray[i].string_ptr;

	mov	rdx, QWORD PTR [rax+rdi+8]

; 478  : 
; 479  :         DebugMsg1(("LocalDir: item=%s\n", tokenarray[i].tokpos ));
; 480  : 
; 481  :         ti.symtype = NULL;
; 482  :         ti.is_ptr = 0;
; 483  :         ti.ptr_memtype = MT_EMPTY;
; 484  :         if ( SIZE_DATAPTR & ( 1 << ModuleInfo.model ) )

	mov	eax, 1
	mov	ecx, DWORD PTR ModuleInfo+360
	shl	al, cl

; 485  :             ti.is_far = TRUE;
; 486  :         else
; 487  :             ti.is_far = FALSE;
; 488  :         ti.Ofssize = ModuleInfo.Ofssize;
; 489  : 
; 490  : #if 0
; 491  :         /* since v1.95 a local hash table is used. No need to search the
; 492  :          * symbol before SymLCreate() is called. SymLCreate() will display
; 493  :          * an error if the symbol is already defined.
; 494  :          */
; 495  :         if ((local = (struct dsym *)SymSearch( name )) && local->sym.state != SYM_UNDEFINED ) {
; 496  :             return( EmitErr( SYMBOL_ALREADY_DEFINED, name ) );
; 497  :         }
; 498  : #endif
; 499  :         local = (struct dsym *)SymLCreate( name );

	mov	rcx, rdx
	test	al, 104					; 00000068H
	mov	QWORD PTR ti$[rbp-113], r14
	movzx	eax, BYTE PTR ModuleInfo+404
	setne	BYTE PTR ti$[rbp-100]
	mov	BYTE PTR ti$[rbp-99], al
	mov	BYTE PTR ti$[rbp-101], r14b
	mov	DWORD PTR ti$[rbp-97], 192		; 000000c0H
	call	SymLCreate
	mov	rbx, rax

; 500  :         if( !local ) { /* if it failed, an error msg has been written already */

	test	rax, rax
	je	$LN50@LocalDir

; 501  :             DebugMsg(("LocalDir: SymLCreate( %s ) failed\n", name ));
; 502  :             return( ERROR );
; 503  :         }
; 504  :         if (ModuleInfo.win64_flags & W64F_HABRAN) local->sym.isparam = FALSE; //clear isparam var, added  by habran

	test	BYTE PTR ModuleInfo+413, 11
	je	SHORT $LN21@LocalDir
	and	BYTE PTR [rax+41], 127			; 0000007fH
$LN21@LocalDir:

; 505  :         local->sym.state = SYM_STACK;
; 506  :         local->sym.isdefined = TRUE;

	or	BYTE PTR [rax+40], 2
	mov	DWORD PTR [rax+32], 5

; 507  :         local->sym.total_length = 1; /* v2.04: added */

	mov	DWORD PTR [rax+64], 1

; 508  :         switch ( ti.Ofssize ) {

	cmp	BYTE PTR ti$[rbp-99], r14b
	je	SHORT $LN22@LocalDir

; 512  :             break;
; 513  : #if AMD64_SUPPORT
; 514  :             /* v2.08: default type for locals in 64-bit is still DWORD (at least in Win64) */
; 515  :             //case USE64: local->sym.mem_type = MT_QWORD; break;
; 516  :             //ti.size = sizeof( uint_64 );
; 517  : #endif
; 518  :         default: 
; 519  :             local->sym.mem_type = MT_DWORD;

	mov	DWORD PTR [rax+36], 3

; 520  :             ti.size = sizeof( uint_32 );

	mov	DWORD PTR ti$[rbp-121], 4
	jmp	SHORT $LN5@LocalDir
$LN22@LocalDir:

; 509  :         case USE16:
; 510  :             local->sym.mem_type = MT_WORD;

	mov	DWORD PTR [rax+36], 1

; 511  :             ti.size = sizeof( uint_16 );

	mov	DWORD PTR ti$[rbp-121], 2
$LN5@LocalDir:

; 521  :             break;
; 522  :         }
; 523  : 
; 524  :         i++; /* go past name */

	mov	eax, DWORD PTR i$[rbp-121]
	inc	eax

; 525  : 
; 526  :         /* get the optional index factor: local name[xx]:... */
; 527  :         if( tokenarray[i].token == T_OP_SQ_BRACKET ) {

	movsxd	rcx, eax
	shl	rcx, 5
	mov	DWORD PTR i$[rbp-121], eax
	cmp	BYTE PTR [rcx+rdi], 91			; 0000005bH
	jne	$LN30@LocalDir

; 528  :             int j;
; 529  :             struct expr opndx;
; 530  :             i++; /* go past '[' */
; 531  :             /* scan for comma or colon. this isn't really necessary,
; 532  :              * but will prevent the expression evaluator from emitting
; 533  :              * confusing error messages.
; 534  :              */
; 535  :             for ( j = i; j < Token_Count; j++ )

	mov	r8d, DWORD PTR ModuleInfo+496
	inc	eax
	mov	DWORD PTR i$[rbp-121], eax
	cmp	eax, r8d
	jge	SHORT $LN60@LocalDir
	movsxd	rcx, eax
	shl	rcx, 5
	add	rcx, rdi
$LL9@LocalDir:

; 536  :                 if ( tokenarray[j].token == T_COMMA ||

	movzx	edx, BYTE PTR [rcx]
	cmp	dl, 44					; 0000002cH
	je	SHORT $LN60@LocalDir
	cmp	dl, 58					; 0000003aH
	je	SHORT $LN60@LocalDir

; 528  :             int j;
; 529  :             struct expr opndx;
; 530  :             i++; /* go past '[' */
; 531  :             /* scan for comma or colon. this isn't really necessary,
; 532  :              * but will prevent the expression evaluator from emitting
; 533  :              * confusing error messages.
; 534  :              */
; 535  :             for ( j = i; j < Token_Count; j++ )

	inc	eax
	add	rcx, 32					; 00000020H
	cmp	eax, r8d
	jl	SHORT $LL9@LocalDir
$LN60@LocalDir:

; 537  :                     tokenarray[j].token == T_COLON)
; 538  :                     break;
; 539  :             if ( ERROR == EvalOperand( &i, tokenarray, j, &opndx, 0 ) )

	lea	r9, QWORD PTR opndx$1[rbp-121]
	mov	BYTE PTR [rsp+32], r14b
	mov	r8d, eax
	lea	rcx, QWORD PTR i$[rbp-121]
	mov	rdx, rdi
	call	EvalOperand
	cmp	eax, -1
	je	$LN50@LocalDir

; 540  :                 return( ERROR );
; 541  :             if ( opndx.kind != EXPR_CONST ) {

	cmp	DWORD PTR opndx$1[rbp-61], r14d
	je	SHORT $LN59@LocalDir

; 542  :                 EmitError( CONSTANT_EXPECTED );

	mov	ecx, 65					; 00000041H
	call	EmitError

; 543  :                 opndx.value = 1;

	mov	eax, 1
	mov	DWORD PTR opndx$1[rbp-121], eax
	jmp	SHORT $LN28@LocalDir
$LN59@LocalDir:
	mov	eax, DWORD PTR opndx$1[rbp-121]
$LN28@LocalDir:

; 544  :             }
; 545  :             /* zero is allowed as value! */
; 546  :             local->sym.total_length = opndx.value;
; 547  :             local->sym.isarray = TRUE;

	or	BYTE PTR [rbx+41], 2
	mov	DWORD PTR [rbx+64], eax

; 548  :             if( tokenarray[i].token == T_CL_SQ_BRACKET ) {

	movsxd	rax, DWORD PTR i$[rbp-121]
	mov	rcx, rax
	shl	rcx, 5
	cmp	BYTE PTR [rcx+rdi], 93			; 0000005dH
	jne	SHORT $LN29@LocalDir

; 549  :                 i++; /* go past ']' */

	inc	eax
	mov	DWORD PTR i$[rbp-121], eax

; 550  :             } else {

	jmp	SHORT $LN30@LocalDir
$LN29@LocalDir:

; 551  :                 EmitError( EXPECTED_CL_SQ_BRACKET );

	mov	ecx, 111				; 0000006fH
	call	EmitError
	mov	eax, DWORD PTR i$[rbp-121]
$LN30@LocalDir:

; 552  :             }
; 553  :         }
; 554  : 
; 555  :         /* get the optional type: local name[xx]:type  */
; 556  :         if( tokenarray[i].token == T_COLON ) {

	movsxd	rcx, eax
	shl	rcx, 5
	cmp	BYTE PTR [rcx+rdi], 58			; 0000003aH
	jne	SHORT $LN34@LocalDir

; 557  :             i++;

	inc	eax

; 558  : 
; 559  :             if ( GetQualifiedType( &i, tokenarray, &ti ) == ERROR )

	lea	r8, QWORD PTR ti$[rbp-121]
	mov	rdx, rdi
	mov	DWORD PTR i$[rbp-121], eax
	lea	rcx, QWORD PTR i$[rbp-121]
	call	GetQualifiedType
	cmp	eax, -1
	je	$LN50@LocalDir

; 561  : 
; 562  :             local->sym.mem_type = ti.mem_type;

	mov	eax, DWORD PTR ti$[rbp-105]
	mov	DWORD PTR [rbx+36], eax

; 563  :             if ( ti.mem_type == MT_TYPE ) {

	cmp	DWORD PTR ti$[rbp-105], 196		; 000000c4H

; 564  :                 local->sym.type = ti.symtype;

	mov	rax, QWORD PTR ti$[rbp-113]
	jne	SHORT $LN33@LocalDir
	mov	QWORD PTR [rbx+80], rax

; 565  :             } else {

	jmp	SHORT $LN34@LocalDir
$LN33@LocalDir:

; 566  :                 local->sym.target_type = ti.symtype;

	mov	QWORD PTR [rbx+48], rax
$LN34@LocalDir:

; 567  :             }
; 568  :             DebugMsg1(("LocalDir: memtype=%X, type=%s, size=%u*%u\n",
; 569  :                        local->sym.mem_type,
; 570  :                        ti.symtype ? ti.symtype->name : "NULL",
; 571  :                        ti.size, local->sym.total_length ));
; 572  :         }
; 573  :         local->sym.is_ptr  = ti.is_ptr;

	movzx	eax, BYTE PTR ti$[rbp-101]
	mov	BYTE PTR [rbx+45], al

; 574  :         local->sym.isfar   = ti.is_far;

	movzx	eax, BYTE PTR ti$[rbp-100]
	shl	al, 4
	xor	al, BYTE PTR [rbx+47]
	and	al, 16
	xor	BYTE PTR [rbx+47], al

; 575  :         local->sym.Ofssize = ti.Ofssize;

	movzx	eax, BYTE PTR ti$[rbp-99]
	mov	BYTE PTR [rbx+44], al

; 576  :         local->sym.ptr_memtype = ti.ptr_memtype;

	movzx	eax, BYTE PTR ti$[rbp-97]
	mov	BYTE PTR [rbx+46], al

; 577  :         local->sym.total_size = ti.size * local->sym.total_length;

	mov	eax, DWORD PTR [rbx+64]
	imul	eax, DWORD PTR ti$[rbp-121]
	mov	DWORD PTR [rbx+56], eax

; 578  : 
; 579  :         /* v2.12: address calculation is now done in SetLocalOffsets() */
; 580  : 
; 581  :         if( info->locallist == NULL ) {

	mov	rcx, QWORD PTR [rsi+16]
	test	rcx, rcx
	jne	SHORT $LN35@LocalDir

; 582  :             info->locallist = local;

	mov	QWORD PTR [rsi+16], rbx

; 583  :         } else {

	jmp	SHORT $LN36@LocalDir
$LN35@LocalDir:

; 584  :             for( curr = info->locallist; curr->nextlocal ; curr = curr->nextlocal );

	mov	rax, QWORD PTR [rcx+112]
	test	rax, rax
	je	SHORT $LN11@LocalDir
$LL12@LocalDir:
	mov	rcx, rax
	mov	rax, QWORD PTR [rax+112]
	test	rax, rax
	jne	SHORT $LL12@LocalDir
$LN11@LocalDir:

; 585  :             curr->nextlocal = local;

	mov	QWORD PTR [rcx+112], rbx
$LN36@LocalDir:

; 586  :         }
; 587  : 
; 588  :         if ( tokenarray[i].token != T_FINAL )

	movsxd	r8, DWORD PTR i$[rbp-121]
	mov	edx, DWORD PTR ModuleInfo+496
	mov	rax, r8
	shl	rax, 5
	movzx	ecx, BYTE PTR [rax+rdi]
	test	cl, cl
	je	SHORT $LN2@LocalDir

; 589  :             if ( tokenarray[i].token == T_COMMA ) {

	cmp	cl, 44					; 0000002cH
	jne	SHORT $LN38@LocalDir

; 590  :                 if ( (i + 1) < Token_Count )

	lea	eax, DWORD PTR [r8+1]
	cmp	eax, edx
	cmovl	r8d, eax
	mov	DWORD PTR i$[rbp-121], r8d
$LN2@LocalDir:

; 594  :             }
; 595  : 
; 596  :     } while ( i < Token_Count );

	cmp	r8d, edx
	jl	$LL4@LocalDir

; 597  : 
; 598  :     return( NOT_ERROR );

	xor	eax, eax
	jmp	SHORT $LN63@LocalDir
$LN50@LocalDir:

; 560  :                 return( ERROR );

	or	eax, -1
	jmp	SHORT $LN63@LocalDir
$LN38@LocalDir:

; 591  :                     i++;
; 592  :             } else {
; 593  :                 return( EmitErr( EXPECTING_COMMA, tokenarray[i].tokpos ) );

	mov	rdx, r8
	mov	ecx, 39					; 00000027H
	shl	rdx, 5
	mov	rdx, QWORD PTR [rdx+rdi+24]
$LN64@LocalDir:

; 475  :             return( EmitErr( SYNTAX_ERROR_EX, tokenarray[i].string_ptr ) );

	call	EmitErr
$LN63@LocalDir:
	mov	rbx, QWORD PTR [rsp+232]
	mov	rsi, QWORD PTR [rsp+240]
	mov	r14, QWORD PTR [rsp+192]

; 599  : }

	add	rsp, 200				; 000000c8H
	pop	rdi
	pop	rbp
	ret	0
$LN46@LocalDir:

; 475  :             return( EmitErr( SYNTAX_ERROR_EX, tokenarray[i].string_ptr ) );

	movsxd	rdx, r8d
	mov	ecx, 209				; 000000d1H
	shl	rdx, 5
	mov	rdx, QWORD PTR [rdx+rdi+8]
	jmp	SHORT $LN64@LocalDir
$LN15@LocalDir:

; 456  :         return( EmitError( PROC_MACRO_MUST_PRECEDE_LOCAL ) );

	mov	ecx, 92					; 0000005cH
	call	EmitError

; 599  : }

	add	rsp, 200				; 000000c8H
	pop	rdi
	pop	rbp
	ret	0
LocalDir ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
;	COMDAT pop_proc
_TEXT	SEGMENT
pop_proc PROC						; COMDAT

; 426  :     if( ProcStack == NULL )

	mov	rax, QWORD PTR ProcStack
	test	rax, rax
	jne	SHORT $LN2@pop_proc

; 429  : }

	ret	0
$LN2@pop_proc:

; 427  :         return( NULL );
; 428  :     return( (struct dsym *)popitem( &ProcStack ) );

	mov	rcx, QWORD PTR [rax]
	mov	QWORD PTR ProcStack, rcx
	mov	rax, QWORD PTR [rax+8]

; 429  : }

	ret	0
pop_proc ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
;	COMDAT push_proc
_TEXT	SEGMENT
proc$ = 48
push_proc PROC						; COMDAT

; 416  : {

	push	rbx
	sub	rsp, 32					; 00000020H

; 417  :     if ( Parse_Pass == PASS_1 ) /* get the locals stored so far */

	cmp	DWORD PTR Parse_Pass, 0
	mov	rbx, rcx
	jne	SHORT $LN2@push_proc

; 418  :         SymGetLocal( (struct asym *)proc );

	call	SymGetLocal
$LN2@push_proc:

; 419  :     pushitem( &ProcStack, proc );

	mov	ecx, 16
	call	LclAlloc
	mov	rdx, QWORD PTR ProcStack
	mov	QWORD PTR [rax], rdx
	mov	QWORD PTR [rax+8], rbx
	mov	QWORD PTR ProcStack, rax

; 420  :     return;
; 421  : }

	add	rsp, 32					; 00000020H
	pop	rbx
	ret	0
push_proc ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
;	COMDAT popitem
_TEXT	SEGMENT
stk$ = 8
popitem	PROC						; COMDAT

; 386  :     void        **stack = stk;
; 387  :     struct qnode *node;
; 388  :     void        *elmt;
; 389  : 
; 390  :     node = (struct qnode *)(*stack);

	mov	rax, QWORD PTR [rcx]

; 391  :     *stack = node->next;

	mov	rdx, QWORD PTR [rax]
	mov	QWORD PTR [rcx], rdx

; 392  :     elmt = (void *)node->elmt;
; 393  :     LclFree( node );
; 394  :     return( elmt );

	mov	rax, QWORD PTR [rax+8]

; 395  : }

	ret	0
popitem	ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
;	COMDAT pushitem
_TEXT	SEGMENT
stk$ = 48
elmt$ = 56
pushitem PROC						; COMDAT

; 373  : {

	mov	QWORD PTR [rsp+8], rbx
	push	rdi
	sub	rsp, 32					; 00000020H
	mov	rdi, rcx
	mov	rbx, rdx

; 374  :     void      **stack = stk;
; 375  :     struct qnode *node;
; 376  : 
; 377  :     node = LclAlloc( sizeof( struct qnode ));

	mov	ecx, 16
	call	LclAlloc

; 378  :     node->next = *stack;

	mov	r8, QWORD PTR [rdi]

; 379  :     node->elmt = elmt;

	mov	QWORD PTR [rax+8], rbx

; 380  :     *stack = node;
; 381  : }

	mov	rbx, QWORD PTR [rsp+48]
	mov	QWORD PTR [rax], r8
	mov	QWORD PTR [rdi], rax
	add	rsp, 32					; 00000020H
	pop	rdi
	ret	0
pushitem ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
_TEXT	SEGMENT
proc$ = 8
buffer$ = 16
ms64_return PROC

; 366  :     /* nothing to do, the caller cleans the stack */
; 367  :     return;
; 368  : }

	ret	0
ms64_return ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
_TEXT	SEGMENT
proc$ = 8
paranode$ = 16
used$ = 24
ms64_pcheck PROC

; 355  :     /* since the parameter names refer the stack-backup locations,
; 356  :      * there's nothing to do here!
; 357  :      * That is, if a parameter's size is > 8, it has to be changed
; 358  :      * to a pointer. This is to be done yet.
; 359  :      */
; 360  :     return( 0 );

	xor	eax, eax

; 361  : }

	ret	0
ms64_pcheck ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
_TEXT	SEGMENT
proc$ = 8
buffer$ = 16
watc_return PROC

; 293  :     int value;
; 294  :     value = 4 * CurrWordSize;
; 295  :     if( proc->e.procinfo->has_vararg == FALSE && proc->e.procinfo->parasize > value )

	mov	r8, QWORD PTR [rcx+96]
	movzx	eax, BYTE PTR ModuleInfo+406
	shl	eax, 2
	test	BYTE PTR [r8+84], 1
	jne	SHORT $LN2@watc_retur
	mov	r8d, DWORD PTR [r8+32]
	cmp	r8d, eax
	jbe	SHORT $LN2@watc_retur

; 296  :         sprintf( buffer + strlen( buffer ), "%d%c", proc->e.procinfo->parasize - value, ModuleInfo.radix != 10 ? 't' : NULLC );

	xor	r9d, r9d
	mov	ecx, 116				; 00000074H
	cmp	BYTE PTR ModuleInfo+396, 10
	cmovne	r9d, ecx
	sub	r8d, eax
	or	rcx, -1
	npad	8
$LL4@watc_retur:
	inc	rcx
	cmp	BYTE PTR [rdx+rcx], 0
	jne	SHORT $LL4@watc_retur
	add	rcx, rdx
	lea	rdx, OFFSET FLAT:$SG11372
	jmp	sprintf
$LN2@watc_retur:

; 297  :     return;
; 298  : }

	ret	0
watc_return ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
_TEXT	SEGMENT
proc$ = 80
paranode$ = 88
used$ = 96
watc_pcheck PROC

; 211  : {

	push	rbx
	push	rbp
	push	r12
	push	r13
	sub	rsp, 40					; 00000028H
	mov	r12, r8
	mov	r13, rdx
	mov	rbx, rcx

; 212  :     static char regname[64];
; 213  :     static char regist[32];
; 214  :     int newflg;
; 215  :     int shift;
; 216  :     int firstreg;
; 217  :     uint_8 Ofssize = GetSymOfssize( &proc->sym );

	call	GetSymOfssize

; 218  :     int size = SizeFromMemtype( paranode->sym.mem_type, paranode->sym.Ofssize, paranode->sym.type );

	movzx	edx, BYTE PTR [r13+44]
	mov	ebp, eax
	mov	r8, QWORD PTR [r13+80]
	mov	ecx, DWORD PTR [r13+36]
	call	SizeFromMemtype

; 219  : 
; 220  :     /* v2.05: VARARG procs don't have register params */
; 221  :     if ( proc->e.procinfo->has_vararg )

	mov	rcx, QWORD PTR [rbx+96]
	mov	r9d, eax
	test	BYTE PTR [rcx+84], 1
	je	SHORT $LN10@watc_pchec

; 222  :         return( 0 );

	xor	eax, eax

; 288  : }

	add	rsp, 40					; 00000028H
	pop	r13
	pop	r12
	pop	rbp
	pop	rbx
	ret	0
$LN10@watc_pchec:
	dec	eax
	mov	QWORD PTR [rsp+96], r14
	test	eax, -4					; fffffffcH
	jne	SHORT $LN44@watc_pchec
	cmp	r9d, 3
	je	SHORT $LN44@watc_pchec

; 225  :         return( 0 );
; 226  : 
; 227  :     /* v2.05: rewritten. The old code didn't allow to "fill holes" */
; 228  :     if ( size == 8 ) {

	cmp	r9d, 8
	je	SHORT $LN43@watc_pchec

; 231  :     } else if ( size == 4 && Ofssize == USE16 ) {

	cmp	r9d, 4
	jne	SHORT $LN14@watc_pchec
	test	bpl, bpl
	jne	SHORT $LN14@watc_pchec

; 232  :         newflg = 3;
; 233  :         shift = 2;

	lea	ecx, QWORD PTR [r9-2]
	lea	r14d, QWORD PTR [r9-1]

; 234  :     } else {

	jmp	SHORT $LN15@watc_pchec
$LN14@watc_pchec:

; 235  :         newflg = 1;
; 236  :         shift = 1;

	mov	ecx, 1
	mov	r14d, ecx
	jmp	SHORT $LN15@watc_pchec
$LN44@watc_pchec:

; 223  : 
; 224  :     if ( size != 1 && size != 2 && size != 4 && size != 8 )

	cmp	r9d, 8
	jne	SHORT $LN42@watc_pchec
$LN43@watc_pchec:

; 229  :         newflg = Ofssize ? 3 : 15;

	test	bpl, bpl
	mov	r14d, 3

; 230  :         shift = Ofssize ? 2 : 4;

	mov	ecx, 2
	mov	eax, 15
	cmovne	eax, r14d
	mov	r14d, eax
	mov	eax, 4
	cmovne	eax, ecx
	mov	ecx, eax
$LN15@watc_pchec:

; 237  :     }
; 238  : 
; 239  :     /* scan if there's a free register (pair/quadrupel) */
; 240  :     for ( firstreg = 0; firstreg < 4 && (newflg & *used ); newflg <<= shift, firstreg += shift );

	mov	eax, DWORD PTR [r12]
	xor	ebx, ebx
	mov	edx, ebx
$LL4@watc_pchec:
	test	eax, r14d
	je	SHORT $LN3@watc_pchec
	shl	r14d, cl
	add	edx, ecx
	cmp	edx, 4
	jl	SHORT $LL4@watc_pchec

; 242  :         return( 0 );

	xor	eax, eax
	mov	r14, QWORD PTR [rsp+96]

; 288  : }

	add	rsp, 40					; 00000028H
	pop	r13
	pop	r12
	pop	rbp
	pop	rbx
	ret	0
$LN3@watc_pchec:

; 241  :     if ( firstreg >= 4 ) /* exit if nothing is free */

	cmp	edx, 4
	jl	SHORT $LN16@watc_pchec
$LN42@watc_pchec:

; 242  :         return( 0 );

	xor	eax, eax
	mov	r14, QWORD PTR [rsp+96]

; 288  : }

	add	rsp, 40					; 00000028H
	pop	r13
	pop	r12
	pop	rbp
	pop	rbx
	ret	0
$LN16@watc_pchec:
	mov	QWORD PTR [rsp+80], rsi

; 243  : 
; 244  :     paranode->sym.state = SYM_TMACRO;
; 245  :     switch ( size ) {

	or	rsi, -1
	mov	QWORD PTR [rsp+88], rdi
	lea	rdi, OFFSET FLAT:?regname@?1??watc_pcheck@@9@9
	mov	QWORD PTR [rsp+32], r15
	mov	DWORD PTR [r13+32], 10
	sub	r9d, 1
	je	$LN17@watc_pchec
	sub	r9d, 1
	je	$LN18@watc_pchec
	sub	r9d, 2
	je	$LN19@watc_pchec
	cmp	r9d, 4
	jne	$LN8@watc_pchec

; 260  :     case 8:
; 261  :         if ( Ofssize ) {
; 262  :             paranode->sym.regist[0] = watc_regs32[firstreg];

	lea	r15, OFFSET FLAT:__ImageBase
	test	bpl, bpl
	je	SHORT $LN23@watc_pchec
	movsxd	rcx, edx
	movzx	eax, WORD PTR watc_regs32[r15+rcx*4]
	mov	WORD PTR [r13+48], ax

; 263  :             paranode->sym.regist[1] = watc_regs32[firstreg+1];

	movzx	eax, WORD PTR watc_regs32[r15+rcx*4+4]
	mov	WORD PTR [r13+50], ax

; 264  :         } else {

	jmp	$LN8@watc_pchec
$LN23@watc_pchec:

; 265  :             /* the AX:BX:CX:DX sequence is for 16-bit only.
; 266  :              * fixme: no support for codeview debug info yet;
; 267  :              * the S_REGISTER record supports max 2 registers only.
; 268  :              */
; 269  :             for( firstreg = 0, regname[0] = NULLC; firstreg < 4; firstreg++ ) {

	mov	BYTE PTR ?regname@?1??watc_pcheck@@9@9, bl
	lea	rbp, OFFSET FLAT:$SG11360
$LL9@watc_pchec:

; 270  :                 GetResWName( watc_regs_qw[firstreg], regname + strlen( regname ) );

	mov	rdx, rsi
$LL49@watc_pchec:
	inc	rdx
	cmp	BYTE PTR [rdi+rdx], 0
	jne	SHORT $LL49@watc_pchec
	mov	ecx, DWORD PTR watc_regs_qw[r15+rbx*4]
	add	rdx, rdi
	call	GetResWName

; 271  :                 if ( firstreg != 3 )

	cmp	rbx, 3
	je	SHORT $LN7@watc_pchec

; 272  :                     strcat( regname, "::");

	lea	rax, QWORD PTR [rdi-1]
$LL47@watc_pchec:
	inc	rax
	cmp	BYTE PTR [rax], 0
	jne	SHORT $LL47@watc_pchec
	xor	edx, edx
$LL48@watc_pchec:
	movzx	ecx, BYTE PTR [rdx+rbp]
	mov	BYTE PTR [rax+rdx], cl
	lea	rdx, QWORD PTR [rdx+1]
	test	cl, cl
	jne	SHORT $LL48@watc_pchec
$LN7@watc_pchec:

; 265  :             /* the AX:BX:CX:DX sequence is for 16-bit only.
; 266  :              * fixme: no support for codeview debug info yet;
; 267  :              * the S_REGISTER record supports max 2 registers only.
; 268  :              */
; 269  :             for( firstreg = 0, regname[0] = NULLC; firstreg < 4; firstreg++ ) {

	inc	rbx
	cmp	rbx, 4
	jl	SHORT $LL9@watc_pchec

; 237  :     }
; 238  : 
; 239  :     /* scan if there's a free register (pair/quadrupel) */
; 240  :     for ( firstreg = 0; firstreg < 4 && (newflg & *used ); newflg <<= shift, firstreg += shift );

	jmp	SHORT $LN8@watc_pchec
$LN19@watc_pchec:

; 252  :     case 4:
; 253  :         if ( Ofssize ) {
; 254  :             paranode->sym.regist[0] = watc_regs32[firstreg];

	lea	r15, OFFSET FLAT:__ImageBase
	test	bpl, bpl
	je	SHORT $LN20@watc_pchec
	movsxd	rax, edx
	movzx	eax, WORD PTR watc_regs32[r15+rax*4]

; 255  :         } else {

	jmp	SHORT $LN51@watc_pchec
$LN20@watc_pchec:

; 256  :             paranode->sym.regist[0] = watc_regs16[firstreg];

	movsxd	rcx, edx
	movzx	eax, WORD PTR watc_regs16[r15+rcx*4]
	mov	WORD PTR [r13+48], ax

; 257  :             paranode->sym.regist[1] = watc_regs16[firstreg+1];

	movzx	eax, WORD PTR watc_regs16[r15+rcx*4+4]
	mov	WORD PTR [r13+50], ax

; 258  :         }
; 259  :         break;

	jmp	SHORT $LN8@watc_pchec
$LN18@watc_pchec:

; 248  :         break;
; 249  :     case 2:
; 250  :         paranode->sym.regist[0] = watc_regs16[firstreg];

	movsxd	rax, edx
	lea	r15, OFFSET FLAT:__ImageBase
	movzx	eax, WORD PTR watc_regs16[r15+rax*4]

; 251  :         break;

	jmp	SHORT $LN51@watc_pchec
$LN17@watc_pchec:

; 246  :     case 1:
; 247  :         paranode->sym.regist[0] = watc_regs8[firstreg];

	movsxd	rax, edx
	lea	r15, OFFSET FLAT:__ImageBase
	movzx	eax, WORD PTR watc_regs8[r15+rax*4]
$LN51@watc_pchec:
	mov	WORD PTR [r13+48], ax
$LN8@watc_pchec:

; 273  :             }
; 274  :         }
; 275  :     }
; 276  :     if ( paranode->sym.regist[1] ) {

	cmp	WORD PTR [r13+50], 0
	mov	r15, QWORD PTR [rsp+32]
	je	SHORT $LN26@watc_pchec

; 277  :         sprintf( regname, "%s::%s",

	movzx	ecx, WORD PTR [r13+48]
	xor	edx, edx
	call	GetResWName
	movzx	ecx, WORD PTR [r13+50]
	lea	rdx, OFFSET FLAT:?regist@?1??watc_pcheck@@9@9
	mov	rbx, rax
	call	GetResWName
	mov	r8, rax
	lea	rdx, OFFSET FLAT:$SG11363
	mov	r9, rbx
	mov	rcx, rdi
	call	sprintf
	jmp	SHORT $LN28@watc_pchec
$LN26@watc_pchec:

; 278  :                 GetResWName( paranode->sym.regist[1], regist ),
; 279  :                 GetResWName( paranode->sym.regist[0], NULL ) );
; 280  :     } else if ( paranode->sym.regist[0] ) {

	movzx	eax, WORD PTR [r13+48]
	test	ax, ax
	je	SHORT $LN28@watc_pchec

; 281  :         GetResWName( paranode->sym.regist[0], regname );

	movzx	ecx, ax
	mov	rdx, rdi
	call	GetResWName
$LN28@watc_pchec:

; 282  :     }
; 283  :     *used |= newflg;

	or	DWORD PTR [r12], r14d
$LL46@watc_pchec:

; 284  :     paranode->sym.string_ptr = LclAlloc( strlen( regname ) + 1 );

	cmp	BYTE PTR [rdi+rsi+1], 0
	lea	rsi, QWORD PTR [rsi+1]
	jne	SHORT $LL46@watc_pchec
	lea	rcx, QWORD PTR [rsi+1]
	call	LclAlloc
	mov	rsi, QWORD PTR [rsp+80]
	mov	QWORD PTR [r13+16], rax
	npad	3
$LL30@watc_pchec:

; 285  :     strcpy( paranode->sym.string_ptr, regname );

	movzx	ecx, BYTE PTR [rdi]
	lea	rdi, QWORD PTR [rdi+1]
	mov	BYTE PTR [rax], cl
	lea	rax, QWORD PTR [rax+1]
	test	cl, cl
	jne	SHORT $LL30@watc_pchec

; 286  :     DebugMsg(("watc_pcheck(%s.%s): size=%u ptr=%u far=%u reg=%s\n", proc->sym.name, paranode->sym.name, size, paranode->sym.is_ptr, paranode->sym.isfar, regname ));
; 287  :     return( 1 );

	mov	rdi, QWORD PTR [rsp+88]
	mov	eax, 1
	mov	r14, QWORD PTR [rsp+96]

; 288  : }

	add	rsp, 40					; 00000028H
	pop	r13
	pop	r12
	pop	rbp
	pop	rbx
	ret	0
watc_pcheck ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
_TEXT	SEGMENT
proc$ = 8
buffer$ = 16
ms32_return PROC

; 331  :     /* v2.07: changed */
; 332  :     //if( proc->e.procinfo->parasize > ( 2 * CurrWordSize ) )
; 333  :     //    sprintf( buffer + strlen( buffer ), "%d%c", proc->e.procinfo->parasize - (2 * CurrWordSize), ModuleInfo.radix != 10 ? 't' : NULLC );
; 334  :     if( proc->e.procinfo->parasize > ( ms32_maxreg[ModuleInfo.Ofssize] * CurrWordSize ) )

	movzx	r8d, BYTE PTR ModuleInfo+404
	lea	r9, OFFSET FLAT:ms32_maxreg
	movzx	eax, BYTE PTR ModuleInfo+406
	mov	r9d, DWORD PTR [r9+r8*4]
	imul	r9d, eax
	mov	rax, QWORD PTR [rcx+96]
	mov	r8d, DWORD PTR [rax+32]
	cmp	r8d, r9d
	jbe	SHORT $LN2@ms32_retur

; 335  :         sprintf( buffer + strlen( buffer ), "%d%c", proc->e.procinfo->parasize - ( ms32_maxreg[ModuleInfo.Ofssize] * CurrWordSize), ModuleInfo.radix != 10 ? 't' : NULLC );

	xor	eax, eax
	mov	ecx, 116				; 00000074H
	cmp	BYTE PTR ModuleInfo+396, 10
	cmovne	eax, ecx
	sub	r8d, r9d
	or	rcx, -1
$LL4@ms32_retur:
	inc	rcx
	cmp	BYTE PTR [rdx+rcx], 0
	jne	SHORT $LL4@ms32_retur
	add	rcx, rdx
	mov	r9d, eax
	lea	rdx, OFFSET FLAT:$SG11390
	jmp	sprintf
$LN2@ms32_retur:

; 336  :     return;
; 337  : }

	ret	0
ms32_return ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
_TEXT	SEGMENT
regname$ = 32
proc$ = 80
paranode$ = 88
used$ = 96
ms32_pcheck PROC

; 310  : {

	mov	QWORD PTR [rsp+8], rbx
	push	rdi
	sub	rsp, 64					; 00000040H
	mov	rdi, rdx
	mov	rbx, r8

; 311  :     char regname[32];
; 312  :     int size = SizeFromMemtype( paranode->sym.mem_type, paranode->sym.Ofssize, paranode->sym.type );

	movzx	edx, BYTE PTR [rdx+44]
	mov	r8, QWORD PTR [rdi+80]
	mov	ecx, DWORD PTR [rdi+36]
	call	SizeFromMemtype

; 313  : 
; 314  :     /* v2.07: 16-bit has 3 register params (AX,DX,BX) */
; 315  :     //if ( size > CurrWordSize || *used >= 2 )
; 316  :     if ( size > CurrWordSize || *used >= ms32_maxreg[ModuleInfo.Ofssize] )

	movzx	ecx, BYTE PTR ModuleInfo+406
	cmp	eax, ecx
	jg	$LN3@ms32_pchec
	movzx	eax, BYTE PTR ModuleInfo+404
	lea	rdx, OFFSET FLAT:__ImageBase
	mov	eax, DWORD PTR ms32_maxreg[rdx+rax*4]
	cmp	DWORD PTR [rbx], eax
	jge	$LN3@ms32_pchec

; 318  :     paranode->sym.state = SYM_TMACRO;

	mov	DWORD PTR [rdi+32], 10

; 319  :     /* v2.10: for codeview debug info, store the register index in the symbol */
; 320  :     paranode->sym.regist[0] = ModuleInfo.Ofssize ? ms32_regs32[*used] : ms32_regs16[*used];

	cmp	BYTE PTR ModuleInfo+404, 0
	movsxd	rax, DWORD PTR [rbx]
	je	SHORT $LN5@ms32_pchec
	mov	ecx, DWORD PTR ms32_regs32[rdx+rax*4]
	jmp	SHORT $LN6@ms32_pchec
$LN5@ms32_pchec:
	mov	ecx, DWORD PTR ms32_regs16[rdx+rax*4]
$LN6@ms32_pchec:
	mov	WORD PTR [rdi+48], cx

; 321  :     GetResWName( ModuleInfo.Ofssize ? ms32_regs32[*used] : ms32_regs16[*used], regname );

	cmp	BYTE PTR ModuleInfo+404, 0
	movsxd	rax, DWORD PTR [rbx]
	je	SHORT $LN7@ms32_pchec
	mov	ecx, DWORD PTR ms32_regs32[rdx+rax*4]
	jmp	SHORT $LN8@ms32_pchec
$LN7@ms32_pchec:
	mov	ecx, DWORD PTR ms32_regs16[rdx+rax*4]
$LN8@ms32_pchec:
	lea	rdx, QWORD PTR regname$[rsp]
	call	GetResWName

; 322  :     paranode->sym.string_ptr = LclAlloc( strlen( regname ) + 1 );

	lea	rax, QWORD PTR regname$[rsp]
	or	rcx, -1
$LL13@ms32_pchec:
	inc	rcx
	cmp	BYTE PTR [rax+rcx], 0
	jne	SHORT $LL13@ms32_pchec
	inc	rcx
	call	LclAlloc
	mov	QWORD PTR [rdi+16], rax

; 323  :     strcpy( paranode->sym.string_ptr, regname );

	lea	rdx, QWORD PTR regname$[rsp]
	npad	4
$LL9@ms32_pchec:
	movzx	ecx, BYTE PTR [rdx]
	lea	rdx, QWORD PTR [rdx+1]
	mov	BYTE PTR [rax], cl
	lea	rax, QWORD PTR [rax+1]
	test	cl, cl
	jne	SHORT $LL9@ms32_pchec

; 324  :     (*used)++;

	inc	DWORD PTR [rbx]

; 325  :     return( 1 );

	mov	eax, 1

; 326  : }

	mov	rbx, QWORD PTR [rsp+80]
	add	rsp, 64					; 00000040H
	pop	rdi
	ret	0
$LN3@ms32_pchec:

; 317  :         return( 0 );

	xor	eax, eax

; 326  : }

	mov	rbx, QWORD PTR [rsp+80]
	add	rsp, 64					; 00000040H
	pop	rdi
	ret	0
ms32_pcheck ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
_TEXT	SEGMENT
ProcCheckOpen PROC

; 2098 : {

$LN60:
	sub	rsp, 40					; 00000028H

; 2099 :     while( CurrProc != NULL ) {

	mov	rax, QWORD PTR CurrProc
	test	rax, rax
	je	$LN3@ProcCheckO
	mov	QWORD PTR [rsp+48], rbx
	mov	QWORD PTR [rsp+56], rsi
	xor	esi, esi
	mov	QWORD PTR [rsp+32], rdi
	npad	11
$LL2@ProcCheckO:

; 2100 :         DebugMsg1(("ProcCheckOpen: unmatched block nesting error, CurrProc=%s\n", CurrProc->sym.name ));
; 2101 :         EmitErr( UNMATCHED_BLOCK_NESTING, CurrProc->sym.name );

	mov	rdx, QWORD PTR [rax+8]
	mov	ecx, 142				; 0000008eH
	call	EmitErr

; 2102 :         ProcFini( CurrProc );

	mov	rdi, QWORD PTR CurrProc
	mov	rax, QWORD PTR ModuleInfo+432
	cmp	QWORD PTR [rdi+24], rax
	jne	SHORT $LN18@ProcCheckO
	call	GetCurrOffset
	jmp	SHORT $LN58@ProcCheckO
$LN18@ProcCheckO:
	mov	rdx, QWORD PTR [rdi+8]
	mov	ecx, 142				; 0000008eH
	call	EmitErr
	mov	rax, QWORD PTR CurrProc
	mov	rcx, QWORD PTR [rax+24]
	mov	eax, DWORD PTR [rcx+16]
$LN58@ProcCheckO:
	sub	eax, DWORD PTR [rdi+16]
	mov	DWORD PTR [rdi+56], eax
	cmp	BYTE PTR Options+13, 2
	jbe	SHORT $LN56@ProcCheckO
	cmp	DWORD PTR Parse_Pass, esi
	jne	$LN57@ProcCheckO
	mov	rax, QWORD PTR [rdi+96]
	mov	rbx, QWORD PTR [rax+8]
	test	rbx, rbx
	je	SHORT $LN7@ProcCheckO
	npad	3
$LL8@ProcCheckO:
	test	BYTE PTR [rbx+40], 1
	jne	SHORT $LN6@ProcCheckO
	mov	r8, QWORD PTR [rbx+8]
	mov	edx, 265				; 00000109H
	mov	ecx, 3
	call	EmitWarn
$LN6@ProcCheckO:
	mov	rbx, QWORD PTR [rbx+112]
	test	rbx, rbx
	jne	SHORT $LL8@ProcCheckO
$LN7@ProcCheckO:
	mov	rbx, QWORD PTR [rdi+96]
	mov	rbx, QWORD PTR [rbx+16]
	test	rbx, rbx
	je	SHORT $LN56@ProcCheckO
	npad	1
$LL11@ProcCheckO:
	test	BYTE PTR [rbx+40], 1
	jne	SHORT $LN9@ProcCheckO
	mov	r8, QWORD PTR [rbx+8]
	mov	edx, 265				; 00000109H
	mov	ecx, 3
	call	EmitWarn
$LN9@ProcCheckO:
	mov	rbx, QWORD PTR [rbx+112]
	test	rbx, rbx
	jne	SHORT $LL11@ProcCheckO
$LN56@ProcCheckO:
	cmp	DWORD PTR Parse_Pass, esi
	jne	SHORT $LN57@ProcCheckO
	cmp	DWORD PTR ModuleInfo+376, 2
	jne	SHORT $LN57@ProcCheckO
	test	BYTE PTR ModuleInfo+413, 2
	je	SHORT $LN57@ProcCheckO
	mov	rax, QWORD PTR sym_ReservedStack
	mov	rdx, QWORD PTR [rdi+96]
	mov	ecx, DWORD PTR [rax+16]
	mov	DWORD PTR [rdx+56], ecx
	mov	rdx, QWORD PTR [rdi+96]
	test	BYTE PTR [rdx+84], 128			; 00000080H
	je	SHORT $LN57@ProcCheckO
	mov	rdx, QWORD PTR [rdx+16]
	test	rdx, rdx
	je	SHORT $LN13@ProcCheckO
$LL14@ProcCheckO:
	mov	rax, QWORD PTR [rdi+96]
	mov	ecx, DWORD PTR [rax+56]
	add	DWORD PTR [rdx+16], ecx
	mov	rdx, QWORD PTR [rdx+112]
	test	rdx, rdx
	jne	SHORT $LL14@ProcCheckO
$LN13@ProcCheckO:
	mov	rax, QWORD PTR [rdi+96]
	mov	r8, QWORD PTR [rax+8]
	test	r8, r8
	je	SHORT $LN57@ProcCheckO
$LL17@ProcCheckO:
	mov	rax, QWORD PTR [rdi+96]
	mov	edx, DWORD PTR [rax+56]
	add	DWORD PTR [r8+16], edx
	mov	r8, QWORD PTR [r8+112]
	test	r8, r8
	jne	SHORT $LL17@ProcCheckO
$LN57@ProcCheckO:
	mov	rax, QWORD PTR [rdi+96]
	test	BYTE PTR [rax+84], 64			; 00000040H
	je	SHORT $LN25@ProcCheckO
	call	LstSetPosition
	mov	rcx, rdi
	call	WriteSEHData
$LN25@ProcCheckO:
	test	DWORD PTR ModuleInfo+408, 2048		; 00000800H
	je	SHORT $LN26@ProcCheckO
	xor	edx, edx
	xor	r8d, r8d
	lea	ecx, QWORD PTR [rdx+7]
	call	LstWrite
$LN26@ProcCheckO:
	cmp	DWORD PTR Parse_Pass, esi
	jne	SHORT $LN27@ProcCheckO
	test	BYTE PTR ProcStatus, -128		; ffffffffffffff80H
	je	SHORT $LN28@ProcCheckO
	mov	rcx, QWORD PTR CurrProc
	mov	rcx, QWORD PTR [rcx+96]
	call	SetLocalOffsets
$LN28@ProcCheckO:
	mov	rcx, QWORD PTR CurrProc
	call	SymGetLocal
$LN27@ProcCheckO:
	mov	rcx, QWORD PTR ProcStack
	test	rcx, rcx
	jne	SHORT $LN32@ProcCheckO
	mov	rax, rsi
	mov	QWORD PTR CurrProc, rax
	jmp	SHORT $LN29@ProcCheckO
$LN32@ProcCheckO:
	mov	rax, QWORD PTR [rcx]
	mov	QWORD PTR ProcStack, rax
	mov	rax, QWORD PTR [rcx+8]
	mov	QWORD PTR CurrProc, rax
	test	rax, rax
	je	SHORT $LN29@ProcCheckO
	mov	rcx, rax
	call	SymSetLocal
	mov	rax, QWORD PTR CurrProc
$LN29@ProcCheckO:
	mov	DWORD PTR ProcStatus, esi
	test	rax, rax
	jne	$LL2@ProcCheckO
	mov	rdi, QWORD PTR [rsp+32]
	mov	rsi, QWORD PTR [rsp+56]
	mov	rbx, QWORD PTR [rsp+48]
$LN3@ProcCheckO:

; 2103 :     }
; 2104 : }

	add	rsp, 40					; 00000028H
	ret	0
ProcCheckOpen ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
_TEXT	SEGMENT
ProcInit PROC

; 3317 :     ProcStack = NULL;

	xor	eax, eax

; 3318 :     CurrProc  = NULL;
; 3319 :     procidx = 1;

	mov	DWORD PTR procidx, 1
	mov	QWORD PTR ProcStack, rax
	mov	QWORD PTR CurrProc, rax

; 3320 :     ProcStatus = 0;

	mov	DWORD PTR ProcStatus, eax

; 3321 :     /* v2.09: reset prolog and epilog mode */
; 3322 :     ModuleInfo.prologuemode = PEM_DEFAULT;

	mov	WORD PTR ModuleInfo+424, ax

; 3323 :     ModuleInfo.epiloguemode = PEM_DEFAULT;
; 3324 :     /* v2.06: no forward references in INVOKE if -Zne is set */
; 3325 :     ModuleInfo.invoke_exprparm = ( Options.strict_masm_compat ? EXPF_NOUNDEF : 0 );

	movzx	eax, BYTE PTR Options+127
	neg	al

; 3326 : #if STACKBASESUPP
; 3327 :     ModuleInfo.basereg[USE16] = T_BP;

	mov	DWORD PTR ModuleInfo+500, 14

; 3328 :     ModuleInfo.basereg[USE32] = T_EBP;

	mov	DWORD PTR ModuleInfo+504, 22
	sbb	cl, cl

; 3329 : #if AMD64_SUPPORT
; 3330 :     ModuleInfo.basereg[USE64] = T_RBP;

	mov	DWORD PTR ModuleInfo+508, 120		; 00000078H
	and	cl, 2

; 3331 : #endif
; 3332 : #endif
; 3333 : #if AMD64_SUPPORT
; 3334 :     unw_segs_defined = 0;

	mov	BYTE PTR unw_segs_defined, 0
	mov	BYTE PTR ModuleInfo+426, cl

; 3335 : #endif
; 3336 : }

	ret	0
ProcInit ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
_TEXT	SEGMENT
tokenarray$ = 48
write_prologue PROC

; 2832 : {

$LN14:
	push	rbx
	sub	rsp, 32					; 00000020H

; 2833 :     /* reset @ProcStatus flag */
; 2834 :     ProcStatus &= ~PRST_PROLOGUE_NOT_DONE;

	and	DWORD PTR ProcStatus, -129		; ffffffffffffff7fH
	mov	rbx, rcx

; 2835 : 
; 2836 : #if AMD64_SUPPORT
; 2837 :     if ( ModuleInfo.fctype == FCT_WIN64 && ( ModuleInfo.win64_flags & W64F_AUTOSTACKSP ) ) {

	cmp	DWORD PTR ModuleInfo+376, 2
	jne	SHORT $LN3@write_prol
	test	BYTE PTR ModuleInfo+413, 2
	je	SHORT $LN3@write_prol

; 2838 :         /* in pass one init reserved stack with 4*8 to force stack frame creation */
; 2839 :         sym_ReservedStack->value = ( Parse_Pass == PASS_1 ? 4 * sizeof( uint_64 ) : CurrProc->e.procinfo->ReservedStack );

	cmp	DWORD PTR Parse_Pass, 0
	jne	SHORT $LN10@write_prol
	mov	edx, 32					; 00000020H
	jmp	SHORT $LN11@write_prol
$LN10@write_prol:
	mov	rax, QWORD PTR CurrProc
	mov	rcx, QWORD PTR [rax+96]
	movsxd	rdx, DWORD PTR [rcx+56]
$LN11@write_prol:
	mov	rax, QWORD PTR sym_ReservedStack
	mov	DWORD PTR [rax+16], edx

; 2840 :         if (Parse_Pass == PASS_1) sym_ReservedStack->hasinvoke = 0;

	cmp	DWORD PTR Parse_Pass, 0
	jne	SHORT $LN4@write_prol
	mov	rax, QWORD PTR sym_ReservedStack
	mov	DWORD PTR [rax+16], 0
$LN3@write_prol:

; 2841 :     }
; 2842 : #endif
; 2843 :     if ( Parse_Pass == PASS_1 ) {

	cmp	DWORD PTR Parse_Pass, 0
	jne	SHORT $LN4@write_prol

; 2844 :         /* v2.12: calculation of offsets of local variables is done delayed now */
; 2845 :         SetLocalOffsets( CurrProc->e.procinfo );

	mov	rcx, QWORD PTR CurrProc
	mov	rcx, QWORD PTR [rcx+96]
	call	SetLocalOffsets
$LN4@write_prol:

; 2846 :     }
; 2847 :     ProcStatus |= PRST_INSIDE_PROLOGUE;

	or	DWORD PTR ProcStatus, 1

; 2848 :     /* there are 3 cases:
; 2849 :      * option prologue:NONE           proc_prologue == NULL
; 2850 :      * option prologue:default        *proc_prologue == NULLC
; 2851 :      * option prologue:usermacro      *proc_prologue != NULLC
; 2852 :      */
; 2853 :     if ( ModuleInfo.prologuemode == PEM_DEFAULT ) {

	movzx	eax, BYTE PTR ModuleInfo+424
	test	al, al
	jne	SHORT $LN5@write_prol

; 2854 :         DebugMsg1(("write_prologue(%s): default prologue\n", CurrProc->sym.name ));
; 2855 :         write_default_prologue();

	call	write_default_prologue
	jmp	SHORT $LN8@write_prol
$LN5@write_prol:

; 2856 :     } else if ( ModuleInfo.prologuemode == PEM_NONE ) {

	cmp	al, 2
	je	SHORT $LN8@write_prol

; 2857 :         DebugMsg1(("write_prologue(%s): prologue is NULL\n", CurrProc->sym.name  ));
; 2858 :     } else {
; 2859 :         DebugMsg1(("write_prologue(%s): userdefined prologue %s\n", CurrProc->sym.name , ModuleInfo.proc_prologue ));
; 2860 :         write_userdef_prologue( tokenarray );

	mov	rcx, rbx
	call	write_userdef_prologue
$LN8@write_prol:

; 2861 :     }
; 2862 :     ProcStatus &= ~PRST_INSIDE_PROLOGUE;

	and	DWORD PTR ProcStatus, -2

; 2863 :     /* v2.10: for debug info, calculate prologue size */
; 2864 :     CurrProc->e.procinfo->size_prolog = GetCurrOffset() - CurrProc->sym.offset;

	call	GetCurrOffset
	mov	rcx, QWORD PTR CurrProc
	sub	al, BYTE PTR [rcx+16]
	mov	rcx, QWORD PTR [rcx+96]
	mov	BYTE PTR [rcx+85], al

; 2865 :     return;
; 2866 : }

	add	rsp, 32					; 00000020H
	pop	rbx
	ret	0
write_prologue ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
_TEXT	SEGMENT
buffer$ = 32
i$ = 656
tokenarray$ = 664
count$ = 672
RetInstr PROC

; 3216 : {

$LN36:
	mov	QWORD PTR [rsp+8], rbx
	mov	QWORD PTR [rsp+16], rbp
	mov	QWORD PTR [rsp+24], rsi
	mov	QWORD PTR [rsp+32], rdi
	push	r14
	sub	rsp, 640				; 00000280H
	movsxd	rdi, ecx

; 3217 :     struct proc_info   *info;
; 3218 :     bool        is_iret = FALSE;

	xor	sil, sil

; 3219 :     char        *p;
; 3220 : #ifdef DEBUG_OUT
; 3221 :     ret_code    rc;
; 3222 : #endif
; 3223 :     char        buffer[MAX_LINE_LEN]; /* stores modified RETN/RETF/IRET instruction */
; 3224 : 
; 3225 :     DebugMsg1(( "RetInstr() enter\n" ));
; 3226 : 
; 3227 : #if AMD64_SUPPORT
; 3228 :     if( tokenarray[i].tokval == T_IRET || tokenarray[i].tokval == T_IRETD || tokenarray[i].tokval == T_IRETQ )

	mov	rbx, rdi
	mov	r14d, r8d
	shl	rbx, 5
	mov	rbp, rdx
	mov	ecx, DWORD PTR [rbx+rdx+16]
	lea	eax, DWORD PTR [rcx-622]
	cmp	eax, 1
	jbe	SHORT $LN5@RetInstr
	cmp	ecx, 1203				; 000004b3H
	jne	SHORT $LN4@RetInstr
$LN5@RetInstr:

; 3229 : #else
; 3230 :     if( tokenarray[i].tokval == T_IRET || tokenarray[i].tokval == T_IRETD )
; 3231 : #endif
; 3232 :         is_iret = TRUE;

	mov	sil, 1
$LN4@RetInstr:

; 3233 : 
; 3234 :     if ( ModuleInfo.epiloguemode == PEM_MACRO ) {

	cmp	BYTE PTR ModuleInfo+425, 1
	jne	SHORT $LN6@RetInstr

; 3235 : #if FASTPASS
; 3236 :         /* don't run userdefined epilogue macro if pass > 1 */
; 3237 :         if ( UseSavedState ) {

	cmp	BYTE PTR UseSavedState, 0
	je	SHORT $LN7@RetInstr

; 3238 :             if ( Parse_Pass > PASS_1 ) {

	cmp	DWORD PTR Parse_Pass, 0
	jbe	SHORT $LN8@RetInstr

; 3239 :                 DebugMsg(( "RetInstr() exit\n" ));
; 3240 :                 //return( NOT_ERROR );
; 3241 :                 return( ParseLine( tokenarray ) );

	mov	rcx, rdx
	call	ParseLine
	jmp	$LN1@RetInstr
$LN8@RetInstr:

; 3242 :             }
; 3243 :             /* handle the current line as if it is REPLACED by the macro content */
; 3244 :             *(LineStoreCurr->line) = ';';

	mov	rax, QWORD PTR LineStoreCurr
	mov	BYTE PTR [rax+16], 59			; 0000003bH
$LN7@RetInstr:

; 3245 :         }
; 3246 : #endif
; 3247 : #ifdef DEBUG_OUT
; 3248 :         rc = write_userdef_epilogue( is_iret, tokenarray );
; 3249 :         DebugMsg(( "RetInstr() exit\n" ));
; 3250 :         return( rc );
; 3251 : #else
; 3252 :         return( write_userdef_epilogue( is_iret, tokenarray ) );

	movzx	ecx, sil
	call	write_userdef_epilogue
	jmp	$LN1@RetInstr
$LN6@RetInstr:

; 3253 : #endif
; 3254 :     }
; 3255 : 
; 3256 :     if ( ModuleInfo.list ) {

	test	DWORD PTR ModuleInfo+408, 2048		; 00000800H
	je	SHORT $LN9@RetInstr

; 3257 :         LstWrite( LSTTYPE_DIRECTIVE, GetCurrOffset(), NULL );

	call	GetCurrOffset
	xor	r8d, r8d
	mov	edx, eax
	lea	ecx, QWORD PTR [r8+4]
	call	LstWrite
$LN9@RetInstr:

; 3258 :     }
; 3259 : 
; 3260 :     strcpy( buffer, tokenarray[i].string_ptr );

	mov	rcx, QWORD PTR [rbx+rbp+8]
	lea	rdx, QWORD PTR buffer$[rsp]
	sub	rdx, rcx
	npad	9
$LL24@RetInstr:
	movzx	eax, BYTE PTR [rcx]
	mov	BYTE PTR [rdx+rcx], al
	lea	rcx, QWORD PTR [rcx+1]
	test	al, al
	jne	SHORT $LL24@RetInstr

; 3261 :     p = buffer + strlen( buffer );

	lea	rcx, QWORD PTR buffer$[rsp]
	or	rax, -1
$LL33@RetInstr:
	inc	rax
	cmp	BYTE PTR [rcx+rax], 0
	jne	SHORT $LL33@RetInstr
	lea	rbx, QWORD PTR buffer$[rsp]
	add	rbx, rax

; 3262 : 
; 3263 :     write_default_epilogue();

	call	write_default_epilogue

; 3264 : 
; 3265 :     info = CurrProc->e.procinfo;

	mov	r8, QWORD PTR CurrProc
	mov	rcx, QWORD PTR [r8+96]

; 3266 : 
; 3267 :     /* skip this part for IRET */
; 3268 :     if( is_iret == FALSE ) {

	test	sil, sil
	jne	SHORT $LN12@RetInstr

; 3269 :         if ( CurrProc->sym.mem_type == MT_FAR )

	cmp	DWORD PTR [r8+36], 130			; 00000082H
	jne	SHORT $LN11@RetInstr

; 3270 :             *p++ = 'f';   /* ret -> retf */

	mov	BYTE PTR [rbx], 102			; 00000066H
	jmp	SHORT $LN34@RetInstr
$LN11@RetInstr:

; 3271 :         else
; 3272 :             *p++ = 'n';     /* ret -> retn */

	mov	BYTE PTR [rbx], 110			; 0000006eH
$LN34@RetInstr:
	inc	rbx
$LN12@RetInstr:

; 3273 :     }
; 3274 :     i++; /* skip directive */

	inc	edi

; 3275 :     if ( info->parasize || ( count != i ) )

	cmp	DWORD PTR [rcx+32], 0
	jne	SHORT $LN14@RetInstr
	cmp	r14d, edi
	je	SHORT $LN13@RetInstr
$LN14@RetInstr:

; 3276 :         *p++ = ' ';

	mov	BYTE PTR [rbx], 32			; 00000020H
	inc	rbx
$LN13@RetInstr:

; 3277 :     *p = NULLC;

	mov	BYTE PTR [rbx], 0

; 3278 :     /* RET without argument? Then calculate the value */
; 3279 :     if( is_iret == FALSE && count == i ) {

	test	sil, sil
	jne	$LN15@RetInstr
	cmp	r14d, edi
	jne	$LN15@RetInstr

; 3280 :         if ( ModuleInfo.epiloguemode != PEM_NONE ) {

	cmp	BYTE PTR ModuleInfo+425, 2
	je	$LN16@RetInstr

; 3281 :             switch( CurrProc->sym.langtype ) {

	mov	eax, DWORD PTR [r8+76]
	cmp	eax, 3
	je	SHORT $LN21@RetInstr
	jle	$LN16@RetInstr
	cmp	eax, 6
	jle	SHORT $LN18@RetInstr
	cmp	eax, 7
	jne	$LN16@RetInstr

; 3289 :             case LANG_FASTCALL:
; 3290 :                 fastcall_tab[ModuleInfo.fctype].handlereturn( CurrProc, buffer );

	movsxd	rax, DWORD PTR ModuleInfo+376
	lea	r9, OFFSET FLAT:fastcall_tab+8
	add	rax, rax
	lea	rdx, QWORD PTR buffer$[rsp]
	mov	rcx, r8
	call	QWORD PTR [r9+rax*8]

; 3291 :                 break;

	jmp	$LN16@RetInstr
$LN18@RetInstr:

; 3282 :             case LANG_BASIC:
; 3283 :             case LANG_FORTRAN:
; 3284 :             case LANG_PASCAL:
; 3285 :                 if( info->parasize != 0 ) {

	mov	r8d, DWORD PTR [rcx+32]
	test	r8d, r8d
	je	SHORT $LN16@RetInstr

; 3286 :                     sprintf( p, "%d%c", info->parasize, ModuleInfo.radix != 10 ? 't' : NULLC );

	xor	r9d, r9d
	lea	rdx, OFFSET FLAT:$SG12549
	cmp	BYTE PTR ModuleInfo+396, 10
	mov	eax, 116				; 00000074H
	mov	rcx, rbx
	cmovne	r9d, eax
	call	sprintf

; 3287 :                 }
; 3288 :                 break;

	jmp	SHORT $LN16@RetInstr
$LN21@RetInstr:

; 3292 :             case LANG_STDCALL:
; 3293 :                 if( !info->has_vararg && info->parasize != 0 ) {

	test	BYTE PTR [rcx+84], 1
	jne	SHORT $LN16@RetInstr
	mov	r8d, DWORD PTR [rcx+32]
	test	r8d, r8d
	je	SHORT $LN16@RetInstr

; 3294 :                     sprintf( p, "%d%c", info->parasize, ModuleInfo.radix != 10 ? 't' : NULLC  );

	xor	r9d, r9d
	lea	rdx, OFFSET FLAT:$SG12553
	cmp	BYTE PTR ModuleInfo+396, 10
	mov	eax, 116				; 00000074H
	mov	rcx, rbx
	cmovne	r9d, eax
	call	sprintf

; 3295 :                 }
; 3296 :                 break;
; 3297 :             }
; 3298 :         }
; 3299 :     } else {

	jmp	SHORT $LN16@RetInstr
$LN15@RetInstr:

; 3300 :         /* v2.04: changed. Now works for both RET nn and IRETx */
; 3301 :         /* v2.06: changed. Now works even if RET has ben "renamed" */
; 3302 :         strcpy( p, tokenarray[i].tokpos );

	movsxd	rax, edi
	shl	rax, 5
	mov	rcx, QWORD PTR [rax+rbp+24]
	sub	rbx, rcx
	npad	11
$LL25@RetInstr:
	movzx	eax, BYTE PTR [rcx]
	mov	BYTE PTR [rbx+rcx], al
	lea	rcx, QWORD PTR [rcx+1]
	test	al, al
	jne	SHORT $LL25@RetInstr
$LN16@RetInstr:

; 3303 :     }
; 3304 :     AddLineQueue( buffer );

	lea	rcx, QWORD PTR buffer$[rsp]
	call	AddLineQueue

; 3305 :     RunLineQueue();

	call	RunLineQueue

; 3306 : 
; 3307 :     DebugMsg1(( "RetInstr() exit\n" ));
; 3308 : 
; 3309 :     return( NOT_ERROR );

	xor	eax, eax
$LN1@RetInstr:

; 3310 : }

	lea	r11, QWORD PTR [rsp+640]
	mov	rbx, QWORD PTR [r11+16]
	mov	rbp, QWORD PTR [r11+24]
	mov	rsi, QWORD PTR [r11+32]
	mov	rdi, QWORD PTR [r11+40]
	mov	rsp, r11
	pop	r14
	ret	0
RetInstr ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
_TEXT	SEGMENT
proc$ = 48
src$ = 56
CopyPrototype PROC

; 1616 : {

$LN21:
	push	rdi
	sub	rsp, 32					; 00000020H

; 1617 :     struct dsym *curr;
; 1618 :     struct dsym *newl;
; 1619 :     struct dsym *oldl;
; 1620 : 
; 1621 :     if ( src->sym.isproc == FALSE )

	test	BYTE PTR [rdx+41], 8
	mov	rdi, rcx
	jne	SHORT $LN8@CopyProtot

; 1622 :         return( ERROR );

	or	eax, -1

; 1649 : }

	add	rsp, 32					; 00000020H
	pop	rdi
	ret	0
$LN8@CopyProtot:

; 1623 :     memcpy(proc->e.procinfo, src->e.procinfo, sizeof( struct proc_info ) );

	mov	rax, QWORD PTR [rdx+96]
	mov	rcx, QWORD PTR [rcx+96]
	mov	QWORD PTR [rsp+48], rbx
	mov	QWORD PTR [rsp+56], rsi

; 1624 :     proc->sym.mem_type = src->sym.mem_type;
; 1625 :     proc->sym.langtype = src->sym.langtype;
; 1626 : #if MANGLERSUPP
; 1627 :     proc->sym.mangler  = src->sym.mangler;
; 1628 : #endif
; 1629 :     proc->sym.ispublic   = src->sym.ispublic;
; 1630 :     /* we use the PROTO part, not the TYPE part */
; 1631 :     //dir->sym.seg_ofssize = src->sym.Ofssize;
; 1632 :     proc->sym.seg_ofssize = src->sym.seg_ofssize;
; 1633 :     proc->sym.isproc = TRUE;
; 1634 :     proc->e.procinfo->paralist = NULL;

	xor	esi, esi
	movups	xmm0, XMMWORD PTR [rax]
	movups	XMMWORD PTR [rcx], xmm0
	movups	xmm1, XMMWORD PTR [rax+16]
	movups	XMMWORD PTR [rcx+16], xmm1
	movups	xmm0, XMMWORD PTR [rax+32]
	movups	XMMWORD PTR [rcx+32], xmm0
	movups	xmm1, XMMWORD PTR [rax+48]
	movups	XMMWORD PTR [rcx+48], xmm1
	movups	xmm0, XMMWORD PTR [rax+64]
	movups	XMMWORD PTR [rcx+64], xmm0
	movsd	xmm1, QWORD PTR [rax+80]
	movsd	QWORD PTR [rcx+80], xmm1
	mov	eax, DWORD PTR [rdx+36]
	mov	DWORD PTR [rdi+36], eax
	mov	eax, DWORD PTR [rdx+76]
	mov	DWORD PTR [rdi+76], eax
	movzx	eax, BYTE PTR [rdx+40]
	xor	al, BYTE PTR [rdi+40]
	and	al, 127					; 0000007fH
	xor	al, BYTE PTR [rdx+40]
	mov	BYTE PTR [rdi+40], al
	movzx	eax, BYTE PTR [rdx+47]
	xor	al, BYTE PTR [rdi+47]
	or	BYTE PTR [rdi+41], 8
	and	al, 3
	xor	BYTE PTR [rdi+47], al
	mov	rax, QWORD PTR [rdi+96]
	mov	QWORD PTR [rax+8], rsi

; 1635 :     for ( curr = src->e.procinfo->paralist; curr; curr = curr->nextparam ) {

	mov	rax, QWORD PTR [rdx+96]
	mov	rbx, QWORD PTR [rax+8]
	test	rbx, rbx
	je	$LN3@CopyProtot
$LL4@CopyProtot:

; 1636 :         newl = LclAlloc( sizeof( struct dsym ) );

	mov	ecx, 120				; 00000078H
	call	LclAlloc

; 1637 :         memcpy( newl, curr, sizeof( struct dsym ) );

	movups	xmm0, XMMWORD PTR [rbx]
	mov	rdx, rax
	movups	XMMWORD PTR [rax], xmm0
	movups	xmm1, XMMWORD PTR [rbx+16]
	movups	XMMWORD PTR [rax+16], xmm1
	movups	xmm0, XMMWORD PTR [rbx+32]
	movups	XMMWORD PTR [rax+32], xmm0
	movups	xmm1, XMMWORD PTR [rbx+48]
	movups	XMMWORD PTR [rax+48], xmm1
	movups	xmm0, XMMWORD PTR [rbx+64]
	movups	XMMWORD PTR [rax+64], xmm0
	movups	xmm1, XMMWORD PTR [rbx+80]
	movups	XMMWORD PTR [rax+80], xmm1
	movups	xmm0, XMMWORD PTR [rbx+96]
	movups	XMMWORD PTR [rax+96], xmm0

; 1638 :         newl->nextparam = NULL;

	mov	QWORD PTR [rax+112], rsi

; 1639 :         if ( proc->e.procinfo->paralist == NULL)

	mov	rax, QWORD PTR [rdi+96]
	mov	rcx, QWORD PTR [rax+8]
	test	rcx, rcx
	jne	SHORT $LN9@CopyProtot

; 1640 :             proc->e.procinfo->paralist = newl;

	mov	QWORD PTR [rax+8], rdx
	jmp	SHORT $LN2@CopyProtot
$LN9@CopyProtot:

; 1641 :         else {
; 1642 :             for ( oldl = proc->e.procinfo->paralist; oldl->nextparam; oldl = oldl->nextparam );

	mov	rax, QWORD PTR [rcx+112]
	test	rax, rax
	je	SHORT $LN6@CopyProtot
$LL7@CopyProtot:
	mov	rcx, rax
	mov	rax, QWORD PTR [rax+112]
	test	rax, rax
	jne	SHORT $LL7@CopyProtot
$LN6@CopyProtot:

; 1643 :             oldl->nextparam = newl;

	mov	QWORD PTR [rcx+112], rdx
$LN2@CopyProtot:

; 1635 :     for ( curr = src->e.procinfo->paralist; curr; curr = curr->nextparam ) {

	mov	rbx, QWORD PTR [rbx+112]
	test	rbx, rbx
	jne	SHORT $LL4@CopyProtot
$LN3@CopyProtot:
	mov	rsi, QWORD PTR [rsp+56]

; 1644 :         }
; 1645 :     }
; 1646 :     DebugMsg1(("CopyPrototype(%s,src=%s): ofssize=%u\n",
; 1647 :                proc->sym.name, src->sym.name, src->sym.seg_ofssize ));
; 1648 :     return( NOT_ERROR );

	xor	eax, eax
	mov	rbx, QWORD PTR [rsp+48]

; 1649 : }

	add	rsp, 32					; 00000020H
	pop	rdi
	ret	0
CopyPrototype ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
_TEXT	SEGMENT
proc$ = 48
DeleteProc PROC

; 1381 : {

$LN15:
	sub	rsp, 40					; 00000028H

; 1382 :     struct dsym *curr;
; 1383 :     struct dsym *next;
; 1384 : 
; 1385 :     DebugMsg(("DeleteProc(%s) enter\n", proc->sym.name ));
; 1386 :     if ( proc->sym.state == SYM_INTERNAL ) {

	cmp	DWORD PTR [rcx+32], 1
	jne	SHORT $LN3@DeleteProc

; 1387 : 
; 1388 :         /* delete all local symbols ( params, locals, labels ) */
; 1389 :         for( curr = proc->e.procinfo->labellist; curr; ) {

	mov	rax, QWORD PTR [rcx+96]
	mov	rcx, QWORD PTR [rax+24]
	test	rcx, rcx
	je	SHORT $LN3@DeleteProc
	mov	QWORD PTR [rsp+32], rbx
	npad	4
$LL2@DeleteProc:

; 1390 :             next = curr->e.nextll;

	mov	rbx, QWORD PTR [rcx+96]

; 1391 :             DebugMsg(("DeleteProc(%s): free %s [next=%p]\n", proc->sym.name, curr->sym.name, curr->next ));
; 1392 :             SymFree( &curr->sym );

	call	SymFree

; 1393 :             curr = next;

	mov	rcx, rbx
	test	rbx, rbx
	jne	SHORT $LL2@DeleteProc
	mov	rbx, QWORD PTR [rsp+32]
$LN3@DeleteProc:

; 1394 :         }
; 1395 : 
; 1396 :         if ( proc->e.procinfo->regslist )
; 1397 :             LclFree( proc->e.procinfo->regslist );
; 1398 : 
; 1399 :         if ( proc->e.procinfo->prologuearg )
; 1400 :             LclFree( proc->e.procinfo->prologuearg );
; 1401 : 
; 1402 :         if ( Options.line_numbers && proc->sym.state == SYM_INTERNAL )
; 1403 :             LclFree( proc->sym.debuginfo );
; 1404 : #if FASTMEM==0 || defined(DEBUG_OUT)
; 1405 :     } else {
; 1406 :         /* PROTOs have just a parameter list, usually without names */
; 1407 :         for( curr = proc->e.procinfo->paralist; curr; ) {
; 1408 :             next = curr->nextparam;
; 1409 :             DebugMsg(("DeleteProc(%s): free %p (%s) [next=%p]\n", proc->sym.name, curr, curr->sym.name, curr->next ));
; 1410 :             SymFree( &curr->sym );
; 1411 :             curr = next;
; 1412 :         }
; 1413 : #endif
; 1414 :     }
; 1415 :     LclFree( proc->e.procinfo );
; 1416 :     return;
; 1417 : }

	add	rsp, 40					; 00000028H
	ret	0
DeleteProc ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
_TEXT	SEGMENT
sym$ = 48
name$ = 56
state$ = 64
CreateProc PROC

; 1329 : {

$LN22:
	mov	QWORD PTR [rsp+8], rbx
	mov	QWORD PTR [rsp+16], rsi
	push	rdi
	sub	rsp, 32					; 00000020H

; 1330 :     if ( sym == NULL )

	lea	rsi, OFFSET FLAT:SymTables+16
	mov	edi, r8d
	mov	rbx, rcx
	test	rcx, rcx
	jne	SHORT $LN4@CreateProc

; 1331 :         sym = ( *name ? SymCreate( name ) : SymAlloc( name ) );

	mov	rcx, rdx
	cmp	BYTE PTR [rdx], bl
	je	SHORT $LN14@CreateProc
	call	SymCreate
	mov	rbx, rax
	jmp	SHORT $LN5@CreateProc
$LN14@CreateProc:
	call	SymAlloc
	mov	rbx, rax
	jmp	SHORT $LN5@CreateProc
$LN4@CreateProc:

; 1332 :     else
; 1333 :         sym_remove_table( ( sym->state == SYM_UNDEFINED ) ? &SymTables[TAB_UNDEF] : &SymTables[TAB_EXT], (struct dsym *)sym );

	cmp	DWORD PTR [rbx+32], 0
	lea	rcx, OFFSET FLAT:SymTables
	mov	rdx, rbx
	cmovne	rcx, rsi
	call	sym_remove_table
$LN5@CreateProc:

; 1334 : 
; 1335 :     if ( sym ) {

	test	rbx, rbx
	je	$LN19@CreateProc

; 1336 :         struct proc_info *info;
; 1337 :         sym->state = state;

	mov	DWORD PTR [rbx+32], edi

; 1338 :         if ( state != SYM_INTERNAL ) {

	cmp	edi, 1
	je	SHORT $LN7@CreateProc

; 1339 :             sym->seg_ofssize = ModuleInfo.Ofssize;

	movzx	eax, BYTE PTR [rbx+47]
	xor	al, BYTE PTR ModuleInfo+404
	and	al, 3
	xor	BYTE PTR [rbx+47], al
$LN7@CreateProc:

; 1340 :         }
; 1341 :         info = LclAlloc( sizeof( struct proc_info ) );

	mov	ecx, 88					; 00000058H
	call	LclAlloc

; 1342 :         ((struct dsym *)sym)->e.procinfo = info;
; 1343 :         info->regslist = NULL;

	xor	edx, edx
	mov	QWORD PTR [rbx+96], rax
	mov	QWORD PTR [rax], rdx

; 1344 :         info->paralist = NULL;

	mov	QWORD PTR [rax+8], rdx

; 1345 :         info->locallist = NULL;

	mov	QWORD PTR [rax+16], rdx

; 1346 :         info->labellist = NULL;

	mov	QWORD PTR [rax+24], rdx

; 1347 :         info->parasize = 0;

	mov	QWORD PTR [rax+32], rdx

; 1348 :         info->localsize = 0;
; 1349 :         info->prologuearg = NULL;

	mov	QWORD PTR [rax+40], rdx

; 1350 :         info->flags = 0;

	mov	BYTE PTR [rax+84], dl

; 1351 :         switch ( sym->state ) {

	mov	edx, DWORD PTR [rbx+32]
	sub	edx, 1
	je	SHORT $LN8@CreateProc
	cmp	edx, 1
	jne	SHORT $LN19@CreateProc

; 1366 :             }
; 1367 :             break;
; 1368 :         case SYM_EXTERNAL:
; 1369 :             sym->weak = TRUE;

	or	BYTE PTR [rbx+47], 8

; 1370 :             sym_add_table( &SymTables[TAB_EXT], (struct dsym *)sym );

	mov	rdx, rbx
	mov	rcx, rsi
	call	sym_add_table

; 1371 :             break;
; 1372 :         }
; 1373 :     }
; 1374 :     return( sym );

	mov	rax, rbx

; 1375 : }

	mov	rbx, QWORD PTR [rsp+48]
	mov	rsi, QWORD PTR [rsp+56]
	add	rsp, 32					; 00000020H
	pop	rdi
	ret	0
$LN8@CreateProc:

; 1352 :         case SYM_INTERNAL:
; 1353 :             /* v2.04: don't use sym_add_table() and thus
; 1354 :              * free the <next> member field!
; 1355 :              */
; 1356 :             if ( SymTables[TAB_PROC].head == NULL )

	cmp	QWORD PTR SymTables+64, 0
	jne	SHORT $LN9@CreateProc

; 1357 :                 SymTables[TAB_PROC].head = (struct dsym *)sym;

	mov	QWORD PTR SymTables+64, rbx
	jmp	SHORT $LN10@CreateProc
$LN9@CreateProc:

; 1358 :             else {
; 1359 :                 SymTables[TAB_PROC].tail->nextproc = (struct dsym *)sym;

	mov	rax, QWORD PTR SymTables+72
	mov	QWORD PTR [rax+112], rbx
$LN10@CreateProc:

; 1360 :             }
; 1361 :             SymTables[TAB_PROC].tail = (struct dsym *)sym;
; 1362 :             procidx++;

	inc	DWORD PTR procidx

; 1363 :             if ( Options.line_numbers ) {

	cmp	BYTE PTR Options+1, 0
	mov	QWORD PTR SymTables+72, rbx
	je	SHORT $LN19@CreateProc

; 1364 :                 sym->debuginfo = LclAlloc( sizeof( struct debug_info ) );

	mov	ecx, 24
	call	LclAlloc
	mov	QWORD PTR [rbx+64], rax

; 1365 :                 sym->debuginfo->file = get_curr_srcfile();

	call	get_curr_srcfile
	mov	rcx, QWORD PTR [rbx+64]
	mov	WORD PTR [rcx+14], ax
$LN19@CreateProc:

; 1375 : }

	mov	rsi, QWORD PTR [rsp+56]
	mov	rax, rbx
	mov	rbx, QWORD PTR [rsp+48]
	add	rsp, 32					; 00000020H
	pop	rdi
	ret	0
CreateProc ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File d:\hjwasm\hjwasm2.13.1s\hjwasm2.13.1s\proc.c
_TEXT	SEGMENT
tv1298 = 96
proc$ = 96
i$ = 104
tokenarray$ = 112
IsPROC$ = 120
langtype$ = 128
ParseProc PROC

; 1000 : {

$LN128:
	mov	BYTE PTR [rsp+32], r9b
	mov	DWORD PTR [rsp+16], edx
	push	rbp
	push	rbx
	push	rsi
	push	rdi
	push	r12
	push	r13
	push	r15
	mov	rbp, rsp
	sub	rsp, 32					; 00000020H

; 1001 :     char            *token;
; 1002 :     uint_16         *regist;
; 1003 :     //int             type;
; 1004 :     //enum lang_type  langtype;
; 1005 :     enum memtype    newmemtype;
; 1006 :     uint_8          newofssize;
; 1007 :     uint_8          oldofssize;
; 1008 : #if FASTPASS
; 1009 :     bool            oldpublic = proc->sym.ispublic;

	movzx	r12d, BYTE PTR [rcx+40]

; 1010 : #endif
; 1011 : 
; 1012 :     /* set some default values */
; 1013 : 
; 1014 :     if ( IsPROC ) {

	lea	rdi, OFFSET FLAT:SpecialTable
	shr	r12b, 7
	xor	r13d, r13d
	movzx	r15d, r9b
	mov	rsi, r8
	mov	rbx, rcx
	test	r9b, r9b
	je	$LN16@ParseProc

; 1015 :         proc->e.procinfo->isexport = ModuleInfo.procs_export;

	mov	eax, DWORD PTR ModuleInfo+408
	mov	rdx, QWORD PTR [rcx+96]
	shr	eax, 3
	shl	al, 2
	xor	al, BYTE PTR [rdx+84]
	and	al, 4
	xor	BYTE PTR [rdx+84], al

; 1016 :         /* don't overwrite a PUBLIC directive for this symbol! */
; 1017 :         if ( ModuleInfo.procs_private == FALSE )

	test	BYTE PTR ModuleInfo+408, 4
	jne	SHORT $LN12@ParseProc

; 1018 :             proc->sym.ispublic = TRUE;

	or	BYTE PTR [rcx+40], 128			; 00000080H
$LN12@ParseProc:

; 1019 : 
; 1020 :         /* set type of epilog code */
; 1021 : #if STACKBASESUPP
; 1022 :         /* v2.11: if base register isn't [E|R]BP, don't use LEAVE! */
; 1023 :         if ( GetRegNo( proc->e.procinfo->basereg ) != 5 ) {

	mov	rdx, QWORD PTR [rcx+96]
	movzx	eax, WORD PTR [rdx+86]
	lea	rcx, QWORD PTR [rax+rax*2]
	cmp	BYTE PTR [rdi+rcx*4+10], 5
	je	SHORT $LN13@ParseProc

; 1024 :             proc->e.procinfo->pe_type = 0;

	and	BYTE PTR [rdx+84], 253			; 000000fdH

; 1025 :         } else

	jmp	SHORT $LN16@ParseProc
$LN13@ParseProc:

; 1026 : #endif
; 1027 :         if ( Options.masm_compat_gencode ) {

	cmp	BYTE PTR Options+128, r13b
	je	SHORT $LN15@ParseProc

; 1028 :             /* v2.07: Masm uses LEAVE if
; 1029 :              * - current code is 32-bit/64-bit or
; 1030 :              * - cpu is .286 or .586+ */
; 1031 :             //proc->e.procinfo->pe_type = ( ( ModuleInfo.curr_cpu & P_CPU_MASK ) >= P_286 );
; 1032 :             proc->e.procinfo->pe_type = ( ModuleInfo.Ofssize > USE16 ||

	cmp	BYTE PTR ModuleInfo+404, r13b
	ja	SHORT $LN85@ParseProc
	mov	eax, DWORD PTR ModuleInfo+392
	and	eax, 240				; 000000f0H
	cmp	eax, 32					; 00000020H
	je	SHORT $LN85@ParseProc
	cmp	eax, 80					; 00000050H
	mov	eax, r13d
	jl	SHORT $LN88@ParseProc
$LN85@ParseProc:
	mov	eax, 1

; 1033 :                                          ( ModuleInfo.curr_cpu & P_CPU_MASK ) == P_286 ||
; 1034 :                                          ( ModuleInfo.curr_cpu & P_CPU_MASK ) >= P_586 ) ? 1 : 0;
; 1035 :         } else {

	jmp	SHORT $LN88@ParseProc
$LN15@ParseProc:

; 1036 :             /* use LEAVE for 286, 386 (and x64) */
; 1037 :             proc->e.procinfo->pe_type = ( ( ModuleInfo.curr_cpu & P_CPU_MASK ) == P_286 ||

	mov	ecx, DWORD PTR ModuleInfo+392
	and	ecx, 240				; 000000f0H
	lea	eax, DWORD PTR [rcx-32]
	test	eax, -81				; ffffffafH
	jne	SHORT $LN108@ParseProc
	mov	eax, 1
	cmp	ecx, 96					; 00000060H
	jne	SHORT $LN88@ParseProc
$LN108@ParseProc:
	mov	eax, r13d
$LN88@ParseProc:
	and	BYTE PTR [rdx+84], 253			; 000000fdH
	and	al, 1
	add	al, al
	or	BYTE PTR [rdx+84], al
$LN16@ParseProc:

; 1038 : #if AMD64_SUPPORT
; 1039 :                                          ( ModuleInfo.curr_cpu & P_CPU_MASK ) == P_64 ||
; 1040 : #endif
; 1041 :                                          ( ModuleInfo.curr_cpu & P_CPU_MASK ) == P_386 ) ? 1 : 0;
; 1042 :         }
; 1043 :     }
; 1044 : 
; 1045 : #if MANGLERSUPP
; 1046 :     /* OW name mangling */
; 1047 :     if( tokenarray[i].token == T_STRING && IsPROC ) {
; 1048 :         /* SetMangler() will ignore LANG_NONE */
; 1049 :         SetMangler( &proc->sym, LANG_NONE, tokenarray[i].string_ptr );
; 1050 :         i++;
; 1051 :     }
; 1052 : #endif
; 1053 : 
; 1054 :     /* 1. attribute is <distance> */
; 1055 :     if ( tokenarray[i].token == T_STYPE &&
; 1056 :         tokenarray[i].tokval >= T_NEAR && tokenarray[i].tokval <= T_FAR32 ) {

	movsxd	rax, DWORD PTR i$[rbp-32]
	shl	rax, 5
	mov	QWORD PTR [rsp+112], r14
	cmp	BYTE PTR [rax+r8], 6
	jne	SHORT $LN17@ParseProc
	mov	ecx, DWORD PTR [rax+r8+16]
	lea	eax, DWORD PTR [rcx-221]
	cmp	eax, 5
	ja	SHORT $LN17@ParseProc

; 1057 :         uint_8 Ofssize = GetSflagsSp( tokenarray[i].tokval );

	lea	rcx, QWORD PTR [rcx+rcx*2]

; 1058 :         /* v2.06: SimpleType is obsolete */
; 1059 :         /* v2.05: FindStdType() is obsolete */
; 1060 :         //type = tokenarray[i].bytval;
; 1061 :         //type = FindStdType(tokenarray[i].value);
; 1062 :         if ( IsPROC ) {
; 1063 :             if ( ( ModuleInfo.Ofssize >= USE32 && Ofssize == USE16 ) ||

	movzx	edx, BYTE PTR ModuleInfo+404
	movzx	r15d, BYTE PTR [rdi+rcx*4+4]
	cmp	BYTE PTR IsPROC$[rbp-32], r13b
	je	SHORT $LN20@ParseProc
	cmp	dl, 1
	jb	SHORT $LN22@ParseProc
	test	r15b, r15b
	je	SHORT $LN21@ParseProc
$LN22@ParseProc:
	test	dl, dl
	jne	SHORT $LN20@ParseProc
	cmp	r15b, 1
	jne	SHORT $LN20@ParseProc
$LN21@ParseProc:

; 1064 :                 ( ModuleInfo.Ofssize == USE16 && Ofssize == USE32 ) ) {
; 1065 :                 EmitError( DISTANCE_INVALID );

	mov	ecx, 171				; 000000abH
	call	EmitError
	movzx	edx, BYTE PTR ModuleInfo+404
$LN20@ParseProc:

; 1066 :             }
; 1067 :         }
; 1068 :         newmemtype = GetMemtypeSp( tokenarray[i].tokval );

	movsxd	rax, DWORD PTR i$[rbp-32]
	shl	rax, 5

; 1069 :         newofssize = (( Ofssize != USE_EMPTY ) ? Ofssize : ModuleInfo.Ofssize );

	cmp	r15b, 254				; 000000feH
	movzx	r14d, dl
	mov	eax, DWORD PTR [rax+rsi+16]
	lea	rcx, QWORD PTR [rax+rax*2]
	movzx	eax, r15b
	movzx	edi, BYTE PTR [rdi+rcx*4+10]
	cmovne	r14d, eax

; 1070 :         i++;

	inc	DWORD PTR i$[rbp-32]

; 1071 :     } else {

	movzx	r15d, BYTE PTR IsPROC$[rbp-32]
	jmp	SHORT $LN18@ParseProc
$LN17@ParseProc:

; 1072 :         newmemtype = ( ( SIZE_CODEPTR & ( 1 << ModuleInfo.model ) ) ? MT_FAR : MT_NEAR );

	mov	ecx, DWORD PTR ModuleInfo+360
	mov	edi, r13d

; 1073 :         newofssize = ModuleInfo.Ofssize;

	movzx	r14d, BYTE PTR ModuleInfo+404
	mov	eax, 1
	shl	eax, cl
	test	al, 112					; 00000070H
	setne	dil
	add	edi, 129				; 00000081H
$LN18@ParseProc:

; 1074 :     }
; 1075 : 
; 1076 :     /* v2.11: GetSymOfssize() cannot handle SYM_TYPE correctly */
; 1077 :     if ( proc->sym.state == SYM_TYPE )

	cmp	DWORD PTR [rbx+32], 7
	jne	SHORT $LN23@ParseProc

; 1078 :         oldofssize = proc->sym.seg_ofssize;

	movzx	eax, BYTE PTR [rbx+47]
	and	al, 3
	jmp	SHORT $LN24@ParseProc
$LN23@ParseProc:

; 1079 :     else
; 1080 :         oldofssize = GetSymOfssize( &proc->sym );

	mov	rcx, rbx
	call	GetSymOfssize
$LN24@ParseProc:

; 1081 : 
; 1082 :     /* did the distance attribute change? */
; 1083 :     if ( proc->sym.mem_type != MT_EMPTY &&

	mov	ecx, DWORD PTR [rbx+36]
	cmp	ecx, 192				; 000000c0H
	je	SHORT $LN25@ParseProc
	cmp	ecx, edi
	jne	SHORT $LN27@ParseProc
	cmp	al, r14b
	je	SHORT $LN25@ParseProc
$LN27@ParseProc:
	lea	eax, DWORD PTR [rcx-129]
	cmp	eax, 1
	jbe	SHORT $LN30@ParseProc

; 1089 :         else {
; 1090 :             return( EmitErr( SYMBOL_REDEFINITION, proc->sym.name ) );

	mov	rdx, QWORD PTR [rbx+8]
	mov	ecx, 143				; 0000008fH
	jmp	$LN124@ParseProc
$LN30@ParseProc:

; 1084 :         ( proc->sym.mem_type != newmemtype ||
; 1085 :          oldofssize != newofssize ) ) {
; 1086 :         DebugMsg(("ParseProc: error, memtype changed, old-new memtype=%X-%X, ofssize=%X-%X\n", proc->sym.mem_type, newmemtype, proc->sym.Ofssize, newofssize));
; 1087 :         if ( proc->sym.mem_type == MT_NEAR || proc->sym.mem_type == MT_FAR )
; 1088 :             EmitError( PROC_AND_PROTO_CALLING_CONV_CONFLICT );

	mov	ecx, 138				; 0000008aH
	call	EmitError

; 1091 :         }
; 1092 :     } else {

	jmp	SHORT $LN31@ParseProc
$LN25@ParseProc:

; 1093 :         proc->sym.mem_type = newmemtype;

	mov	DWORD PTR [rbx+36], edi

; 1094 :         if ( IsPROC == FALSE )

	test	r15b, r15b
	jne	SHORT $LN31@ParseProc

; 1095 :             proc->sym.seg_ofssize = newofssize;

	and	BYTE PTR [rbx+47], 252			; 000000fcH
	and	r14b, 3
	or	BYTE PTR [rbx+47], r14b
$LN31@ParseProc:

; 1096 :     }
; 1097 : 
; 1098 :     /* 2. attribute is <langtype> */
; 1099 :     /* v2.09: the default language value is now a function argument. This is because
; 1100 :      * EXTERN[DEF] allows to set the language attribute by:
; 1101 :      * EXTERN[DEF] <langtype> <name> PROTO ...
; 1102 :      * ( see CreateProto() in extern.c )
; 1103 :      */
; 1104 :     //langtype = ModuleInfo.langtype; /* set the default value */
; 1105 :     GetLangType( &i, tokenarray, &langtype ); /* optionally overwrite the value */

	lea	r8, QWORD PTR langtype$[rbp-32]
	mov	rdx, rsi
	lea	rcx, QWORD PTR i$[rbp-32]
	call	GetLangType

; 1106 :     
; 1107 : 	/* John Hankinson: 2016-02-10 Allows Linux64 to utlise Win64 ABI */
; 1108 : 	#if AMD64_SUPPORT
; 1109 : 	if (Options.output_format == OFORMAT_ELF)

	cmp	DWORD PTR Options+144, 3
	mov	edi, 7
	mov	eax, DWORD PTR langtype$[rbp-32]

; 1110 : 		langtype = LANG_FASTCALL;
; 1111 : 	#endif
; 1112 : 
; 1113 : 	/* has language changed? */
; 1114 :     if ( proc->sym.langtype != LANG_NONE && proc->sym.langtype != langtype ) {

	mov	edx, DWORD PTR [rbx+76]
	cmove	eax, edi
	mov	DWORD PTR langtype$[rbp-32], eax
	test	edx, edx
	je	SHORT $LN33@ParseProc
	cmp	edx, eax
	je	SHORT $LN33@ParseProc

; 1115 :         DebugMsg(("ParseProc: error, language changed, %u - %u\n", proc->sym.langtype, langtype ));
; 1116 :         EmitError( PROC_AND_PROTO_CALLING_CONV_CONFLICT );

	mov	ecx, 138				; 0000008aH
	call	EmitError

; 1117 :     } else

	jmp	SHORT $LN34@ParseProc
$LN33@ParseProc:

; 1118 :         proc->sym.langtype = langtype;

	mov	DWORD PTR [rbx+76], eax
$LN34@ParseProc:

; 1119 : 
; 1120 : 	/* John Hankinson: 2016-02-10 Allows Linux64 to utlise Win64 ABI */
; 1121 : 	#if AMD64_SUPPORT
; 1122 : 	if (proc->sym.langtype == LANG_NONE && Options.output_format == OFORMAT_ELF)

	cmp	DWORD PTR [rbx+76], r13d
	jne	SHORT $LN116@ParseProc
	cmp	DWORD PTR Options+144, 3
	jne	SHORT $LN116@ParseProc

; 1123 : 	{
; 1124 : 		proc->sym.langtype = LANG_FASTCALL;

	mov	DWORD PTR [rbx+76], edi
$LN116@ParseProc:

; 1125 : 	}
; 1126 : 	#endif
; 1127 : 
; 1128 :     /* 3. attribute is <visibility> */
; 1129 :     /* note that reserved word PUBLIC is a directive! */
; 1130 :     /* PROTO does NOT accept PUBLIC! However,
; 1131 :      * PROTO accepts PRIVATE and EXPORT, but these attributes are just ignored!
; 1132 :      */
; 1133 : 
; 1134 :     if ( tokenarray[i].token == T_ID || tokenarray[i].token == T_DIRECTIVE ) {

	movsxd	rdx, DWORD PTR i$[rbp-32]
	mov	rax, rdx
	shl	rax, 5
	lea	rdi, QWORD PTR [rax+rsi]
	movzx	eax, BYTE PTR [rax+rsi]
	cmp	al, 8
	je	SHORT $LN37@ParseProc
	cmp	al, 3
	jne	$LN44@ParseProc
$LN37@ParseProc:

; 1135 :         token = tokenarray[i].string_ptr;

	mov	rdi, QWORD PTR [rdi+8]

; 1136 :         if ( _stricmp( token, "PRIVATE") == 0 ) {

	lea	rdx, OFFSET FLAT:$SG11712
	mov	rcx, rdi
	call	_stricmp
	test	eax, eax
	jne	SHORT $LN38@ParseProc

; 1137 :             if ( IsPROC ) { /* v2.11: ignore PRIVATE for PROTO */

	test	r15b, r15b
	je	SHORT $LN40@ParseProc

; 1138 :                 proc->sym.ispublic = FALSE;

	and	BYTE PTR [rbx+40], 123			; 0000007bH

; 1139 : #if FASTPASS
; 1140 :                 /* error if there was a PUBLIC directive! */
; 1141 :                 proc->sym.scoped = TRUE;

	or	BYTE PTR [rbx+40], 4

; 1142 :                 if ( oldpublic ) {

	test	r12b, r12b
	je	SHORT $LN41@ParseProc

; 1143 :                     SkipSavedState(); /* do a full pass-2 scan */

	call	SkipSavedState

; 1144 :                 }
; 1145 : #endif
; 1146 :                 proc->e.procinfo->isexport = FALSE;
; 1147 :             }
; 1148 :             i++;

	jmp	SHORT $LN41@ParseProc
$LN38@ParseProc:

; 1149 :         } else if ( IsPROC && (_stricmp(token, "PUBLIC") == 0 ) ) {

	test	r15b, r15b
	je	SHORT $LN42@ParseProc
	lea	rdx, OFFSET FLAT:$SG11717
	mov	rcx, rdi
	call	_stricmp
	test	eax, eax
	jne	SHORT $LN42@ParseProc

; 1150 :             proc->sym.ispublic = TRUE;

	or	BYTE PTR [rbx+40], 128			; 00000080H
$LN41@ParseProc:

; 1151 :             proc->e.procinfo->isexport = FALSE;

	mov	rax, QWORD PTR [rbx+96]
	and	BYTE PTR [rax+84], 251			; 000000fbH
$LN40@ParseProc:

; 1152 :             i++;

	mov	edx, DWORD PTR i$[rbp-32]
	inc	edx
	mov	DWORD PTR i$[rbp-32], edx
	jmp	SHORT $LN44@ParseProc
$LN42@ParseProc:

; 1153 :         } else if ( _stricmp(token, "EXPORT") == 0 ) {

	lea	rdx, OFFSET FLAT:$SG11719
	mov	rcx, rdi
	call	_stricmp
	test	eax, eax
	jne	SHORT $LN118@ParseProc

; 1154 :             DebugMsg1(("ParseProc(%s): EXPORT detected\n", proc->sym.name ));
; 1155 :             if ( IsPROC ) { /* v2.11: ignore EXPORT for PROTO */

	test	r15b, r15b
	je	SHORT $LN40@ParseProc

; 1156 :                 proc->sym.ispublic = TRUE;

	or	BYTE PTR [rbx+40], 128			; 00000080H

; 1157 :                 proc->e.procinfo->isexport = TRUE;

	mov	rax, QWORD PTR [rbx+96]
	or	BYTE PTR [rax+84], 4

; 1158 :                 /* v2.11: no export for 16-bit near */
; 1159 :                 if ( ModuleInfo.Ofssize == USE16 && proc->sym.mem_type == MT_NEAR )

	cmp	BYTE PTR ModuleInfo+404, r13b
	jne	SHORT $LN40@ParseProc
	cmp	DWORD PTR [rbx+36], 129			; 00000081H
	jne	SHORT $LN40@ParseProc

; 1160 :                     EmitErr( EXPORT_MUST_BE_FAR, proc->sym.name );

	mov	rdx, QWORD PTR [rbx+8]
	mov	ecx, 276				; 00000114H
	call	EmitErr
	mov	edx, DWORD PTR i$[rbp-32]
	inc	edx
	mov	DWORD PTR i$[rbp-32], edx
	jmp	SHORT $LN44@ParseProc
$LN118@ParseProc:

; 1161 :             }
; 1162 :             i++;

	mov	edx, DWORD PTR i$[rbp-32]
$LN44@ParseProc:

; 1163 :         }
; 1164 :     }
; 1165 : 
; 1166 :     /* 4. attribute is <prologuearg>, for PROC only.
; 1167 :      * it must be enclosed in <>
; 1168 :      */
; 1169 :     if ( IsPROC && tokenarray[i].token == T_STRING && tokenarray[i].string_delim == '<' ) {

	test	r15b, r15b
	je	$LN121@ParseProc
	movsxd	rcx, edx
	shl	rcx, 5
	add	rcx, rsi
	cmp	BYTE PTR [rcx], 9
	jne	$LN121@ParseProc
	cmp	BYTE PTR [rcx+1], 60			; 0000003cH
	jne	$LN121@ParseProc

; 1170 :         int idx = Token_Count + 1;

	mov	eax, DWORD PTR ModuleInfo+496
	lea	r12d, DWORD PTR [rax+1]

; 1171 :         int max;
; 1172 :         if ( ModuleInfo.prologuemode == PEM_NONE )

	movzx	eax, BYTE PTR ModuleInfo+424
	cmp	al, 2
	je	$LN3@ParseProc

; 1173 :             ; /* no prologue at all */
; 1174 :         else if ( ModuleInfo.prologuemode == PEM_MACRO ) {

	cmp	al, 1
	jne	SHORT $LN50@ParseProc

; 1175 :             proc->e.procinfo->prologuearg = LclAlloc( tokenarray[i].stringlen + 1 );

	mov	ecx, DWORD PTR [rcx+16]
	inc	ecx
	call	LclAlloc
	mov	rcx, QWORD PTR [rbx+96]
	mov	QWORD PTR [rcx+40], rax

; 1176 :             strcpy( proc->e.procinfo->prologuearg, tokenarray[i].string_ptr );

	movsxd	rax, DWORD PTR i$[rbp-32]
	shl	rax, 5
	mov	rcx, QWORD PTR [rax+rsi+8]
	mov	rax, QWORD PTR [rbx+96]
	mov	rdx, QWORD PTR [rax+40]
	npad	4
$LL91@ParseProc:
	movzx	eax, BYTE PTR [rcx]
	inc	rcx
	mov	BYTE PTR [rdx], al
	lea	rdx, QWORD PTR [rdx+1]
	test	al, al
	jne	SHORT $LL91@ParseProc

; 1177 :         } else {

	jmp	$LN125@ParseProc
$LN50@ParseProc:

; 1178 :             /* check the argument. The default prologue
; 1179 :              understands FORCEFRAME and LOADDS only
; 1180 :              */
; 1181 :             max = Tokenize( tokenarray[i].string_ptr, idx, tokenarray, TOK_RESCAN );

	mov	rcx, QWORD PTR [rcx+8]
	mov	r9d, 1
	mov	r8, rsi
	movsxd	r15, r12d
	mov	edx, r12d
	call	Tokenize

; 1182 :             for ( ; idx < max; idx++ ) {

	cdqe
	mov	QWORD PTR tv1298[rbp-32], rax
	cmp	r15, rax
	jge	$LN120@ParseProc

; 1170 :         int idx = Token_Count + 1;

	mov	rdi, r15
	lea	r14, QWORD PTR [r15+2]
	shl	rdi, 5
	add	rdi, rsi
	shl	r14, 5
	add	r14, rsi
$LL4@ParseProc:

; 1183 :                 if ( tokenarray[idx].token == T_ID ) {

	cmp	BYTE PTR [rdi], 8
	jne	$LN52@ParseProc

; 1184 :                     if ( _stricmp( tokenarray[idx].string_ptr, "FORCEFRAME") == 0 ) {

	mov	rcx, QWORD PTR [rdi+8]
	lea	rdx, OFFSET FLAT:$SG11731
	call	_stricmp
	test	eax, eax
	jne	SHORT $LN54@ParseProc

; 1185 :                         proc->e.procinfo->forceframe = TRUE;

	mov	rax, QWORD PTR [rbx+96]
	or	BYTE PTR [rax+84], 8
	jmp	SHORT $LN59@ParseProc
$LN54@ParseProc:

; 1186 : #if AMD64_SUPPORT
; 1187 :                     } else if ( ModuleInfo.Ofssize != USE64 && (_stricmp( tokenarray[idx].string_ptr, "LOADDS") == 0 ) ) {

	cmp	BYTE PTR ModuleInfo+404, 2
	je	$LN56@ParseProc
	mov	rcx, QWORD PTR [rdi+8]
	lea	rdx, OFFSET FLAT:$SG11734
	call	_stricmp
	test	eax, eax
	jne	$LN56@ParseProc

; 1188 : #else
; 1189 :                     } else if ( _stricmp( tokenarray[idx].string_ptr, "LOADDS") == 0 ) {
; 1190 : #endif
; 1191 :                         if ( ModuleInfo.model == MODEL_FLAT ) {

	cmp	DWORD PTR ModuleInfo+360, 7
	jne	SHORT $LN58@ParseProc

; 1192 :                             EmitWarn( 2, LOADDS_IGNORED_IN_FLAT_MODEL );

	mov	edx, 226				; 000000e2H
	lea	ecx, QWORD PTR [rax+2]
	call	EmitWarn

; 1193 :                         } else

	jmp	SHORT $LN59@ParseProc
$LN58@ParseProc:

; 1194 :                             proc->e.procinfo->loadds = TRUE;

	mov	rax, QWORD PTR [rbx+96]
	or	BYTE PTR [rax+84], 16
$LN59@ParseProc:

; 1197 :                     }
; 1198 :                     if ( tokenarray[idx+1].token == T_COMMA && tokenarray[idx+2].token != T_FINAL)

	cmp	BYTE PTR [rdi+32], 44			; 0000002cH
	jne	SHORT $LN2@ParseProc
	cmp	BYTE PTR [r14], r13b
	je	SHORT $LN2@ParseProc

; 1199 :                         idx++;

	inc	r12d
	inc	r15
	add	rdi, 32					; 00000020H
	add	r14, 32					; 00000020H
$LN2@ParseProc:

; 1182 :             for ( ; idx < max; idx++ ) {

	inc	r12d
	inc	r15
	add	rdi, 32					; 00000020H
	add	r14, 32					; 00000020H
	cmp	r15, QWORD PTR tv1298[rbp-32]
	jl	$LL4@ParseProc
$LN120@ParseProc:
	movzx	r15d, BYTE PTR IsPROC$[rbp-32]
$LN125@ParseProc:

; 1200 :                 } else {
; 1201 :                     return( EmitErr( SYNTAX_ERROR_EX, tokenarray[idx].string_ptr ) );

	mov	edx, DWORD PTR i$[rbp-32]
$LN3@ParseProc:

; 1202 :                 }
; 1203 :             }
; 1204 :         }
; 1205 :         i++;

	inc	edx
	mov	DWORD PTR i$[rbp-32], edx
$LN121@ParseProc:

; 1206 :     }
; 1207 : 
; 1208 : #if AMD64_SUPPORT
; 1209 :     /* check for optional FRAME[:exc_proc] */
; 1210 :     if ( ModuleInfo.Ofssize == USE64 &&
; 1211 :         IsPROC &&
; 1212 :         tokenarray[i].token == T_RES_ID &&

	cmp	BYTE PTR ModuleInfo+404, 2
	jne	$LN61@ParseProc
	test	r15b, r15b
	je	$LN61@ParseProc
	movsxd	rax, edx
	shl	rax, 5
	cmp	BYTE PTR [rax+rsi], 7
	jne	$LN61@ParseProc
	cmp	DWORD PTR [rax+rsi+16], 263		; 00000107H
	jne	$LN61@ParseProc
	mov	eax, DWORD PTR Options+144
	add	eax, -2					; fffffffeH
	cmp	eax, 1
	jbe	SHORT $LN62@ParseProc

; 1213 :         tokenarray[i].tokval == T_FRAME ) {
; 1214 : 		/* v2.05: don't accept FRAME for ELF  - 2016-02-10 John Hankinson allowed ELF64 to use FRAME/Win64 ABI */
; 1215 : 		if (Options.output_format != OFORMAT_COFF && Options.output_format != OFORMAT_ELF
; 1216 : #if PE_SUPPORT
; 1217 :             && ModuleInfo.sub_format != SFORMAT_PE

	cmp	DWORD PTR ModuleInfo+372, 2
	je	SHORT $LN62@ParseProc

; 1218 : #endif
; 1219 :            ) {
; 1220 :             return( EmitErr( NOT_SUPPORTED_WITH_CURR_FORMAT, GetResWName( T_FRAME, NULL ) ) );

	xor	edx, edx
	mov	ecx, 263				; 00000107H
	call	GetResWName
	mov	rdx, rax
	mov	ecx, 224				; 000000e0H
	jmp	$LN124@ParseProc
$LN56@ParseProc:

; 1195 :                     } else {
; 1196 :                         return( EmitErr( UNKNOWN_DEFAULT_PROLOGUE_ARGUMENT, tokenarray[idx].string_ptr ) );

	movsxd	rdx, r12d
	mov	ecx, 225				; 000000e1H
	shl	rdx, 5
	mov	rdx, QWORD PTR [rdx+rsi+8]
	jmp	$LN124@ParseProc
$LN52@ParseProc:

; 1200 :                 } else {
; 1201 :                     return( EmitErr( SYNTAX_ERROR_EX, tokenarray[idx].string_ptr ) );

	movsxd	rdx, r12d
	shl	rdx, 5
	mov	rdx, QWORD PTR [rdx+rsi+8]
	jmp	$LN126@ParseProc
$LN62@ParseProc:

; 1221 :         }
; 1222 :         i++;

	inc	edx

; 1223 :         if( tokenarray[i].token == T_COLON ) {

	movsxd	rax, edx
	shl	rax, 5
	mov	DWORD PTR i$[rbp-32], edx
	cmp	BYTE PTR [rax+rsi], 58			; 0000003aH
	jne	SHORT $LN63@ParseProc

; 1224 :             struct asym *sym;
; 1225 :             i++;

	inc	edx

; 1226 :             if ( tokenarray[i].token != T_ID ) {

	movsxd	rcx, edx
	shl	rcx, 5
	mov	DWORD PTR i$[rbp-32], edx
	cmp	BYTE PTR [rcx+rsi], 8
	je	SHORT $LN65@ParseProc

; 1227 :                 return( EmitErr( SYNTAX_ERROR_EX, tokenarray[i].string_ptr ) );

	mov	rdx, QWORD PTR [rcx+rsi+8]
	jmp	$LN126@ParseProc
$LN65@ParseProc:

; 1228 :             }
; 1229 :             sym = SymSearch( tokenarray[i].string_ptr );

	mov	rcx, QWORD PTR [rcx+rsi+8]
	call	SymFind
	mov	rdi, rax

; 1230 :             if ( sym == NULL ) {

	test	rax, rax
	jne	SHORT $LN66@ParseProc

; 1231 :                 sym = SymCreate( tokenarray[i].string_ptr );

	movsxd	rcx, DWORD PTR i$[rbp-32]
	shl	rcx, 5
	mov	rcx, QWORD PTR [rcx+rsi+8]
	call	SymCreate

; 1232 :                 sym->state = SYM_UNDEFINED;
; 1233 :                 sym->used = TRUE;
; 1234 :                 sym_add_table( &SymTables[TAB_UNDEF], (struct dsym *)sym ); /* add UNDEFINED */

	mov	rdx, rax
	lea	rcx, OFFSET FLAT:SymTables
	mov	rdi, rax
	or	BYTE PTR [rax+40], 1
	mov	DWORD PTR [rax+32], r13d
	call	sym_add_table
$LN68@ParseProc:

; 1239 :             }
; 1240 :             proc->e.procinfo->exc_handler = sym;

	mov	rax, QWORD PTR [rbx+96]
	mov	QWORD PTR [rax+48], rdi

; 1241 :             i++;

	inc	DWORD PTR i$[rbp-32]

; 1242 :         } else

	jmp	SHORT $LN64@ParseProc
$LN66@ParseProc:
	cmp	DWORD PTR [rax+32], 2
	jbe	SHORT $LN68@ParseProc

; 1235 :             } else if ( sym->state != SYM_UNDEFINED &&
; 1236 :                        sym->state != SYM_INTERNAL &&
; 1237 :                        sym->state != SYM_EXTERNAL ) {
; 1238 :                 return( EmitErr( SYMBOL_REDEFINITION, sym->name ) );

	mov	rdx, QWORD PTR [rax+8]
	mov	ecx, 143				; 0000008fH
	jmp	$LN124@ParseProc
$LN63@ParseProc:

; 1243 :             proc->e.procinfo->exc_handler = NULL;

	mov	rax, QWORD PTR [rbx+96]
	mov	QWORD PTR [rax+48], r13
$LN64@ParseProc:

; 1244 :         proc->e.procinfo->isframe = TRUE;

	mov	rax, QWORD PTR [rbx+96]
	or	BYTE PTR [rax+84], 64			; 00000040H
	mov	edx, DWORD PTR i$[rbp-32]
$LN61@ParseProc:

; 1245 :     }
; 1246 : #endif
; 1247 :     /* check for USES */
; 1248 :     if ( tokenarray[i].token == T_ID && _stricmp( tokenarray[i].string_ptr, "USES" ) == 0 ) {

	movsxd	rax, edx
	shl	rax, 5
	cmp	BYTE PTR [rax+rsi], 8
	jne	$LN9@ParseProc
	mov	rcx, QWORD PTR [rax+rsi+8]
	lea	rdx, OFFSET FLAT:$SG11747
	call	_stricmp
	test	eax, eax
	jne	$LN123@ParseProc

; 1249 :         int cnt;
; 1250 :         int j;
; 1251 :         if ( !IsPROC ) {/* not for PROTO! */

	test	r15b, r15b
	jne	SHORT $LN70@ParseProc

; 1252 :             DebugMsg(("ParseProc: USES found in PROTO\n"));
; 1253 :             EmitErr( SYNTAX_ERROR_EX, tokenarray[i].string_ptr );

	movsxd	rdx, DWORD PTR i$[rbp-32]
	mov	ecx, 209				; 000000d1H
	shl	rdx, 5
	mov	rdx, QWORD PTR [rdx+rsi+8]
	call	EmitErr
$LN70@ParseProc:

; 1254 :         }
; 1255 :         i++;

	mov	eax, DWORD PTR i$[rbp-32]
	inc	eax

; 1256 :         /* count register names which follow */
; 1257 :         for ( cnt = 0, j = i; tokenarray[j].token == T_REG; j++, cnt++ );

	movsxd	rcx, eax
	mov	rdx, rcx
	mov	DWORD PTR i$[rbp-32], eax
	shl	rdx, 5
	cmp	BYTE PTR [rdx+rsi], 2
	jne	$LN107@ParseProc
	shl	rcx, 5
	add	rcx, rsi
$LL7@ParseProc:
	lea	rcx, QWORD PTR [rcx+32]
	inc	r13d
	cmp	BYTE PTR [rcx], 2
	je	SHORT $LL7@ParseProc

; 1258 : 
; 1259 :         if ( cnt == 0 ) {

	test	r13d, r13d
	je	SHORT $LN107@ParseProc

; 1262 :         } else {
; 1263 :             regist = LclAlloc( (cnt + 1) * sizeof( uint_16 ) );

	lea	eax, DWORD PTR [r13+1]
	movsxd	rcx, eax
	add	rcx, rcx
	call	LclAlloc

; 1264 :             proc->e.procinfo->regslist = regist;

	mov	rcx, QWORD PTR [rbx+96]

; 1265 :             *regist++ = cnt;

	lea	rdi, QWORD PTR [rax+2]
	mov	QWORD PTR [rcx], rax
	mov	WORD PTR [rax], r13w

; 1266 :             /* read in registers */
; 1267 :             for( ; tokenarray[i].token == T_REG; i++ ) {

	movsxd	rdx, DWORD PTR i$[rbp-32]
	mov	rcx, rdx
	shl	rcx, 5
	cmp	BYTE PTR [rcx+rsi], 2
	lea	rax, QWORD PTR [rcx+rsi]
	jne	SHORT $LN9@ParseProc
$LL10@ParseProc:

; 1268 :                 if ( SizeFromRegister( tokenarray[i].tokval ) == 1 ) {

	mov	ecx, DWORD PTR [rax+16]
	call	SizeFromRegister
	cmp	eax, 1
	jne	SHORT $LN73@ParseProc

; 1269 :                     EmitError( INVALID_USE_OF_REGISTER );

	mov	ecx, 170				; 000000aaH
	call	EmitError
$LN73@ParseProc:

; 1270 :                 }
; 1271 :                 *regist++ = tokenarray[i].tokval;

	movsxd	rax, DWORD PTR i$[rbp-32]
	shl	rax, 5
	movzx	eax, WORD PTR [rax+rsi+16]
	mov	WORD PTR [rdi], ax
	add	rdi, 2
	mov	edx, DWORD PTR i$[rbp-32]
	inc	edx
	movsxd	rax, edx
	shl	rax, 5
	add	rax, rsi
	mov	DWORD PTR i$[rbp-32], edx
	cmp	BYTE PTR [rax], 2
	je	SHORT $LL10@ParseProc

; 1266 :             /* read in registers */
; 1267 :             for( ; tokenarray[i].token == T_REG; i++ ) {

	jmp	SHORT $LN9@ParseProc
$LN107@ParseProc:

; 1260 :             DebugMsg(("ParseProc: no registers for regslist\n"));
; 1261 :             EmitErr( SYNTAX_ERROR_EX, tokenarray[i-1].tokpos );

	mov	rdx, QWORD PTR [rdx+rsi-8]
	mov	ecx, 209				; 000000d1H
	call	EmitErr
$LN123@ParseProc:
	mov	edx, DWORD PTR i$[rbp-32]
$LN9@ParseProc:

; 1272 :             }
; 1273 :         }
; 1274 :     }
; 1275 : 
; 1276 :     /* the parameters must follow */
; 1277 :     if ( tokenarray[i].token == T_STYPE || tokenarray[i].token == T_RES_ID || tokenarray[i].token == T_DIRECTIVE ) {

	movsxd	r8, edx
	shl	r8, 5
	movzx	ecx, BYTE PTR [r8+rsi]
	lea	eax, DWORD PTR [rcx-6]
	cmp	al, 1
	jbe	$LN75@ParseProc
	cmp	cl, 3
	je	$LN75@ParseProc

; 1279 :     }
; 1280 : 
; 1281 :     /* skip optional comma */
; 1282 :     if ( tokenarray[i].token == T_COMMA )

	cmp	cl, 44					; 0000002cH
	jne	SHORT $LN76@ParseProc

; 1283 :         i++;

	inc	edx
	mov	DWORD PTR i$[rbp-32], edx
$LN76@ParseProc:

; 1284 : 
; 1285 :     DebugMsg1(("ParseProc(%s): i=%u, Token_Count=%u, CurrWordSize=%u\n", proc->sym.name, i, Token_Count, CurrWordSize ));
; 1286 : 
; 1287 :     if( i >= Token_Count ) {

	cmp	edx, DWORD PTR ModuleInfo+496
	jl	SHORT $LN77@ParseProc

; 1288 :         /* procedure has no parameters at all */
; 1289 :         if ( proc->e.procinfo->paralist != NULL )

	mov	rax, QWORD PTR [rbx+96]
	cmp	QWORD PTR [rax+8], 0
	je	SHORT $LN83@ParseProc

; 1290 :             EmitErr( CONFLICTING_PARAMETER_DEFINITION, "" );

	lea	rdx, OFFSET FLAT:$SG11758
	mov	ecx, 137				; 00000089H
	call	EmitErr

; 1302 :             /* do proceed if the parameter scan returns an error */
; 1303 :             ;//return( ERROR );
; 1304 :     }
; 1305 : 
; 1306 :     /* v2.11: isdefined and isproc now set here */
; 1307 :     proc->sym.isdefined = TRUE;

	or	BYTE PTR [rbx+40], 2

; 1308 :     proc->sym.isproc = TRUE;

	or	BYTE PTR [rbx+41], 8

; 1309 :     //proc->e.procinfo->init_done = TRUE;
; 1310 :     DebugMsg1(("ParseProc(%s): memtype=%Xh parasize=%u\n", proc->sym.name, proc->sym.mem_type, proc->e.procinfo->parasize));
; 1311 : 
; 1312 :     return( NOT_ERROR );

	xor	eax, eax
	jmp	SHORT $LN1@ParseProc
$LN77@ParseProc:

; 1291 :     } else if( proc->sym.langtype == LANG_NONE ) {

	cmp	DWORD PTR [rbx+76], 0
	jne	SHORT $LN80@ParseProc

; 1292 :         EmitError( LANG_MUST_BE_SPECIFIED );

	mov	ecx, 91					; 0000005bH
	call	EmitError

; 1302 :             /* do proceed if the parameter scan returns an error */
; 1303 :             ;//return( ERROR );
; 1304 :     }
; 1305 : 
; 1306 :     /* v2.11: isdefined and isproc now set here */
; 1307 :     proc->sym.isdefined = TRUE;

	or	BYTE PTR [rbx+40], 2

; 1308 :     proc->sym.isproc = TRUE;

	or	BYTE PTR [rbx+41], 8

; 1309 :     //proc->e.procinfo->init_done = TRUE;
; 1310 :     DebugMsg1(("ParseProc(%s): memtype=%Xh parasize=%u\n", proc->sym.name, proc->sym.mem_type, proc->e.procinfo->parasize));
; 1311 : 
; 1312 :     return( NOT_ERROR );

	xor	eax, eax
	jmp	SHORT $LN1@ParseProc
$LN80@ParseProc:

; 1293 :     } else {
; 1294 :         /* v2.05: set PROC's vararg flag BEFORE params are scanned! */
; 1295 :         if ( tokenarray[Token_Count - 1].token == T_RES_ID &&

	movsxd	rax, DWORD PTR ModuleInfo+496
	shl	rax, 5
	cmp	BYTE PTR [rax+rsi-32], 7
	jne	SHORT $LN82@ParseProc
	cmp	DWORD PTR [rax+rsi-16], 262		; 00000106H
	jne	SHORT $LN82@ParseProc

; 1296 :             tokenarray[Token_Count - 1].tokval == T_VARARG )
; 1297 :             proc->e.procinfo->has_vararg = TRUE;

	mov	rax, QWORD PTR [rbx+96]
	or	BYTE PTR [rax+84], 1
	mov	edx, DWORD PTR i$[rbp-32]
$LN82@ParseProc:

; 1298 :         /* v2.04: removed, comma is checked above already */
; 1299 :         //if( tokenarray[i].token == T_COMMA )
; 1300 :         //    i++;
; 1301 :         if ( ERROR == ParseParams( proc, i, tokenarray, IsPROC ) )

	movzx	r9d, r15b
	mov	r8, rsi
	mov	rcx, rbx
	call	ParseParams
$LN83@ParseProc:

; 1302 :             /* do proceed if the parameter scan returns an error */
; 1303 :             ;//return( ERROR );
; 1304 :     }
; 1305 : 
; 1306 :     /* v2.11: isdefined and isproc now set here */
; 1307 :     proc->sym.isdefined = TRUE;

	or	BYTE PTR [rbx+40], 2

; 1308 :     proc->sym.isproc = TRUE;

	or	BYTE PTR [rbx+41], 8

; 1309 :     //proc->e.procinfo->init_done = TRUE;
; 1310 :     DebugMsg1(("ParseProc(%s): memtype=%Xh parasize=%u\n", proc->sym.name, proc->sym.mem_type, proc->e.procinfo->parasize));
; 1311 : 
; 1312 :     return( NOT_ERROR );

	xor	eax, eax
	jmp	SHORT $LN1@ParseProc
$LN75@ParseProc:

; 1278 :         return( EmitErr( SYNTAX_ERROR_EX, tokenarray[i].string_ptr ) );

	mov	rdx, QWORD PTR [r8+rsi+8]
$LN126@ParseProc:
	mov	ecx, 209				; 000000d1H
$LN124@ParseProc:
	call	EmitErr
$LN1@ParseProc:
	mov	r14, QWORD PTR [rsp+112]

; 1313 : }

	add	rsp, 32					; 00000020H
	pop	r15
	pop	r13
	pop	r12
	pop	rdi
	pop	rsi
	pop	rbx
	pop	rbp
	ret	0
ParseProc ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File c:\program files (x86)\windows kits\10\include\10.0.10150.0\ucrt\stdio.h
;	COMDAT sprintf
_TEXT	SEGMENT
_Buffer$ = 80
_Format$ = 88
sprintf	PROC						; COMDAT

; 1776 : {

$LN8:
	mov	QWORD PTR [rsp+16], rdx
	mov	QWORD PTR [rsp+24], r8
	mov	QWORD PTR [rsp+32], r9
	push	rbx
	push	rsi
	push	rdi
	sub	rsp, 48					; 00000030H
	mov	rbx, rdx

; 1777 :     int _Result;
; 1778 :     va_list _ArgList;
; 1779 :     __crt_va_start(_ArgList, _Format);

	lea	rsi, QWORD PTR _Format$[rsp+8]
	mov	rdi, rcx

; 1780 : 
; 1781 :     #pragma warning(push)
; 1782 :     #pragma warning(disable: 4996) // Deprecation
; 1783 :     _Result = _vsprintf_l(_Buffer, _Format, NULL, _ArgList);

	call	__local_stdio_printf_options
	mov	r9, rbx
	mov	QWORD PTR [rsp+40], rsi
	or	rbx, -1
	mov	QWORD PTR [rsp+32], 0
	mov	r8, rbx
	mov	rdx, rdi
	mov	rcx, QWORD PTR [rax]
	or	rcx, 1
	call	__stdio_common_vsprintf
	test	eax, eax
	cmovs	eax, ebx

; 1784 :     #pragma warning(pop)
; 1785 : 
; 1786 :     __crt_va_end(_ArgList);
; 1787 :     return _Result;
; 1788 : }

	add	rsp, 48					; 00000030H
	pop	rdi
	pop	rsi
	pop	rbx
	ret	0
sprintf	ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File c:\program files (x86)\windows kits\10\include\10.0.10150.0\ucrt\stdio.h
;	COMDAT _vsprintf_l
_TEXT	SEGMENT
_Buffer$ = 64
_Format$ = 72
_Locale$ = 80
_ArgList$ = 88
_vsprintf_l PROC					; COMDAT

; 1455 : {

$LN6:
	mov	QWORD PTR [rsp+8], rbx
	mov	QWORD PTR [rsp+16], rbp
	mov	QWORD PTR [rsp+24], rsi
	push	rdi
	sub	rsp, 48					; 00000030H
	mov	rbx, r9
	mov	rdi, r8
	mov	rsi, rdx
	mov	rbp, rcx

; 1456 :     #pragma warning(push)
; 1457 :     #pragma warning(disable: 4996) // Deprecation
; 1458 :     return _vsnprintf_l(_Buffer, (size_t)-1, _Format, _Locale, _ArgList);

	call	__local_stdio_printf_options
	mov	QWORD PTR [rsp+40], rbx
	mov	r9, rsi
	or	rbx, -1
	mov	QWORD PTR [rsp+32], rdi
	mov	r8, rbx
	mov	rdx, rbp
	mov	rcx, QWORD PTR [rax]
	or	rcx, 1
	call	__stdio_common_vsprintf

; 1459 :     #pragma warning(pop)
; 1460 : }

	mov	rbp, QWORD PTR [rsp+72]
	test	eax, eax
	mov	rsi, QWORD PTR [rsp+80]
	cmovs	eax, ebx
	mov	rbx, QWORD PTR [rsp+64]
	add	rsp, 48					; 00000030H
	pop	rdi
	ret	0
_vsprintf_l ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File c:\program files (x86)\windows kits\10\include\10.0.10150.0\ucrt\stdio.h
;	COMDAT _vsnprintf_l
_TEXT	SEGMENT
_Buffer$ = 64
_BufferCount$ = 72
_Format$ = 80
_Locale$ = 88
_ArgList$ = 96
_vsnprintf_l PROC					; COMDAT

; 1385 : {

$LN4:
	mov	QWORD PTR [rsp+8], rbx
	mov	QWORD PTR [rsp+16], rbp
	mov	QWORD PTR [rsp+24], rsi
	push	rdi
	sub	rsp, 48					; 00000030H
	mov	rbx, r9
	mov	rdi, r8
	mov	rsi, rdx
	mov	rbp, rcx

; 1386 :     int const _Result = __stdio_common_vsprintf(

	call	__local_stdio_printf_options
	mov	r9, rdi
	mov	r8, rsi
	mov	rdx, rbp
	mov	rcx, QWORD PTR [rax]
	mov	rax, QWORD PTR _ArgList$[rsp]
	or	rcx, 1
	mov	QWORD PTR [rsp+40], rax
	mov	QWORD PTR [rsp+32], rbx
	call	__stdio_common_vsprintf

; 1387 :         _CRT_INTERNAL_LOCAL_PRINTF_OPTIONS | _CRT_INTERNAL_PRINTF_LEGACY_VSPRINTF_NULL_TERMINATION,
; 1388 :         _Buffer, _BufferCount, _Format, _Locale, _ArgList);
; 1389 : 
; 1390 :     return _Result < 0 ? -1 : _Result;
; 1391 : }

	mov	rbx, QWORD PTR [rsp+64]
	or	ecx, -1
	mov	rbp, QWORD PTR [rsp+72]
	test	eax, eax
	mov	rsi, QWORD PTR [rsp+80]
	cmovs	eax, ecx
	add	rsp, 48					; 00000030H
	pop	rdi
	ret	0
_vsnprintf_l ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File c:\program files (x86)\windows kits\10\include\10.0.10150.0\ucrt\stdio.h
;	COMDAT printf
_TEXT	SEGMENT
_Format$ = 80
printf	PROC						; COMDAT

; 950  : {

$LN6:
	mov	QWORD PTR [rsp+8], rcx
	mov	QWORD PTR [rsp+16], rdx
	mov	QWORD PTR [rsp+24], r8
	mov	QWORD PTR [rsp+32], r9
	push	rbx
	push	rsi
	push	rdi
	sub	rsp, 48					; 00000030H
	mov	rdi, rcx

; 951  :     int _Result;
; 952  :     va_list _ArgList;
; 953  :     __crt_va_start(_ArgList, _Format);

	lea	rsi, QWORD PTR _Format$[rsp+8]

; 954  :     _Result = _vfprintf_l(stdout, _Format, NULL, _ArgList);

	mov	ecx, 1
	call	__acrt_iob_func
	mov	rbx, rax
	call	__local_stdio_printf_options
	xor	r9d, r9d
	mov	QWORD PTR [rsp+32], rsi
	mov	r8, rdi
	mov	rdx, rbx
	mov	rcx, QWORD PTR [rax]
	call	__stdio_common_vfprintf

; 955  :     __crt_va_end(_ArgList);
; 956  :     return _Result;
; 957  : }

	add	rsp, 48					; 00000030H
	pop	rdi
	pop	rsi
	pop	rbx
	ret	0
printf	ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File c:\program files (x86)\windows kits\10\include\10.0.10150.0\ucrt\stdio.h
;	COMDAT _vfprintf_l
_TEXT	SEGMENT
_Stream$ = 64
_Format$ = 72
_Locale$ = 80
_ArgList$ = 88
_vfprintf_l PROC					; COMDAT

; 638  : {

$LN4:
	mov	QWORD PTR [rsp+8], rbx
	mov	QWORD PTR [rsp+16], rbp
	mov	QWORD PTR [rsp+24], rsi
	push	rdi
	sub	rsp, 48					; 00000030H
	mov	rbx, r9
	mov	rdi, r8
	mov	rsi, rdx
	mov	rbp, rcx

; 639  :     return __stdio_common_vfprintf(_CRT_INTERNAL_LOCAL_PRINTF_OPTIONS, _Stream, _Format, _Locale, _ArgList);

	call	__local_stdio_printf_options
	mov	r9, rdi
	mov	QWORD PTR [rsp+32], rbx
	mov	r8, rsi
	mov	rdx, rbp
	mov	rcx, QWORD PTR [rax]
	call	__stdio_common_vfprintf

; 640  : }

	mov	rbx, QWORD PTR [rsp+64]
	mov	rbp, QWORD PTR [rsp+72]
	mov	rsi, QWORD PTR [rsp+80]
	add	rsp, 48					; 00000030H
	pop	rdi
	ret	0
_vfprintf_l ENDP
_TEXT	ENDS
; Function compile flags: /Ogtpy
; File c:\program files (x86)\windows kits\10\include\10.0.10150.0\ucrt\corecrt_stdio_config.h
;	COMDAT __local_stdio_printf_options
_TEXT	SEGMENT
__local_stdio_printf_options PROC			; COMDAT

; 74   :     static unsigned __int64 _OptionsStorage;
; 75   :     return &_OptionsStorage;

	lea	rax, OFFSET FLAT:?_OptionsStorage@?1??__local_stdio_printf_options@@9@9 ; `__local_stdio_printf_options'::`2'::_OptionsStorage

; 76   : }

	ret	0
__local_stdio_printf_options ENDP
_TEXT	ENDS
END
