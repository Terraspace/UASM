...
$pdata$IncBinDirective DD imagerel $LN41
	DD	imagerel $LN41+432
	DD	imagerel $unwind$IncBinDirective
$pdata$0$IncBinDirective DD imagerel $LN41+432
	DD	imagerel $LN41+556
	DD	imagerel $chain$0$IncBinDirective
$pdata$1$IncBinDirective DD imagerel $LN41+556
	DD	imagerel $LN41+587
...
$chain$0$IncBinDirective DD 020821H
	...
	DD	imagerel $LN41+432
	DD	imagerel $unwind$IncBinDirective
$chain$1$IncBinDirective DD 021H
	DD	imagerel $LN41
	DD	imagerel $LN41+432
	DD	imagerel $unwind$IncBinDirective
...
; 408  :     }
; 409  : 
; 410  :     /* don't touch Option fields! if anything at all, ModuleInfo.name may be modified.
; 411  :      * However, since the directive is ignored by Masm, nothing is done.
; 412  :      */
; 413  : //  strncpy( ModuleInfo.name, tokenarray[i].string_ptr, sizeof( ModuleInfo.name ) );
; 414  : //  ModuleInfo.name[ sizeof( ModuleInfo.name ) - 1] = NULLC;
...
; 384  :     /* if a module name is set with -nm, ignore NAME directive! */
; 385  :     /* v2.08: removed, since Options.names isn't touched at all */
; 386  :     //if( Options.names[OPTN_MODULE_NAME] != NULL )
; 387  :     //    return( NOT_ERROR );
; 388  : 
; 389  :     i++; /* skip directive */

	inc	ecx

; 390  : 
; 391  :     /* improper use of NAME is difficult to see since it is a nop
; 392  :      therefore some syntax checks are implemented:
; 393  :      - no 'name' structs, unions, records, typedefs!
; 394  :      - no 'name' struct fields!
; 395  :      - no 'name' segments!
; 396  :      - no 'name:' label!
; 397  :      */
; 398  :     if ( CurrStruct != NULL ||
; 399  :         ( tokenarray[i].token == T_DIRECTIVE &&
; 400  :          ( tokenarray[i].tokval == T_SEGMENT ||
; 401  :           tokenarray[i].tokval == T_STRUCT  ||
...
; 406  :          tokenarray[i].token == T_COLON ) {
; 407  :         return( EmitErr( SYNTAX_ERROR_EX, tokenarray[i-1].tokpos ) );
...
; 260  :     if ( file = SearchFile( StringBufferEnd, FALSE ) ) {
	...
	mov	rdi, rax
	test	rax, rax
	je	SHORT $LN26@IncBinDire

; 261  :         /* transfer file content to the current segment. */
; 262  :         if ( fileoffset )
...
; 264  :         for( ; sizemax; sizemax-- ) {
	...
	test	esi, esi
	je	SHORT $LN36@IncBinDire
	npad	4
$LL4@IncBinDire:

; 265  :             int ch = fgetc( file );

	mov	rcx, rdi
	call	fgetc
	mov	ebx, eax

; 266  :             if ( ( ch == EOF ) && feof( file ) )
...
; 299  : 
; 300  :     if ( tokenarray[i].token != T_STRING ||
...
; 301  :         tokenarray[i].string_delim != '<' ) {
; 302  :         DebugMsg(("AliasDirective: first argument is not a literal: %s\n", tokenarray[i].string_ptr ));
; 303  :         return( EmitError( TEXT_ITEM_REQUIRED ) );
; 304  :     }
; 305  : 
...
; 312  :     }
; 313  : 
; 314  :     if ( tokenarray[i+2].token != T_STRING ||
...
; 318  :     }
; 319  :     subst = tokenarray[i+2].string_ptr;

	mov	QWORD PTR [rsp+56], rsi
	mov	rsi, QWORD PTR [rax+rdx+8]

; 320  : 
; 321  :     if ( tokenarray[i+3].token != T_FINAL ) {
...
; 326  :     sym = SymSearch( tokenarray[i].string_ptr );
...
; 327  :     if ( sym == NULL || sym->state == SYM_UNDEFINED ) {
...
; 351  :     }
; 352  :     if ( sym->state != SYM_ALIAS || ( strcmp( sym->substitute->name, subst ) != 0 )) {
...
; 357  :     /* for COFF+ELF, make sure <actual_name> is "global" (EXTERNAL or
; 358  :      * public INTERNAL). For OMF, there's no check at all. */
; 359  :     if ( Parse_Pass != PASS_1 ) {
...
; 360  :         if ( Options.output_format == OFORMAT_COFF
; 361  : #if ELF_SUPPORT
; 362  :              || Options.output_format == OFORMAT_ELF
; 363  : #endif
; 364  :            ) {
; 365  :             if ( sym->substitute->state == SYM_UNDEFINED ) {
...
; 368  :                        ( sym->substitute->state != SYM_INTERNAL || sym->substitute->ispublic == FALSE ) ) {
; 369  :                 return( EmitErr( MUST_BE_PUBLIC_OR_EXTERNAL, subst ) );
...
; 353  :         DebugMsg(("AliasDirective: symbol redefinition\n"));
; 354  :         return( EmitErr( SYMBOL_REDEFINITION, sym->name ) );
...
; 328  :         struct asym *sym2;
; 329  :         /* v2.04b: adjusted to new field <substitute> */
; 330  :         sym2 = SymSearch( subst );
...
; 333  :             sym2->state = SYM_UNDEFINED;
; 334  :             sym_add_table( &SymTables[TAB_UNDEF], (struct dsym *)sym2 );
...
; 341  :             sym = SymCreate( tokenarray[i].string_ptr );
...
; 342  :         else
; 343  :             sym_remove_table( &SymTables[TAB_UNDEF], (struct dsym *)sym );
...
; 344  : 
; 345  :         sym->state = SYM_ALIAS;

	mov	DWORD PTR [rbx+32], 8

; 346  :         sym->substitute = sym2;
; 347  :         /* v2.10: copy language type of alias */
; 348  :         sym->langtype = sym2->langtype;
; 349  :         sym_add_table( &SymTables[TAB_ALIAS], (struct dsym *)sym ); /* add ALIAS */
...
; 316  :         DebugMsg(("AliasDirective: second argument is not a literal: %s\n", tokenarray[i+2].string_ptr ));
; 317  :         return( EmitError( TEXT_ITEM_REQUIRED ) );
...
; 311  :         return( EmitErr( SYNTAX_ERROR_EX, tokenarray[i+1].string_ptr ) );
...
; 425  :     uint_8          oldradix;
; 426  :     struct expr     opndx;
; 427  : 
; 428  :     /* to get the .radix parameter, enforce radix 10 and retokenize! */
; 429  :     oldradix = ModuleInfo.radix;

	movzx	ebx, BYTE PTR ModuleInfo+396
	mov	rdi, rdx

; 430  :     ModuleInfo.radix = 10;
; 431  :     i++; /* skip directive token */
...
; 434  :     /* v2.11: flag NOUNDEF added - no forward ref possible */
; 435  :     if ( EvalOperand( &i, tokenarray, Token_Count, &opndx, EXPF_NOUNDEF ) == ERROR ) {
...
; 443  :         return( EmitErr( SYNTAX_ERROR_EX, tokenarray[i].tokpos ) );
...
; 444  :     }
; 445  :     if ( opndx.value > 16 || opndx.value < 2 || opndx.hvalue != 0 ) {
...
; 446  :         return( EmitError( INVALID_RADIX_TAG ) );
...
; 461  :         return( EmitErr( SYNTAX_ERROR_EX, tokenarray[i+1].tokpos ) );
...
; 462  :     }
; 463  : #if COFF_SUPPORT || ELF_SUPPORT || PE_SUPPORT
; 464  :     if ( Options.output_format == OFORMAT_COFF
; 465  : #if ELF_SUPPORT
...
; 474  :     } else
; 475  : #endif
; 476  :         ModuleInfo.segorder = GetSflagsSp( tokenarray[i].tokval );
...
; 470  : #endif
; 471  :        ) {
; 472  :         if ( Parse_Pass == PASS_1 )

	cmp	DWORD PTR Parse_Pass, 0
	jne	SHORT $LN6@SegOrderDi

; 473  :             EmitWarn( 2, NOT_SUPPORTED_WITH_CURR_FORMAT, _strupr( tokenarray[i].string_ptr ) );
